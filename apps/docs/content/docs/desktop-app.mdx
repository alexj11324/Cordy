---
title: Desktop app
description: Install Orvilo Desktop, use desktop tabs and the built-in daemon, and connect to a self-hosted instance.
---

import { Callout } from "fumadocs-ui/components/callout";

Orvilo Desktop is available for macOS, Windows, and Linux. It uses the same account and workspace data as the web app, but it manages the daemon on your machine automatically and keeps a separate set of tabs for each workspace.
...
You can be signed in to both at the same time. As long as they connect to the same Orvilo service, the data they show is shared.
...
After installing, sign in with the same email you use on the web. Once signed in, Desktop starts its own bundled Orvilo CLI and detects the AI coding tools already installed on the machine.
...
After sign-in, Desktop creates a dedicated CLI profile for the current Orvilo service and uses it to start the daemon. The profile lives at:

```text
~/.patchbay/profiles/desktop-<host>/
```

It never reads or overwrites the default profile you use in the terminal. If you also start another daemon manually, Orvilo shows them as separate runtimes.
...
Desktop connects to Orvilo Cloud by default. To connect to a self-hosted instance, create `desktop.json` inside the `.patchbay` directory of your home directory:
...
## Windows Defender flags Orvilo as a virus
...
**This is a false positive.** The `!ml` suffix means the verdict came from Defender's machine-learning heuristics rather than a malware signature. Orvilo's Windows builds are not Authenticode code-signed yet, and a freshly published unsigned binary that starts background processes and opens network connections is exactly the profile those heuristics score as suspicious. Every Windows artifact is built by GitHub Actions from the public source in this repository.
...
1. **Restore the quarantined file** — Windows Security → *Virus & threat protection* → *Protection history* → select the Orvilo item → *Actions* → *Restore*.
2. **Add an exclusion** so it isn't quarantined again — *Virus & threat protection* → *Manage settings* → *Exclusions* → *Add an exclusion* → *Folder*, and add both:
   ...
   Both paths matter: when the bundled CLI goes missing, Desktop downloads a replacement into `%APPDATA%\Orvilo\bin`, so excluding only the install folder leaves you in a loop where the replacement gets quarantined too.
...
<Callout type="warning">
Only add these exclusions if you installed Orvilo from [GitHub Releases](https://github.com/alexj11324/Cordy/releases) and the checksum matches. An exclusion disables real-time protection for everything in that folder.
</Callout>
