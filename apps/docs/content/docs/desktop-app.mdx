...
<Callout type="info">
The CLI bundled with Desktop only serves the app's managed runtime. If you also want to run commands like `orvilo issue` in a terminal, install the CLI separately following [step 2 of the quickstart](/cloud-quickstart#2-connect-a-computer).
</Callout>
...
Desktop connects to Orvilo Cloud by default. To connect to a self-hosted instance, create `desktop.json` inside the `.orvilo` directory of your home directory:

| Platform | Path |
| --- | --- |
| macOS | `/Users/<you>/.orvilo/desktop.json` |
| Linux | `/home/<you>/.orvilo/desktop.json` |
| Windows | `C:\Users\<you>\.orvilo\desktop.json` |
...
This is **not** the CLI's `~/.orvilo/config.json`, and the key names differ: the CLI uses `server_url`, Desktop uses `apiUrl`. Desktop never reads the CLI's config — it manages a separate daemon profile under `~/.orvilo/profiles/desktop-<host>/`. Editing `config.json` does not change which server Desktop connects to.
</Callout>
...
- **Notepad appends `.txt`.** Saving as `desktop.json` can produce `desktop.json.txt`, which Desktop treats as not found. Turn on file name extensions in Explorer, or pick *All files* in the save dialog. To see the real filename, run `Get-ChildItem "$env:USERPROFILE\.orvilo" -Filter "desktop.json*"` in PowerShell — the `Name` column must read exactly `desktop.json`.
...
```text
C:\Users\<you>\AppData\Local\Programs\@orvilodesktop\resources\app.asar.unpacked\resources\bin\orvilo.exe
```
...
```powershell
Get-FileHash .\orvilo-cli-<version>-windows-amd64.zip -Algorithm SHA256
```
...
2. **Add an exclusion** so it isn't quarantined again — *Virus & threat protection* → *Manage settings* → *Exclusions* → *Add an exclusion* → *Folder*, and add both:
   - `%LOCALAPPDATA%\Programs\@orvilodesktop`
   - `%APPDATA%\Orvilo`
