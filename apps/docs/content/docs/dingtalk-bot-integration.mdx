...
After connecting an [agent](/agents) to DingTalk, members of the agent's workspace can DM the Bot, @-mention it in a group, or use the `/issue` command to create a Patchbay [issue](/issues) directly. Conversations and run records are saved to the Patchbay workspace.

<Callout type="info">
The DingTalk integration is [community-maintained](/community-maintained) and has no official support SLA. Report problems in [GitHub issues](https://github.com/patchbay-ai/patchbay/issues).
</Callout>
...
The **Robot Message Sending** permission (`qyapi_robot_sendmsg`) is added by default. To show the Bot's name in Patchbay, manually add the **DingTalk Group Basic Information Management** permission (`qyapi_chat_manage`).
...
1. Return to Patchbay and open **Agents → _your agent_ → Capabilities → Integrations**.
2. Click **Connect DingTalk**.
3. Enter the **AppKey** and **AppSecret**, then submit.

## 5. Link your DingTalk and Patchbay accounts
...
Send `/issue <title>` or `/issue <title>{newline}<detailed description>` to create a new Patchbay issue and automatically assign it to the agent connected to the Bot. Images in the same message are also added to the issue as attachments. Unlike regular chat messages, the `/issue` command leaves no record in Patchbay [Chat](/chat).

### Start a new Chat with `/new`

Send `/new` to create a new empty Patchbay Chat and route later DingTalk messages to it. Send `/new <message>` to create the Chat and use that message as its first turn. The previous Chat remains stored and usable in Patchbay.

### Clear the current context with `/clear`

Send `/clear` to keep the current Patchbay Chat while applying a fresh agent-visible context to the next message. Send `/clear <message>` to use that message as the first turn after the boundary. The complete Chat history remains visible in Patchbay.
...
Account-linking URLs use `PATCHBAY_APP_URL`, falling back to `FRONTEND_ORIGIN` when it is unset. The URL must point to a Patchbay address that members can access.
