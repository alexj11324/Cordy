...
智能体默认最多同时执行 6 条 task；单个守护进程默认最多同时执行 20 条。达到上限时，新 task 会留在队列中，等已有执行结束后再开始。运行时离线时，task 也会继续排队；只有该运行时心跳缺席超过重连宽限期、且该 task 自己也已等满这么久时才会失败；所以排在忙碌运行时后面的长队列不会因为等太久而被判失败，路由给已经离线的机器时也仍然留有完整一个宽限期把它唤醒。
...
这表示另一条在途 task 正在使用同一个本地目录。Patchbay 会等待目录锁释放，避免两个智能体同时修改同一份文件。

这种等待只存在于目录的 `in_place`（“原地”）模式。如果这个目录是 git 仓库，把资源切到 `worktree`（“并行”）就没有队列了：每条 task 拿到自己的 worktree，并以分支形式交回结果，谁都不用等谁。见[项目资源](/project-resources#task-如何共享这个目录)。

否则通常只需等前一条 task 结束。若前一条已经卡住，可以从它的执行日志中停止；也可以为当前智能体选择其他本地目录。这个目录互斥由守护进程在内存中维护，不写磁盘锁文件；怀疑锁状态异常时运行 `patchbay daemon restart` 即可释放，没有需要手动删除的文件。
...
先在执行电脑的终端中直接运行同一款工具。工具自身也无法开始时，先修复工具登录或配置，再从执行日志重试 task。

## 实时更新失效

task 仍能执行、但评论和状态不能实时出现，通常是 WebSocket 没有连通。
...
仍然无法定位时，到 [GitHub 任务](https://github.com/patchbay-ai/patchbay/issues) 搜索已有问题或提交新的任务。

## 接下来

- [守护进程与运行时](/daemon-runtimes) — 运行时注册与在线状态机制。
- [Task](/tasks) — 状态、超时与失败原因参考。
