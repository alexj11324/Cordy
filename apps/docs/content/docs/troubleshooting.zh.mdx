...
自托管部署还需要从**执行电脑**请求 API 的 `/health`——服务器本机测试无法暴露执行电脑侧的 DNS、TLS 或防火墙问题。需要修改地址时，重新运行 `orvilo setup self-host`，或检查当前 profile 的 `server_url`：
...
如果运行时列表缺少预期工具，先在同一个系统账号和 `PATH` 中确认工具可以运行并且已经登录，再执行 `orvilo daemon restart`。
...
否则通常只需等前一条 task 结束。若前一条已经卡住，可以从它的执行日志中停止；也可以为当前智能体选择其他本地目录。这个目录互斥由守护进程在内存中维护，不写磁盘锁文件；怀疑锁状态异常时运行 `orvilo daemon restart` 即可释放，没有需要手动删除的文件。
...
| 组件 | 查看方式 |
| --- | --- |
| 后台守护进程 | `orvilo daemon logs --lines 100` |
| 实时跟随守护进程日志 | `orvilo daemon logs --follow` |
| 默认 profile 的日志文件 | `~/.orvilo/daemon.log` |
| 默认 profile 的启动或崩溃日志 | `~/.orvilo/daemon.err.log` |
| 命名 profile | `~/.orvilo/profiles/<name>/` 下的对应日志 |
| Docker backend | `docker compose -f docker-compose.selfhost.yml logs -f backend` |
| 浏览器 | 开发者工具中的 Console 和 Network |

哪个文件是当前活跃的日志，取决于守护进程启动时使用的 profile；早前守护进程残留的旧日志同样能正常读出来，最容易让人调错文件。不要靠猜去开文件：`orvilo daemon logs` 会在输出日志内容前先打印它解析到的绝对路径。读命名 profile 的日志时加上 `--profile <name>`。
