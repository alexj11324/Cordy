---
title: Issues
description: An issue holds one piece of work's context, executor, status, and execution history.
---
...
| **Executor** | A workspace member, an agent, or a team. |
...
| **Activity and Agent thread** | Comments, status changes, tasks, and results returned by agents. |

![Issue detail: description, discussion, properties on the right, and the shared Agent thread](/images/docs/quickstart-run-result.webp)

## Create an issue

Create one from the **Issues** page or inside a project — a title is enough, and every other property can be filled in later.

Each issue has a number, such as `PB-123`. The digits increment within the workspace; after an admin changes the workspace's issue prefix, numbers display with the new prefix. See [Workspaces](/workspaces#create-a-workspace).
...
| **Team** | The team leader receives it and decides who handles it. |

When assigned to an agent or a team, the issue enqueues immediately unless it is in `backlog`; if the runtime is offline, the task waits in the queue. Archived agents and teams cannot be assigned.

Assignment does not bypass an agent's Access; when assigning to a team, the leader's Access is what gets checked. See [Assign issues to agents](/assigning-issues).
...
So a `Code Review` status in the `in_review` category ends an automation run exactly like `in_review` does, and moving an issue out of `backlog` into a `Rework` status in the `todo` category starts the assigned agent exactly like moving it to `todo` would.
...
Sub-issues can be given a **stage** to advance in batches — 1, 2, 3. When every sub-issue in the earliest unfinished stage reaches `done` or `cancelled`, the parent issue receives a "sub-issues completed" notification; when the parent's executor is an agent, it wakes up and decides whether to start the next stage. Sub-issues without a stage count as one batch and notify once when they all finish.

## Switch views

The Issues page offers five views — list, board, table, Gantt, and swimlane — filterable and sortable by status, executor, project, and other properties. They all show the same issues.
