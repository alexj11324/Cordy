...
Changing an issue's assignee or status does not stop a run that has already started. To interrupt one, cancel the corresponding task from the agent's activity tab. Active tasks are cancelled along with an issue only when the issue is deleted.
</Callout>
...
Clicking retry on a failed or cancelled task invokes **the agent that ran that task at the time**. Even if the issue has since been reassigned, the retry does not switch to the new assignee.
