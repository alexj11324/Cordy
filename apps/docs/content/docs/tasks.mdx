...
Codex has a separate, same-task recovery path for confirmed model capacity errors and temporary rate limits. After Codex finishes its own retries and its process has exited, the daemon waits 15 seconds, 30 seconds, 1 minute, 2 minutes, then 5 minutes between further attempts. There is no attempt limit while the task remains active. The transcript shows each wait; stopping the task cancels recovery. Recovery keeps the original conversation and working directory and refuses to start a replacement conversation if resume fails. A successful connection alone does not complete the task.

Codex's `usageLimitExceeded` and `sessionBudgetExceeded` stop recovery immediately. A bare HTTP 429, an unknown error, or an ordinary 503 without a confirmed capacity error does not enable this path. Existing network-failure retries remain unchanged. This recovery currently applies to the Codex adapter; other ACP adapters retain their existing behavior.

An Automation's **run only** mode does not create retry tasks automatically, to avoid overlapping with the next scheduled run. The **create issue** mode produces regular issue tasks, so infrastructure failures still retry under the rules above. Codex capacity recovery stays inside the existing run in either mode. Both modes show their final result in the Automation run history.
...
| `provider_capacity_or_rate_limit` | Rate limited or out of capacity (429/529) | Confirmed Codex capacity errors recover automatically; other cases require a manual retry |
