...
An issue holds the goal, discussion, executor, and final status of a piece of work; a task records one agent run against that work.
...
When an agent is working on an issue, a **Working** card appears in the activity thread — the same place as comments. It shows who is working, a live status, and a nested reply so you can keep talking while the run continues. Open the **agent conversation** from the card (or from the agent's activity row) to see the full thread. If the agent is still working, your message opens a Side Chat.

The timeline updates over the realtime connection while the page stays open — new comments, status changes, and Working cards appear without a manual reload.

From here you can:

- Open the **agent conversation** from an activity row to keep chatting on that issue. If the agent is still working, your message opens a Side Chat.
- Stop tasks that are deferred, queued, starting, waiting for a local directory, or running.
- Retry a failed or cancelled task.

![Working agent card in the issue timeline](/images/docs/quickstart-run-result.webp)

<Callout type="warning">
Changing an issue's executor or status does not stop a run that has already started. To interrupt one, cancel the corresponding task from the agent's activity tab. Active tasks are cancelled along with an issue only when the issue is deleted.
</Callout>
...
The failure reasons shown in agent activity and usage stats fall into two groups: unprefixed reason codes are recorded by the platform; `agent_error.*` codes are classified from the AI coding tool's own errors.
...
Clicking retry on a failed or cancelled task invokes **the agent that ran that task at the time**. Even if the issue has since been reassigned, the retry does not switch to the new executor.
