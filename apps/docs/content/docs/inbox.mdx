...
- an issue you subscribe to gets a new comment, or its executor, status, priority, or dates change;
...
Changing the executor does not remove existing subscriptions. Subscribe or unsubscribe anytime on the issue page; after unsubscribing, ordinary updates stop reaching your inbox, while direct @-mentions still notify you individually.
...
The inbox is only for workspace members. Even when an agent is the executor or a subscriber, it never reads inbox notifications.

@-mentioning an agent in a comment, assigning an issue to it, or triggering an automation creates a run directly instead of leaving the agent a notification. `@all` notifies members only.
