...
- whether each target comes from a direct @-mention, the issue assignee, or a team leader;
...
3. A top-level comment that matches neither case goes to the issue's agent assignee; when the assignee is a team, the leader handles it. A plain reply directly to a member does not fall back to the assignee; @-mention an agent or team explicitly when you want one to join.

When a comment explicitly @-mentions other targets (including members), the assignee fallback no longer applies.

When a reply triggers an agent other than the assignee, Patchbay schedules a delayed fallback run for the assignee: if the target agent replies within about 5 minutes, the fallback is cancelled automatically; otherwise the assignee starts a run too.
