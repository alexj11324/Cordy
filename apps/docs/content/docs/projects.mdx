...
The lead can be a member or an agent, and marks who coordinates the project. It is not a permission setting: it does not automatically set an executor on the project's issues, and making an agent the lead does not automatically run it. Starting a run still takes an executor, an @mention, or an automation trigger.
