...
1. **Dedicated Unix user.** Create a `orvilo` user, give it only the repositories and credentials the agents need, and run the daemon as that user. Your own account stays untouched.
...
- **Per-task working directory.** Each task gets its own workdir under `~/orvilo_workspaces/`, so concurrent tasks do not collide on the same checkout.
...
To confirm the effective Codex configuration, read the managed block in the task's `config.toml` under the task's `CODEX_HOME` — the section between the `# BEGIN orvilo-managed` and `# END orvilo-managed` markers is written by the daemon on every run.
