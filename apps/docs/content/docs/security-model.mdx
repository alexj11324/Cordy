...
- **Task-scoped API tokens.** The `ORVILO_TOKEN` handed to a task is bound to that agent and that task by the server, so a task cannot act as you or as another agent through the Patchbay API.
