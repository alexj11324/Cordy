...
1. 在托管的 `accounts` 服务中配置 Google provider。
2. 把 broker 回调地址加入 **Authorized redirect URIs**：

   ```text
   https://accounts.example.com/oauth/google/callback
   ```

3. Desktop 登录只使用 broker 的 `/oauth/google` 与
   `/oauth/google/callback`。broker 只通过 `patchbay://auth/callback` 返回
   一次性 PKCE code，Rust API 在 `/api/desktop-handoff/redeem` 兑换它。
...
浏览器登录默认保持 30 天。可以用 `AUTH_TOKEN_TTL` 调整，支持 duration 或正整数秒数：
