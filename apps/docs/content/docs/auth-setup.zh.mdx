...
不要在公网实例中启用固定验证码。生产环境的组合是 `APP_ENV=production` 加空的 `ORVILO_DEV_VERIFICATION_CODE`。
</Callout>
...
`email` 默认取 `ORVILO_DEV_EMAIL`（再回退到 `dev@localhost`）；`redirect` 只接受同源路径；除非传 `onboarding=keep`，否则会直接标记引导完成。在本仓库中，`make dev-login` 会打印该链接、bearer token 和要打开的工作区。

<Callout type="warning">
绝不要在公网实例上设置 `ORVILO_DEV_LOGIN`：任何知道邮箱地址的人都能免验证码登录为该用户。`APP_ENV=production` 时它会被忽略，且只有该变量为 `1` 时后端才会注册这个路由。
</Callout>
