---
title: Telegram Bot
description: Connect a Cordy agent to your own Telegram Bot for private chats, group mentions, topics, and /issue.
---

import { Callout } from "fumadocs-ui/components/callout";

Cordy uses a Bot that you create with Telegram's official [@BotFather](https://t.me/BotFather). One Bot maps to one Cordy agent. Create a separate Bot for each agent that needs its own Telegram identity.
...
- A Cordy workspace owner or admin must connect the Bot.
...
4. Keep **Group Privacy** enabled. Cordy only needs commands, explicit @-mentions, and replies to the Bot in groups.
...
1. In Cordy, open **Agents**, select an agent, and open **Integrations**.
2. Click **Connect Telegram**.
3. Paste the Bot token and click **Connect**.

Cordy calls Telegram to verify the Bot, checks that no outgoing webhook conflicts with long polling, encrypts the token, and starts one supervised `getUpdates` connection. A Bot already connected to another agent or workspace must be disconnected there first.

## First use and account binding

The first time a member messages the Bot, it sends a single-use Cordy account-binding link. Open it, sign in to the same Cordy workspace, then return to Telegram and send the message again. The link expires after 15 minutes; message the Bot again to get a new link.
...
- `/issue <title>` creates a Cordy issue; following lines are used as its optional description. `/issue` without a title returns usage guidance.
...
Open **Settings → Integrations → Telegram** to view connected Bots. Owners and admins can disconnect one. Disconnecting stops long polling and outbound replies but preserves Cordy conversations and audit records.
...
- **409 polling conflict**: another Cordy instance or process is polling the same Bot. Stop the other consumer or use a separate Bot per environment.
