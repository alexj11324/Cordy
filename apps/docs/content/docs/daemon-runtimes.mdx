...
description: How Patchbay connects computers, discovers AI coding tools, and executes tasks.
---

import { Callout } from "fumadocs-ui/components/callout";

Patchbay records and coordinates the work; connected computers execute it. The **daemon** on a computer claims tasks and invokes the AI coding tools installed on that machine.

## Daemon vs runtime

- The **daemon** is the Patchbay background process running on one computer. It connects to the server, discovers local tools, claims tasks, and reports results back.
...
The AI coding tools a local runtime invokes, those tools' own login credentials, and your local code directories all stay on the connected computer. The Patchbay server does not execute commands on behalf of local tools, and it does not automatically upload your entire working directory.
...
With Patchbay Desktop, the app starts the daemon automatically — no extra command needed.

On the web, a remote computer, or a headless environment, [install the Patchbay CLI](/cli) first, then run:
...
| Command | Purpose |
| --- | --- |
| `patchbay daemon status` | Show daemon and connection status |
| `patchbay daemon logs -f` | Follow the logs |
| `patchbay daemon restart` | Restart the daemon and re-detect local tools |
| `patchbay daemon stop` | Stop the daemon |
| `patchbay daemon start --foreground` | Run in the current terminal, for debugging |

To place task workspaces on another disk, persist a root for the current profile with `patchbay config set workspaces_root <path>`, or pass `--workspaces-root <path>` to `daemon start` or `daemon restart`. The flag overrides `PATCHBAY_WORKSPACES_ROOT`, which overrides the profile config. Existing task directories are not moved when the root changes.
...
- Tasks already queued wait for the runtime to recover. They fail only once it has stopped heartbeating for longer than the reconnect grace **and** the task has itself been queued that long, so a runtime that is merely busy keeps its backlog, and work routed to an already-offline runtime still gets a full grace to wait.
...
Once a limit is reached, new tasks keep queueing. You can adjust a single agent's concurrency in the agent settings, and the machine-wide cap through `PATCHBAY_DAEMON_MAX_CONCURRENT_TASKS`. Parallel runs compete for machine capacity, tool account quota, and the same working directory all at once.
...
A custom profile does not add a new communication protocol. You still pick one of the protocol families Patchbay already supports (the tool's integration protocol type; see [AI coding tools comparison](/providers)), and the command itself must be compatible with that family.
...
Everything you type in the command field stays directly after the executable, ahead of the arguments Patchbay adds:

```text
<your command> <your fixed arguments> <Patchbay's protocol arguments> <the agent's custom arguments>
```

This is what makes a subcommand-style wrapper work. If your command is `ccms start q36`, the tool sees `start q36` first and can select its subcommand before Patchbay's `-p` and the rest arrive — which is the only order such a wrapper accepts.
...
- **Patchbay's own values win a conflict.** If your fixed arguments set a flag Patchbay also sets, Patchbay's value comes later and takes effect. Most importantly, a model chosen on the agent overrides a `--model` pinned in the profile. To pin a model for everyone, leave the agents' model field empty.
...
```bash
patchbay runtime profile set-path <profile-id> --path /absolute/path/to/command
```

Remove the path override:

```bash
patchbay runtime profile unset-path <profile-id>
```
...
1. Run `patchbay daemon status` to confirm the daemon is running.
2. Run `patchbay daemon logs -f` to look for login, network, or tool-detection errors.
3. Run `command -v <tool-command>` in the same environment to confirm the daemon can find it.
4. Open Patchbay's **Runtimes** page and check whether the target computer and the matching AI coding tool show as online.
5. After a tool install, a path change, or a profile update, run `patchbay daemon restart`.
