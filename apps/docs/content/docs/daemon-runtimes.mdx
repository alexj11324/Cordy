...
To place task workspaces on another disk, persist a root for the current profile with `patchbay config set workspaces_root <path>`, or pass `--workspaces-root <path>` to `daemon start` or `daemon restart`. The flag overrides `ORVILO_WORKSPACES_ROOT`, which overrides the profile config. Existing task directories are not moved when the root changes.
...
Once a limit is reached, new tasks keep queueing. You can adjust a single agent's concurrency in the agent settings, and the machine-wide cap through `ORVILO_DAEMON_MAX_CONCURRENT_TASKS`. Parallel runs compete for machine capacity, tool account quota, and the same working directory all at once.
