...
| Command | Purpose |
| --- | --- |
| `orvilo daemon status` | Show daemon and connection status |
| `orvilo daemon logs -f` | Follow the logs |
| `orvilo daemon restart` | Restart the daemon and re-detect local tools |
| `orvilo daemon stop` | Stop the daemon |
| `orvilo daemon start --foreground` | Run in the current terminal, for debugging |

To place task workspaces on another disk, persist a root for the current profile with `orvilo config set workspaces_root <path>`, or pass `--workspaces-root <path>` to `daemon start` or `daemon restart`. The flag overrides `ORVILO_WORKSPACES_ROOT`, which overrides the profile config. Existing task directories are not moved when the root changes.
...
```bash
orvilo runtime profile set-path <profile-id> --path /absolute/path/to/command
```

Remove the path override:

```bash
orvilo runtime profile unset-path <profile-id>
```
...
1. Run `orvilo daemon status` to confirm the daemon is running.
2. Run `orvilo daemon logs -f` to look for login, network, or tool-detection errors.
...
5. After a tool install, a path change, or a profile update, run `orvilo daemon restart`.
