...
description: How Orvilo connects computers, discovers AI coding tools, and executes tasks.
---

import { Callout } from "fumadocs-ui/components/callout";

Orvilo records and coordinates the work; connected computers execute it. The **daemon** on a computer claims tasks and invokes the AI coding tools installed on that machine.

## Daemon vs runtime

- The **daemon** is the Orvilo background process running on one computer. It connects to the server, discovers local tools, claims tasks, and reports results back.
...
The AI coding tools a local runtime invokes, those tools' own login credentials, and your local code directories all stay on the connected computer. The Orvilo server does not execute commands on behalf of local tools, and it does not automatically upload your entire working directory.
...
With Orvilo Desktop, the app starts the daemon automatically — no extra command needed.

On the web, a remote computer, or a headless environment, [install the Orvilo CLI](/cli) first, then run:
...
A custom profile does not add a new communication protocol. You still pick one of the protocol families Orvilo already supports (the tool's integration protocol type; see [AI coding tools comparison](/providers)), and the command itself must be compatible with that family.
...
Everything you type in the command field stays directly after the executable, ahead of the arguments Orvilo adds:

```text
<your command> <your fixed arguments> <Orvilo's protocol arguments> <the agent's custom arguments>
```

This is what makes a subcommand-style wrapper work. If your command is `ccms start q36`, the tool sees `start q36` first and can select its subcommand before Orvilo's `-p` and the rest arrive — which is the only order such a wrapper accepts.
...
- **Orvilo's own values win a conflict.** If your fixed arguments set a flag Orvilo also sets, Orvilo's value comes later and takes effect. Most importantly, a model chosen on the agent overrides a `--model` pinned in the profile. To pin a model for everyone, leave the agents' model field empty.
...
4. Open Orvilo's **Runtimes** page and check whether the target computer and the matching AI coding tool show as online.
