...
DeepSeek Harness のモデルカタログはランタイム自身が提供します（`dsh --profile orvilo --list-models`）。モデル ID は `provider/model` 形式（例: `deepseek-official/deepseek-chat`）なので、一覧から完全な ID を選択してください。
