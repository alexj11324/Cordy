...
A chat is **not bound to any issue**: the agent doesn't automatically receive an issue's description and comments, but it can still use the Patchbay CLI to query workspaces, projects, issues, and skills within its permissions.
...
![A chat with an agent: a question answered with the result organized into a table](/images/docs/chat-conversation.webp)
...
4. After you send, Patchbay creates a run for the message, and the reply comes back to the current conversation.
...
Before the first message, Chat shows up to three suggestions configured for the selected agent. Selecting one copies its full prompt into the composer without sending it, so you can adapt it first. Agents without configured suggestions use localized general-purpose examples.
...
| Clear owner, executor, reviewer, status, priority, and deliverable | **Issue** |
...
Direct chats created in Patchbay are visible only to their creator; other workspace members and admins cannot read them. Conclusions the team needs to see together should be written into an issue, a project description, or a skill.

## Multi-turn context

A conversation tries to continue the original session in the AI coding tool, so later messages don't need to repeat earlier content. Patchbay stores the session ID and routes subsequent runs to a runtime that can access that session.
