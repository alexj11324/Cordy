...
Orvilo signs users in with email verification codes by default; Google OAuth can be added on top. Existing users can always sign in again — signup restrictions only decide whether new accounts can be created.

## Email verification codes

After the user enters an email address, Orvilo sends a 6-digit verification code. The code is valid for 10 minutes; once it verifies, the browser receives a sign-in cookie.
...
2. Add the Orvilo frontend's callback URL to **Authorized redirect URIs**:
...
- If the invitee already has an Orvilo account, they can sign in and accept the invitation.
