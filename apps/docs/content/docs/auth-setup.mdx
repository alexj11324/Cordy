...
1. Configure Google as a provider in the managed `accounts` service.
2. Register the broker callback at **Authorized redirect URIs**:

   ```text
   https://accounts.example.com/oauth/google/callback
   ```

3. Keep the desktop flow on the broker's `/oauth/google` entry and
   `/oauth/google/callback` callback. The broker returns only a one-time
   PKCE-bound code through `patchbay://auth/callback`; the Rust API redeems it
   at `/api/desktop-handoff/redeem`.
...
Browser sign-ins last 30 days by default. Adjust with `AUTH_TOKEN_TTL`, which accepts a duration or a positive integer of seconds:
