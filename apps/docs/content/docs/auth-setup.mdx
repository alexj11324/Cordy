...
Patchbay signs users in with email verification codes by default; Google OAuth can be added on top. Existing users can always sign in again — signup restrictions only decide whether new accounts can be created.

## Email verification codes

After the user enters an email address, Patchbay sends a 6-digit verification code. The code is valid for 10 minutes; once it verifies, the browser receives a sign-in cookie.
...
Do not enable a fixed code on a publicly reachable instance. The production combination is `APP_ENV=production` with an empty `PATCHBAY_DEV_VERIFICATION_CODE`.
</Callout>

## Skipping the login page in development

A fixed code still costs a code request and a form. To sign in with neither:
...
The backend then serves `/auth/dev-login`:

- `GET /auth/dev-login?email=dev@localhost&redirect=/dev/issues` sets the session cookie and redirects into the web app, so opening the URL is the login.
- `POST /auth/dev-login` with `{"email":"dev@localhost"}` returns the same `{ token, user }` as `verify-code`, for curl and scripts.

`email` defaults to `PATCHBAY_DEV_EMAIL` (then `dev@localhost`), `redirect` accepts a same-origin path only, and onboarding is marked complete unless you pass `onboarding=keep`. In this repository, `make dev-login` prints the URL, the bearer token and the workspace to open.

<Callout type="warning">
Never set `PATCHBAY_DEV_LOGIN` on a publicly reachable instance: anyone who knows an email address can sign in as that user with no code. It is ignored when `APP_ENV=production`, and the route is not registered at all unless the variable is `1`.
</Callout>

## Google sign-in

1. Create an OAuth 2.0 client in the [Google Cloud Console](https://console.cloud.google.com/).
2. Add the Patchbay frontend's callback URL to **Authorized redirect URIs**:

   ```text
   https://patchbay.example.com/auth/callback
   ```
...
3. Set:
   ...
   GOOGLE_REDIRECT_URI=https://patchbay.example.com/auth/callback
   ```
...
- If the invitee already has a Patchbay account, they can sign in and accept the invitation.
