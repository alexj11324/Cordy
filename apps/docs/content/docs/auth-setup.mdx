...
Do not enable a fixed code on a publicly reachable instance. The production combination is `APP_ENV=production` with an empty `ORVILO_DEV_VERIFICATION_CODE`.
</Callout>
...
`email` defaults to `ORVILO_DEV_EMAIL` (then `dev@localhost`), `redirect` accepts a same-origin path only, and onboarding is marked complete unless you pass `onboarding=keep`. In this repository, `make dev-login` prints the URL, the bearer token and the workspace to open.

<Callout type="warning">
Never set `ORVILO_DEV_LOGIN` on a publicly reachable instance: anyone who knows an email address can sign in as that user with no code. It is ignored when `APP_ENV=production`, and the route is not registered at all unless the variable is `1`.
</Callout>
