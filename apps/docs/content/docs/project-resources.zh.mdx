...
- **结果是你仓库里的一个分支**，命名为 `agent/<agent>/<task>`。执行记录的**运行详情**面板里会显示分支名（可直接复制），也可以用 `git branch` 找到它。用 `git log` / `git diff` review，然后自己决定 merge 还是 cherry-pick。Patchbay 不会替你合并。中途失败的任务同样会报告它的分支，因为它已经把智能体做出来的部分提交进去了。
