...
First work out which layer the problem lives in: the Patchbay service, the daemon, the runtime, or the AI coding tool. These commands are usually enough to surface the first meaningful error:

```bash
patchbay version
patchbay auth status
patchbay daemon status --output json
patchbay daemon logs --lines 100
```
...
`/health` only says the API process is responding; `/readyz` checks whether the database is reachable. The Rust entrypoint runs migrations before serving, so a migration failure prevents readiness. Include the error, relevant logs, CLI version, and operating system when reporting a problem; remove tokens, email addresses, and other sensitive details before submitting.
...
- The daemon is connected to the wrong Patchbay service.
...
Self-hosted deployments should also request the API's `/health` from the **executing computer** — testing on the server itself cannot expose DNS, TLS, or firewall problems on the executing computer's side. To change the address, rerun `patchbay setup self-host`, or check the current profile's `server_url`:
...
Open the issue's **Agent thread** and check the task's current status and what it is waiting for.
...
If the runtime list is missing an expected tool, first confirm the tool runs and is signed in under the same system account and `PATH`, then run `patchbay daemon restart`.

### `waiting_local_directory` status

This means another in-flight task is using the same local directory. Patchbay waits for the directory lock to be released so two agents never modify the same files at once.
...
Otherwise you just wait for the earlier task to finish. If it is stuck, stop it from its Agent thread, or pick a different local directory for the current agent. This directory mutex lives in the daemon's memory — no lock file is written to disk. If you suspect stale lock state, `patchbay daemon restart` releases it; there is nothing to delete manually.

## AI coding tool fails to start

An online daemon does not mean the tool itself works. Open the run's Agent thread and focus on:
...
Run the same tool directly in a terminal on the executing computer first. If the tool cannot start on its own, fix its sign-in or configuration, then continue the task from the Agent thread.
...
When `SMTP_HOST` and Resend are both configured, Patchbay prefers SMTP. For configuration, see [Sign-in and signup](/auth-setup#email-verification-codes).
...
If the backend still fails to start with `refusing to drop legacy daily rollups`, the automatic backfill did not complete (for example, it failed partway, or the SQL was applied directly instead of through `migrate up`). With PostgreSQL running, use the binary already shipped in the backend image. Override the normal entrypoint because that entrypoint exits on the failed migration:
...
A source checkout is an alternative only when a Rust toolchain is installed:

```bash
make rust-build
DATABASE_URL='postgres://...' ./bin/backfill_task_usage_hourly
```

Useful flags: `--dry-run` previews without writing; `--sleep-between-slices` adds a pause between slices to reduce read pressure on a busy instance. The command works in monthly slices and is idempotent — rerun it directly after an interruption. It holds an advisory lock and is mutually exclusive with the server's scheduled rollup, so it cannot produce duplicate or inconsistent rollup data. When it finishes, restart the backend and confirm `/readyz` returns HTTP 200 with `{"status":"ready"}`. The handler only checks database reachability; successful server startup separately confirms that the entrypoint migration completed.
...
If it is another Patchbay checkout, run `make stop` in that directory first. Otherwise stop the offending program normally, or change the current service's port. Public ports 80/443 are listened on by a reverse proxy such as Caddy or Nginx.

## Log locations

| Component | How to view |
| --- | --- |
| Background daemon | `patchbay daemon logs --lines 100` |
| Follow daemon logs live | `patchbay daemon logs --follow` |
| Default profile's log file | `~/.patchbay/daemon.log` |
| Default profile's startup or crash log | `~/.patchbay/daemon.err.log` |
| Named profiles | the matching logs under `~/.patchbay/profiles/<name>/` |
...
If you still cannot pin it down, search existing issues or file a new one on [GitHub Issues](https://github.com/patchbay-ai/patchbay/issues).
