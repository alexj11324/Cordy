...
First work out which layer the problem lives in: the Orvilo service, the daemon, the runtime, or the AI coding tool. These commands are usually enough to surface the first meaningful error:
...
- The daemon is connected to the wrong Orvilo service.
...
This means another in-flight task is using the same local directory. Orvilo waits for the directory lock to be released so two agents never modify the same files at once.
...
When `SMTP_HOST` and Resend are both configured, Orvilo prefers SMTP. For configuration, see [Sign-in and signup](/auth-setup#email-verification-codes).
...
If it is another Orvilo checkout, run `make stop` in that directory first. Otherwise stop the offending program normally, or change the current service's port. Public ports 80/443 are listened on by a reverse proxy such as Caddy or Nginx.
