...
Self-hosted deployments should also request the API's `/health` from the **executing computer** — testing on the server itself cannot expose DNS, TLS, or firewall problems on the executing computer's side. To change the address, rerun `orvilo setup self-host`, or check the current profile's `server_url`:
...
If the runtime list is missing an expected tool, first confirm the tool runs and is signed in under the same system account and `PATH`, then run `orvilo daemon restart`.
...
## `orvilo setup` reports the server is not reachable
...
| --- | --- |
| Background daemon | `orvilo daemon logs --lines 100` |
| Follow daemon logs live | `orvilo daemon logs --follow` |
| Default profile's log file | `~/.orvilo/daemon.log` |
| Default profile's startup or crash log | `~/.orvilo/daemon.err.log` |
| Named profiles | the matching logs under `~/.orvilo/profiles/<name>/` |
