...
description: ワークスペースごとにセルフホストの Forgejo、Gitea、GitLab インスタンスを接続し、タスク番号を参照する Pull Request / Merge Request を対応するタスクへ自動的に紐づけ、マージ時に「完了」へ移動して CI ステータスを表示します。
---
...
Patchbay はワークスペースごとにセルフホストの Git ホスティング、**Forgejo**、**Gitea**、**GitLab** のいずれかへ接続します。接続後、ブランチ名、タイトル、本文にタスク番号（例: `MUL-123`）を含む Pull Request（GitLab では Merge Request）は、対応する[タスク](/issues)へ**自動的に紐づけられ**、タスクサイドバーの **Pull requests** に表示されます。終了キーワードを含む PR がマージされると、タスクは「完了」へ移動します。head commit の CI はカード上のチェックバーとして表示されます。
...
- **タスクの紐づけ** — タイトル、本文、ブランチの番号で PR をタスクへ紐づけます。マージ済み PR に終了キーワード（`Closes`/`Fixes`/`Resolves MUL-123`）があり、open の関連 PR が残っていなければ、タスクを「完了」へ移動します。
