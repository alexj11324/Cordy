...
Patchbay はワークスペースごとにセルフホストの Git ホスティング、**Forgejo**、**Gitea**、**GitLab** のいずれかへ接続します。接続後、ブランチ名、タイトル、本文にイシュー番号（例: `PB-123`）を含む Pull Request（GitLab では Merge Request）は、対応する[イシュー](/issues)へ**自動的に紐づけられ**、イシューサイドバーの **Pull requests** に表示されます。終了キーワードを含む PR がマージされると、イシューは「完了」へ移動します。head commit の CI はカード上のチェックバーとして表示されます。
...
- **イシューの紐づけ** — タイトル、本文、ブランチの番号で PR をイシューへ紐づけます。マージ済み PR に終了キーワード（`Closes`/`Fixes`/`Resolves PB-123`）があり、open の関連 PR が残っていなければ、イシューを「完了」へ移動します。
