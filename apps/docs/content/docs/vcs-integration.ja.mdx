...
<Callout type="warn">
**セルフホスト版 Orvilo 専用です。** この連携は Orvilo を**自分でデプロイしている**場合のみ利用でき、Orvilo Cloud では提供されません。ここでの「セルフホスト」は **Orvilo 自体**のセルフホストを意味します。通常は独自ネットワーク内の Git インスタンスへアクセスするために使用します。サーバー運用者が `ORVILO_VCS_INTEGRATION_ENABLED=true` を設定して機能を有効にし、`ORVILO_VCS_SECRET_KEY` も設定する必要があります。それまでは、設定 → 連携にこのセクションは表示されません。
</Callout>

Orvilo はワークスペースごとにセルフホストの Git ホスティング、**Forgejo**、**Gitea**、**GitLab** のいずれかへ接続します。接続後、ブランチ名、タイトル、本文にタスク番号（例: `MUL-123`）を含む Pull Request（GitLab では Merge Request）は、対応する[タスク](/issues)へ**自動的に紐づけられ**、タスクサイドバーの **Pull requests** に表示されます。終了キーワードを含む PR がマージされると、タスクは「完了」へ移動します。head commit の CI はカード上のチェックバーとして表示されます。
...
`ORVILO_PUBLIC_URL` にサーバーの公開ベース URL を設定すると、Orvilo がそのまま貼り付けられる Webhook URL を表示できます。設定しない場合、画面には Webhook のパスだけが表示されるため、origin を自分で補います。
...
2. Orvilo で **設定 → 連携 → Git ホスティング**を開きます。
3. **プロバイダー**を選択し、インスタンス URL（例: `https://forgejo.example.com`）とアクセストークンを入力して **接続**をクリックします。Orvilo は保存前に、そのトークンでインスタンスへアクセスして検証します。
...
Orvilo は保存済みの secret で各 delivery を検証するため、一致する secret がない Webhook は拒否されます。
