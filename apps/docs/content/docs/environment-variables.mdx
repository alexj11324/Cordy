...
```dotenv
DATABASE_URL=postgres://user:password@postgres:5432/orvilo?sslmode=require
JWT_SECRET=<long-random-secret>
APP_ENV=production
FRONTEND_ORIGIN=https://orvilo.example.com
ORVILO_APP_URL=https://orvilo.example.com
ORVILO_PUBLIC_URL=https://api.orvilo.example.com
```
...
|---|---|---|
| `DATABASE_URL` | local `orvilo` database | PostgreSQL connection string |
...
| `ORVILO_DAEMON_SERVER_URL` | falls back to `ORVILO_PUBLIC_URL`, then `ORVILO_APP_URL` / `FRONTEND_ORIGIN` | Server URL the API process inserts into `orvilo setup self-host`; set it when daemons reach the API at a different URL than the public webhook URL |
...
| `ORVILO_DAEMON_AUTO_RELOAD` | `true` | Whether to restart into a `orvilo` binary replaced on disk out of band (`brew upgrade`, a re-download, a local build). Independent of `ORVILO_DAEMON_AUTO_UPDATE` |
| `ORVILO_WORKSPACES_ROOT` | `~/orvilo_workspaces` | Root directory for task working directories |
...
Precedence is command-line flag → environment variable → `~/.orvilo/config.json` → built-in default. For watchdog behavior, see [Daemon and runtimes](/daemon-runtimes).

### Persisting daemon configuration

Common daemon-side settings can also be written to `~/.orvilo/config.json` instead of relying on shell environment variables; named profiles keep theirs at `~/.orvilo/profiles/<name>/config.json`:

```bash
orvilo config set poll_interval 10s
orvilo config show
```
...
- Passing an empty string clears a persisted value and falls back to the env var or built-in default, for example `orvilo config set poll_interval ""`.
