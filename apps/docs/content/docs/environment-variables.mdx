...
description: Common server, storage, integration, and runtime configuration for self-hosting Orvilo.
---

import { Callout } from "fumadocs-ui/components/callout";

Orvilo reads environment variables at process startup. After changing one, restart the affected API, web, or daemon process. Docker Compose's `docker compose restart` does not re-read `.env`; recreate containers with `up -d` for changes to take effect.
...
When `S3_BUCKET` is unset, Orvilo uses local disk.
...
| Variable | Default | Description |
|---|---|---|
| `ORVILO_SERVER_URL` | `ws://localhost:8080/ws` | Orvilo API / WebSocket URL; also accepts `http(s)` |
...
|---|---|---|
| `server_url` | `ws://localhost:8080/ws` | Orvilo API / WebSocket URL |
