...
description: Common server, storage, integration, and runtime configuration for self-hosting Cordy.
---

import { Callout } from "fumadocs-ui/components/callout";

Cordy reads environment variables at process startup. After changing one, restart the affected API, web, or daemon process. Docker Compose's `docker compose restart` does not re-read `.env`; recreate containers with `up -d` for changes to take effect.
...
```dotenv
DATABASE_URL=postgres://user:password@postgres:5432/cordy?sslmode=require
JWT_SECRET=<long-random-secret>
APP_ENV=production
FRONTEND_ORIGIN=https://cordy.example.com
CORDY_APP_URL=https://cordy.example.com
CORDY_PUBLIC_URL=https://api.cordy.example.com
```
...
`JWT_SECRET` is required in production: with `APP_ENV=production` the backend refuses to boot if it is empty or a known placeholder (generate one with `openssl rand -hex 32`). Do not set `CORDY_DEV_VERIFICATION_CODE`.
</Callout>
...
|---|---|---|
| `DATABASE_URL` | local `cordy` database | PostgreSQL connection string |
...
| `CORDY_SHUTDOWN_HOLD_DURATION` | `0` | How long to wait after a termination signal before graceful shutdown begins |
| `CORDY_RUNTIME_RECONNECT_GRACE` | `3h` | How long an offline runtime may reconnect before its in-flight tasks fail; values below `150s` are clamped |
...
| `CORDY_APP_URL` | falls back to `FRONTEND_ORIGIN` | User-reachable web URL; CLI sign-in and account-link URLs use it |
| `CORDY_PUBLIC_URL` | empty | Public API URL; used for webhook URLs and runtime connection instructions |
...
| `RESEND_FROM_EMAIL` | `noreply@cordy.ai` | Sender address; must belong to a verified domain |
...
| `CORDY_DEV_VERIFICATION_CODE` | empty | Fixed 6-digit test code for non-production environments |

For the exact allowlist evaluation order, see [Sign-in and signup](/auth-setup#signup-restrictions).

## Attachment storage

When `S3_BUCKET` is unset, Cordy uses local disk.
...
| `CORDY_TRUSTED_PROXIES` | empty | Trusted proxy CIDRs for automation webhooks and realtime connections |
...
| Lark | `CORDY_LARK_SECRET_KEY` | Base64-encoded 32-byte credential encryption key |
| Slack | `CORDY_SLACK_SECRET_KEY` | Base64-encoded 32-byte token encryption key |
| Telegram | `CORDY_TELEGRAM_SECRET_KEY` | Base64-encoded 32-byte Bot token encryption key |
| Personal WeChat | `CORDY_WEIXIN_SECRET_KEY` | Base64-encoded 32-byte iLink token and conversation-context encryption key |
| Composio | `COMPOSIO_API_KEY` | Enables Composio tool connections |
| Composio | `COMPOSIO_CALLBACK_BASE_URL` | Callback API URL; can fall back to `CORDY_PUBLIC_URL` |
| Composio | `COMPOSIO_STATE_SECRET` | OAuth state signing secret; can be derived from `JWT_SECRET` |
| Self-hosted Git | `CORDY_VCS_INTEGRATION_ENABLED` | Forgejo/Gitea/GitLab integration switch; enabled by default in compose |
| Self-hosted Git | `CORDY_VCS_SECRET_KEY` | Base64-encoded 32-byte encryption key (`openssl rand -base64 32`); the feature is entirely unavailable without it |
...
| Variable | Default | Description |
|---|---|---|
| `CORDY_LLM_API_KEY` | empty | OpenAI-compatible API key |
| `CORDY_LLM_BASE_URL` | empty | OpenAI-compatible endpoint |
| `CORDY_LLM_DEFAULT_MODEL` | `gpt-5.6-luna` | Used when a request does not specify a model |
| `CORDY_LLM_MAX_RETRIES` | `2` | Retry ceiling per call; `0` disables retries, `1`–`5` cap it at N |

`CORDY_LLM_MAX_RETRIES` is the single source for the retry policy. Leave it unset for the default of 2, set `0` to send exactly one request per call, or set 1–5 to cap retries at that many. It is a ceiling, not a quota: only retryable failures consume it, and a success or the caller's own deadline can end the call sooner. Any other value — negative, non-numeric, or above 5 — fails startup instead of being corrected silently. The ceiling is a latency budget: backoff starts at 0.5s and doubles to an 8s cap, so a larger budget would outlast the callers' own deadlines and turn a retryable failure into a timeout. Retries cover connection failures and HTTP 408, 409, 429 and 5xx; all other 4xx responses are returned as-is. The server logs the effective policy at startup as `llm retry policy`, with no credentials in the line.
...
<Callout type="info">
This covers the assist layer only. Running an agent is a separate data path: when an agent answers a chat, your daemon executes that agent's AI coding tool using the tool's own credentials, and does not forward the `CORDY_LLM_*` settings above to it. (The task-scoped Cordy connection variables the agent needs are injected by the daemon separately.) Emptying the variables above does not affect that path — govern it through the agent's runtime configuration.
</Callout>
...
| Variable | Default | Description |
|---|---|---|
| `CORDY_SERVER_URL` | `ws://localhost:8080/ws` | Cordy API / WebSocket URL; also accepts `http(s)` |
| `CORDY_DAEMON_DEVICE_NAME` | hostname | Device name shown in the runtime list |
| `CORDY_AGENT_RUNTIME_NAME` | `Local Agent` | Runtime display name |
| `CORDY_DAEMON_POLL_INTERVAL` | `30s` | Task polling interval when no wake event arrives |
| `CORDY_DAEMON_HEARTBEAT_INTERVAL` | `15s` | Heartbeat interval |
| `CORDY_DAEMON_MAX_CONCURRENT_TASKS` | `20` | Concurrent task ceiling per daemon |
| `CORDY_AGENT_TIMEOUT` | `0` | Absolute time limit per run; `0` means no limit |
| `CORDY_AGENT_IDLE_WATCHDOG` | `30m` | Silence ceiling with no output and no tool execution |
| `CORDY_AGENT_TOOL_WATCHDOG` | `2h` | Silence ceiling for a single tool call |
| `CORDY_OPENCODE_IDLE_WATCHDOG` | `10m` | OpenCode-specific silence threshold |
| `CORDY_CODEX_SEMANTIC_INACTIVITY_TIMEOUT` | `10m` | Codex semantic-silence threshold |
| `CORDY_CODEX_FIRST_TURN_TIMEOUT` | `0` | Explicit override for the Codex first-turn no-progress ceiling; `0` keeps the default. The effective first-turn wait stays bounded by `CORDY_CODEX_SEMANTIC_INACTIVITY_TIMEOUT` and the overall execution timeout — set `CORDY_CODEX_SEMANTIC_INACTIVITY_TIMEOUT` strictly above this value (with some margin), or the wait is truncated to it and the model-catalog startup retry is skipped. Equal values are not enough: the semantic timer is armed first, so at equal durations the retry can still be lost |
| `CORDY_CODEX_HANDSHAKE_TIMEOUT` | `30s` | Codex app-server startup handshake ceiling |
| `CORDY_DAEMON_AUTO_UPDATE` | Cloud `true`; self-hosted `false` | Whether to check for and apply CLI updates automatically |
| `CORDY_DAEMON_AUTO_UPDATE_INTERVAL` | `6h` | Update check interval |
| `CORDY_DAEMON_AUTO_RELOAD` | `true` | Whether to restart into a `cordy` binary replaced on disk out of band (`brew upgrade`, a re-download, a local build). Independent of `CORDY_DAEMON_AUTO_UPDATE` |
| `CORDY_WORKSPACES_ROOT` | `~/cordy_workspaces` | Root directory for task working directories |
| `CORDY_AGENT_TEMP_BASE` | `/tmp` (Linux/macOS) | Linux/macOS only. Parent directory for private per-task temp dirs; must be an existing, writable absolute directory, and an invalid value fails task startup instead of falling back to `/tmp`. Pick a short path — child tools may bind AF_UNIX sockets under it, and `sun_path` is limited to 108 bytes on Linux and 104 on macOS |
| `CORDY_KEEP_ENV_AFTER_TASK` | `false` | Keep task directories for debugging |

Each AI coding tool accepts `CORDY_<PROVIDER>_PATH` and `CORDY_<PROVIDER>_MODEL` to override the command path and default model. QwenPaw and MiniMax Code have no model variable because Cordy never sends them a model; MiniMax Code's path variable is `CORDY_MCODE_PATH` — see [AI coding tools comparison](/providers#model-sources). DeepSeek Harness supports `CORDY_DSH_PATH` and `CORDY_DSH_MODEL` (a model id from the dsh catalog, e.g. `deepseek-official/deepseek-chat`). Dim supports `CORDY_DIM_PATH` and `CORDY_DIM_MODEL` (a model id from the dim catalog). Machine-wide default arguments via `CORDY_<PROVIDER>_ARGS` are currently supported for five tools: Claude Code, Codex, CodeBuddy, Qwen Code, and QwenPaw. The variables are `CORDY_CLAUDE_ARGS`, `CORDY_CODEX_ARGS`, `CORDY_CODEBUDDY_ARGS`, `CORDY_QWEN_ARGS`, and `CORDY_QWENPAW_ARGS`. For example:

```dotenv
CORDY_CLAUDE_PATH=/opt/bin/claude
CORDY_CLAUDE_ARGS=--max-turns 40
```

Precedence is command-line flag → environment variable → `~/.cordy/config.json` → built-in default. For watchdog behavior, see [Daemon and runtimes](/daemon-runtimes).

### Persisting daemon configuration

Common daemon-side settings can also be written to `~/.cordy/config.json` instead of relying on shell environment variables; named profiles keep theirs at `~/.cordy/profiles/<name>/config.json`:

```bash
cordy config set poll_interval 10s
cordy config show
```
...
|---|---|---|
| `server_url` | `ws://localhost:8080/ws` | Cordy API / WebSocket URL |
...
- Passing an empty string clears a persisted value and falls back to the env var or built-in default, for example `cordy config set poll_interval ""`.
