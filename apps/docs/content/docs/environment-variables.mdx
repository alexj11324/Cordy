...
| `RESEND_FROM_EMAIL` | `noreply@patchbay.ai` | Sender address; must belong to a verified domain |
...
| Plugins | `PATCHBAY_PLUGIN_API_URL` | Complete versioned Plugin Public API base URL, such as `https://plugin-api.example.com/v1`; falls back to `PATCHBAY_PUBLIC_URL` + `/v1` |
