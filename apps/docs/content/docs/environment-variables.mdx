...
description: Common server, storage, integration, and runtime configuration for self-hosting Patchbay.
---

import { Callout } from "fumadocs-ui/components/callout";

Patchbay reads environment variables at process startup. After changing one, restart the affected API, web, or daemon process. Docker Compose's `docker compose restart` does not re-read `.env`; recreate containers with `up -d` for changes to take effect.
...
```dotenv
DATABASE_URL=postgres://user:password@postgres:5432/patchbay?sslmode=require
JWT_SECRET=<long-random-secret>
APP_ENV=production
FRONTEND_ORIGIN=https://patchbay.example.com
PATCHBAY_APP_URL=https://patchbay.example.com
PATCHBAY_PUBLIC_URL=https://api.patchbay.example.com
```
...
`JWT_SECRET` is required in production: with `APP_ENV=production` the backend refuses to boot if it is empty or a known placeholder (generate one with `openssl rand -hex 32`). Do not set `PATCHBAY_DEV_VERIFICATION_CODE` or `PATCHBAY_DEV_LOGIN`.
</Callout>
...
|---|---|---|
| `DATABASE_URL` | local `patchbay` database | PostgreSQL connection string |
...
| `PATCHBAY_DATABASE_STARTUP_TIMEOUT` | `3m` | Container-level retry budget shared by migration and API startup; set to `0` for one fail-fast attempt per phase |
| `PATCHBAY_DATABASE_CONNECT_TIMEOUT` | `5s` | Fallback timeout for each startup connection attempt; native pgx `connect_timeout`, `PGCONNECT_TIMEOUT`, or service settings take precedence |
...
| `PATCHBAY_SHUTDOWN_HOLD_DURATION` | `0` | How long to wait after a termination signal before graceful shutdown begins |
| `PATCHBAY_RUNTIME_RECONNECT_GRACE` | `3h` | How long an offline runtime may reconnect before its in-flight tasks fail; values below `150s` are clamped |
...
| `PATCHBAY_APP_URL` | falls back to `FRONTEND_ORIGIN` | User-reachable web URL; CLI sign-in and account-link URLs use it |
| `PATCHBAY_PUBLIC_URL` | empty | Public API URL; used for webhook URLs and runtime connection instructions |
| `PATCHBAY_DAEMON_SERVER_URL` | falls back to `PATCHBAY_PUBLIC_URL`, then `PATCHBAY_APP_URL` / `FRONTEND_ORIGIN` | Server URL the API process inserts into `patchbay setup self-host`; set it when daemons reach the API at a different URL than the public webhook URL |
...
<Callout type="warning">
`PATCHBAY_DAEMON_SERVER_URL` is returned by the unauthenticated `/api/config` endpoint and is visible to clients. Treat it as public configuration; never put credentials, tokens, or other secrets in it.
</Callout>
...
| `RESEND_FROM_EMAIL` | empty | Required when Resend is enabled; must be a sender in a domain verified in your Resend account |
...
| `PATCHBAY_DEV_VERIFICATION_CODE` | empty | Fixed 6-digit test code for non-production environments |
| `PATCHBAY_DEV_LOGIN` | empty | `1` serves `/auth/dev-login`, which signs in as any email with no code, in non-production environments |
| `PATCHBAY_DEV_EMAIL` | `dev@localhost` | Default email for `/auth/dev-login` and the local dev fixtures |

For the exact allowlist evaluation order, see [Sign-in and signup](/auth-setup#signup-restrictions).

## Attachment storage

When `S3_BUCKET` is unset, Patchbay uses local disk.
...
| `RATE_LIMIT_DEV_LOGIN` | `60` | Per-IP `/auth/dev-login` requests per minute (only when `PATCHBAY_DEV_LOGIN=1`) |
...
| `PATCHBAY_TRUSTED_PROXIES` | empty | Trusted proxy CIDRs for automation webhooks and realtime connections |
...
| Lark | `PATCHBAY_LARK_SECRET_KEY` | Base64-encoded 32-byte credential encryption key |
| Slack | `PATCHBAY_SLACK_SECRET_KEY` | Base64-encoded 32-byte token encryption key |
| Telegram | `PATCHBAY_TELEGRAM_SECRET_KEY` | Base64-encoded 32-byte Bot token encryption key |
| Composio | `COMPOSIO_API_KEY` | Enables Composio tool connections |
| Composio | `COMPOSIO_CALLBACK_BASE_URL` | Callback API URL; can fall back to `PATCHBAY_PUBLIC_URL` |
| Composio | `COMPOSIO_STATE_SECRET` | OAuth state signing secret; can be derived from `JWT_SECRET` |
| Self-hosted Git | `PATCHBAY_VCS_INTEGRATION_ENABLED` | Forgejo/Gitea/GitLab integration switch; enabled by default in compose |
| Self-hosted Git | `PATCHBAY_VCS_SECRET_KEY` | Base64-encoded 32-byte encryption key (`openssl rand -base64 32`); the feature is entirely unavailable without it |
| Plugins | `PATCHBAY_PLUGIN_SECRET_KEY` | Base64-encoded 32-byte key for stored secrets and encrypted surface launch URLs |
| Plugins | `PATCHBAY_PLUGIN_SURFACE_ORIGIN` | Dedicated cookie-free browser origin routed to this backend; must differ from app/API origins and preserve `Host` |
| Plugins | `PATCHBAY_PLUGIN_API_URL` | Complete versioned Plugin Public API base URL, such as `https://plugin-api.example.com/api/v1/plugin`; falls back to `PATCHBAY_PUBLIC_URL` + `/api/v1/plugin` |
| Plugins | `PATCHBAY_PLUGIN_DIR` | Optional absolute directory used to publish local plugin bundles during development |
...
| Variable | Default | Description |
|---|---|---|
| `PATCHBAY_LLM_API_KEY` | empty | OpenAI-compatible API key |
| `PATCHBAY_LLM_BASE_URL` | empty | OpenAI-compatible endpoint |
| `PATCHBAY_LLM_DEFAULT_MODEL` | `gpt-5.6-luna` | Used when a request does not specify a model |
| `PATCHBAY_LLM_MAX_RETRIES` | `2` | Retry ceiling per call; `0` disables retries, `1`–`5` cap it at N |

`PATCHBAY_LLM_MAX_RETRIES` is the single source for the retry policy. Leave it unset for the default of 2, set `0` to send exactly one request per call, or set 1–5 to cap retries at that many. It is a ceiling, not a quota: only retryable failures consume it, and a success or the caller's own deadline can end the call sooner. Any other value — negative, non-numeric, or above 5 — fails startup instead of being corrected silently. The ceiling is a latency budget: backoff starts at 0.5s and doubles to an 8s cap, so a larger budget would outlast the callers' own deadlines and turn a retryable failure into a timeout. Retries cover connection failures and HTTP 408, 409, 429 and 5xx; all other 4xx responses are returned as-is. The server logs the effective policy at startup as `llm retry policy`, with no credentials in the line.
...
<Callout type="info">
This covers the assist layer only. Running an agent is a separate data path: when an agent answers a chat, your daemon executes that agent's AI coding tool using the tool's own credentials, and does not forward the `PATCHBAY_LLM_*` settings above to it. (The task-scoped Patchbay connection variables the agent needs are injected by the daemon separately.) Emptying the variables above does not affect that path — govern it through the agent's runtime configuration.
</Callout>
...
| Variable | Default | Description |
|---|---|---|
| `PATCHBAY_SERVER_URL` | `ws://localhost:8080/ws` | Patchbay API / WebSocket URL; also accepts `http(s)` |
| `PATCHBAY_DAEMON_DEVICE_NAME` | hostname | Device name shown in the runtime list |
| `PATCHBAY_AGENT_RUNTIME_NAME` | `Local Agent` | Runtime display name |
| `PATCHBAY_DAEMON_POLL_INTERVAL` | `30s` | Task polling interval when no wake event arrives |
| `PATCHBAY_DAEMON_HEARTBEAT_INTERVAL` | `15s` | Heartbeat interval |
| `PATCHBAY_DAEMON_MAX_CONCURRENT_TASKS` | `20` | Concurrent task ceiling per daemon |
| `PATCHBAY_AGENT_TIMEOUT` | `0` | Absolute time limit per run; `0` means no limit |
| `PATCHBAY_AGENT_IDLE_WATCHDOG` | `2h` | Silence ceiling with no output and no tool execution; `0` disables the watchdog entirely |
| `PATCHBAY_AGENT_TOOL_WATCHDOG` | same as `PATCHBAY_AGENT_IDLE_WATCHDOG` | Silence ceiling for a single tool call; set it only to give tools more room than the model gets, `0` never force-stops a tool call |
| `PATCHBAY_OPENCODE_IDLE_WATCHDOG` | `10m` | OpenCode-specific silence threshold |
| `PATCHBAY_CODEX_SEMANTIC_INACTIVITY_TIMEOUT` | same as `PATCHBAY_AGENT_IDLE_WATCHDOG` | Codex semantic-silence threshold. Codex's own timer cannot see that a tool is running, so it tracks the larger of the idle / tool budgets instead of holding a separate, shorter ceiling |
| `PATCHBAY_CODEX_FIRST_TURN_TIMEOUT` | `0` | Explicit override for the Codex first-turn no-progress ceiling; `0` keeps the default. The effective first-turn wait stays bounded by `PATCHBAY_CODEX_SEMANTIC_INACTIVITY_TIMEOUT` and the overall execution timeout — set `PATCHBAY_CODEX_SEMANTIC_INACTIVITY_TIMEOUT` strictly above this value (with some margin), or the wait is truncated to it and the model-catalog startup retry is skipped. Equal values are not enough: the semantic timer is armed first, so at equal durations the retry can still be lost |
| `PATCHBAY_CODEX_HANDSHAKE_TIMEOUT` | `30s`; `thread/start`, `thread/resume`: `60s` | Codex app-server startup handshake ceilings. An explicit value overrides both budgets globally |
| `PATCHBAY_DAEMON_AUTO_UPDATE` | Cloud `true`; self-hosted `false` | Whether to check for and apply CLI updates automatically |
| `PATCHBAY_DAEMON_AUTO_UPDATE_INTERVAL` | `6h` | Update check interval |
| `PATCHBAY_DAEMON_AUTO_RELOAD` | `true` | Whether to restart into a `patchbay` binary replaced on disk out of band (`brew upgrade`, a re-download, a local build). Independent of `PATCHBAY_DAEMON_AUTO_UPDATE` |
| `PATCHBAY_WORKSPACES_ROOT` | `~/patchbay_workspaces` | Root directory for task working directories |
| `PATCHBAY_AGENT_TEMP_BASE` | `/tmp` (Linux/macOS) | Linux/macOS only. Parent directory for private per-task temp dirs; must be an existing, writable absolute directory, and an invalid value fails task startup instead of falling back to `/tmp`. Pick a short path — child tools may bind AF_UNIX sockets under it, and `sun_path` is limited to 108 bytes on Linux and 104 on macOS |
| `PATCHBAY_KEEP_ENV_AFTER_TASK` | `false` | Keep task directories for debugging |

Each AI coding tool accepts `PATCHBAY_<PROVIDER>_PATH` and `PATCHBAY_<PROVIDER>_MODEL` to override the command path and default model. QwenPaw and MiniMax Code have no model variable because Patchbay never sends them a model; MiniMax Code's path variable is `PATCHBAY_MCODE_PATH` — see [AI coding tools comparison](/providers#model-sources). DeepSeek Harness supports `PATCHBAY_DSH_PATH` and `PATCHBAY_DSH_MODEL` (a model id from the dsh catalog, e.g. `deepseek-official/deepseek-chat`). Dim supports `PATCHBAY_DIM_PATH` and `PATCHBAY_DIM_MODEL` (a model id from the dim catalog). ZeroClaw supports `PATCHBAY_ZEROCLAW_PATH` but has no model variable; its agent profile owns the model selection. Machine-wide default arguments via `PATCHBAY_<PROVIDER>_ARGS` are currently supported for five tools: Claude Code, Codex, CodeBuddy, Qwen Code, and QwenPaw. The variables are `PATCHBAY_CLAUDE_ARGS`, `PATCHBAY_CODEX_ARGS`, `PATCHBAY_CODEBUDDY_ARGS`, `PATCHBAY_QWEN_ARGS`, and `PATCHBAY_QWENPAW_ARGS`. For example:

```dotenv
PATCHBAY_CLAUDE_PATH=/opt/bin/claude
PATCHBAY_CLAUDE_ARGS=--max-turns 40
```

Precedence is command-line flag → environment variable → `~/.patchbay/config.json` → built-in default. For watchdog behavior, see [Daemon and runtimes](/daemon-runtimes).

### Persisting daemon configuration

Common daemon-side settings can also be written to `~/.patchbay/config.json` instead of relying on shell environment variables; named profiles keep theirs at `~/.patchbay/profiles/<name>/config.json`:

```bash
patchbay config set poll_interval 10s
...
|---|---|---|
| `server_url` | `ws://localhost:8080/ws` | Patchbay API / WebSocket URL |
...
- Passing an empty string clears a persisted value and falls back to the env var or built-in default, for example `patchbay config set poll_interval ""`.
