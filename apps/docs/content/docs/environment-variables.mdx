...
| `AUTH_TOKEN_TTL` | `720h` (30 days) | Lifetime of browser JWTs and cookies; accepts a duration or a positive integer of seconds |
...
| `RESEND_FROM_EMAIL` | `noreply@example.com` | Placeholder sender; replace it with an address on a verified domain before enabling email |
...
| Personal WeChat | `PATCHBAY_WEIXIN_SECRET_KEY` | Base64-encoded 32-byte iLink token and conversation-context encryption key |
...
| `PATCHBAY_AGENT_IDLE_WATCHDOG` | `30m` | Silence ceiling with no output and no tool execution |
| `PATCHBAY_AGENT_TOOL_WATCHDOG` | `2h` | Silence ceiling for a single tool call |
| `PATCHBAY_OPENCODE_IDLE_WATCHDOG` | `10m` | OpenCode-specific silence threshold |
| `PATCHBAY_CODEX_SEMANTIC_INACTIVITY_TIMEOUT` | `10m` | Codex semantic-silence threshold |
...
| `PATCHBAY_CODEX_HANDSHAKE_TIMEOUT` | `30s` | Codex app-server startup handshake ceiling |
...
- Duration keys accept positive durations (such as `10s`, `2h`); `0s` and negative values are rejected. The one exception is `agent_timeout`: `0s` is valid and explicitly disables the run time limit.
