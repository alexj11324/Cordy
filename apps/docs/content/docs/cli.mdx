---
title: Using the CLI
description: Install and log in to the Patchbay CLI, and manage workspaces, issues, agents, and runtimes from the terminal.
---

import { Callout } from "fumadocs-ui/components/callout";

The Patchbay CLI both connects local runtimes and manages workspaces, issues, agents, and automations from the terminal. This page covers the common paths; the flags your installed version supports are always defined by each command's own `--help`.
...
If you only need to sign in again without overwriting other configuration, run `patchbay login`. On a machine without a browser, create a personal access token in the web settings first, then enter it with:
...
Subsequent commands use this workspace. A single command can override it with `--workspace-id`, or you can set `PATCHBAY_WORKSPACE_ID`.
...
patchbay issue search "login failure"

# Create and update
patchbay issue create --title "Fix login failure"
patchbay issue status MUL-123 in_progress
patchbay issue assign MUL-123 --to "Backend Agent"  # sets the executor

# Comments and runs
patchbay issue comment list MUL-123
patchbay issue comment add MUL-123 --content "Check the regression tests first"
...
```bash
patchbay issue create --title "Upgrade notes" --description-stdin < notes.md
...
Deleting a runtime that still has active agents bound to it is refused by default. `patchbay runtime delete <id> --cascade` unbinds those agents, preserves their configuration and history, and cancels their active tasks.
...
| Command | Purpose |
| --- | --- |
| `issue` | Create, update, set owner/executor/reviewer roles, and search issues; manage comments, subscribers, labels, properties, and runs |
| `project` | Manage projects and their resources |
| `label`, `property` | Manage workspace labels and custom properties |
| `agent`, `skill`, `team` | Manage agents, skills, and teams |
| `automation` | Manage automations, triggers, and run history |
...
`patchbay chat` reads **the external chat session an agent is currently handling**; it is mainly for agents in chat integrations, not a general command for browsing arbitrary workspace conversations.
...
Scripts should use JSON output rather than parsing the terminal-oriented tables. The output formats and pagination flags each command supports are defined by `patchbay <command> --help`.

## Profiles and configuration

The default configuration lives in `~/.patchbay/config.json`. Use `--profile <name>` to isolate a separate set of server address, token, default workspace, and daemon state:
...
Named profiles live in `~/.patchbay/profiles/<name>/config.json`. Inspect the current values:
...
This protects implicit Patchbay CLI profile resolution while preserving the normal `HOME` and XDG variables used by other provider tools. It is not an OS filesystem boundary: processes running as the same system user may still open an explicitly known path. Use a dedicated user, container, VM, or equivalent isolation when that stronger guarantee is required.

<Callout type="warning">
CLI configuration files contain tokens that can access Patchbay as you. Don't commit them to a repository, upload them to logs, or share them with others.
</Callout>

## Command reference

The tables below cover every current top-level command, grouped the way the CLI itself groups them. All commands accept the global flags `--server-url`, `--workspace-id`, `--profile`, and `--debug`. `--output` is registered per command, and the default format varies: `list` commands default to a table, while `get` and `create` commands mostly default to JSON. Full flags are defined by `patchbay <command> --help`.

### Core

| Command | Subcommand | Purpose | Key flags |
| --- | --- | --- | --- |
| `issue` | `list` | List issues | `--status`, `--priority`, `--owner`, `--owner-id`, `--executor`, `--executor-id`, `--project`, `--metadata` (repeatable), `--limit`, `--offset`, `--sort`, `--full-id` |
| | `get <id>` | Show a single issue | |
| | `create` | Create an issue | `--title` (required), `--description` / `--description-stdin` / `--description-file`, `--status`, `--priority`, `--owner` / `--owner-id`, `--executor` / `--executor-id`, `--reviewer` / `--reviewer-id`, `--parent`, `--stage`, `--project`, `--start-date`, `--due-date`, `--attachment` (repeatable) |
| | `update <id>` | Update issue fields | Same role-specific fields as `create`, plus `--position`, `--no-start` |
| | `assign <id>` | Set or clear the executor | `--to` (fuzzy name match against agents and teams), `--to-id`, `--unassign`, `--no-start` |
...
| | `dependency-graph get <id>` / `apply <parent-id>` | Read or atomically apply a typed dependency graph | `get`: `--output`; `apply`: `--idempotency-key`, `--plan-file` / `--plan-stdin`, `--allow-external-file`, `--output` |
...
| | `rerun <id>` | Re-enqueue a run for the current executor | |
...
| `automation` | `list/get/create/update/delete` | Manage automations | `create`: `--title`, `--agent`, `--mode` (all required), `--project`, `--subscriber` (repeatable) |
...
| | `mcp list/add/update/remove` | Manage the workspace's MCP server library | Writes are owner/admin. A server added here is assigned to no agent — give it to one with `patchbay agent mcp add`. `list` shows names and transports only; the stored entry is write-only and never returned. `--server-config-file` / `--server-config-stdin` keep tokens out of shell history |
...
| `team` | `list/get/create/update/delete` | Manage teams (`delete` archives) | |
| | `member list/add/set-role/remove` | Manage team members | |
...
| Command | Subcommand | Purpose | Key flags |
| --- | --- | --- | --- |
| `daemon` | `start` | Start the local daemon | `--foreground`, `--device-name`, `--runtime-name`, `--workspaces-root`, `--poll-interval`, `--heartbeat-interval`, `--agent-timeout` (`0` means no limit), `--max-concurrent-tasks`, `--no-auto-update`; each has a matching `PATCHBAY_*` environment variable |
...
Use a typed plan when a parent issue is split into tasks with hard prerequisites:
...
The plan's current role contract is explicit: `owner` is a workspace member;
`executor` and `candidate_executors` are agents or teams; `reviewer` may be a
member, agent, or team. The server validates the plan and derives `waves`;
`get` and successful `apply` return the parent, children, nodes, edges, and
readiness `unlock_condition`. Reusing the idempotency key replays the same
plan. Graph mutations publish `dependency_graph:updated` for connected clients.

## Driving Patchbay from another coding agent

If most of your work already happens inside Codex, Claude Code, or Cursor, you can
operate Patchbay from there instead of switching to a terminal. The
[Patchbay CLI skill](https://github.com/patchbay-ai/patchbay-cli) teaches those agents
...
effects that mentions, status changes, and owner/executor/reviewer changes carry.
...
- [Patchbay CLI skill](https://github.com/patchbay-ai/patchbay-cli) — operate Patchbay from Codex, Claude Code, or Cursor.
