---
title: Using the CLI
description: Install and log in to the Orvilo CLI, and manage workspaces, issues, agents, and runtimes from the terminal.
---

import { Callout } from "fumadocs-ui/components/callout";

The Orvilo CLI both connects local runtimes and manages workspaces, issues, agents, and automations from the terminal. This page covers the common paths; the flags your installed version supports are always defined by each command's own `--help`.
...
This protects implicit Orvilo CLI profile resolution while preserving the normal `HOME` and XDG variables used by other provider tools. It is not an OS filesystem boundary: processes running as the same system user may still open an explicitly known path. Use a dedicated user, container, VM, or equivalent isolation when that stronger guarantee is required.

<Callout type="warning">
CLI configuration files contain tokens that can access Orvilo as you. Don't commit them to a repository, upload them to logs, or share them with others.
</Callout>
...
## Driving Orvilo from another coding agent

If most of your work already happens inside Codex, Claude Code, or Cursor, you can
operate Orvilo from there instead of switching to a terminal. The
[Orvilo CLI skill](https://github.com/alexj11324/Cordy-cli) teaches those agents
...
- [Orvilo CLI skill](https://github.com/alexj11324/Cordy-cli) — operate Orvilo from Codex, Claude Code, or Cursor.
