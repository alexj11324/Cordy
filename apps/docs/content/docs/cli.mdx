...
Subsequent commands use this workspace. A single command can override it with `--workspace-id`, or you can set `ORVILO_WORKSPACE_ID`.
...
| Command | Subcommand | Purpose | Key flags |
| --- | --- | --- | --- |
| `daemon` | `start` | Start the local daemon | `--foreground`, `--device-name`, `--runtime-name`, `--workspaces-root`, `--poll-interval`, `--heartbeat-interval`, `--agent-timeout` (`0` means no limit), `--max-concurrent-tasks`, `--no-auto-update`; each has a matching `ORVILO_*` environment variable |
