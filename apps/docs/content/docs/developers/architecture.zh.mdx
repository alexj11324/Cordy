---
title: 项目架构
description: 了解 Patchbay 的客户端、Go 服务、执行守护进程和共享前端包如何协作。
---

Patchbay 由一个 Go backend、多种客户端和运行在执行电脑上的守护进程组成。PostgreSQL 保存协作数据；守护进程领取 task，并调用本机的 AI 编程工具。
...
| 目录 | 职责 | 主要技术 |
| --- | --- | --- |
| `server/` | API、认证、task 调度、集成、CLI 和守护进程 | Go、Chi、sqlc、gorilla/websocket |
| `apps/web/` | 浏览器客户端和应用入口 | Next.js App Router |
...
例如，一项 Web 与 Desktop 都需要的任务功能通常会涉及：
...
- **TanStack Query** 保存任务、智能体、成员、收件箱等服务端数据；
...
- `internal/realtime/` 把任务、评论、收件箱等变化推送给用户客户端；
...
1. 用户设置任务的 executor、提及智能体，或由自动化触发；
...
任务有彼此独立的角色：member 是 owner，agent/team 是 executor，member/agent/team 是 reviewer。新增查询、缓存 key 或实时事件时，必须保留工作区和具体角色，不能把这些角色压成一个统一的角色字段。
