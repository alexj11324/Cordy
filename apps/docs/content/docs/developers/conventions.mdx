...
The Chinese pages under `apps/docs/content/docs/*.zh.mdx` are the de facto voice standard for everything else on this page, and their prose follows the table below as well.

### `issue` is the product's task — translate it

`issue` is the English product name for the thing a user files and an agent works on. In Chinese it is **the everyday word for "task"**:

| Entity | en | zh-Hans |
| --- | --- | --- |
| the filed unit of work (`issue`) | Issue | **任务** |
| one agent execution run (`task`) | Task | `task` |

Both nouns are user-visible and they are **not** the same thing — an issue can carry many runs. Never let one locale spell both with the same word, which is why the run keeps a visually distinct form.
...
- **`issue` in the "problem" sense** (runtime health) is a plain noun, not the entity: `{{count}} issues` on a machine card is `{{count}} 个异常`, never the entity word.

**Why `issue` is translated while `skill` is not**: users file and read issues all day, and "issue" has no meaning in Chinese outside of dev jargon. The everyday task word is what people already say for this. `skill` is a Patchbay-specific concept where no local word carries the meaning.
...
- **`automation` → "自动化"**: in Chinese, "automation" associates with Tesla's "自动驾驶" and doesn't match what the feature does (run tasks on a schedule). Notion and Feishu both use "自动化"; that's the industry consensus.
...
| Owner / Executor / Reviewer / Reporter | Owner / Executor / Reviewer / Reporter |
...
- "Set the executor" → "设置任务 executor"
