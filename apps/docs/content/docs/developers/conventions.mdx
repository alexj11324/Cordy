...
- **`skill`** keeps lowercase English in Chinese text — an Orvilo-specific concept with no established Chinese term; titles may capitalize as `Skills`.
...
**Why `issue` is translated while `skill` is not**: users file and read issues all day, and "issue" has no meaning in Chinese, Japanese, or Korean outside of dev jargon. The everyday task word is what people already say for this. `skill` is an Orvilo-specific concept where no local word carries the meaning.
