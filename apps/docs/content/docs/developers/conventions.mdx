...
If you write Patchbay code, change a translation, or write Chinese product copy, this is the page to reference.
...
| `packages/ui` | nothing | `@patchbay/core`, business logic |
...
| `apps/mobile/` | types and pure functions from `@patchbay/core` | Web/Desktop React pages, stores, and platform implementations |
...
- Standard `rustfmt` + `clippy`. No exceptions.
- Handler files mirror domains: `agent.rs`, `auth.rs`, `runtime.rs`.
- Tests live in colocated `#[cfg(test)]` modules or the crate's `tests/` directory.
- Parse and validate boundary input explicitly; never panic on untrusted UUIDs or other request fields.
...
`AgentRuntime.name` is the daemon's raw technical name (e.g. `Codex (host)`); the user's alias lives in `custom_name`. User-visible copy must never render `runtime.name` directly — use the shared helpers so aliases and provider stay consistent (PB-5248, #5260):
...
Every issue has a human-readable key like `PB-123`: workspace `issue_prefix` (uppercase letters and digits, typically 3 chars, max 10) + sequence number. Workspace admins can change the prefix in Settings → General; changing it renumbers every existing issue. Keys in PR titles, branch names, docs, and chat are human-readable/search context only; they never establish a Work Product relation. Use the current key for human references and explicitly Attach the Work Product when a relation is intended.

### Comments in code

English only. The repo enforces this for both Rust and TypeScript. If you find a Chinese comment in code, it's a bug — replace it.
...
This is the **mandatory** glossary for every translation PR. It used to live at `packages/views/locales/glossary.md`; that file has been removed and this page replaces it.

### The core distinction: everyday noun vs Patchbay-specific term

Patchbay's product nouns split into two categories:
...
- **Patchbay-specific term** — a concept no local word carries (`skill`), or a schema-level identifier a user may have to type or match (`todo`, `in_progress`, `task_id`). Render as **lowercase English** so it reads like a type name.
...
- **Code references and literal commands** stay English: `patchbay issue ...`, the `/issue` Slack and Lark slash command.
- **`skill`** keeps lowercase English in Chinese text — a Patchbay-specific concept with no established Chinese term; titles may capitalize as `Skills`.
...
**Why `issue` is translated while `skill` is not**: users file and read issues all day, and "issue" has no meaning in Chinese, Japanese, or Korean outside of dev jargon. The everyday task word is what people already say for this. `skill` is a Patchbay-specific concept where no local word carries the meaning.
...
- **`automation` → "自动化"**: use the established functional term for scheduled task execution. Notion and Feishu both use "自动化"; this keeps the product concept precise.

### Don't translate — brands and acronyms

| Category | Terms |
| --- | --- |
| Brands | **Patchbay**, GitHub, Slack, Google, Anthropic, OpenAI, Claude, Codex, Cursor, Linear, Jira |
...
1. Apply it in the relevant locale JSONs / AGENTS.md / docs page
