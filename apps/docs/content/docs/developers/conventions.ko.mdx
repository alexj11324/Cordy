...
Patchbay 코드를 작성하거나, 번역을 변경하거나, 중국어 제품 카피를 작성한다면 이 페이지를 참고하세요.
...
| `packages/ui` | 없음 | `@patchbay/core`, 비즈니스 로직 |
...
| `apps/mobile/` | `@patchbay/core`의 유형과 순수 함수 | Web/Desktop의 React 페이지, store, 플랫폼 구현 |
...
- 표준 `rustfmt` + `clippy`. 예외 없음.
- Handler 파일은 도메인을 반영: `agent.rs`, `auth.rs`, `runtime.rs`
- 테스트는 같은 파일의 `#[cfg(test)]` module 또는 crate의 `tests/` directory에 배치
- 경계 입력은 명시적으로 parse하고 validate하며, 신뢰할 수 없는 UUID나 request field에서 panic하지 않기
...
모든 이슈에는 `PB-123` 같은 사람이 읽을 수 있는 키가 있습니다: 워크스페이스 `issue_prefix`(대문자와 숫자, 보통 3자, 최대 10자) + 일련번호. 워크스페이스 admin은 Settings → General에서 접두사를 변경할 수 있고, 변경하면 기존의 모든 이슈가 다시 번호 매겨집니다. PR 제목, 브랜치 이름, 문서와 채팅 속 키는 사람이 읽고 검색하기 위한 문맥일 뿐 Work Product 관계를 만들지 않습니다. 관계가 필요하면 현재 키를 사람용 참조로 사용하고 Work Product를 명시적으로 Attach하세요.

### 코드 내 주석

영어만 사용합니다. 레포는 Rust와 TypeScript 모두에 이를 강제합니다. 코드에서 중국어 주석을 발견하면 그것은 버그이니 교체하세요.
...
Patchbay의 제품 명사는 두 가지 범주로 나뉩니다:

- **엔티티(Entity)** — URL, 데이터베이스 row, API 타입을 가집니다. 중국어 텍스트에서는 **소문자 영문**으로 표기하여 시각적으로 타입 이름처럼 읽히게 하고 "이것은 Patchbay 시스템 엔티티다"라는 신호를 줍니다.
...
`issue` / `skill` / `task`는 Patchbay의 핵심 엔티티입니다. 스키마 컬럼, API 필드, 제품 UI 레이블이 모두 영어입니다. 중국어 텍스트에서는 **혼합 규칙**을 따르며 — 무엇을 사용할지는 단어가 어디에 나타나는지에 따라 달라집니다:
...
**`issue` / `skill` / `task`가 `project` / `automation`처럼 중국어로 강제 번역되지 않는 이유**:
...
- **`skill`**: 정착된 중국어 용어가 없는 Patchbay 고유 개념입니다.
...
- **`automation` → "自动化"**: 일정에 따라 task를 실행하는 기능을 정확히 설명하는 확립된 용어입니다. Notion과 Feishu 모두 "自动化"를 사용해 제품 용어를 일관되게 유지합니다.

### 번역하지 않음 — 브랜드와 약어

| 범주 | 용어 |
| --- | --- |
| 브랜드 | **Patchbay**, GitHub, Slack, Google, Anthropic, OpenAI, Claude, Codex, Cursor, Linear, Jira |
