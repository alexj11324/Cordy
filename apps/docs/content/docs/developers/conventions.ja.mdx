...
- **Patchbay 固有語** — その言語に対応する語がない概念（`skill`）、またはユーザーが入力したり突き合わせたりするスキーマレベルの識別子（`todo`、`in_progress`、`task_id`）。**小文字の英語**で表記し、型名のように読ませます。
...
| エージェントの 1 回の実行（`task`） | Task | `task` | **作業** | **작업** |

どちらの名詞もユーザーの目に触れ、しかも**同じものではありません** — 1 つの issue が複数の実行を抱えることがあります。どのロケールでも両方を同じ語で綴ってはいけません。実行が各ロケールで視覚的に区別できる形を保っているのはそのためです。

変わらないもの:

- **API / DB フィールド**は常に `issue` / `task` / `skill`: `issue_status`、`task_id`、`skill_uuid`。
- **コード参照とコマンドそのもの**は英語のまま: `patchbay issue ...`、Slack と Lark の `/issue` スラッシュコマンド。
- **`skill`** は中国語テキストでも小文字の英語のまま — 定着した訳語がない Patchbay 固有の概念です。タイトルでは `Skills` のように大文字にできます。
- **「問題」の意味の `issue`**（ランタイムのヘルス）はエンティティではなく普通名詞です。マシンカードの `{{count}} issues` は `{{count}} 个异常` / `問題 {{count}} 件` / `문제 {{count}}개` であり、エンティティの語は使いません。

**なぜ `issue` は翻訳し、`skill` は翻訳しないのか**: ユーザーは毎日 issue を登録し、読んでいます。そして "issue" という語は、中国語・日本語・韓国語では開発者の隠語以外に意味を持ちません。現地の人がもともとこの対象を指して使っているのは「タスク」に当たる日常語です。`skill` は、その意味を担える現地語が存在しない Patchbay 固有の概念です。
...
- "Set the executor" → "タスクの executor を設定"
