...
Patchbay のコードを書く、翻訳を変更する、あるいは中国語の製品コピーを書く場合は、このページを参照してください。
...
| `packages/ui` | なし | `@patchbay/core`, ビジネスロジック |
...
| `apps/mobile/` | `@patchbay/core` の型と純粋関数 | Web/Desktop の React ページ、store、プラットフォーム実装 |
...
- 標準の `rustfmt` + `clippy`。例外なし。
- Handler ファイルはドメインを反映する: `agent.rs`, `auth.rs`, `runtime.rs`
- テストは同じファイルの `#[cfg(test)]` module または crate の `tests/` directory に配置
- 境界入力は明示的に parse と validate を行い、信頼できない UUID や request field で panic しない
...
すべてのイシューには `PB-123` のような人が読めるキーがあります。ワークスペースの `issue_prefix`（大文字と数字、通常 3 文字、最大 10 文字）+ 連番です。ワークスペースの admin は Settings → General で接頭辞を変更できますが、変更すると既存のすべてのイシューが番号を振り直されます。PR タイトル、ブランチ名、ドキュメント、チャット内のキーは人間向けの表示・検索コンテキストに過ぎず、Work Product の関係を作りません。関係が必要な場合は現在のキーを人間向け参照として使い、Work Product を明示的に Attach してください。

### コード内のコメント

英語のみです。リポジトリは Rust と TypeScript の両方でこれを強制します。コード内に中国語のコメントを見つけたら、それはバグなので置き換えてください。
...
Patchbay の製品名詞は 2 つのカテゴリに分かれます。

- **エンティティ（Entity）** — URL、データベースの row、API の型を持ちます。中国語のテキストでは**小文字の英語**で表記し、視覚的に型名のように読めて「これは Patchbay のシステムエンティティだ」というシグナルを与えます。
...
`issue` / `skill` / `task` は Patchbay の中核エンティティです。スキーマのカラム、API のフィールド、製品 UI のラベルがすべて英語です。中国語のテキストでは**混合ルール**に従い、何を使うかは単語がどこに現れるかによって変わります。
...
**`issue` / `skill` / `task` が `project` / `automation` のように中国語へ強制的に翻訳されない理由**:
...
- **`skill`**: 定着した中国語の用語がない Patchbay 固有の概念です。
...
- **`automation` → "自动化"**: スケジュールに従ってタスクを実行する機能を正確に表す、確立した用語です。Notion も Feishu も "自动化" を使っており、製品用語を統一できます。

### 翻訳しない — ブランドと頭字語

| カテゴリ | 用語 |
| --- | --- |
| ブランド | **Patchbay**, GitHub, Slack, Google, Anthropic, OpenAI, Claude, Codex, Cursor, Linear, Jira |
