...
写 Patchbay 代码、改翻译、写中文产品文案，都从这一页查。
...
| `packages/ui` | 无业务依赖 | `@patchbay/core`、业务逻辑 |
...
| `apps/mobile/` | `@patchbay/core` 的类型和纯函数 | Web/Desktop 的 React 页面、store 和平台实现 |

Web 和 Desktop 都有的逻辑，**必须**抽到共享包。"小段重复"也不算例外。Mobile 拥有独立的 UI、数据层和发布流程，只共享类型与纯函数。
...
- 不创建数据库 foreign key，也不使用级联删除或级联更新；关系校验和清理由应用层完成
- 所有索引都使用 `CREATE INDEX CONCURRENTLY` 或 `CREATE UNIQUE INDEX CONCURRENTLY`，每条 concurrent index 单独占一个 migration 文件

### Rust

- 标准 `rustfmt` + `clippy`，无例外
- Handler 文件按域命名：`agent.rs`、`auth.rs`、`runtime.rs`
- 测试放在同文件的 `#[cfg(test)]` module 或 crate 的 `tests/` directory
- 对边界输入显式 parse 和 validate，不要因不可信 UUID 或其他 request field 而 panic
...
- 网络响应必须用 `packages/core/api/schema.ts` 中的 `parseWithFallback` 和 zod schema 解析，不能直接 `as T`
- 新增或修改 endpoint 时，同步更新 schema，并覆盖字段缺失或格式错误的测试
- 下游 UI 对可选字段提供默认值；服务端枚举的 `switch` 必须有 `default` 分支
- 已安装的 Desktop 可能连接更新的 backend，不能假设前后端永远同版本

### Runtime 显示名

`AgentRuntime.name` 是 daemon 的原始技术名（例如 `Codex (host)`），用户别名在 `custom_name`。用户可见文案不得直接渲染 `runtime.name`，要用共享 helper，保证别名和 provider 一致（PB-5248、#5260）：
...
每个 issue 有人类可读的编号，比如 `PB-123`：工作区 `issue_prefix`（大写字母和数字，通常 3 个字符，最长 10 个）+ 流水号。工作区管理员可以在**设置 → 通用**中修改前缀；修改会让所有现有 issue 重新编号。PR 标题、分支名、文档和聊天中的编号只用于人类阅读和搜索，永远不会建立 Work Product 关联；需要关联时，请使用当前编号作人类引用，并显式 Attach 对应产物。

### 代码注释

**只允许英文**。Rust 和 TypeScript 都强制。如果在代码里看到中文注释，那就是 bug，替换掉。
...
这是每个翻译 PR 都必须遵守的术语表。原本在 `packages/views/locales/glossary.md`，那个文件已删除，由这一页取代。

### 核心区分：日常词 vs Patchbay 专有词

Patchbay 的产品名词分两类：

- **日常词** —— 用户会直接说出口的词。**完整翻译成中文**，是不是数据库实体都一样：`issue` → 任务、`workspace` → 工作区、`project` → 项目。
- **Patchbay 专有词** —— 中文里没有对应词的概念（`skill`），或用户可能要照着敲、照着比对的 schema 标识符（`todo`、`in_progress`、`task_id`）。**用小写英文**呈现，读起来像类型名。

其余规则以 `apps/docs/content/docs/*.zh.mdx` 为准 —— docs 是已经实战 20+ 篇的 CN voice 标准。但 docs 正文里的 `issue` 还没换成「任务」，正在单独清理；这个词以下面的表格为准，不要参考那些页面。

### `issue` 就是产品里的「任务」，必须翻译

`issue` 是这个「用户提出、智能体去做的事」的英文产品名。在英文之外的每种语言里，它就是当地最日常的那个「任务」词：

| 实体 | en | zh-Hans | ja | ko |
| --- | --- | --- | --- | --- |
| 提交出来的一件事（`issue`） | Issue | **任务** | **タスク** | **태스크** |
| 智能体的一次执行（`task`） | Task | `task` | **作業** | **작업** |

这两个词用户都看得到，而且**不是一回事** —— 一个任务下面可以跑很多次执行。任何一种语言都不能把两者写成同一个词，所以「执行」在每种语言里都保留一个视觉上区分得开的写法。

不跟着改的部分：

- **API / DB 字段**永远是 `issue` / `task` / `skill`：`issue_status`、`task_id`、`skill_uuid`。
- **代码引用和字面命令**保持英文：`patchbay issue ...`、Slack 与飞书的 `/issue` 斜杠命令。
- **`skill`** 在中文里仍用小写英文 —— Patchbay 特有概念，没有公认中文译法；标题可大写为 `Skills`。
- **表示「问题」的 issue** 不是这个实体：机器卡片上的 `{{count}} issues` 是 `{{count}} 个异常`，不是 `任务`。

**为什么 `issue` 要译，而 `skill` 不译**：用户每天都在提任务、看任务，而 "issue" 这个词在中文、日文、韩文里除了开发圈黑话之外没有含义，当地人本来就用「任务」在说这件事。`skill` 则是 Patchbay 特有概念，没有哪个中文词能承载它的意思。

其它产品名词同样按「有没有稳定的本地日常词」来判断：

- **`project` 翻成"项目"**：中文里早就稳定的日常词。飞书 / Tower / Teambition / PingCode / GitHub Projects 中文版 0 例外都翻译成"项目"，没有产品保留 `project`。
- **`automation` 翻成"自动化"**：这是描述按周期运行 task 的准确功能名称。Notion / 飞书都用"自动化"，也能保持产品概念一致。
...
| 类别 | 词 |
| --- | --- |
| 品牌 | **Patchbay**、GitHub、Slack、Google、Anthropic、OpenAI、Claude、Codex、Cursor、Linear、Jira |
...
- "Create new issue" → "新建任务"（`任务` 是中文，不加空格）
...
1. 把规则在相关 locale JSON / AGENTS.md / docs 页面里同步落地
