...
- Go 1.26.6, temporarily required only by the legacy `make check` compatibility gate, not by the production backend
...
Migrations still live in `server/migrations/`; production query modules live in `server-rs/crates/cordy-db/src/queries/`.
...
`make check` runs TypeScript checks, locked Rust workspace tests, the legacy Go compatibility gate, and Playwright E2E. CI also builds and lints according to the changed scope and runs platform- or installer-specific tests.
