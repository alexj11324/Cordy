...
Patchbay is a Rust backend and pnpm monorepo. The simplest way to start locally is `make dev`; it prepares the environment, source-matched runtime, database, and migrations for the current checkout, then starts the complete Electron client and API.
