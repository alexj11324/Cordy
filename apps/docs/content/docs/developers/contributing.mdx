...
`make up` allocates this checkout's ports and database under a lock and records them, so several checkouts and worktrees run side by side and `make status` / `make down` / `make destroy` can act on exactly one of them. It prints the addresses it allocated; `make dev` is the alternative that uses the default `http://localhost:3000` and `http://localhost:8080` and runs everything in the foreground of one terminal.
