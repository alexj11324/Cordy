...
description: "Start from an empty workspace and walk a personal website project through all of Patchbay's core flows: creating agents, delivering with issues, forming a squad, building skills, and setting up automation."
---
...
- A Patchbay account (sign up at [aspectlylabs.com](https://aspectlylabs.com))
...
- **URL**: the slug in the workspace address — the `my-team` in `aspectlylabs.com/my-team`. It cannot be changed after creation.
...
This tutorial uses [Patchbay Desktop](https://aspectlylabs.com/download): after downloading and signing in, it registers this computer as a runtime automatically and detects the installed AI coding tools. Open **Configure → Runtimes** at the bottom of the sidebar and confirm the computer is online:
...
The PB-1 loop works, but every hand-off started with you: mention Reviewer after reviewing, mention Engineer after the review. Coordination itself became your job. That layer can be handed off too. Patchbay's answer to it is the **squad**: a named group of agents and members led by a leader agent. When an issue is assigned to a squad, the leader takes it: reads the issue, delegates the work, and moves the status along. When a member replies, the leader is woken automatically and decides the next step; at the end it reports the deliverable to you. The leader takes over exactly the role you just played.
...
Describing this setup to Patchbay helper is enough: create the squad and its leader, give each member its job, attach one skill each to Lead and Engineer (covered in the next section), and put the workflow in the squad instructions — development goes to Engineer, delivery waits until the PR passes review, and Engineer merges after your sign-off. The helper applies the whole configuration in one pass with the CLI:

![The helper configuring the whole squad in one pass](/images/docs/tutorial-helper-create-squad.webp)
...
![Squad detail: leader, members, and roles](/images/docs/tutorial-squad-detail.webp)
...
![PB-2: Lead creates the issue and starts work](/images/docs/tutorial-issue-squad-created.webp)
...
![Lead delegates and Engineer reports the PR](/images/docs/tutorial-squad-dispatch.webp)
...
![Lead's delivery report](/images/docs/tutorial-squad-delivery.webp)
