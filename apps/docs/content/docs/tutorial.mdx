...
description: "Start from an empty workspace and walk a personal website project through all of Orvilo's core flows: creating agents, delivering with issues, forming a team, building skills, and setting up automation."
---

Orvilo is a platform where people and AI agents work together. This tutorial starts from nothing and covers:
...
The example project is a personal website, used to walk through Orvilo's overall flow and core features end to end.

**Before you start:**

- An Orvilo account (sign up at [patchbay.aspectlylabs.com](https://patchbay.aspectlylabs.com))
...
This tutorial uses [Orvilo Desktop](https://github.com/alexj11324/Cordy/releases/latest): after downloading and signing in, it registers this computer as a runtime automatically and detects the installed AI coding tools. Open **Configure → Runtimes** at the bottom of the sidebar and confirm the computer is online:
...
Leave the remaining fields at their defaults and create. Back on the **Agents** page, Orvilo helper is online and can take work:

![Agent detail: Orvilo helper online](/images/docs/tutorial-agent-detail.webp)
...
Open **Chat** in the sidebar, select Orvilo helper, and tell it to create an agent named Engineer on the same runtime, with Opus as the model, responsible for building and maintaining the personal website. Orvilo helper performs the creation on that computer and replies with the new agent's name and model. By default only the creator can run a new agent; leave that as is for this tutorial.

![Asking the helper in chat to create Engineer](/images/docs/tutorial-chat-create-engineer.webp)

Open **Agents → Engineer** to check the result: the model is Opus, and the instructions were generated by Orvilo helper — scope of responsibility, how to get repository code, and requirements for submitting changes. They can be edited at any time.
...
That completes the setup. From here on, this is how day-to-day work runs in Orvilo: requirements are written as issues and handed to agents.
...
A skill is a knowledge package for an agent: one `SKILL.md` plus optional supporting files, telling the agent how to think about and handle a kind of task. Orvilo follows the Anthropic Agent Skills open standard, so any skill that matches the spec can be imported directly; once attached to an agent, it syncs to the runtime at execution time. Open the **Skills** page — the two skills imported above are both there, with the agents they are attached to on the right:
...
Read the files of a third-party skill before importing it: Orvilo does not sandbox them, and a skill's content is handed to the AI coding tool as-is.
...
Configuring it takes one sentence: tell Orvilo helper that at nine every Monday morning, Reviewer should run a full check over the site's repository — redundancy and dead code, questionable structure, places that could follow conventions better — writing findings into issue comments and changing no code:
...
Agents are first-class members and all collaboration stays on the issue timeline — that is the way of working Orvilo provides. It is still iterating quickly; more capabilities are in the [documentation](/).

## Next steps

- [Core concepts](/concepts) — meet all of Orvilo's core objects in three minutes.
