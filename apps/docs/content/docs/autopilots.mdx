...
Each autopilot stores a Runbook, an assignee, and one or more triggers. When triggered, Patchbay creates an issue or runs the agent directly, and keeps a record of every run.
...
After adding a webhook trigger, Patchbay generates a unique URL. Send it a JSON object or array to trigger the autopilot:
...
Patchbay periodically checks whether recent runs keep failing: when the past 7 days have at least 50 completed or failed runs with a failure rate of 90%, the system pauses the autopilot and notifies its creator; fix the cause, then resume it manually.
