...
<Callout type="info">
Before an agent can execute, the computer that runs it needs at least one [supported AI coding tool](/providers) installed and signed in — for example Claude Code, Codex, or Cursor. Orvilo does not ship these tools.
</Callout>

## 1. Sign in and open your workspace

Sign in on the web ([patchbay.aspectlylabs.com](https://patchbay.aspectlylabs.com)) or with [Orvilo Desktop](https://github.com/alexj11324/Cordy/releases/latest). Email verification codes and Google are both supported.
...
3. Submit. Orvilo creates the issue, sets the agent as the executor, and starts executing immediately.

![New issue dialog in agent mode: Created by set to Orvilo Assistant](/images/docs/quickstart-new-issue.webp)
...
- [How Orvilo works](/how-patchbay-works) — how issues, agents, runtimes, and each run relate.
