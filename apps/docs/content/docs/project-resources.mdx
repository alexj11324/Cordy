...
- **The result is a branch in your repo**, named `agent/<agent>/<task>`. The shared **Agent thread** shows the branch name (and lets you copy it); you can also find it with `git branch`. Review it with `git log` / `git diff`, and merge or cherry-pick it yourself. Patchbay never merges it for you. A run that failed partway still reports its branch, because it still committed whatever the agent had produced.
