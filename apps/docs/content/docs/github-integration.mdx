...
- Attaching never records close intent: merging still auto-closes only via `Closes` / `Fixes` / `Resolves` in the title or body.

Both paths coexist and are idempotent: an attached PR later matched by the webhook keeps one card, and the webhook backfills or refreshes its metadata (including the installation link). Keeping the identifier in the title/branch remains recommended as the fallback, but it's no longer the only way the sidebar lights up.
