...
description: Link pull requests to Orvilo issues and watch development progress from the issue.
---

import { Callout } from "fumadocs-ui/components/callout";

Once GitHub is connected, Orvilo automatically links pull requests based on issue identifiers. Issue detail shows PR state, change size, CI results, and merge conflicts in place.
...
Self-hosted Orvilo can also connect self-hosted Forgejo, Gitea, or GitLab instances in parallel, with the same PR auto-linking, merge-to-Done, and CI display. The entry point is **Settings → Integrations → Git hosting**; see [Self-hosted Git hosting](/vcs-integration). Orvilo Cloud does not offer this entry point.
...
5. Return to Orvilo after the installation completes.
...
<Callout type="info">
The **GitHub connection** decides which repositories Orvilo receives PR events from; the **code repositories** setting decides which repositories agents can pick when running tasks. They serve different purposes and are configured separately.
</Callout>
...
| **PR card → CI & mergeability** | For each linked PR, Orvilo fetches an authenticated GitHub API snapshot and mirrors its **CI status** and **mergeability** onto the card (see **What the PR card shows** below). |
...
Orvilo ignores case and only matches the current workspace's issue prefix. One PR can link to multiple issues.
...
CI status and mergeability come from snapshots Orvilo pulls from the GitHub API, and the two are independent; merged or closed PRs no longer show either. When GitHub is temporarily unavailable, the card keeps the last snapshot and marks it stale instead of going blank.
...
A merged PR doesn't necessarily mean the issue is finished. Orvilo only moves an issue to `Done` when all of the following hold:
...
The same GitHub App installation can be connected to multiple Orvilo workspaces. GitHub events flow into each workspace separately and are matched against each workspace's own issue prefix.
...
Clicking **Disconnect** in **Settings → GitHub** only removes the relationship between the current Orvilo workspace and the installation; it does not uninstall the App from GitHub for you. Existing PR records are kept, and new events stop flowing into that workspace.

To revoke repository authorization on the GitHub side, uninstall the App or adjust its repository scope from your personal or organization GitHub App installations page. After uninstalling, every Orvilo workspace linked to that installation stops receiving events.

## Self-hosting setup

Orvilo Cloud doesn't need this section. Self-hosting requires creating your own GitHub App first.
...
| Field | Value |
|---|---|
| Homepage URL | Your Orvilo frontend address, e.g. `https://patchbay.example.com` |
...
If you don't need CI shown in Orvilo, you can skip the Checks and Commit statuses permissions and their events. Contents is still required — without it the snapshot query fails outright and the PR card also loses merge state.
...
The next two variables are required for the PR card to show CI status and mergeability — Orvilo uses them to authenticate as the App and pull snapshots:
