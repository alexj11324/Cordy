...
| Co-authored-by | Adds `Co-authored-by: orvilo-agent <github@aspectlylabs.com>` to commits created by agents. |
...
| Field | Value |
|---|---|
| Homepage URL | Your Orvilo frontend address, e.g. `https://orvilo.example.com` |
...
Take the slug from the App's public URL. For example, the slug of `https://github.com/apps/orvilo-acme` is `orvilo-acme`.

```dotenv
GITHUB_APP_SLUG=orvilo-acme
