...
DeepSeek Harness advertises its model catalog from the runtime itself (`dsh --profile orvilo --list-models`). Its model ids use the `provider/model` form (for example `deepseek-official/deepseek-chat`), so select the full id from the list.
