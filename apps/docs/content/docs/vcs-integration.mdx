...
network. A server operator must enable it (`ORVILO_VCS_INTEGRATION_ENABLED=true`)
and set `ORVILO_VCS_SECRET_KEY`; until then the section does not appear in
...
```sh
ORVILO_VCS_SECRET_KEY=<base64 32-byte key>
```
...
Set `ORVILO_PUBLIC_URL` to the server's public base URL so Patchbay can show a ready-to-paste webhook URL. Without it, the UI shows only the webhook path and you prepend your own origin.
