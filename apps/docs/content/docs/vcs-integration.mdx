...
<Callout type="warn">
**Self-hosted Cordy only.** This integration is available only when you run
Cordy yourself; it is not offered on Cordy Cloud. "Self-hosted" here means
**Cordy** is self-hosted — typically so it can reach a Git instance on your own
network. A server operator must enable it (`CORDY_VCS_INTEGRATION_ENABLED=true`)
and set `CORDY_VCS_SECRET_KEY`; until then the section does not appear in
Settings → Integrations.
</Callout>

Cordy connects to a self-hosted Git provider per workspace: **Forgejo**, **Gitea**, or **GitLab**. Once connected, any pull request (GitLab: merge request) whose branch name, title, or body contains an issue identifier (for example `PB-123`) is **auto-linked** to that [issue](/issues), appears under **Pull requests** in the issue sidebar, and — when merged with a closing keyword — moves the issue to **Done**. CI for the head commit shows as a checks bar on the card.
...
```sh
CORDY_VCS_INTEGRATION_ENABLED=true
```
...
```sh
CORDY_VCS_SECRET_KEY=<base64 32-byte key>
```
...
Set `CORDY_PUBLIC_URL` to the server's public base URL so Cordy can show a ready-to-paste webhook URL. Without it, the UI shows only the webhook path and you prepend your own origin.
...
2. In Cordy, open **Settings → Integrations → Git providers**.
3. Choose the **provider**, enter the instance URL (for example `https://forgejo.example.com`) and the access token, then **Connect**. Cordy validates the token against the instance before saving.
...
Cordy authenticates every delivery against the stored secret, so a webhook without the matching secret is rejected.
