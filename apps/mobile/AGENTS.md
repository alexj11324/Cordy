...
- `import type` from `@cordy/core/types/*` (zero runtime coupling)
- Pure functions from `@cordy/core/`
...
1. **Read the payload.** Find the event in `@cordy/core/types/events.ts`. Note the fields; decide if patch is possible (full object) or invalidate is required (just an id).
