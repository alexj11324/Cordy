...
For cross-app sharing rules, see the root `AGENTS.md`. This file documents the locked tech-stack baseline and the few mobile-specific rules — so AI doesn't suggest outdated alternatives.

## What mobile may import from `packages/`

- `import type` from `@patchbay/core/types/*` (zero runtime coupling)
- Pure functions from `@patchbay/core/`
...
- **State enums / transitions** — render every status / priority / inbox type / comment type, with a sensible fallback for unknown values (per the root `AGENTS.md`). Never silently drop a category.
...
1. **Read the payload.** Find the event in `@patchbay/core/types/events.ts`. Note the fields; decide if patch is possible (full object) or invalidate is required (just an id).
...
- **Android falls back to a regular modal** — no rounded corners, no native drag. `apps/mobile/AGENTS.md` treats iOS as the primary target so this is acceptable, but document inline at the call site if a particular feature must work identically on both.
...
status / priority / executor / label / project / due-date all sit next
to each other). Mixing centered cards (for status/priority, short
fixed lists) with formSheet routes (for executor/label/project, long
