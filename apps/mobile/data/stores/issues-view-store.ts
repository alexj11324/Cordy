 ...
 * My Issues has its own `assigned (Owned) / created / agents` scopes.
 *
 * The `scope` filter is **client-side** on owner/executor type — see
 ...
 * `@orvilo/core/issues/stores/issues-scope-store` — mobile only
 * `import type` from `@orvilo/core/types/*` per Sharing Principles, and
...
import type { IssuePriority, IssueStatus } from "@orvilo/core/types";
