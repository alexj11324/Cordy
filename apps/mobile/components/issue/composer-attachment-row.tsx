...
export type MentionChipType = "member" | "agent" | "team" | "all" | "issue";
...
export interface MentionChip {
   ...
   *  human identifier (e.g. "PB-123"), which is what the chip + the
...
function MentionChipView({
  ...
  // Issue chips show the bare identifier (e.g. "PB-123") — no leading @.
...
function AttachmentChipView({ item, onRemove, onRetry }: AttachmentChipProps) {
  ...
  const onPress = () => {
    ...
    if (isImage) {
      ...
      // `/api/attachments/{id}/download` (PB-2976). RN's `Linking.openURL`
