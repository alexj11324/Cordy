...
import { pickLocale, type SupportedLocale } from "@cordy/core/i18n";
...
import { useHasOnboarded } from "@cordy/core/paths";
import { setCurrentWorkspace } from "@cordy/core/platform";
import { ThemeProvider } from "@cordy/ui/components/common/theme-provider";
...
import { captureEvent } from "@cordy/core/analytics";
...
function AppContent() {
  ...
  // can pick the matching CLI profile (server_url from ~/.cordy config).
  useEffect(() => {
    if (!runtimeConfig) return;
    window.daemonAPI.setTargetApiUrl(runtimeConfig.apiUrl);
  }, [runtimeConfig]);

  // Listen for invite IDs delivered via deep link (cordy://invite/<id>).
  ...
  // Listen for auth token delivered via deep link (cordy://auth/callback?token=...).
    ...
    <div className="flex h-screen items-center justify-center bg-background p-8 text-foreground">
      <div className="max-w-xl rounded-lg border bg-card p-6 shadow-sm">
        ...
        <p className="mt-3 text-body text-muted-foreground">
          Cordy Desktop could not load <code>~/.cordy/desktop.json</code>. Fix or remove the file and restart the app.
        </p>
