...
const PREFS_PATH = join(homedir(), ".cordy", "desktop_prefs.json");
...
// `cordy daemon start` blocks until the daemon reports ready, polling /health
// for up to its own startup timeout (45s in server/cmd/cordy/cmd_daemon.go) to
 ...
 * a stuck start is an auth problem. Hits the same endpoint `cordy auth status`
 ...
 * the user's default CLI profile at `~/.cordy/` — neither its files nor its
 * health port.
 */
...
function findCliOnPath(): string | null {
  const candidates = process.platform === "win32" ? ["cordy.exe"] : ["cordy"];
...
function bundledCliPath(): string {
  const binName = process.platform === "win32" ? "cordy.exe" : "cordy";
...
/**
 * Returns a usable `cordy` binary path. Priority:
...
async function mintPat(jwt: string): Promise<string> {
  ...
  const res = await fetch(url, {
    ...
    body: JSON.stringify({ name: "Cordy Desktop" }),
  });
  if (!res.ok) {
...
function desktopSpawnEnv(): NodeJS.ProcessEnv {
  return { ...process.env, CORDY_LAUNCHED_BY: "desktop" };
}

async function startDaemon(): Promise<{ success: boolean; error?: string }> {
  const bin = await resolveCliBinary();
  if (!bin) return { success: false, error: "cordy CLI is not installed" };
