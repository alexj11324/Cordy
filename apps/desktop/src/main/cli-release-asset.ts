const RELEASE_ARCHIVE_PREFIX = "orvilo-cli-";
...
export function selectPlatformReleaseAssetName(
  ...
  // Prefer the versioned `orvilo-cli-<v>-<os>-<arch>.<ext>` name; fall
  // back to the legacy `orvilo_<os>_<arch>.<ext>` so older releases that
  ...
  const legacyName = `orvilo_${os}_${mappedArch}.${ext}`;
