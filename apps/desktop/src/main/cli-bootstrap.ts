...
// Desktop prefers the bundled `cordy` CLI shipped inside the app for
...
const GITHUB_LATEST_BASE =
  "https://github.com/cordy-ai/cordy/releases/latest/download";

function binaryName(): string {
  return process.platform === "win32" ? "cordy.exe" : "cordy";
}
...
async function installFresh(): Promise<string> {
  ...
  const workDir = join(tmpdir(), `cordy-cli-${Date.now()}`);
...
/**
 * Returns the path to a usable `cordy` binary. If one is already present at
