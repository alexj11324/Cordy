...
describe("deriveVersion (real git describe)", () => {
  ...
  function initRepo() {
    const dir = mkdtempSync(join(tmpdir(), "cordy-desktop-ver-"));
    repos.push(dir);
    const run = (...args) =>
      execFileSync("git", args, { cwd: dir, encoding: "utf-8" });
    run("init", "-q");
    run("config", "user.email", "test@cordy.ai");
...
describe("electron-builder.yml packaging config", () => {
  // Regression guard for github.com/alexj11324/Cordy/issues/5595. The
