 ...
 *
 * `start_url` is deliberately NOT "/". The root path first resolves the
 * signed-in workspace and otherwise enters the login flow. "/inbox" is one of
 * `LEGACY_ROUTE_SEGMENTS`, which proxy.ts resolves per session: signed in with
 * a known workspace it lands on that workspace's inbox, signed in without one
 * it lands on /login (which resolves against the workspace list), and signed
 * out it lands on /login too. All three are pinned in manifest.test.ts,
 * because a launcher icon has no URL bar to recover from a wrong destination.
 *
