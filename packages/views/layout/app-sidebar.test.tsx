...
vi.mock("@cordy/ui/components/ui/collapsible", () => ({
...
vi.mock("@cordy/ui/components/common/actor-avatar", () => ({ ActorAvatar: () => <span /> }));
...
vi.mock("@cordy/core/api", async (importOriginal) => {
  const actual = await importOriginal<typeof import("@cordy/core/api")>();
  return {
    ...actual,
    api: {
      ...actual.api,
...
vi.mock("@cordy/core/issues/queries", () => ({ issueDetailOptions: () => ({ queryKey: ["issue"] }) }));
vi.mock("@cordy/core/issues/stores/create-mode-store", () => ({
  useCreateModeStore: { getState: () => ({ lastMode: "agent" }) },
  openCreateIssueWithPreference: vi.fn(),
}));
vi.mock("@cordy/core/issues/stores/draft-store", () => ({ useIssueDraftStore: () => false }));
vi.mock("@cordy/core/modals", () => ({ useModalStore: { getState: () => ({ modal: null, open: vi.fn() }) } }));
vi.mock("@cordy/core/pins/mutations", () => ({ useDeletePin: () => ({ mutate: deletePin }), useReorderPins: () => ({ mutate: vi.fn() }) }));
vi.mock("@cordy/core/pins/queries", () => ({ pinListOptions: () => ({ queryKey: ["pins"] }) }));
vi.mock("@cordy/core/projects/queries", () => ({ projectDetailOptions: () => ({ queryKey: ["project"] }) }));
