...
import { larkInstallationsOptions, larkKeys } from "@cordy/core/lark";
import { api, ApiError } from "@cordy/core/api";
import type { LarkInstallation, LarkInstallStatusResponse } from "@cordy/core/types";
...
// install path is per-agent (each Cordy Agent gets exactly one Bot —
...
function InstallationRow({
  ...
  const { t } = useT("settings");
  // The bot is bound 1:1 to a Cordy Agent (per the (workspace_id,
  // agent_id) UNIQUE in lark_installation). Render the Cordy agent's
