...
describe("surface entry resolution", () => {
  ...
  it("refuses a plaintext source on a real host", () => {
    expect(resolveSurfaceEntry(installation({ source_url: "http://example.com/patchbay.plugin.json" }), SURFACE)).toBeNull();
  });
  ...
  it("allows plain HTTP from loopback so a surface can be developed locally", () => {
    ...
    expect(resolveSurfaceEntry(installation({ source_url: "http://localhost:8787/patchbay.plugin.json" }), SURFACE))
      .toBe("http://localhost:8787/ui/main.js");
    expect(resolveSurfaceEntry(installation({ source_url: "http://127.0.0.1:8787/patchbay.plugin.json" }), SURFACE))
      .toBe("http://127.0.0.1:8787/ui/main.js");
  });

  it("does not treat a hostname that merely contains localhost as loopback", () => {
    expect(resolveSurfaceEntry(installation({ source_url: "http://localhost.evil.test/patchbay.plugin.json" }), SURFACE)).toBeNull();
    expect(resolveSurfaceEntry(installation({ source_url: "http://notlocalhost/patchbay.plugin.json" }), SURFACE)).toBeNull();
  });
});
