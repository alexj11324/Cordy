...
// is agent-only.
function MembersCell({ team }: { team: Team }) {
  const preview = team.member_preview ?? [];
  const count = team.member_count ?? preview.length;
  if (count === 0) {
    return (
...
// once archived a team is gone from the UI.
// ---------------------------------------------------------------------------
...
function TeamRowActions({ team }: { team: Team }) {
  const { t } = useT("teams");
...
const COLUMN_KEYS: TeamColumnKey[] = ["members", "creator", "created"];
const SORT_FIELDS: TeamSortField[] = ["name", "members", "created"];
...
function TeamListToolbar({
  ...
  scopeCounts: Record<TeamsScope, number>;
  filters: TeamListFilters;
  onToggleFilter: (key: keyof TeamListFilters, value: string) => void;
  onClearFilters: () => void;
  ...
  const SCOPE_LABELS: Record<TeamsScope, string> = {
    mine: t(($) => $.scope.mine),
    all: t(($) => $.scope.all),
  };
  const SORT_LABELS: Record<TeamSortField, string> = {
    name: t(($) => $.page.table.name),
    members: t(($) => $.page.table.members),
    created: t(($) => $.page.table.created),
  };
  const COLUMN_LABELS: Record<TeamColumnKey, string> = {
    members: t(($) => $.page.table.members),
      ...
      <div className="flex min-w-0 items-center gap-2">
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="start" className="w-auto">
            <DropdownMenuRadioGroup
              value={scope}
              onValueChange={(value) => onScopeChange(value as TeamsScope)}
            >
              {TEAM_SCOPES.map((s) => (
              ...
              <DropdownMenu>
                ...
                <DropdownMenuContent align="start" className="w-auto">
                  <DropdownMenuRadioGroup
                    value={sortField}
                    onValueChange={(v) =>
                      onSortFieldChange(v as TeamSortField)
                    }
                  >
...
export function TeamsPage() {
  ...
  const scope = useTeamsViewStore((s) => s.scope);
  const setScope = useTeamsViewStore((s) => s.setScope);
  ...
  const handleSort = useTeamsViewStore((s) => s.toggleSort);
  const handleSortFieldSelect = useTeamsViewStore((s) => s.setSortField);
  ...
  const scopeCounts = useMemo<Record<TeamsScope, number>>(() => {
    let mine = 0;
    if (currentUser) {
      for (const s of teams) if (s.creator_id === currentUser.id) mine++;
    }
    return { mine, all: teams.length };
  }, [teams, currentUser]);

  // Rows within the current scope, unfiltered — filter option lists + the
  // "n / total" denominator derive from this.
  const scopeRows = useMemo<Team[]>(() => {
    return teams.filter((s) => {
      if (scope === "mine") {
        return !!currentUser && s.creator_id === currentUser.id;
      }
      return true;
    });
  }, [teams, scope, currentUser]);
              ...
              {rows.length === 0 ? (
                ...
                rows.map((team) => (
                  <ListGridRow
                    ...
                    {...rowLink(p.teamDetail(team.id), team.name)}
                  >
                    <NameCell team={team} />
                    <LeaderCell
                      leaderId={team.leader_id}
                      leader={agentsById.get(team.leader_id)}
                    />
                    {isColVisible("members") ? (
                      <MembersCell team={team} />
                    ) : (
                    ...
                    {isColVisible("creator") ? (
                      <ListGridCell className="hidden gap-1.5 @2xl:flex">
                        <ActorAvatar
                          actorType="member"
                          actorId={team.creator_id}
                          size="sm"
                        />
                        <span className="min-w-0 truncate text-caption text-muted-foreground">
                          {membersById.get(team.creator_id)?.name ??
                            team.creator_id.slice(0, 8)}
                        </span>
                      </ListGridCell>
                    ) : (
                      <ListGridCell className="hidden px-0 @2xl:flex" />
    ...
    <div className="min-h-0 flex-1 overflow-auto @container">
      <ListGrid
        ...
        style={columnTrackVars(
          (key) => !TEAM_DEFAULT_HIDDEN_COLUMNS.includes(key),
          true,
        )}
      >
        <ListGridHeader>
