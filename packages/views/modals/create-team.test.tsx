...
// "Create Team" is both the dialog title and the submit button label. Always
// pick by role so tests don't depend on DOM order between the two.
function getSubmitButton(): HTMLButtonElement {
  const btn = screen
    .getAllByRole("button")
    .find((b) => b.textContent === "Create Team");
  if (!btn) throw new Error("Create Team submit button not found");
...
function renderModal() {
  ...
  render(
    <I18nProvider locale="en" resources={TEST_RESOURCES}>
      <CreateTeamModal onClose={onClose} />
    </I18nProvider>,
  );
  return { onClose };
}
...
describe("CreateTeamModal", () => {
  ...
  it("binds the name and description inputs in the identity row", () => {
    ...
    const desc = screen.getByPlaceholderText(/Describe what this team/i) as HTMLInputElement;
  ...
  it("auto-clears an additional-members entry when the same agent is picked as leader", async () => {
    ...
    // payload sent to addTeamMember (none — leader was the only pick).
    mocks.createTeam.mockResolvedValue(makeTeam({ leader_id: "agent-mine-2" }));
    ...
    await waitFor(() => {
      expect(mocks.createTeam).toHaveBeenCalledTimes(1);
    });
    // addTeamMember must NOT be called for the agent we promoted to leader.
  ...
  it("on success with partial member failure shows success + warning toasts and still navigates", async () => {
    ...
    await waitFor(() => {
      expect(mocks.addTeamMember).toHaveBeenCalledTimes(2);
    });
    await waitFor(() => {
      expect(mocks.toastSuccess).toHaveBeenCalledTimes(1);
    });
  ...
  it("on createTeam failure shows an error toast, does not navigate, and re-enables submit", async () => {
    renderModal();
    fireEvent.change(screen.getByPlaceholderText(/e\.g\. Frontend Team/i), {
      target: { value: "Boom Team" },
    });
    fireEvent.click(firstMatch("MineAgentOne"));

    mocks.createTeam.mockRejectedValueOnce(new Error("server down"));
    ...
    // Submit button is re-enabled (textContent reads "Create Team" again,
    // not "Creating...").
