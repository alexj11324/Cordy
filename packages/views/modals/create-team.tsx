    ...
    try {
      ...
      if (selectedMembers.length > 0) {
        await Promise.allSettled(
          selectedMembers.map(async (m) => {
            try {
              ...
              toast.warning(
                t(($) => $.create_team.toast_member_add_failed, {
                  name: m.name,
                  error:
        ...
        queryClient.invalidateQueries({
          queryKey: [...workspaceKeys.teams(wsId), team.id, "members"],
        });
      }

      onClose();
      toast.success(t(($) => $.create_team.toast_created));
      router.push(wsPaths.teamDetail(team.id));
    } catch (err) {
      toast.error(
        err instanceof Error
          ? err.message
          : t(($) => $.create_team.toast_failed),
      );
      setCreating(false);
    }
  };
      ...
      <DialogContent className="p-0 gap-0 flex flex-col overflow-hidden !top-1/2 !left-1/2 !-translate-x-1/2 !-translate-y-1/2 !w-full !max-w-2xl !h-[85vh]">
              ...
              <div className="flex-1 min-w-0 space-y-3">
                <div>
                  ...
                  <Input
                    ...
                    placeholder={t(($) => $.create_team.name_placeholder)}
                    className="mt-1"
                    onKeyDown={(e) => {
                ...
                <div>
                  <Label className="text-caption text-muted-foreground">
                    {t(($) => $.create_team.description_label)}
                  </Label>
                  <Input
                    type="text"
                    value={description}
                    onChange={(e) => setDescription(e.target.value)}
                    placeholder={t(($) => $.create_team.description_placeholder)}
        ...
        <div className="flex items-center justify-end gap-2 border-t bg-background px-5 py-3">
          <Button variant="ghost" onClick={onClose}>
            {t(($) => $.create_team.cancel)}
          </Button>
          <Button onClick={() => void handleSubmit()} disabled={!canSubmit}>
            {creating
              ? t(($) => $.create_team.submitting)
          ...
          <PopoverTrigger className="flex w-full min-w-0 items-center gap-3 rounded-lg border border-border bg-background px-3 py-2.5 text-left text-body transition-colors hover:bg-muted">
            ...
            <div className="min-w-0 flex-1">
              <div className="truncate font-medium">
                {selected?.name ?? t(($) => $.create_team.leader_placeholder)}
              </div>
          ...
          <PopoverContent align="start" className="w-[var(--anchor-width)] p-0">
            <div className="border-b px-2 py-1.5">
              <input
                ...
                placeholder={t(($) => $.create_team.picker_search_placeholder)}
                className="w-full bg-transparent text-body placeholder:text-muted-foreground outline-none"
              />
            </div>
            <div className="max-h-72 overflow-y-auto p-1">
              {filteredMine.length > 0 && (
                <PickerSection label={t(($) => $.create_team.group_my_agents)}>
                  {filteredMine.map((a) => (
                    <PickerItem
                      key={a.id}
                      selected={value === a.id}
              ...
              {filteredOthers.length > 0 && (
                <PickerSection label={t(($) => $.create_team.group_workspace_agents)}>
...
function AdditionalMembersPicker({
  ...
  return (
    <div>
      <Label className="text-caption text-muted-foreground">
        {t(($) => $.create_team.members_label)}{" "}
        <span className="text-muted-foreground">
          {t(($) => $.create_team.members_optional)}
        </span>
      </Label>
      <p className="mt-0.5 mb-1.5 text-caption text-muted-foreground">
        {t(($) => $.create_team.members_hint)}
      </p>
      ...
      >
        ...
        <PopoverTrigger
          render={
            <div
              ...
              aria-expanded={open}
              aria-controls="team-member-listbox"
              tabIndex={0}
              ...
              {value.length === 0 ? (
                <>
                  ...
                  <span className="min-w-0 flex-1 truncate text-muted-foreground">
                    {t(($) => $.create_team.members_placeholder)}
                  </span>
                </>
              ) : (
                <div className="flex min-w-0 flex-1 flex-wrap items-center gap-1.5">
                  ...
                  {value.length > CHIP_DISPLAY_LIMIT && (
                    <span className="rounded-md bg-muted px-1.5 py-0.5 text-caption font-medium text-muted-foreground tabular-nums">
                      {t(($) => $.create_team.members_more_count, {
        ...
        <PopoverContent align="start" className="w-[var(--anchor-width)] p-0">
          ...
          <div id="team-member-listbox" role="listbox" className="max-h-72 overflow-y-auto p-1">
            {filteredMine.length > 0 && (
              <PickerSection label={t(($) => $.create_team.group_my_agents)}>
                {filteredMine.map((a) => (
                  <PickerItem
            ...
            {filteredMembers.length > 0 && (
              <PickerSection label={t(($) => $.create_team.group_members)}>
  ...
  return (
    <span className="inline-flex items-center gap-1 rounded-full border bg-background px-1.5 py-0.5 text-caption">
      ...
      <button
        ...
        aria-label={t(($) => $.create_team.members_remove_aria, { name: m.name })}
