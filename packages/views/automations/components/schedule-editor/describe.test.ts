...
): TFunction<"automations"> {
  const i18n = createI18n(locale, { [locale]: { automations: resources } });
  return i18n.getFixedT(locale, "automations") as TFunction<"automations">;
}

describe("describeSchedule (en)", () => {
  const t = fixedT("en", enAutomations);
...
describe("describeSchedule (zh-Hans)", () => {
  const t = fixedT("zh-Hans", zhAutomations);
  const describeExpr = (expr: string) => describeSchedule(t, parseCron(expr, TZ));
