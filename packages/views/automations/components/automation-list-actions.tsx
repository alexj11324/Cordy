  ...
  const handleDelete = async () => {
    ...
    try {
      for (const row of rows) {
        await deleteAutomation.mutateAsync(row.id);
      }
      onOpenChange(false);
      onDeleted?.();
    } catch (err) {
...
function useSetStatus() {
  ...
  return async (rows: Automation[], status: "active" | "paused") => {
    try {
      for (const row of rows) {
        if (row.status === status) continue;
        await updateAutomation.mutateAsync({ id: row.id, status });
      }
    } catch (err) {
      toast.error(err instanceof Error ? err.message : String(err));
    }
...
export function AutomationRowActions({ row }: { row: Automation }) {
  const { t } = useT("automations");
      ...
      <DropdownMenu>
        ...
        <DropdownMenuContent align="end" className="w-48">
          <DropdownMenuItem
            onClick={() =>
              intentNavigate(
                wsPaths.automationDetail(row.id),
                "foreground-tab",
                row.title,
              )
            }
...
export function AutomationBatchToolbar({
  rows,
  onClear,
}: {
  rows: Automation[];
