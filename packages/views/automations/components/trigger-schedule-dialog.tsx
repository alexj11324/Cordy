...
export function TriggerScheduleDialog({
  ...
  onSaveDraft?: (config: ScheduleConfig) => void;
  ...
  const submit = async () => {
    ...
    try {
      ...
      const schedule = {
        automationId,
        cron_expression: toCron(config),
        timezone: config.timezone,
      };
      if (trigger)
        await updateTrigger.mutateAsync({ ...schedule, triggerId: trigger.id });
      ...
      <DialogContent className="max-w-md">
        ...
        <div className="min-w-0 space-y-4 pt-2">
          ...
          <div className="flex justify-end gap-2 pt-1">
            ...
            <Button
              size="sm"
              disabled={submitting || !gate.scheduleValid}
              onClick={submit}
            >
              {submitting
                ? t(($) => $.dialog.saving)
