...
export function AutomationDialog(props: AutomationDialogProps) {
  ...
  // Editing an automation that has no schedule: the panel has nothing to
  ...
  // copy / done affordances. This avoids the "now go find your automation
    ...
    try {
      ...
      if (isCreate) {
        ...
        if (triggerOk) {
          ...
          // Partial success: automation saved, schedule failed. Show the
    ...
    <Dialog open={open} onOpenChange={onOpenChange}>
      ...
      >
        ...
        <div className="flex items-center justify-between gap-3 px-5 py-3 border-t shrink-0 bg-background">
          {/* The hint drops out while the schedule section states the automation
...
// ---------------------------------------------------------------------------
// Post-create state for webhook automations: shows the freshly minted URL
