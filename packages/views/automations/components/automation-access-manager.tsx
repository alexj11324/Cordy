...
// Grant / revoke explicit write access to an automation. Members-only, mirroring
...
// Save action.
export function AutomationAccessManager({
  automationId,
  collaborators,
}: {
  automationId: string;
  ...
  const grant = useGrantAutomationAccess();
  const revoke = useRevokeAutomationAccess();
  ...
  const handleGrant = async (userId: string) => {
    try {
      await grant.mutateAsync({ automationId, userId });
  ...
  const handleRevoke = async (userId: string) => {
    try {
      await revoke.mutateAsync({ automationId, userId });
