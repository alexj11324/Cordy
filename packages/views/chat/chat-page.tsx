...
export function ChatPage() {
  ...
  // Compact compose can be dismissed by a topics-view request. 0 is inert
  // (same convention as focusRequest) so the initial mount — including
  // StrictMode's replay — does not close a compose pane that `?agent=` just
  // opened.
  useEffect(() => {
    if (topicsViewRequest === 0) return;
  ...
  useEffect(() => {
    ...
    if (agentIntentRevision !== observedAgentIntentRevision.current) {
      // A sibling surface (compact thread list, new-chat control, or a
      // later explicit selection) made a choice while this deep link was
      // still pending. Consume the stale intent without starting a
      // different chat.
