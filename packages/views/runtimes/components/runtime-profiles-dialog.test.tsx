...
describe("RuntimeProfilesDialog", () => {
  ...
  it("shows the custom empty state and keeps built-in protocols collapsed", () => {
    ...
    expect(
      screen.getByText("Create your first custom Harness"),
    ).toBeInTheDocument();
    expect(
    ...
    expect(
      screen.getAllByRole("button", { name: "New custom Harness" }),
    ).toHaveLength(1);
  });
  ...
  it("renders custom profiles before the collapsed built-in reference section", () => {
    ...
    const customTitle = screen.getByText("Custom Harnesses (1)");
  ...
  it("opens the focused create flow from a machine and closes after creation", async () => {
    ...
    expect(
      screen.getByRole("heading", { name: "New custom Harness" }),
    ).toBeInTheDocument();
    ...
    expect(screen.getByRole("link", { name: "View setup guide" })).toHaveAttribute(
      "href",
      "https://orvilo.aspectlylabs.com/docs/daemon-runtimes#custom-runtime-profiles",
    );
    expect(screen.getByText("Step 1 of 2")).toBeInTheDocument();
    expect(
      screen.queryByText("Create your first custom Harness"),
    ).not.toBeInTheDocument();
  ...
  it("opens a custom Harness directly in edit mode", () => {
    renderDialog({
      intent: "edit",
