...
describe("runtime docs links", () => {
  it.each([
    ["en", "https://patchbay.ai/docs/daemon-runtimes"],
    ["zh-Hans", "https://patchbay.ai/docs/zh/daemon-runtimes"],
    ["ja", "https://patchbay.ai/docs/ja/daemon-runtimes"],
    ["ko", "https://patchbay.ai/docs/ko/daemon-runtimes"],
  ...
  it("adds the localized custom runtime section", () => {
    expect(customRuntimeDocsHref("zh-Hans")).toBe(
      `https://patchbay.ai/docs/zh/daemon-runtimes#${encodeURIComponent("自定义运行时配置")}`,
    );
  });
});
