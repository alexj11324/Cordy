...
export const IssueAgentActivityIndicator = memo(function IssueAgentActivityIndicator({
  ...
  if (!hoverCard) {
    return (
      <span className="inline-flex shrink-0 items-center gap-1" data-issue-agent-activity={isRunning ? "running" : "queued"}>{badge}</span>
    );
  }
  ...
  return (
    <HoverCard>
      <HoverCardTrigger
        ...
        render={
          <span className="inline-flex shrink-0 items-center gap-1" data-issue-agent-activity={isRunning ? "running" : "queued"} />
        }
      >
        {badge}
      </HoverCardTrigger>
