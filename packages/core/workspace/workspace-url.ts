// Brand host shown as the workspace URL prefix on the managed Cordy Cloud,
...
// self-hosted server that has not set CORDY_APP_URL / FRONTEND_ORIGIN), so
// this literal must remain the ultimate fallback.
const BRAND_WORKSPACE_HOST = "cordy.ai";
 ...
 * self-hosted instances show their own domain instead of `cordy.ai`. Falls
