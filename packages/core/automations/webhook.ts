 ...
 *
 * Resolution order:
 *  1. trigger.webhook_url — present only when PATCHBAY_PUBLIC_URL is set on the
 ...
 */
export function buildAutomationWebhookUrl(params: {
  trigger: Pick<AutomationTrigger, "kind" | "webhook_token" | "webhook_path" | "webhook_url">;
  apiBaseUrl?: string;
  ...
  const path =
    ...
    (trigger.webhook_token ? `/api/webhooks/automations/${trigger.webhook_token}` : null);
  if (!path) return null;
 ...
 * the automation. The origin and the `/api/webhooks/automations/` prefix carry no
 ...
 */
export function maskAutomationWebhookUrl(url: string): string {
