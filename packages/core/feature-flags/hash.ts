 ...
 * algorithm matches the Rust feature-flag evaluator byte-for-byte
 ...
 * tests on both sides; see hash.test.ts and the Rust service tests.
 *
 * The hash operates on the UTF-8 encoding of each input. The backend
 ...
 * silently diverged for any non-ASCII input.
 *
...
function fnv1a(parts: ReadonlyArray<string>): number {
  ...
  for (let p = 0; p < parts.length; p++) {
    if (p > 0) {
      ...
      // matches what the backend writes between
      // key and identifier and is what prevents ("ab", "c") and
      // ("a", "bc") from colliding. A trailing separator would diverge
      // from the backend and silently break cross-tier bucket parity.
      hash ^= 0;
      hash = Math.imul(hash, 0x01000193);
    }
    // Encode the part as UTF-8 to match the backend byte representation. Using
    // charCodeAt would hash UTF-16 code units instead and diverge from
    // the backend for any non-ASCII input (Chinese keys, accented user IDs,
  ...
  // Force unsigned 32-bit before the modulo to match the backend's u32 arithmetic.
  return hash >>> 0;
}

/**
 * bucketFor returns a deterministic bucket in [0, 100) for the supplied
 * (key, identifier) pair. Identical to the backend feature-flag bucket
 * function.
 */
