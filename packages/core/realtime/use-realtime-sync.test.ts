...
describe("applyChatDoneToCache", () => {
  it("writes structured attribution into the inline assistant row", () => {
    const qc = createQueryClient();
    qc.setQueryData<ChatMessage[]>(messagesKey, [userMessage()]);

    applyChatDoneToCache(qc, donePayload({
      sources: [{ id: "docs", url: "https://example.test/docs", title: "Docs" }],
      citations: [{ source_id: "docs", start: 0, end: 4 }],
    }));

    expect(qc.getQueryData<ChatMessage[]>(messagesKey)?.at(-1)).toMatchObject({
      sources: [{ id: "docs", url: "https://example.test/docs", title: "Docs" }],
      citations: [{ source_id: "docs", start: 0, end: 4 }],
    });
  });
