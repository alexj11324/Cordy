...
export interface BootstrapPatrickInput {
   ...
   * mid-flow and Patrick was created in the previously-active workspace.
   */
...
export interface BootstrapPatrickResult {
  agent: PatrickBootstrapResponse;
  chatSession: ChatSession;
}

/**
 * Creates or reuses the workspace's Patrick and starts its opening conversation.
 *
 ...
 * opening turn under the session lock. Nothing about Patrick's configuration is
 ...
 */
export async function bootstrapPatrick(
  input: BootstrapPatrickInput,
): Promise<
  BootstrapPatrickResult & {
    kickoff: Awaited<ReturnType<typeof api.startPatrickOnboarding>>;
  }
> {
  const agent = await api.createPatrickAgent(
    {
  ...
  if (!chatSession) {
    throw new Error("Patrick onboarding session was not returned");
  }
  ...
  const kickoff = await api.startPatrickOnboarding(
    chatSession.id,
    { language: input.language },
    input.workspaceSlug,
  );
...
export function useBootstrapPatrick(workspaceId: string) {
  const queryClient = useQueryClient();

  return useMutation({
    mutationFn: (input: BootstrapPatrickInput) => bootstrapPatrick(input),
