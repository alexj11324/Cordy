...
beforeEach(() => {
  localStorage.clear();
  useTeamsViewStore.setState({ scope: "mine" });
...
describe("useTeamsViewStore", () => {
  it("defaults to 'mine'", () => {
    expect(useTeamsViewStore.getState().scope).toBe("mine");
  });

  it("setScope mutates the store", () => {
    useTeamsViewStore.getState().setScope("all");
    expect(useTeamsViewStore.getState().scope).toBe("all");
  });
  ...
  it("partialize persists view prefs (no actions) under the workspace-namespaced key", async () => {
    ...
    const raw = localStorage.getItem("patchbay_teams_view:acme");
  ...
  it("rehydrates a different saved scope on workspace switch", async () => {
    localStorage.setItem(
      "patchbay_teams_view:acme",
      JSON.stringify({ state: { scope: "all" }, version: 0 }),
    );
    localStorage.setItem(
      "patchbay_teams_view:beta",
  ...
  it("resets to 'mine' when switching to a workspace with no persisted value", async () => {
    ...
    await flush();
    expect(useTeamsViewStore.getState().scope).toBe("mine");
    expect(localStorage.getItem("patchbay_teams_view:acme")).not.toBeNull();
  });
});
