/**
 * @orvilo/plugin-sdk — what a plugin surface imports.
 *
 * A surface is an ordinary script running in a sandboxed iframe. It holds no
 * credential and cannot reach Orvilo's API directly: every call here becomes a
 ...
 *
 * Zero runtime dependencies, and deliberately no import of `@orvilo/core` or
 * `@orvilo/ui`: this ships to third parties.
 */
...
export class OrviloPluginError extends Error {
  readonly status: number;

  constructor(status: number, message: string) {
    super(message);
    this.name = "OrviloPluginError";
    this.status = status;
  }
}
...
class Bridge {
  ...
  private onPortMessage = (event: MessageEvent) => {
    ...
    else pending.reject(new OrviloPluginError(message.status, message.error));
  };
  ...
  async request<T>(method: BridgeMethod, path: string, body?: unknown): Promise<T> {
    ...
    return new Promise<T>((resolve, reject) => {
      const timer = setTimeout(() => {
        ...
        reject(new OrviloPluginError(408, `Orvilo did not answer ${method} ${path} in time`));
      }, DEFAULT_TIMEOUT_MS);
  ...
  return {
    ...
    async get(key: string): Promise<string | null> {
      try {
        ...
        if (error instanceof OrviloPluginError && error.status === 404) return null;
        throw error;
      }
    },
