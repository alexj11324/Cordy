...
const DATABASE_URL = process.env.DATABASE_URL ?? "postgres://cordy:cordy@localhost:5432/cordy?sslmode=disable";
...
export class TestApiClient {
  ...
  async login(email: string, name: string) {
    ...
    try {
      ...
      const configuredDevCode = process.env.CORDY_DEV_VERIFICATION_CODE?.trim();
