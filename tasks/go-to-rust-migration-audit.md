...
| [~] | AUDIT-004 | 主线切片已交付 | Lark、WeCom、DingTalk、Slack、Telegram、Composio、VCS、GHSnapshot 与 channel media production lifecycle 已交付 | verification 在 T-60 一次性收口 supervisor/lease、外部凭证 smoke、坏凭证/网络失败、回滚和不可测原因矩阵；review/fix 异步回写 | 主 agent 当前无新的不重叠迁移缺口；最终退出依赖异步 V/R/F 直接证据 | PR #532..#536/#538..#541；§5.3、§6.2、§20..§28、§75 | 主 agent；独立 V/R/F subagent |
...
| [~] | AUDIT-008 | 进行中 | route parity 和部分 wire tests 已有；T-54 已把未接入生产字段的 `cordy-util::Ulid` utility 切到 Go-compatible Crockford codec，并创建 Ready PR #577；T-54A 已把 daemon event ID 生成器切到共享 `ulid` crate，并创建 Ready PR #579；T-54B 已把 realtime/daemon 的全部 ULID 生产调用收口到 `cordy-util`，创建 Ready PR #580；T-54C 已补齐 Go/Rust Redis event envelope 的固定字段、缺失字段和 scope routing contract；T-54D 已把 realtime created_at/heartbeat 切到 Go-compatible RFC3339Nano，并创建 Ready PR #582；T-54E 已将 handler/service 已迁移的 RFC3339Nano 输出统一到 `cordy-util`，并创建 Ready PR #583；T-54F 已将 PostHog batch timestamp 切到同一 helper，并创建 Ready PR #584；T-57 已实现共享 HTTP 错误 envelope framing 并创建 Ready PR #585；T-58 已实现 issue response projection 的 null/empty 与 activity timestamp 契约并创建 Ready PR #586；T-59 已收口 DB model/query serializers 的 reserved-key、nullable legacy-row 和 cross-language JSON residual，并创建 Ready PR #588 | 收口 #577/#579/#580/#581/#582/#583/#584/#585/#586/#588 异步 V/R/F；在可用 workspace resolver 与数据库后补跑真实编译、测试和旧行读取，继续按 T-59 的完整 residual wire/schema 契约收口 AUDIT-008，避免单字段微型 PR | utility contract 不是生产兼容或 Go 下线证据；事件切片依赖 AUDIT-002 daemon/realtime 入口；T-57/T-58/T-59 复用现有 helper、投影、model/query/event/serde 入口 | PR #577/#579/#580/#581/#582/#583/#584/#585/#586/#588；§6.2、§64、§66、§67、§68、§69、§70、§71、§72、§73、§74 | 主 agent；独立 V/R/F subagent |
| [~] | AUDIT-009 | 进行中 | 默认入口、pprof 和 logger 文档已有部分更新；T-55 已将 backfill runbook 切到 Rust 入口并创建 Ready PR #578；T-61 已将四种语言的 troubleshooting backfill 命令和 self-host readiness 说明对齐 Rust 入口/响应，并补充 pprof 客户端边界说明 | 收口 #578/#590 异步 V/R/F，并在 T-60 新鲜产物可用后完成 install/systemd/release/rollback/pprof/metrics 文档逐项复核 | 增量文档依赖对应实现；最终退出依赖 AUDIT-001..008 的真实路径与 T-60 产物证据 | PR #523/#524/#525/#578/#590；§6.2、§65、§76 | 主 agent；独立 V/R/F subagent |
...
59. `[~]` `T-54E / §70` AUDIT-008 handler/service RFC3339Nano helper centralization（Ready PR #583，待异步退出证据）
...
任务盘点冻结（2026-08-28 UTC）：本次审计把当前已知、尚未闭合的工作归并为既有 T-53、T-59 至 T-61，
并保留 T-56 作为唯一最终退休门。后续实现阶段不再为单字段、单文件、单测试或新发现的同类缺口
创建新 ID；发现的问题必须归入这四个已登记边界或既有 T-ID，并在对应章节记录，不得绕过台账开工。
...
本索引同时是高风险 API/DB/provider/daemon/security/backfill/CLI 回归矩阵的唯一归属。冻结审计后，矩阵的补证据和必要
contract test 继续回写 T-53/§63；不再为同一回归面创建新的 T-ID。跨边界 wire/schema、旧 schema 和 golden/round-trip
向量单独归入 T-59/§74，生产构建与生命周期验收归入 T-60/§75。
...
## 71. [~] AUDIT-008 analytics PostHog RFC3339Nano timestamp wire compatibility（T-54F）

本项在开始修改 analytics 生产 payload 前登记。Go `server/internal/analytics/posthog.go` 的 batch `captureItem.Timestamp`
使用 `e.Timestamp.UTC().Format(time.RFC3339Nano)`；Rust `cordy-analytics::posthog::build_capture_items` 当前使用 chrono
`SecondsFormat::AutoSi`，对 4/7/8 位有效小数会改变 wire 字符串（例如 `.123400000` → `.123`，Go 为 `.1234`）。

范围只把既有 PostHog batch timestamp 调用切到 T-54D 已验证的 `cordy-util::rfc3339_nano`，补一个固定 fractional vector；
保留 batch schema、properties enrichment、queue/backpressure、HTTP timeout、失败计数、配置和默认 host，不新增 analytics
service、fallback 或 API。

- 默认生产路径：`cordy_analytics::new_from_env` 继续创建唯一 PostHog client，`build_capture_items` 继续生成 `/batch/` payload，
  仅 timestamp formatter 改变。
- Go 是否可下线：不能。该切片只收口 PostHog timestamp 字符串；其余 analytics 事件/属性、handler/daemon 时间字段、真实
...
## 72. [~] AUDIT-008 shared HTTP error envelope framing（T-57）

本项在开始编码前登记，选择 AUDIT-008 中仍未收口的“错误 envelope 与 JSON framing”完整契约面。Go
`server/internal/handler/handler.go` 的 `writeError`/`writeErrorCode` 委托 `writeJSON`，其生产错误响应有固定的
`application/json` content type、精确 `Content-Length`、尾部换行，以及 `encoding/json` 对 `<`、`>`、`&` 和
U+2028/U+2029 的安全转义；Rust `cordy-handler::error::{error_response,error_code_response}` 当前直接使用 Axum
`Json`，没有这些 framing 约束，现有测试也只检查可解析的 JSON 字段，不能证明 Go-compatible wire contract。

范围只收口公共 HTTP 错误响应 helper 及其所有现有生产调用：保持 `{"error": ...}`、
`{"error": ..., "code": ...}` 字段、HTTP status、错误 taxonomy、日志和 fail-closed 语义不变；让 helper 输出
Go-compatible UTF-8 JSON bytes、尾部换行、`Content-Type: application/json` 和准确 `Content-Length`，并为错误消息的
HTML-sensitive 字符及 Unicode line separator 建固定向量。`domain_error`、daemon claim failure、connector error
和其他调用方继续复用现有 helper；不批量改写成功响应、不开第二套 error service、不新增 fallback client 或测试框架。

- 默认生产路径：`cordy-handler` 的 `error_response`/`error_code_response` 仍由现有 HTTP router、daemon claim、provider
  connector 和 API handler 调用；只改变错误 body framing，不改变路由、状态码、错误文本、配置选择或生产 assembly。
- Go 是否可下线：不能。T-57 只证明共享错误 envelope 的 Rust wire framing；其他 JSON null/empty、DB/旧数据兼容、真实 API
  smoke、AUDIT-001..009 和最终 Go 退休门仍未完成。
- 依赖：已迁移并稳定的 `cordy-handler::error` 调用链；不依赖外部凭证或数据库，验证可使用现有 handler crate。
- owner：主 agent 负责 Rust helper、生产调用保持、固定 wire vectors、机械检查、提交/推送和 Ready PR；独立
  verification/reviewer/fixer 负责编译、测试、契约审查和回归修复。
- 实现：Rust `cordy-handler::error::json_response` 已按 Go framing 输出，产品 commit 为
  `b2a42c4f3f74b3fb4ac13a9218313cd3859aa8a6`，独立 fixer 的格式提交为
  `8b485522e8605a1820e4ec3f127004c38bb1229a`；Ready PR #585。
- 验证：fixed-stable `rustfmt --edition 2021 --check` 与 range `git diff --check` 通过；locked/offline
  `cargo metadata --no-deps` 通过。完整 metadata、handler build/check/clippy/test-no-run 以及三个精确错误测试均在
  workspace resolution 阶段 exit 101，继承 #563 的 `hyper-util 0.1.20` `runtime` feature 不存在，matched/executed 为
  0/0；未将其记录为编译或测试通过。reviewer 无 P0–P3、安全、配置或 Ponytail finding；运行时 API smoke 因同一 resolver
  阻断未执行。
...
- owner：主 agent 负责 Rust projection、生产 caller 保持、golden vectors、机械检查、提交/推送和 Ready PR；独立
  verification/reviewer/fixer 负责编译、测试、契约审查和回归修复。
- 实现：Rust `IssueResponse::from_issue` 已将 `last_activity_at` 切到已迁移的
  `cordy_util::rfc3339_nano`；产品 commit 为 `f6925272`，独立 fixer 的 rebase/format 提交为
  `fc8e6777936371c3dadcd0e71266fd80d3ce3553`；Ready PR #586。变更范围只保留 issue.rs 与 T-58 台账。
- 验证：`merge-base --is-ancestor 87ae1dfb HEAD`、range `git diff --check` 与 fixed-stable
  `rustfmt --edition 2021 --check server-rs/crates/cordy-handler/src/issue.rs` 均 PASS；Cargo.lock 未变化，locked/offline
  `cargo metadata --no-deps` PASS。完整 metadata、handler build/check/clippy/test-no-run 与两个精确 issue 测试均在 workspace
  resolution 阶段 exit 101，继承 #563 的 `hyper-util 0.1.20` `runtime` feature 不存在，matched/executed 为 0/0；未记录为
  编译或测试通过。reviewer 在最终 tip 确认原 stacking/scope P1 已关闭，无 correctness/security/config/P0–P3/Ponytail finding；
  runtime API smoke 因同一 resolver 阻断未执行。
...
round-trip 和 Go/Rust cross-language fixture。T-54C/D/E 的既有生产改动不重做；本项只处理静态审计仍无法证明
等价的残余。

范围以跨边界 wire/schema 契约为单位，优先复用 `cordy-db` models/queries、`cordy-protocol`、`cordy-events`、
`cordy-util` formatter/ID codec 和现有 handler/realtime serializers。需要时同时调整生产序列化或旧数据读取，
并用固定向量证明 null/empty、时间、ID、event envelope 和 DB nullable/enum 行为；不拆成单字段 PR，不新建第二套
serializer、migration framework 或 fallback。

- 默认生产路径：现有 `cordy-server` API、`cordy-daemon`/`cordy-realtime` event、`cordy-migrate` 读取/写入和
  provider payload 继续使用唯一 Rust models/serializers；有效配置不选择测试 Stub/Noop/Fake。
- Go 是否可下线：不能。必须先完成 T-53 回归矩阵、T-60 生产验收和 T-61 文档复核，并通过最终全仓引用审计。
...
- 主 agent 仅执行 `git diff --check`；没有运行 cargo、rustfmt、测试、DB 或长编译命令。产品 commit `a8ec9582`，台账
  commit `de0ad432`，格式修复 `ee6480b2`，reviewer P3 去重修复 `4563816f`；最终 tip 已推送至 Ready PR #588
  （<https://github.com/alexj11324/Cordy/pull/588>）。独立 verifier 确认最终四个 touched Rust 文件 rustfmt 与静态字段覆盖
  通过，但 workspace 的 `hyper-util 0.1.20` `runtime` resolver blocker 使完整 metadata/build/check/test 及两个精确
  测试均为 exit 101、matched/executed 0/0；`DATABASE_URL`、`psql`、`pg_isready` 不可用，旧行读取未执行。reviewer
  的唯一 P3 重复 fixture 已由 fixer 最小删除并验证。故本项仍为 `[~]`，不能声称 AUDIT-008 已完成或删除 Go。

## 75. [~] AUDIT-001..006 一次性生产构建/启动/升级/回滚验收（T-60）

本项把 P0 生产闭环集中为一个验收矩阵，减少用户重复跑 CI：覆盖 Rust server/CLI/migrate/backfill 构建与
`--help`/version/退出码、Docker/Helm/systemd/default Makefile 入口、fresh PostgreSQL、`/health`/`/readyz`、
daemon registration→claim→execute→reconcile→shutdown、provider 正/负配置、channel lease/media、pprof/metrics、
升级/降级和 self-host exact-image rollback。它是生产验证/发布切片，不重新实现已交付业务。

范围按真实产物和默认路径运行完整矩阵；缺凭证、网络不可达、平台不可用、磁盘/DNS/resolver 等限制必须记录
...
本项一次性复核 `README`、install/self-host、Docker/Helm、systemd、release/compatibility/rollback、
pprof/metrics/logger/operator troubleshooting 文档与 T-60 实际产物和命令一致性。清除或标记仍属兼容回滚路径的
Go 命令，不把文档文字更新当作生产切换证据。

范围只改与已确认 Rust 入口、binary 名称、参数、环境变量、健康检查、迁移/backfill、升级/回滚和诊断流程直接
相关的文档；由新鲜产物命令逐项复核。不得新增第二套安装器、runbook 或抽象；文档不适用时记录理由。

- 默认生产路径：文档必须指向 T-60 验证过的 Rust binaries/images/units，并明确兼容回滚边界和 Go 尚不可删除的
  状态；不改变运行时配置选择。
- Go 是否可下线：不能。T-61 是 T-56 的文档/运维证据前置，不代表 Go 已删除。
- 依赖：T-60 新鲜产物与 AUDIT-001/006 已有入口；T-53/T-59 提供契约名称和错误/兼容边界。
