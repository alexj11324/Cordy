...
| C2 | agents_probe/agents_refresh/local_skills/skill_cache/prompt/artifact_matcher/claude_plugins/slash_skill（Go ≈3k 行） | src/{agents_probe,agents_refresh,local_skills,skill_cache,prompt,runtime_config_sections,claude_plugins,artifact_matcher,slash_skill}.rs | 🔄 **主体已落，重修轮完成**——首批交付 8 模块 ≈3.4k 行 + 47 测试（PR #5 `agent/nora/7dab541b6d98`）。重修轮（CORD-12）：① agents_refresh.go 策略半区落地 agents_refresh.rs——三间隔常量、next_converge_backoff、ConvergeRetryState 循环节流策略、AgentsRefreshHost 接缝、refresh_agent_availability 单向合并、providers_missing_runtimes 扫描、builtin_versions_from_payload、workspaces_behind_on_versions 版本滞后扫描、partition_demotable_runtimes 降级分拆、RevivedRuntimes，13 新测试；编排半区（loop 本体/register 调用/claim barrier）仍待 daemon.go 核心。② ArtifactMatcher 三份收一份：canonical = artifact_matcher.rs（含 build_pattern_set 迁入、basenames_sorted/is_empty 访问器），diskusage.rs 与 gc.rs 的内联副本删除改引；顺带修掉 diskusage 版 safeRelativePath 吃掉前导 "../" 的偏离。③ 全 crate cargo fmt 清零（首批交付遗留 fmt 漂移一并修复） |

**CORD-12 重修轮集成验收（20260822）：cordy-daemon clippy --all-targets -D warnings 0 错误、fmt 干净、332 测试全过（319→332）；全 workspace clippy 0 错误、1069 测试 0 失败。**
...
- [ ] **S10. CLI bins**：cordy/migrate/backfill×3
  - [x] CLI foundation 首批真实切片：新增 `cordy-cli`（clap）与独立 Make 运行/构建入口；迁移所需的 profile/task-local config 解析、API 请求身份头/超时/错误与退出码，并交付 `version`/根 `--version`（version/commit/date/go/os/arch 构建元数据与 text/json）、`auth status/logout`（task token 隔离、table/json状态、当前 profile 锁定原子登出）、`config show/set`（白名单校验、task-local/profile 隔离、保留未知字段的锁定原子写）、`user profile get/update`（table/json，update 支持安全的 inline/stdin/file description 输入）和 `workspace list/get/create/update`（workspace-free鉴权枚举、UUID/slug/短前缀解析、安全 inline/stdin/file 文本输入、table/json与默认标记），不注册未实现命令。生产 release workflow 保持 Go CLI，待命令面与 update/release 链闭环后切换。
