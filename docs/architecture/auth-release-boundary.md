...
the Go API to mint a short-lived, one-time `ovd_` code. The custom-protocol URL
contains only that code and state. The verifier never leaves Desktop, and the
reusable Orvilo bearer is returned only by the HTTPS redeem endpoint.
...
code storage and Orvilo session issuance. There is no Rust origin dependency.

The broker image is independently named `orvilo-auth-broker`. Release jobs
