...
| Managed Slack credentials     | `377bb52a9` plus integration commit `96afd5368` preserve Hoplite `6c4be491f`, add refresh/lifetime validation, encrypted rotation, provider-health observations and server lifecycle. Initial persistence failure reproduced in [33759435477](https://github.com/alexj11324/Cordy/actions/runs/33759435477). | Backend build and PostgreSQL-backed race tests passed in [33761740116](https://github.com/alexj11324/Cordy/actions/runs/33761740116); sqlc reported parameter-order drift, corrected from its actual output. Replacement CI and real Slack authorization/refresh/revoke remain. |
| Workspace messaging Hub       | Rust `patchbay-channel-engine/src/hub.rs` and router support workspace-owned installs with per-conversation `/agents` selection. The inspected Go engine has no Hub stage; managed Slack still resolves the nil installation Agent ID.                                                                       | Port visibility/permission filtering, selection, persistence, pending-run switch fence, command replies and adapter wiring.                                                                                                                                                     |
