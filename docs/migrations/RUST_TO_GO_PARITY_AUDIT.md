...
| Workspace messaging Hub       | `278a2b69b` ports selection, invocation permissions, transactional persistence, pending-run fencing, localized replies and adapter wiring. `284aa95a1` isolates resume state by Agent; `1eb6b8cee` integrates the final concurrent Hoplite guard. RED failures were reproduced; implementation verification is pending. | Reopen the PR with user confirmation, run backend/sqlc/replacement CI and address failures; then real provider and setup/UI acceptance. |
