...
- Standalone settings navigation is 256px (`md:w-64`), matching the app Sidebar's `SIDEBAR_WIDTH_DEFAULT`; selected/hover states, typography hierarchy, card borders, and content inset remain aligned with the existing sidebar tokens. Its desktop surface now uses the same gradient, transparency, blur, saturation, border, shadow, and reduced-transparency fallback as the main Sidebar; compact widths remain opaque like the Sidebar sheet. Embedded settings continue to use the existing controls through the adapter fallback.
