...
// authors and the Orvilo host. It must stay independent from private handlers,
// services, and database implementations.
//
// A plugin relates to Orvilo in exactly three ways:
//
//   - Action   (plugin -> Orvilo): host capabilities the plugin calls.
//   - Hook     (Orvilo -> plugin): plugin capabilities the host calls.
