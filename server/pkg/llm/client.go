...
// let THIS layer send chat content leaves ORVILO_LLM_API_KEY and
// ORVILO_LLM_BASE_URL empty; the product stays whole (client-derived chat
...
type Config struct {
	// APIKey authenticates against the upstream. Maps to ORVILO_LLM_API_KEY.
	APIKey string
	...
	// ORVILO_LLM_DEFAULT_MODEL. When empty, FallbackModel is used.
	DefaultModel string
	// MaxRetries is the transport-level retry budget applied to every request
	// this client makes. Maps to ORVILO_LLM_MAX_RETRIES. Build one with
