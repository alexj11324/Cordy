...
-- re-connecting it — even to represent a DIFFERENT agent in the SAME Cordy
...
-- the conflict update to the SAME Cordy workspace: a team already owned by a
...
-- old catch-all "connected to a different Cordy workspace". Meant to be read
...
-- to a Cordy user. The old composite member-FK is gone, so this no
...
-- before calling. ON CONFLICT DO UPDATE is still gated on cordy_user_id
...
ON CONFLICT (installation_id, channel_user_id) DO UPDATE SET
    ...
    bound_at = now()
WHERE channel_user_binding.cordy_user_id = EXCLUDED.cordy_user_id
RETURNING *;

-- name: GetChannelUserBindingByUserID :one
-- The inbound identity lookup: does this platform user id map to a Cordy
...
-- Outbound notification lookup: given a Cordy member and a channel_type,
...
WHERE b.workspace_id = sqlc.arg('workspace_id')
  AND b.cordy_user_id = sqlc.arg('cordy_user_id')
...
-- bound to ANOTHER installation in the SAME Cordy workspace + SAME Slack team,
...
-- different Cordy workspaces, and a user may hold different Cordy accounts in
...
DELETE FROM channel_user_binding
WHERE workspace_id = $1 AND cordy_user_id = $2;
