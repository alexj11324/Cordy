...
-- name: ArchiveAutomation :exec
UPDATE automation
SET status = 'archived', pause_reason = NULL, updated_at = now()
WHERE id = $1;

-- name: PauseAutomationsByUnboundAgents :many
...
-- name: PauseAutomationsByUnrunnableTeam :many
...
-- name: CreateAutomationRuleVersion :one
...
VALUES (
    @automation_id, @workspace_id, @published_by_type, sqlc.narg(published_by_id),
    COALESCE(sqlc.narg(config_summary), '{}'::jsonb)
)
RETURNING *;

-- name: GetActiveAutomationRuleVersion :one
...
SELECT * FROM automation_rule_version
...
-- name: SetAutomationTriggerPublishersByAutomation :exec
...
INSERT INTO automation_run (
    ...
    sqlc.narg('team_id'), sqlc.narg('planned_at'),
...
-- name: GetAutomationRunByTriggerAndPlanned :one
...
-- name: GetAutomationRunByWebhookDelivery :one
SELECT * FROM automation_run
WHERE webhook_delivery_id = $1
LIMIT 1;

-- name: GetAutomationRunByQuotaReservation :one
SELECT * FROM automation_run
WHERE quota_reservation_id = $1
LIMIT 1;

-- name: RecoverPartialAutomationRun :one
...
-- name: UpdateAutomationRunTerminalWithQuota :one
...
-- name: ListSchedulableAutomationTriggers :many
...
-- name: GetAutomationTaskByRun :one
...
-- name: FailAutomationRunsByIssue :many
...
-- name: SelectAutomationsExceedingFailureThreshold :many
...
-- name: SystemPauseAutomation :one
...
-- name: ListAutomationSubscribers :many
...
SELECT s.* FROM automation_subscriber AS s
...
-- name: ListAutomationSubscribersForAutomations :many
...
-- name: ListAutomationCollaborators :many
-- ORDER BY created_at keeps row rendering stable across refreshes.
SELECT * FROM automation_collaborator
WHERE automation_id = $1
ORDER BY created_at ASC, user_id ASC;

-- name: AddAutomationCollaborator :one
