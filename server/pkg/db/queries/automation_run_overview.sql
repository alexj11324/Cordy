...
WHERE a.workspace_id = sqlc.arg(workspace_id)
  ...
  AND (a.title ILIKE '%' || sqlc.arg(search)::text || '%'
    OR COALESCE(r.failure_reason, '') ILIKE '%' || sqlc.arg(search)::text || '%')
  AND (cardinality(sqlc.arg(statuses)::text[]) = 0 OR r.status = ANY(sqlc.arg(statuses)::text[]))
ORDER BY r.triggered_at DESC, r.id DESC
LIMIT sqlc.arg(page_limit) OFFSET sqlc.arg(page_offset);

-- name: WorkspaceAutomationRunSummary :one
SELECT count(*) FILTER (WHERE
  (a.title ILIKE '%' || sqlc.arg(search)::text || '%'
    OR COALESCE(r.failure_reason, '') ILIKE '%' || sqlc.arg(search)::text || '%')
  AND (cardinality(sqlc.arg(statuses)::text[]) = 0 OR r.status = ANY(sqlc.arg(statuses)::text[])))::bigint AS total,
