...
const activateIssueCreatedTask = `-- name: ActivateIssueCreatedTask :one
...
func (q *Queries) ActivateIssueCreatedTask(ctx context.Context, id pgtype.UUID) (AgentTaskQueue, error) {
	row := q.db.QueryRow(ctx, activateIssueCreatedTask, id)
	var i AgentTaskQueue
	err := row.Scan(
		&i.ID,
		&i.AgentID,
...
SELECT
    ...
    $11,
    CASE
        WHEN COALESCE($12::boolean, FALSE)
...
type CreateAgentTaskParams struct {
	...
	HandoffNote          pgtype.Text   `json:"handoff_note"`
	TeamID               pgtype.UUID   `json:"team_id"`
	IssueCreatedPending  pgtype.Bool   `json:"issue_created_pending"`
...
SELECT
    $1, parent.agent_id, parent.runtime_id, parent.issue_id, 'deferred', LEAST(parent.priority, 4),
    LEFT($2::text, 200),
    ((CASE
...
WHERE candidate.context->>'agent_thread_idempotency_key' = $1::text
...
func (q *Queries) GetAgentThreadContinuationByIdempotency(ctx context.Context, arg GetAgentThreadContinuationByIdempotencyParams) (AgentTaskQueue, error) {
	row := q.db.QueryRow(ctx, getAgentThreadContinuationByIdempotency, arg.IdempotencyKey, arg.ParentTaskID)
	var i AgentTaskQueue
	err := row.Scan(
		&i.ID,
		&i.AgentID,
...
func (q *Queries) GetLatestTaskRoleForIssueAndAgent(ctx context.Context, arg GetLatestTaskRoleForIssueAndAgentParams) (GetLatestTaskRoleForIssueAndAgentRow, error) {
	...
	err := row.Scan(&i.IsLeaderTask, &i.TeamID)
	return i, err
}
...
WITH RECURSIVE thread(id, agent_id, runtime_id, session_id, depth) AS (
    ...
    WHERE root.id = $2 AND root.issue_id IS NULL
...
WHERE task.id IN (SELECT id FROM thread) AND task.issue_id IS NULL
  AND lock_task_owner_rows(NULL, $1, NULL)
`
...
func (q *Queries) LinkTaskToIssue(ctx context.Context, arg LinkTaskToIssueParams) error {
	_, err := q.db.Exec(ctx, linkTaskToIssue, arg.IssueID, arg.ID)
	return err
}
...
const listPendingIssueCreatedTasksByRuntime = `-- name: ListPendingIssueCreatedTasksByRuntime :many
SELECT id, agent_id, issue_id, status, priority, dispatched_at, started_at, completed_at, result, error, created_at, context, runtime_id, session_id, work_dir, trigger_comment_id, chat_session_id, automation_run_id, attempt, max_attempts, parent_task_id, failure_reason, trigger_summary, force_fresh_session, is_leader_task, wait_reason, initiator_user_id, handoff_note, prepare_lease_expires_at, team_id, runtime_mcp_overlay, escalation_for_task_id, fire_at, originator_user_id, runtime_connected_apps, coalesced_comment_ids, delivered_comment_ids, chat_input_task_id, chat_finalize_deferred_at, originator_source, delegated_from_task_id, retry_of_task_id, rerun_of_task_id, rule_version_id, trigger_evidence_kind, trigger_evidence_ref_id, accountable_user_id, session_rollout_missing, retired_session_id, quick_actions_disabled, regenerate_quick_actions_for, branch_name, durable_work_dir, channel_context_revision, execution_lane_key, model_id, policy_revision, failover_reason FROM agent_task_queue
WHERE runtime_id = $1
  AND status = 'queued'
  AND context->>'issue_created_pending' = 'true'
...
func (q *Queries) ListPendingIssueCreatedTasksByRuntime(ctx context.Context, runtimeID pgtype.UUID) ([]AgentTaskQueue, error) {
	rows, err := q.db.Query(ctx, listPendingIssueCreatedTasksByRuntime, runtimeID)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
...
const lockAgentForAutomationAssignment = `-- name: LockAgentForAutomationAssignment :one
SELECT id, workspace_id, name, avatar_url, runtime_mode, runtime_config, visibility, status, max_concurrent_tasks, owner_id, created_at, updated_at, description, runtime_id, instructions, archived_at, archived_by, custom_env, custom_args, mcp_config, model, thinking_level, composio_toolkit_allowlist, permission_mode, kind, system_key, disabled_runtime_skills, service_tier, conversation_starters FROM agent
WHERE id = $1 AND workspace_id = $2 AND kind = 'user'
FOR SHARE
`

type LockAgentForAutomationAssignmentParams struct {
	ID          pgtype.UUID `json:"id"`
	WorkspaceID pgtype.UUID `json:"workspace_id"`
}
...
func (q *Queries) LockAgentForAutomationAssignment(ctx context.Context, arg LockAgentForAutomationAssignmentParams) (Agent, error) {
	row := q.db.QueryRow(ctx, lockAgentForAutomationAssignment, arg.ID, arg.WorkspaceID)
	var i Agent
	err := row.Scan(
		&i.ID,
		&i.WorkspaceID,
...
const lockAgentThreadAgent = `-- name: LockAgentThreadAgent :one
SELECT agent.id
FROM agent
...
func (q *Queries) LockAgentThreadAgent(ctx context.Context, taskID pgtype.UUID) (pgtype.UUID, error) {
	row := q.db.QueryRow(ctx, lockAgentThreadAgent, taskID)
	var id pgtype.UUID
	err := row.Scan(&id)
	return id, err
}
...
const setIssueCreatedTaskRuntimeOverlay = `-- name: SetIssueCreatedTaskRuntimeOverlay :execrows
UPDATE agent_task_queue
...
type SetIssueCreatedTaskRuntimeOverlayParams struct {
	ID                   pgtype.UUID `json:"id"`
	RuntimeMcpOverlay    []byte      `json:"runtime_mcp_overlay"`
	RuntimeConnectedApps []byte      `json:"runtime_connected_apps"`
	OriginatorUserID     pgtype.UUID `json:"originator_user_id"`
...
func (q *Queries) SetIssueCreatedTaskRuntimeOverlay(ctx context.Context, arg SetIssueCreatedTaskRuntimeOverlayParams) (int64, error) {
	result, err := q.db.Exec(ctx, setIssueCreatedTaskRuntimeOverlay,
		arg.ID,
		arg.RuntimeMcpOverlay,
		arg.RuntimeConnectedApps,
		arg.OriginatorUserID,
