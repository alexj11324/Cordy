...
func TestOpenAPICoversCapabilityLedger(t *testing.T) {
	...
	for _, operation := range Operations {
		...
		if got, _ := raw["x-orvilo-contract"].(string); got != string(operation.Contract) {
			t.Errorf("%s %s x-orvilo-contract = %q, want %q", operation.Method, operation.Path, got, operation.Contract)
		}
		gotScope, _ := raw["x-orvilo-scope"].(string)
		if gotScope != operation.Policy.Scope {
			t.Errorf("%s %s x-orvilo-scope = %q, want %q", operation.Method, operation.Path, gotScope, operation.Policy.Scope)
		}
	}
