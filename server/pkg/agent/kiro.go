...
func (b *kiroBackend) Execute(ctx context.Context, prompt string, opts ExecOptions) (*Session, error) {
	...
	// (goal_complete or `patchbay issue comment add`). We record the status
	...
	c := &hermesClient{
		...
		onMessage: func(msg Message) {
			...
			if msg.Type == MessageToolUse {
				...
				// Recognize `patchbay issue comment add` by its command
...
// `patchbay issue comment add` invocation. It keys purely off the command
...
func isKiroIssueCommentAddCommand(command string) bool {
	...
	// `sh -c "patchbay issue comment add ..."`; unwrap a single such layer so
	...
	if executable != "patchbay" && !strings.HasSuffix(executable, "/patchbay") {
		return false
	}
