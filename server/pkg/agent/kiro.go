...
func (b *kiroBackend) Execute(ctx context.Context, prompt string, opts ExecOptions) (*Session, error) {
	...
	// (goal_complete or `orvilo issue comment add`). We record the status
	...
	c := &hermesClient{
		...
		onMessage: func(msg Message) {
			...
			if msg.Type == MessageToolUse {
				...
				// Recognize `orvilo issue comment add` by its command
...
// `orvilo issue comment add` invocation. It keys purely off the command
...
func isKiroIssueCommentAddCommand(command string) bool {
	...
	// `sh -c "orvilo issue comment add ..."`; unwrap a single such layer so
	...
	if executable != "orvilo" && !strings.HasSuffix(executable, "/orvilo") {
		return false
	}
...
// like `ORVILO_TOKEN=x orvilo issue comment add ...` is still recognized.
