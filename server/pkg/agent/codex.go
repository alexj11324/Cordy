...
const (
	patchbayCodexMcpBeginMarker = "# BEGIN patchbay-managed mcp_servers (do not edit; regenerated by daemon)"
	patchbayCodexMcpEndMarker   = "# END patchbay-managed mcp_servers"
)

var codexMcpBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(patchbayCodexMcpBeginMarker) +
		`.*?^` + regexp.QuoteMeta(patchbayCodexMcpEndMarker) + `\n*`)
...
func ensureCodexMcpConfig(configPath string, mcpConfig json.RawMessage, logger *slog.Logger) error {
	...
	if managed {
		...
		if block == "" {
			block = patchbayCodexMcpBeginMarker + "\n" + patchbayCodexMcpEndMarker + "\n"
		}
		if stripped == "" {
			updated = block
		} else {
...
func renderCodexMcpServersBlock(raw json.RawMessage) (string, bool, error) {
	...
	sb.WriteString(patchbayCodexMcpEndMarker)
	sb.WriteString("\n")
