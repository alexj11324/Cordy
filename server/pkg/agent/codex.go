...
const (
	orviloCodexMcpBeginMarker = "# BEGIN orvilo-managed mcp_servers (do not edit; regenerated by daemon)"
	orviloCodexMcpEndMarker   = "# END orvilo-managed mcp_servers"
)

var codexMcpBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(orviloCodexMcpBeginMarker) +
		`.*?^` + regexp.QuoteMeta(orviloCodexMcpEndMarker) + `\n*`)
...
func ensureCodexMcpConfig(configPath string, mcpConfig json.RawMessage, logger *slog.Logger) error {
	...
	if managed {
		...
		if block == "" {
			block = orviloCodexMcpBeginMarker + "\n" + orviloCodexMcpEndMarker + "\n"
		}
		if stripped == "" {
			updated = block
		} else {
...
func renderCodexMcpServersBlock(raw json.RawMessage) (string, bool, error) {
	...
	sb.WriteString(orviloCodexMcpEndMarker)
	sb.WriteString("\n")
