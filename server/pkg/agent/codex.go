...
const (
	cordyCodexMcpBeginMarker = "# BEGIN cordy-managed mcp_servers (do not edit; regenerated by daemon)"
	cordyCodexMcpEndMarker   = "# END cordy-managed mcp_servers"
)

var codexMcpBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(cordyCodexMcpBeginMarker) +
		`.*?^` + regexp.QuoteMeta(cordyCodexMcpEndMarker) + `\n*`)
...
func ensureCodexMcpConfig(configPath string, mcpConfig json.RawMessage, logger *slog.Logger) error {
	...
	if managed {
		...
		if block == "" {
			block = cordyCodexMcpBeginMarker + "\n" + cordyCodexMcpEndMarker + "\n"
		}
		if stripped == "" {
			updated = block
		} else {
...
func renderCodexMcpServersBlock(raw json.RawMessage) (string, bool, error) {
	...
	sb.WriteString(cordyCodexMcpBeginMarker)
	sb.WriteString("\n")
	...
	sb.WriteString(cordyCodexMcpEndMarker)
	sb.WriteString("\n")
...
func isCordyMcpSelectorKey(k string) bool {
	switch k {
...
//
// An explicit configured override (CORDY_CODEX_FIRST_TURN_TIMEOUT) is honored
