...
func TestCodeArtsRealSmoke(t *testing.T) {
	if os.Getenv("ORVILO_RUN_REAL_AGENT_SMOKE") != "1" {
		t.Skip("set ORVILO_RUN_REAL_AGENT_SMOKE=1 to run an authenticated CodeArts smoke test")
	}

	backend, err := ResolveBackend("codearts", Config{
		ExecutablePath: os.Getenv("ORVILO_CODEARTS_PATH"),
		Logger:         slog.Default(),
	})
	if err != nil {
		t.Fatal(err)
	...
	session, err := backend.Execute(ctx, "Reply with exactly CODEARTS_SMOKE_OK and nothing else.", ExecOptions{
		Cwd:     t.TempDir(),
		Model:   os.Getenv("ORVILO_CODEARTS_MODEL"),
