...
// the cordy CLI version reported by the daemon at registration time. The
...
// binding, no-retry semantics on `cordy issue create` failure — see PR
...
// MinHandoffCLIVersion is the lowest cordy CLI version whose daemon renders
...
var (
	ErrCLIVersionMissing = errors.New("cordy CLI version not reported by daemon")
	ErrCLIVersionTooOld  = errors.New("cordy CLI version is below required minimum")
)
