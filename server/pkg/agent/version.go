...
// the orvilo CLI version reported by the daemon at registration time. The
...
// binding, no-retry semantics on `orvilo issue create` failure — see PR
...
// MinHandoffCLIVersion is the lowest orvilo CLI version whose daemon renders
...
var (
	ErrCLIVersionMissing = errors.New("orvilo CLI version not reported by daemon")
	ErrCLIVersionTooOld  = errors.New("orvilo CLI version is below required minimum")
)
