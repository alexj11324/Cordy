...
// dshBackend drives the Cordy DSH bundle over its versioned JSONL
...
// this package only translates those events into Cordy's Backend contract.
...
func (b *dshBackend) Execute(ctx context.Context, prompt string, opts ExecOptions) (*Session, error) {
	...
	if requestID == "" {
		requestID = "cordy-" + strconv.FormatInt(time.Now().UnixNano(), 10)
	}
