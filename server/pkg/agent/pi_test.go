...
func TestBuildPiArgsIgnoresSystemPrompt(t *testing.T) {
	args := buildPiArgs("/tmp/s.jsonl", ExecOptions{
		SystemPrompt: "the entire orvilo runtime brief",
	}, slog.Default())
	...
	for _, a := range args {
		...
		if a == "the entire orvilo runtime brief" {
...
func TestStripPiToolCallMarkup(t *testing.T) {
	tests := map[string]string{
		...
		`before response:bash{command:<|"|>orvilo issue comment list issue-id --all --output json<|"|>} after`: "before  after",
...
// slash (`claude/claude-opus-5` under provider `orvilo-anthropic`). Splitting
...
func TestBuildPiArgsPassesQualifiedSelectorWhole(t *testing.T) {
	args := buildPiArgs("/tmp/s.jsonl", ExecOptions{Model: "orvilo-anthropic/claude/claude-opus-5"}, slog.Default())

	joined := strings.Join(args, " ")
	if !strings.Contains(joined, "--model orvilo-anthropic/claude/claude-opus-5") {
