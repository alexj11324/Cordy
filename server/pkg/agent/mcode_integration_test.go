...
func TestMcodeRealACPContextAndToolSmoke(t *testing.T) {
	...
	workDir := t.TempDir()
	writeMcodeSmokeFile(t, filepath.Join(workDir, "AGENTS.md"), `# Cordy smoke context

For every response in this workspace, include the exact marker AGENTS-MCODE-OK.
`)
	writeMcodeSmokeFile(t, filepath.Join(workDir, ".minimax", "skills", "cordy-smoke", "SKILL.md"), `---
name: cordy-smoke
description: Use when asked to run the Cordy MiniMax Code integration smoke.
---
# Cordy smoke skill
...
`)
	...
	session, err := backend.Execute(ctx,
		"Run the cordy-smoke skill. Follow the workspace instructions and return its requested evidence.",
