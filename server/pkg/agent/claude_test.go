...
func TestMergeEnvFiltersClaudeCodeVars(t *testing.T) {
	...
	if !found["CORDY_SERVER_URL=https://task.example"] || !found["CORDY_TOKEN=mat_task"] {
		t.Fatalf("expected explicit task CORDY_* values to be appended, got %v", env)
	}
}
...
func TestResumeWasRejected(t *testing.T) {
	...
	cases := []struct {
		...
		{
			// Verbatim from cordy-ai/cordy#5704 (Claude Code 2.1.207,
...
func TestBuildClaudeArgsManagedSkillSettingsWins(t *testing.T) {
	args := buildClaudeArgs(ExecOptions{
		ClaudeSettingsPath: "/tmp/cordy-claude-settings.json",
	...
	if !strings.Contains(joined, "--settings /tmp/cordy-claude-settings.json") {
