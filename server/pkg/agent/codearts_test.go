...
func TestCodeArtsExecuteUsesNativeRunFlagsAndStdin(t *testing.T) {
	...
	session, err := backend.Execute(t.Context(), prompt, ExecOptions{
		...
		CustomArgs: []string{
			...
			"--dir", "wrong", "--variant", "high", "--title", "Orvilo task",
		},
	})
	if err != nil {
		t.Fatal(err)
	...
	want := []string{"wrapper", "run", "--format", "json", "--auto", "--model", "huaweicloud-maas/deepseek-v3.2", "--session", "ses_existing", "--title", "Orvilo task"}
