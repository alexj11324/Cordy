...
// qwenBlockedArgs are owned by Patchbay. Qwen accepts the task prompt and stream
// protocol as flags, so custom args must not replace either. Model/session are
// also selected by Patchbay, and safe mode disables the QWEN.md context file.
