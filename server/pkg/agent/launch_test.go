...
// lands. It asserts the argv Cordy builds, not that any particular third-party
...
func TestLogAgentCommandRedactsTextAndJSON(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			cmd := &exec.Cmd{Path: "/opt/cordy/bin/codex", Args: append([]string{"codex"}, args...)}
			...
			for _, diagnostic := range []string{
				"agent command", "provider", "codex", "/opt/cordy/bin/codex",
...
// twenty-one backends, and CORDY_QWENPAW_ARGS shipped plumbed-but-dropped
