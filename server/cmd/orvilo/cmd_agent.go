...
// part of their injected environment from silently resolving Orvilo state
...
func requireTaskLocalConfigRoot() error {
	...
	if strings.TrimSpace(os.Getenv(cli.TaskConfigRootEnv)) == "" {
		...
		if markerPath := leftoverDaemonTaskMarkerPath(); markerPath != "" {
			return fmt.Errorf("daemon-managed task requires a task-local Orvilo config root in %s%s", cli.TaskConfigRootEnv, leftoverMarkerSuffix(markerPath))
		}
		return fmt.Errorf("daemon-managed task requires a task-local Orvilo config root in %s%s", cli.TaskConfigRootEnv, daemonPortOnlyContextHint())
	}
	return nil
}
