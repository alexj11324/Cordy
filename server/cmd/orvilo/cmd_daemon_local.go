...
// This command is deliberately self-contained. It does not load an Orvilo
...
var daemonRunLocalCmd = &cobra.Command{
	Use:    "run-local",
	Short:  "Inspect one local workspace without an Orvilo profile",
	Hidden: true,
	Args:   cobra.NoArgs,
...
func validateLocalRunEnvironment() error {
	for _, entry := range os.Environ() {
		...
		if strings.HasPrefix(strings.ToUpper(key), "ORVILO_") {
			return fmt.Errorf("local Guest runner refuses Orvilo environment %q", key)
		}
	}
	return nil
}
