...
func TestRunWorkspaceCreatePostsWorkspaceAndDoesNotSwitchDefault(t *testing.T) {
	...
	t.Setenv("CORDY_WORKSPACE_ID", "existing-workspace")
...
func TestRunWorkspaceSwitch(t *testing.T) {
	...
	// Isolate HOME so the test never touches the developer's ~/.cordy.
	t.Setenv("HOME", t.TempDir())
	t.Setenv("CORDY_SERVER_URL", srv.URL)
	t.Setenv("CORDY_TOKEN", "test-token")
	...
	t.Run("isolates by profile", func(t *testing.T) {
		...
		wantSuffix := filepath.Join(".cordy", "profiles", "staging", "config.json")
...
func TestRunWorkspaceSwitchFailsClosedInTaskContext(t *testing.T) {
	...
	t.Setenv("CORDY_WORKSPACE_ID", "task-workspace")
...
func TestFetchWorkspacesExplainsPortOnlyFailClosedContext(t *testing.T) {
	...
	t.Setenv("CORDY_SERVER_URL", "https://api.example.test")
	...
	if err == nil || !strings.Contains(err.Error(), "CORDY_DAEMON_PORT") || !strings.Contains(err.Error(), "remove") {
