...
func TestRunConfigSetPersistsSupportedKeysInProfile(t *testing.T) {
	...
	workspacesRoot := filepath.Join(t.TempDir(), "cordy-dev")
...
func TestApplyConfigSetSupportsDaemonKeys(t *testing.T) {
	...
	cfg := cli.CLIConfig{}
	workspacesRoot := filepath.Join(t.TempDir(), "cordy")
...
func TestApplyConfigSetNormalizesWorkspacesRoot(t *testing.T) {
	...
	cfg := cli.CLIConfig{}
	if err := applyConfigSet(&cfg, "workspaces_root", filepath.Join("data", "cordy")); err != nil {
		t.Fatalf("applyConfigSet: %v", err)
	}
	want := filepath.Join(cwd, "data", "cordy")
...
func TestApplyConfigSetPositiveDurationRoundTripsToDaemonResolver(t *testing.T) {
	const envName = "TEST_CORDY_PERSISTED_DURATION"
	t.Setenv(envName, "")
