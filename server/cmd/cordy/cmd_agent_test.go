...
func TestHumanLocalCommandDistinguishesPortHintFromTaskIdentity(t *testing.T) {
	t.Chdir(t.TempDir())
	t.Setenv("CORDY_AGENT_ID", "")
	t.Setenv("CORDY_TASK_ID", "")
	t.Setenv(cli.TaskConfigRootEnv, "")
	t.Setenv("CORDY_DAEMON_PORT", "20032")

	if err := requireHumanLocalCommand("login"); err != nil {
		t.Fatalf("port-only host context rejected login: %v", err)
	}

	t.Setenv(cli.TaskConfigRootEnv, filepath.Join(t.TempDir(), "task-cordy"))
...
func TestHumanLocalCommandRejectsExplicitTaskIdentity(t *testing.T) {
	for _, tc := range []struct {
		...
		{name: "agent ID blocks setup", envName: "CORDY_AGENT_ID", command: "setup"},
		{name: "task ID blocks daemon stop", envName: "CORDY_TASK_ID", command: "daemon stop"},
	} {
...
func TestMissingServerConfigMessageExplainsPortOnlyContext(t *testing.T) {
	...
	if !strings.Contains(message, "CORDY_DAEMON_PORT") || !strings.Contains(message, "remove") {
...
// impersonation escape: a sandbox fault strips every CORDY_* env var from
// an agent subprocess, which then runs `cordy` from the *parent* directory
...
func TestNewAPIClient_WorkdirParentEscapeFailsClosed(t *testing.T) {
	...
	// Daemon-owned tree: {root}/.cordy marker + {root}/{ws}/{task}/workdir
	...
	t.Setenv("CORDY_TASK_ID", "")
	t.Setenv("CORDY_DAEMON_PORT", "")
	t.Setenv("CORDY_TOKEN", "")
	t.Setenv("CORDY_SERVER_URL", "http://127.0.0.1:8080")
...
// agent task (CORDY_AGENT_ID / CORDY_TASK_ID set), the CLI must NOT
// silently read the user-global ~/.cordy/config.json to recover a missing
...
func TestResolveWorkspaceID_AgentContextSkipsConfig(t *testing.T) {
	...
	t.Run("agent context with explicit env uses env", func(t *testing.T) {
		t.Setenv("CORDY_AGENT_ID", "agent-123")
		t.Setenv("CORDY_TASK_ID", "task-456")
		t.Setenv("CORDY_DAEMON_PORT", "")
		t.Setenv("CORDY_WORKSPACE_ID", "env-ws")
	...
	t.Run("daemon port marker also skips config", func(t *testing.T) {
		t.Setenv("CORDY_AGENT_ID", "")
		t.Setenv("CORDY_TASK_ID", "")
		t.Setenv("CORDY_DAEMON_PORT", "27182")
		t.Setenv("CORDY_WORKSPACE_ID", "")
...
func TestResolveToken_AgentContextSkipsConfig(t *testing.T) {
	...
	t.Run("agent context without env never reads config", func(t *testing.T) {
		...
		if got := resolveToken(testCmd()); got != "" {
			t.Fatalf("resolveToken() = %q, want empty in agent context without CORDY_TOKEN", got)
		}
	})
	...
	t.Run("daemon port marker without env never reads config", func(t *testing.T) {
		...
		if got := resolveToken(testCmd()); got != "" {
			t.Fatalf("resolveToken() = %q, want empty in daemon-managed context without CORDY_TOKEN", got)
		}
	})
	...
	t.Run("agent context uses explicit task token env", func(t *testing.T) {
		...
		t.Setenv("CORDY_TASK_ID", "task-456")
		t.Setenv("CORDY_DAEMON_PORT", "")
		t.Setenv("CORDY_TOKEN", "mat_task_token")

		if got := resolveToken(testCmd()); got != "mat_task_token" {
			t.Fatalf("resolveToken() = %q, want CORDY_TOKEN", got)
		}
	})
	...
	t.Run("daemon port set without agent context avoids config fallback", func(t *testing.T) {
		...
		t.Setenv("CORDY_TASK_ID", "")
		t.Setenv("CORDY_TOKEN", "")
		t.Setenv("CORDY_DAEMON_PORT", "19514")
	...
	t.Run("daemon port set with explicit task token uses task token", func(t *testing.T) {
		...
		if got := resolveToken(testCmd()); got != "mat_task_token" {
			t.Fatalf("resolveToken() = %q, want CORDY_TOKEN (task token wins over daemon signal)", got)
		}
	})

	// CORDY_SERVER_URL is a user-facing env var that may be set in a
	// normal shell. It is NOT a daemon identity signal — only
	// CORDY_DAEMON_PORT is. The config fallback must still work when
	// SERVER_URL is set but no daemon signal is present.
	t.Run("CORDY_SERVER_URL alone does not block config fallback", func(t *testing.T) {
		t.Setenv("CORDY_AGENT_ID", "")
		t.Setenv("CORDY_TASK_ID", "")
		t.Setenv("CORDY_TOKEN", "")
		t.Setenv("CORDY_DAEMON_PORT", "")
		t.Setenv("CORDY_SERVER_URL", "https://api.cordy.ai")
...
func TestNewAPIClient_AgentContextRequiresTaskToken(t *testing.T) {
	t.Setenv("CORDY_SERVER_URL", "http://127.0.0.1:8080")
	t.Setenv("CORDY_WORKSPACE_ID", "workspace-123")
	t.Setenv("CORDY_AGENT_ID", "agent-123")
	t.Setenv("CORDY_TASK_ID", "task-456")
	...
	t.Run("member token fails closed", func(t *testing.T) {
		t.Setenv("CORDY_TOKEN", "mul_member_token")
...
func TestNewAPIClient_DaemonPortRequiresTaskToken(t *testing.T) {
	...
	if !strings.Contains(err.Error(), "CORDY_DAEMON_PORT") || !strings.Contains(err.Error(), "remove") {
...
func TestAgentUpdateNoFieldsErrorPointsAtEnvCommand(t *testing.T) {
	t.Setenv("HOME", t.TempDir())
	t.Setenv("CORDY_SERVER_URL", "http://127.0.0.1:0")
	t.Setenv("CORDY_WORKSPACE_ID", "test-ws")
	...
	msg := err.Error()
	if !strings.Contains(msg, "cordy agent env set") {
		t.Fatalf("no-fields error must direct users to `cordy agent env set`; got: %q", msg)
	}
}
...
func TestAgentUpdateDoesNotExposeCustomEnvFlags(t *testing.T) {
	for _, flag := range []string{"custom-env", "custom-env-stdin", "custom-env-file"} {
		if agentUpdateCmd.Flag(flag) != nil {
			t.Errorf("agent update must NOT expose --%s after MUL-2600; use `cordy agent env set` instead", flag)
		}
	}
}
