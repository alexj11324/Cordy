...
var workspaceCreateCmd = &cobra.Command{
	...
	Long: "Creates a new workspace and adds you as its owner. Both --name and " +
		...
		"this profile — run 'cordy workspace switch <slug>' afterward if you " +
		"want subsequent commands to target the new workspace.",
	Example: "  cordy workspace create --name \"Support Team\" --slug support-team --issue-prefix SUP",
	Args:    cobra.NoArgs,
...
var workspaceMemberInviteCmd = &cobra.Command{
	...
	Long: "Sends a workspace invitation to an email address. The invitee gets a " +
		...
		"UUID, a slug, or a short UUID prefix (≥4 hex chars) as shown in " +
		"'workspace list'; if omitted the current default workspace is used " +
		"(--workspace-id / CORDY_WORKSPACE_ID / profile default).\n\n" +
...
var workspaceSwitchCmd = &cobra.Command{
	...
	Long: "Sets the default workspace for the current profile after verifying you " +
		...
		"commands without --workspace-id or CORDY_WORKSPACE_ID will target " +
		"this workspace.\n\n" +
		"Resolution priority (highest to lowest): --workspace-id flag, " +
		"CORDY_WORKSPACE_ID env, profile default (set by this command).\n\n" +
		"For low-level use, 'cordy config set workspace_id <id>' writes the " +
...
var workspaceMcpCmd = &cobra.Command{
	...
	Long: "Manages the workspace's library of MCP servers. A server added here " +
		...
		"to that agent ('cordy agent mcp add'), which also carries a per-agent " +
...
var workspaceMcpAddCmd = &cobra.Command{
	...
	Long: "Adds an MCP server to the workspace library. It is assigned to no " +
		"agent — use 'cordy agent mcp add <agent-id> <server-id>' to give it to " +
		"one.\n\n" +
	...
	Example: "  cordy workspace mcp add linear --server-config-file ./linear.json\n" +
		"  echo '{\"url\":\"https://mcp.example\"}' | cordy workspace mcp add example --server-config-stdin",
	Args: cobra.RangeArgs(1, 2),
...
func fetchWorkspaces(ctx context.Context, cmd *cobra.Command) ([]workspaceSummary, error) {
	...
	if token == "" {
		return nil, fmt.Errorf("not authenticated: run 'cordy login' first%s", daemonPortOnlyContextHint())
	}
...
func resolveWorkspaceByIDOrSlug(workspaces []workspaceSummary, target string) (workspaceSummary, error) {
	...
	return workspaceSummary{}, fmt.Errorf("workspace %q not found or you do not have access; run 'cordy workspace list' to see options", target)
}
...
func runWorkspaceGet(cmd *cobra.Command, args []string) error {
	...
	if wsID == "" {
		return fmt.Errorf("workspace ID is required: pass an id/slug/prefix as argument or set CORDY_WORKSPACE_ID")
	}
