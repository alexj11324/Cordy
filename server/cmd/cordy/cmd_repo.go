...
func runRepoCheckout(cmd *cobra.Command, args []string) error {
	...
	daemonPort := os.Getenv("CORDY_DAEMON_PORT")
	if daemonPort == "" {
		return fmt.Errorf("CORDY_DAEMON_PORT not set (this command is intended to be run by an agent inside a daemon task)")
	}

	workspaceID := os.Getenv("CORDY_WORKSPACE_ID")
	agentName := os.Getenv("CORDY_AGENT_NAME")
	taskID := os.Getenv("CORDY_TASK_ID")
	taskToken := os.Getenv("CORDY_TOKEN")
	if taskToken == "" {
		return fmt.Errorf("CORDY_TOKEN not set (repo checkout requires the active task credential)")
	}
	...
	reqBody := map[string]any{
		...
		"task_id":       taskID,
		"checkout_mode": strings.TrimSpace(os.Getenv("CORDY_REPO_CHECKOUT_MODE")),
		"retry_busy":    true,
	}
	...
	for {
		...
		if resp.StatusCode == http.StatusServiceUnavailable && resp.Header.Get("X-Cordy-Retryable") == "repo-busy" {
