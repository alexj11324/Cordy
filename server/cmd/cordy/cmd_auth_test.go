...
func TestResolveAppURL(t *testing.T) {
	...
	t.Run("prefers CORDY_APP_URL", func(t *testing.T) {
		t.Setenv("CORDY_APP_URL", "http://localhost:14000")
...
func TestBrowserLoginInstructionsSSHRemoteHint(t *testing.T) {
	const loginURL = "https://cordy.ai/login?cli_callback=http%3A%2F%2Flocalhost%3A43689%2Fcallback"
...
func TestLoginTokenFlagWiring(t *testing.T) {
	...
	if tokenFlag.NoOptDefVal != tokenPromptSentinel {
		t.Fatalf("loginCmd --token NoOptDefVal = %q, want %q (legacy `cordy login --token` prompt mode would break)", tokenFlag.NoOptDefVal, tokenPromptSentinel)
	}
}

// TestLoginTokenHelpOutputRendersCleanly renders loginCmd's flag help through
// the same pflag path `cordy login -h` uses (FlagUsagesWrapped) and locks the
...
func TestHumanAuthCommandsFailClosedInTaskContext(t *testing.T) {
	...
	t.Setenv("CORDY_AGENT_ID", "agent-test")
	t.Setenv("CORDY_TASK_ID", "task-test")
	t.Setenv("CORDY_TOKEN", "mat_task_sentinel")
	t.Setenv("CORDY_SERVER_URL", "https://task.invalid")
	t.Setenv("CORDY_TASK_CONFIG_ROOT", filepath.Join(t.TempDir(), "task-cordy"))
...
// `cordy login --token`. The original implementation hardcoded `mul_`
// only, which rejected legitimate Cordy Cloud Node PATs (`mcn_`) at
