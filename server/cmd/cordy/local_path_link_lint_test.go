...
func TestFindLocalPathLinksAllowsLegitimateContent(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			body: "the PR is at [#42](https://github.com/cordy-ai/cordy/pull/42)",
		},
		{
			name: "mention link",
...
func TestGuardLocalPathLinksOnlyFiresInAgentContext(t *testing.T) {
	...
	t.Run("human PAT context is never linted", func(t *testing.T) {
		// No CORDY_AGENT_ID / CORDY_TASK_ID: a person running the CLI. Their
...
func TestGuardLocalPathLinksHintIsPerCommand(t *testing.T) {
	...
	msg := err.Error()
	if !strings.Contains(msg, "`cordy issue update` cannot carry files") {
		t.Errorf("update hint missing, got: %v", msg)
	}
	if !strings.Contains(msg, "cordy issue comment add <issue-id> --attachment <path>") {
		t.Errorf("update hint must redirect to comment add, got: %v", msg)
	}
	if strings.Contains(msg, "cordy issue update --attachment") {
