...
func TestRunRepoCheckoutForwardsManagedCheckoutMode(t *testing.T) {
	...
	t.Setenv("CORDY_DAEMON_PORT", strings.TrimPrefix(srv.URL, "http://127.0.0.1:"))
	t.Setenv("CORDY_WORKSPACE_ID", "ws-1")
	t.Setenv("CORDY_AGENT_NAME", "Test Agent")
	t.Setenv("CORDY_TASK_ID", "task-1")
	t.Setenv("CORDY_TOKEN", "mat_repo_checkout_test")
	t.Setenv("CORDY_REPO_CHECKOUT_MODE", "isolated")
...
func TestRunRepoCheckoutRetriesServiceUnavailable(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		attempts++
		if attempts == 1 {
			w.Header().Set("X-Cordy-Retryable", "repo-busy")
