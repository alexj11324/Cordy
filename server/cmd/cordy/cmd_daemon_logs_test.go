...
// ~/.cordy/profiles/<name>/daemon.log while ~/.cordy/daemon.log may still
...
func TestDaemonLogSourcePathResolvesPerProfile(t *testing.T) {
	t.Run("default profile", func(t *testing.T) {
		...
		want := filepath.Join(home, ".cordy", "daemon.log")
		if got != want {
	...
	t.Run("named profile", func(t *testing.T) {
		home := mkProfiles(t, "desktop-api.cordy.ai")

		got, err := daemonLogSourcePath("desktop-api.cordy.ai")
		if err != nil {
			t.Fatalf("daemonLogSourcePath = %v, want nil", err)
		}
		want := filepath.Join(home, ".cordy", "profiles", "desktop-api.cordy.ai", "daemon.log")
		if got != want {
			t.Fatalf("daemonLogSourcePath = %q, want %q", got, want)
		}
	})
...
func TestRunDaemonLogsAnnouncesResolvedPath(t *testing.T) {
	cases := []struct {
		...
		{"named profile", "desktop-api.cordy.ai", "desktop-api.cordy.ai", 50, false},
		{"named profile following", "desktop-api.cordy.ai", "desktop-api.cordy.ai", 50, true},
	}
...
// The announcement goes to stderr, so `cordy daemon logs | grep ...` and a
...
func TestRunDaemonLogsMissingFileNamesProfilePath(t *testing.T) {
	...
	seedDaemonLog(t, filepath.Join(home, ".cordy", "daemon.log"))

	cmd, errOut := daemonLogsCmdFor(t, "desktop-api.cordy.ai", 50, false)
	rec := stubTailLog(t, errOut)
