...
// mkProfiles creates ~/.cordy/profiles/<name> for each name under a fresh
...
func mkProfiles(t *testing.T, names ...string) string {
	...
	for _, name := range names {
		dir := filepath.Join(home, ".cordy", "profiles", filepath.FromSlash(name))
...
func TestRequireKnownProfile(t *testing.T) {
	...
	t.Run("existing profile passes", func(t *testing.T) {
		mkProfiles(t, "dev", "desktop-api.cordy.ai")
		if err := requireKnownProfile("desktop-api.cordy.ai"); err != nil {
			t.Fatalf("requireKnownProfile = %v, want nil", err)
		}
	})

	t.Run("unknown profile lists the known ones sorted", func(t *testing.T) {
		mkProfiles(t, "prod", "dev", "desktop-api.cordy.ai")

		err := requireKnownProfile("desktop-api.cordy")
		var unknown *unknownProfileError
		if !errors.As(err, &unknown) {
			t.Fatalf("requireKnownProfile = %v, want *unknownProfileError", err)
		}
		if unknown.Profile != "desktop-api.cordy" {
			t.Fatalf("Profile = %q, want the name the user passed", unknown.Profile)
		}
		want := []string{"desktop-api.cordy.ai", "dev", "prod"}
		...
		if !strings.Contains(msg, `"desktop-api.cordy"`) || !strings.Contains(msg, "desktop-api.cordy.ai") {
			t.Fatalf("error message %q must name both the bad profile and the known ones", msg)
		}
	})

	// Regression: profile names may contain separators. `cordy --profile
	// team/dev config set ...` creates ~/.cordy/profiles/team/dev, so
	...
	t.Run("no profiles root yet points at login", func(t *testing.T) {
		...
		if !strings.Contains(unknown.Error(), "cordy login --profile staging") {
...
func TestDaemonStatusUnknownProfile(t *testing.T) {
	t.Run("text mode fails without touching stdout", func(t *testing.T) {
		...
		mkProfiles(t, "desktop-api.cordy.ai")

		out, err := captureStdout(t, func() error {
			return runDaemonStatus(daemonStatusCmdFor(t, "desktop-api.cordy", ""), nil)
		})
		var unknown *unknownProfileError
		if !errors.As(err, &unknown) {
	...
	t.Run("json mode prints one document and exits non-zero", func(t *testing.T) {
		clearDaemonTaskEnv(t)
		mkProfiles(t, "desktop-api.cordy.ai", "dev")

		out, err := captureStdout(t, func() error {
			return runDaemonStatus(daemonStatusCmdFor(t, "desktop-api.cordy", "json"), nil)
		})
		...
		if payload.Profile != "desktop-api.cordy" {
			t.Fatalf("profile = %q, want the name the user passed", payload.Profile)
		}
		want := []string{"desktop-api.cordy.ai", "dev"}
...
// The default profile owns ~/.cordy directly and has no profiles/ entry, so
...
func TestDaemonLifecycleCommandsRejectUnknownProfile(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			...
			cmd := daemonStatusCmdFor(t, "desktop-api.cordy", "")
...
func TestDaemonStatusTaskContextRejectsProfileBeforeListing(t *testing.T) {
	...
	t.Setenv("CORDY_DAEMON_PORT", "19601")
