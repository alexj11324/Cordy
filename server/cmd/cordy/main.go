...
var rootCmd = &cobra.Command{
	Use:           "cordy",
	Short:         "Cordy CLI — local agent runtime and management tool",
	Long:          "Work seamlessly with Cordy from the command line.",
	SilenceUsage:  true,
	SilenceErrors: true,
}
...
func init() {
	...
	rootCmd.SetVersionTemplate("cordy {{.Version}}\n")
	...
	rootCmd.PersistentFlags().String("server-url", "", "Cordy server URL (env: CORDY_SERVER_URL)")
	rootCmd.PersistentFlags().String("workspace-id", "", "Workspace ID (env: CORDY_WORKSPACE_ID)")
	rootCmd.PersistentFlags().String("profile", "", "Configuration profile name (e.g. dev) — isolates config, daemon state, and workspaces")
	rootCmd.PersistentFlags().BoolVar(&debugFlag, "debug", false, "Print full error details on failure (env: CORDY_DEBUG)")
