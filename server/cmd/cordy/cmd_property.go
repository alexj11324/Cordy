...
// cordy property {list|get|create|update|archive|unarchive} — workspace
// custom property definitions, and cordy issue property {list|set|unset} —
...
flags:
  cordy property create --name Severity --type select \
...
The actor types hold workspace members; they take no options:
  cordy property create --name Reviewer --type actor`,
	Args: exactArgs(0),
