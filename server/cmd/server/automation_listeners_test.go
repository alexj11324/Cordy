...
func TestAutomationRunOnlyTaskTerminalEventsUpdateRun(t *testing.T) {
	...
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			...
			run, err := automationSvc.DispatchAutomation(ctx, ap, pgtype.UUID{}, "manual", nil)
			if err != nil {
...
// linkedIssueAutomationFixture is the starting state every create_issue
// linked-issue listener test shares: a dispatched create_issue run sitting in
// issue_created with exactly one issue task that carries no automation_run_id
// (so it must be reached via the issue_id lookup, not SyncRunFromTask).
type linkedIssueAutomationFixture struct {
...
// dispatchCreateIssueAutomation creates an active create_issue automation,
// dispatches it, and returns the linked run plus its single issue task.
// Cleanup (automation, issue, tasks, comments) is registered on t.
func dispatchCreateIssueAutomation(t *testing.T, title string) linkedIssueAutomationFixture {
	t.Helper()
	ctx := context.Background()
	queries := db.New(testPool)
	bus := events.New()
	...
	run, err := automationSvc.DispatchAutomation(ctx, ap, pgtype.UUID{}, "schedule", nil)
	if err != nil {
		t.Fatalf("DispatchAutomation: %v", err)
	}
	...
	if tasks[0].AutomationRunID.Valid {
		t.Fatal("create_issue issue task unexpectedly has automation_run_id; test must exercise linked issue lookup")
	}
	if run.Status != "issue_created" {
		t.Fatalf("expected pre-failure run status issue_created, got %q", run.Status)
	}

	return linkedIssueAutomationFixture{taskSvc: taskSvc, queries: queries, run: run, taskID: tasks[0].ID}
}
...
// TestAutomationCreateIssueTaskNoProgressFailureUpdatesRun is the original
// VEN-661 regression: a Codex no-progress failure with no retries left fails
// the linked run.
func TestAutomationCreateIssueTaskNoProgressFailureUpdatesRun(t *testing.T) {
	ctx := context.Background()
	f := dispatchCreateIssueAutomation(t, "Create-issue no-progress listener")
	...
	updatedRun, err := f.queries.GetAutomationRun(ctx, f.run.ID)
	if err != nil {
		t.Fatalf("GetAutomationRun: %v", err)
	}
...
// TestAutomationCreateIssueTaskAgentErrorFailureUpdatesRun covers the VEN-662
...
func TestAutomationCreateIssueTaskAgentErrorFailureUpdatesRun(t *testing.T) {
	ctx := context.Background()
	f := dispatchCreateIssueAutomation(t, "Create-issue agent-error listener")
...
// TestAutomationCreateIssueTaskRetryPendingKeepsRunOpen locks in the wait guard:
...
func TestAutomationCreateIssueTaskRetryPendingKeepsRunOpen(t *testing.T) {
	ctx := context.Background()
	f := dispatchCreateIssueAutomation(t, "Create-issue retry-pending listener")
...
// TestAutomationDispatchSkipsWhenRuntimeOffline locks in the MUL-1899
// admission gate: when the assignee agent's runtime is not online we must
// record a `skipped` automation_run with a failure_reason and NOT enqueue an
// agent_task_queue row. This is the fix for "活跃 schedule 持续给离线 local
// agent 入队".
func TestAutomationDispatchSkipsWhenRuntimeOffline(t *testing.T) {
	ctx := context.Background()
	queries := db.New(testPool)
	bus := events.New()
	taskSvc := service.NewTaskService(queries, testPool, nil, bus)
	...
	ap, err := queries.CreateAutomation(ctx, db.CreateAutomationParams{
		WorkspaceID:        parseUUID(testWorkspaceID),
		Title:              "Offline-runtime automation",
...
// TestAutomationCreateIssueDispatchCreatesIssueWhenRuntimeOffline locks in the
// audit-trail contract for tracked automations: create_issue mode must still
...
func TestAutomationCreateIssueDispatchCreatesIssueWhenRuntimeOffline(t *testing.T) {
	ctx := context.Background()
	queries := db.New(testPool)
	bus := events.New()
	taskSvc := service.NewTaskService(queries, testPool, nil, bus)
	...
	ap, err := queries.CreateAutomation(ctx, db.CreateAutomationParams{
		WorkspaceID:        parseUUID(testWorkspaceID),
		Title:              "Offline create-issue automation",
...
// and task creation), DispatchAutomation must return (run, nil) with
...
//  3. Triggering the automation. Admission has already loaded the agent +
...
func TestManualTriggerDoesNotErrorOnPostAdmissionSkip(t *testing.T) {
	...
	ap, err := queries.CreateAutomation(ctx, db.CreateAutomationParams{
		WorkspaceID:        parseUUID(testWorkspaceID),
		Title:              "Manual-trigger skip automation",
	...
	run, err := automationSvc.DispatchAutomation(ctx, ap, pgtype.UUID{}, "manual", nil)
	if err != nil {
		t.Fatalf("manual DispatchAutomation returned error (would 500 the handler): %v", err)
	}
	if run == nil {
		t.Fatal("expected a run, got nil")
	}
