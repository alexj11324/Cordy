...
// automation_schedule_dispatch JobSpec: an active automation, a
...
func setupAutomationScheduleJob(t *testing.T, cron string) (db.AutomationTrigger, *scheduler.Manager, *service.AutomationService) {
	t.Helper()
	...
	if _, err := testPool.Exec(ctx,
		`UPDATE automation_trigger SET created_at = now() - INTERVAL '2 minute' WHERE id = $1`,
		trigger.ID,
	); err != nil {
	...
	t.Cleanup(func() {
		...
		_, _ = testPool.Exec(bg,
			...
			scheduler.ScopeKindAutomationTrigger, util.UUIDToString(trigger.ID),
		)
		_, _ = testPool.Exec(bg, `DELETE FROM automation WHERE id = $1`, ap.ID)
	})

	mgr := scheduler.NewManager(testPool, scheduler.Options{
		RunnerID: "automation-job-test",
	})
	if err := mgr.Register(scheduler.AutomationScheduleDispatchJob(testPool, queries, automationSvc)); err != nil {
		t.Fatalf("register automation_schedule_dispatch job: %v", err)
	}

	return trigger, mgr, automationSvc
}

// TestAutomationScheduleJobDispatchesOnce verifies the end-to-end
// happy path: one tick of the JobSpec produces exactly one
// sys_cron_executions row (SUCCESS) and exactly one automation_run
// row tagged with the canonical UTC planned_at. This is the
// occurrence-level idempotency contract from MUL-3551 §1.
func TestAutomationScheduleJobDispatchesOnce(t *testing.T) {
	ctx := context.Background()

	// `*/1 * * * *` fires every minute. Trigger.created_at is
	// backdated 2 minutes in the fixture so there is always at least
	// one due occurrence.
	trigger, mgr, _ := setupAutomationScheduleJob(t, "*/1 * * * *")
	...
	`, scheduler.JobNameAutomationScheduleDispatch, scheduler.ScopeKindAutomationTrigger,
	...
	// Exactly one automation_run with planned_at set.
	var runRows int
	...
	if runRows != 1 || !plannedAtValid {
		t.Fatalf("expected 1 automation_run with planned_at set, got %d rows planned_at_valid=%v", runRows, plannedAtValid)
	}
...
// TestAutomationScheduleJobMissedSchedulesCollapse covers MUL-3551 §4:
...
func TestAutomationScheduleJobMissedSchedulesCollapse(t *testing.T) {
	ctx := context.Background()

	trigger, mgr, _ := setupAutomationScheduleJob(t, "*/5 * * * *")

	// Force a large historical window: the trigger thinks it was
	// registered an hour ago, so without collapse the hook would
	// emit 12 occurrences.
	if _, err := testPool.Exec(ctx,
		`UPDATE automation_trigger SET created_at = now() - INTERVAL '1 hour' WHERE id = $1`,
		trigger.ID,
	); err != nil {
		t.Fatalf("backdate trigger.created_at: %v", err)
	}
	...
	var runRows int
	if err := testPool.QueryRow(ctx, `
		SELECT COUNT(*) FROM automation_run WHERE trigger_id = $1
	`, trigger.ID).Scan(&runRows); err != nil {
		t.Fatalf("count run rows: %v", err)
	}
	if runRows != 1 {
		t.Fatalf("missed schedules must collapse to a single automation_run, got %d", runRows)
	}
}

// TestAutomationScheduleJobCrashRecovery covers MUL-3551 §5: a runner
...
// dispatch path is re-entered, DispatchAutomationForPlan sees the
...
// the automation_run.
func TestAutomationScheduleJobCrashRecovery(t *testing.T) {
	ctx := context.Background()
	...
	if err := testPool.QueryRow(ctx, `
		SELECT id, task_id IS NOT NULL FROM automation_run WHERE trigger_id = $1
	...
	// died before its terminal UPDATE landed. The automation_run row
	// from tick 1 stays as the "complete" snapshot — that is what
	// DispatchAutomationForPlan must reuse on the retry.
	if _, err := testPool.Exec(ctx, `
	...
	// Exactly one automation_run, and it is the SAME one tick 1
	// created — DispatchAutomationForPlan's idempotency + the
	// complete-run reuse path in isAutomationRunComplete prevent a
	// duplicate.
	rows, err := testPool.Query(ctx, `SELECT id FROM automation_run WHERE trigger_id = $1`, trigger.ID)
	if err != nil {
	...
	if len(seenIDs) != 1 {
		t.Fatalf("crash recovery must keep automation_run at exactly one row, got %d", len(seenIDs))
	}
...
// TestAutomationScheduleJobTwoRunnersSingleWinner covers the
...
// uniqueness key.
func TestAutomationScheduleJobTwoRunnersSingleWinner(t *testing.T) {
	ctx := context.Background()
	queries := db.New(testPool)
	bus := events.New()
	taskSvc := service.NewTaskService(queries, testPool, nil, bus)
	...
	if err := mgrA.Register(scheduler.AutomationScheduleDispatchJob(testPool, queries, automationSvc)); err != nil {
		t.Fatalf("register A: %v", err)
	}
	if err := mgrB.Register(scheduler.AutomationScheduleDispatchJob(testPool, queries, automationSvc)); err != nil {
	...
	if runRows != execRows {
		t.Fatalf("expected 1 automation_run per exec row, got exec=%d run=%d", execRows, runRows)
	}
}

// TestAutomationScheduleJobDisabledTriggerSkips locks in that a
// trigger toggled off between scope-list and handler run is treated
// as a SUCCESS no-op — no automation_run created. This protects
// against the race the legacy goroutine could not prove safe (it
// reloaded the automation, but never re-checked the trigger's
// enabled flag in-handler).
func TestAutomationScheduleJobDisabledTriggerSkips(t *testing.T) {
	ctx := context.Background()
	...
	// covered by the unit-level automation_inactive case below.)
	if _, err := testPool.Exec(ctx,
		`UPDATE automation_trigger SET enabled = FALSE WHERE id = $1`, trigger.ID,
	); err != nil {
...
// TestAutomationScheduleJobPausedAutomationSkipsAtHandler covers the
// in-handler race window: scope-list says "active" at tick start,
// but the automation is paused before the per-scope handler runs
// (e.g. by an HTTP PUT that landed between scope-list and dispatch).
// The handler MUST re-read automation.status and treat the run as a
// SUCCESS no-op (skipped_reason: "automation_inactive") instead of
// firing a real dispatch.
//
// Driving this via `mgr.RunOnce` would not exercise the guard —
// scope-list's SQL filter already excludes paused automations so the
...
func TestAutomationScheduleJobPausedAutomationSkipsAtHandler(t *testing.T) {
	ctx := context.Background()
	queries := db.New(testPool)
	bus := events.New()
	taskSvc := service.NewTaskService(queries, testPool, nil, bus)
	...
	// Pause the parent automation AFTER setup — simulates the
	// "paused after scope-list" race the in-handler guard exists for.
	if _, err := queries.UpdateAutomation(ctx, db.UpdateAutomationParams{
	...
	job := scheduler.AutomationScheduleDispatchJob(testPool, queries, automationSvc)

	// Drive the handler directly. plan_time is arbitrary — what we
	// are asserting is that the handler re-reads automation.status
	// and returns a SUCCESS no-op WITHOUT calling DispatchAutomationForPlan.
	planTime := time.Now().UTC().Truncate(time.Minute)
	result, err := job.Handler(ctx, scheduler.HandlerInput{
		Job:       &job,
		Scope:     scheduler.Scope{Kind: scheduler.ScopeKindAutomationTrigger, ID: util.UUIDToString(trigger.ID)},
		PlanTime:  planTime,
		Attempt:   1,
	...
	if result.RowsAffected != 0 {
		t.Fatalf("paused automation must produce a no-op (rows_affected=0), got %d", result.RowsAffected)
	}
	if got, _ := result.Result["skipped_reason"].(string); got != "automation_inactive" {
		t.Fatalf("expected skipped_reason=automation_inactive, got %q", got)
	}

	// And — critically — no automation_run should have been created
	...
	if runRows != 0 {
		t.Fatalf("paused-automation handler guard must not create a run, got %d", runRows)
	}
}

// TestAutomationScheduleJobBadCronStaysSilent locks in the failure
...
// automation_trigger.cron_expression, the planner hook must fail
...
// row, and NO automation_run created. The bad cron MUST NOT silently
// look like a SUCCESS.
func TestAutomationScheduleJobBadCronStaysSilent(t *testing.T) {
	ctx := context.Background()

	trigger, mgr, _ := setupAutomationScheduleJob(t, "*/1 * * * *")

	if _, err := testPool.Exec(ctx,
		`UPDATE automation_trigger SET cron_expression = $2 WHERE id = $1`,
...
// seedColdStartTrigger creates an automation + a schedule trigger
// (kind='schedule', timezone=UTC, given cron) and returns the
// trigger plus the queries/service handles. Cleanup of the automation
...
// outcome.
func seedColdStartTrigger(t *testing.T, cron string) (db.AutomationTrigger, *db.Queries, *service.AutomationService) {
	t.Helper()
	ctx := context.Background()
	queries := db.New(testPool)
	bus := events.New()
	...
	return trigger, queries, automationSvc
}

// TestAutomationScheduleJobColdStartHonorsLastFiredAt is the regression
...
func TestAutomationScheduleJobColdStartHonorsLastFiredAt(t *testing.T) {
	ctx := context.Background()
	trigger, queries, automationSvc := seedColdStartTrigger(t, "0 17 * * 1-5")
	...
	scope := scheduler.Scope{
		Kind: scheduler.ScopeKindAutomationTrigger,
...
// TestAutomationScheduleJobColdStartBrandNewTriggerStillFires is the
// counterpart to TestAutomationScheduleJobColdStartHonorsLastFiredAt:
...
func TestAutomationScheduleJobColdStartBrandNewTriggerStillFires(t *testing.T) {
	ctx := context.Background()
	trigger, queries, automationSvc := seedColdStartTrigger(t, "0 12 * * *")
...
// TestAutomationScheduleJobColdStartSkipsStaleMissedOccurrence covers the
...
func TestAutomationScheduleJobColdStartSkipsStaleMissedOccurrence(t *testing.T) {
	ctx := context.Background()
	trigger, queries, automationSvc := seedColdStartTrigger(t, "7 9 * * *")
