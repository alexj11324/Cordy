...
func TestRedisClientName(t *testing.T) {
	tests := []struct {
		...
		{"empty_suffix_returns_existing", "cordy-api:store", "", "cordy-api:store"},
		{"empty_existing_uses_default_prefix", "", "store", "cordy-api:store"},
...
func TestNewNamedRedisClient_SetsClientName(t *testing.T) {
	...
	opts := client.Options()
	if opts.ClientName != "cordy-api:store" {
		t.Errorf("ClientName = %q, want %q", opts.ClientName, "cordy-api:store")
	}
}
...
func TestNewNamedRedisClient_DisableClientName_InvalidValue(t *testing.T) {
	...
	if opts.ClientName != "cordy-api:store" {
		t.Errorf("ClientName = %q, want %q (invalid env should fall back to naming enabled)", opts.ClientName, "cordy-api:store")
	}
}
...
func TestJWTSecretBootError(t *testing.T) {
	...
	tests := []struct {
		...
		{"production_with_code_default_is_rejected", "cordy-dev-secret-change-in-production", "production", true},
