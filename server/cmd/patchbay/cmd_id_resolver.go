	...
	if _, err := normalizeUUIDPrefix(trimmed); err == nil {
		return resolvedID{}, fmt.Errorf(
			"issue ref %q looks like a short UUID prefix; short prefixes are no longer supported for issues. "+
				"Use the issue key (e.g. MUL-123) shown by `patchbay issue list`, or pass the full UUID (run a list command with --full-id to copy it)",
			input,
		)
	}
	return resolvedID{}, fmt.Errorf(
		"issue ref %q is not a recognized issue reference; use the issue key (e.g. MUL-123) shown by `patchbay issue list`, or pass the full UUID",
		input,
	)
}
...
func resolveTaskRunID(ctx context.Context, client *cli.APIClient, issueID, input string) (resolvedID, error) {
	...
	if strings.TrimSpace(issueID) == "" {
		return resolvedID{}, fmt.Errorf("short task run prefixes require --issue <issue-id>; pass a full task UUID or run `patchbay issue runs <issue-id> --full-id`")
	}
...
func (l actorDisplayLookup) loadTeams() {
	if l.state == nil || l.state.teamsLoaded {
		return
	}
	l.state.teamsLoaded = true
	l.state.teams = map[string]string{}
	if l.client == nil || l.client.WorkspaceID == "" {
		return
	}
	var teams []map[string]any
	if err := l.client.GetJSON(l.ctx, "/api/teams", &teams); err == nil {
		for _, s := range teams {
			if id := strVal(s, "id"); id != "" {
				l.state.teams[id] = strVal(s, "name")
			}
		}
	}
}
...
func (l actorDisplayLookup) actor(actorType, id string) string {
	...
	case "team":
		l.loadTeams()
		if l.state != nil && l.state.teams != nil {
			if name := l.state.teams[id]; name != "" {
				return "team:" + name
			}
		}
	}
