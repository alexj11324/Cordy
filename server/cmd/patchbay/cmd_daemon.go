...
import (
	...
	logger_pkg "github.com/patchbay-ai/patchbay/server/internal/logger"
...
func requireDaemonRestartPreflight(cmd *cobra.Command, profile string) error {
	...
	if err := cli.NewAPIClient(baseURL, "", cfg.Token).GetJSON(ctx, "/api/me", nil); err != nil {
		...
		return fmt.Errorf("refusing to restart: cannot reach the Patchbay server at %s (%w); the running daemon was left untouched.\nMake sure the server is running and reachable, then rerun 'patchbay daemon restart'", baseURL, err)
	}
	return nil
}
...
func describeDaemonManager(launchedBy string) string {
	if launchedBy == "desktop" {
		return "Patchbay Desktop app (start and stop it from the app)"
	}
	return launchedBy
}
