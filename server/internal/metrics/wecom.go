...
func NewWecomMetrics() *WecomMetrics {
	...
	return &WecomMetrics{
		...
		OutboundSkipped: prometheus.NewCounterVec(prometheus.CounterOpts{
			...
			Help: "Completions this adapter was never going to deliver to WeCom, by reason. Kept apart from dropped because none of these is a delivery failure: origin_not_channel is a question typed in Orvilo on a WeCom-bound session, installation_revoked means there is no longer an installation to deliver through, nothing_to_say is an empty completion carrying no file. Counting them as drops would make ordinary web usage read as a WeCom outage.",
		}, []string{"reason"}),
