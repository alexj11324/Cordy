...
func TestBusinessMetricsRegistryExposesAllFamilies(t *testing.T) {
	...
	exerciseEvent(m, analytics.EventTeamCreated, nil)
	exerciseEvent(m, analytics.EventAutomationCreated, map[string]any{"cadence": "manual"})
	...
	exerciseEvent(m, analytics.EventAutomationRunStarted, map[string]any{"cadence": "manual", "trigger_kind": "manual"})
	exerciseEvent(m, analytics.EventAutomationRunCompleted, map[string]any{"cadence": "manual", "trigger_kind": "manual"})
	exerciseEvent(m, analytics.EventAutomationRunFailed, map[string]any{"cadence": "manual", "trigger_kind": "manual"})
	...
	m.RecordAutomationRunSkipped("manual", "throttled")
	...
	m.RecordEntitlementDecision("automation_runs", "observe", "cache_fresh")
	m.RecordEntitlementVersionRegression()
	m.RecordAutomationQuotaDecision("observe", "manual", "admitted")
