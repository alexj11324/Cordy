...
// app-level token (xapp-) into Patchbay (the paste path lives in byo_install.go).
...
var (
	...
	// already connected to a live owner in a DIFFERENT Patchbay workspace — it
	...
	ErrTeamOwnedByAnotherWorkspace = errors.New("slack: this Slack app is already connected to a different Patchbay workspace")
...
// the pasted Slack app is already connected to a DIFFERENT agent or Patchbay
...
func (s *InstallService) persistInstall(ctx context.Context, p installPersist, limit *int64) (db.ChannelInstallation, error) {
	...
	// owner (existing agent, including an archived one) is left in place and trips
...
// ListByWorkspace returns every Slack installation in the workspace (installed and
...
// (ListConnectableInstallations filters to installed and unpaused), so its Socket Mode connection
