...
// failed) read so the unified `cordy chat history` / `cordy chat thread`
...
// `cordy chat history` (channel overview) and `cordy chat thread [id]`
...
// boundary for `cordy chat thread <id>` (the agent supplies only a
...
// drills into with `cordy chat thread <id>`. Backs `cordy chat history`.
...
// threads) reads its linear conversation. Backs `cordy chat thread [id]`.
...
// agent can drill in with `cordy chat thread <id>`.
