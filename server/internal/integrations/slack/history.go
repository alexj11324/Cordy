...
// failed) read so the unified `patchbay chat history` / `patchbay chat thread`
...
// `patchbay chat history` (channel overview) and `patchbay chat thread [id]`
...
// boundary for `patchbay chat thread <id>` (the agent supplies only a
...
// drills into with `patchbay chat thread <id>`. Backs `patchbay chat history`.
...
// threads) reads its linear conversation. Backs `patchbay chat thread [id]`.
...
// agent can drill in with `patchbay chat thread <id>`.
