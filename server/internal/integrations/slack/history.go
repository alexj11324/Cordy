...
// failed) read so the unified `orvilo chat history` / `orvilo chat thread`
...
// `orvilo chat history` (channel overview) and `orvilo chat thread [id]`
...
// boundary for `orvilo chat thread <id>` (the agent supplies only a
...
// drills into with `orvilo chat thread <id>`. Backs `orvilo chat history`.
...
// threads) reads its linear conversation. Backs `orvilo chat thread [id]`.
...
// agent can drill in with `orvilo chat thread <id>`.
