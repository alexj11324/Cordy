...
func (p *Patcher) processEvent(ctx context.Context, e events.Event) error {
	...
	if err != nil {
		if errors.Is(err, pgx.ErrNoRows) {
			// Direct Patchbay task or violated snapshot invariant — fail closed.
			return nil
		}
	...
	// that originated in Lark, but their replies belong only in Patchbay.
	...
	if InstallationStatus(inst.Status) != InstallationInstalled {
