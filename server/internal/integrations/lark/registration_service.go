...
func (s *RegistrationService) BeginInstall(ctx context.Context, p BeginInstallParams) (BeginInstallResult, error) {
	...
	// bot reads "<agent> - Orvilo" instead of "{用户姓名}的智能助手".
	agentName := ""
	if p.AgentID.Valid {
...
// "connected to a different Orvilo workspace" that lied when the real owner sat
...
func (s *RegistrationService) liveOwnerConflictMessage(ctx context.Context, requestingWorkspaceID pgtype.UUID, appID string) string {
	...
	case owner.WorkspaceID != requestingWorkspaceID:
		return "This Feishu app is already connected to a different Orvilo workspace. Disconnect it there before connecting it here."
...
// "<agent> - Orvilo" instead of Lark's auto-generated
...
// "Orvilo" rather than a dangling " - Orvilo".
