	...
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			rep := NewLarkOutcomeReplier(OutcomeReplierConfig{
				...
				Queries: stubReplierQueries{}, AppURL: "https://cordy.test", Logger: log,
			})
...
func TestLarkOutcomeReplierUsesAppURLForWebLinks(t *testing.T) {
	...
	if got := stub.bindingCalls[0].BindURL; got != "https://app.cordy.test/lark/bind?token=token+with+space" {
	...
	if !strings.Contains(stub.textOut[0].Text, "https://app.cordy.test/issues/MUL-42") {
...
func TestLarkOutcomeReplierIssueCreatedSendsConfirmation(t *testing.T) {
	...
	if !strings.Contains(got.Text, "https://cordy.test/issues/MUL-42") {
		t.Errorf("text should embed the deep link back to Cordy; got %q", got.Text)
	}
