...
// can't accidentally pass a Patchbay user UUID where a Lark open_id is
// expected.
type OpenID string

// ChatID identifies a Lark conversation (p2p or group). One ChatID maps
// to one Patchbay chat_session via lark_chat_session_binding.
type ChatID string
...
const (
	...
	// DropReasonNonWorkspaceMember — the sender resolved to a Patchbay
