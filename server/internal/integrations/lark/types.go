...
// can't accidentally pass an Orvilo user UUID where a Lark open_id is
// expected.
type OpenID string

// ChatID identifies a Lark conversation (p2p or group). One ChatID maps
// to one Orvilo chat_session via lark_chat_session_binding.
type ChatID string
...
// separate clouds with distinct hosts; a single Orvilo deployment serves
...
const (
	...
	// DropReasonNonWorkspaceMember — the sender resolved to an Orvilo
