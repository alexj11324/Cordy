...
// can't accidentally pass a Cordy user UUID where a Lark open_id is
// expected.
type OpenID string

// ChatID identifies a Lark conversation (p2p or group). One ChatID maps
// to one Cordy chat_session via lark_chat_session_binding.
type ChatID string
...
const (
	...
	// DropReasonNonWorkspaceMember — the sender resolved to a Cordy
