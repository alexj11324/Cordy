...
const (
	...
	// Operators do NOT set CORDY_LARK_HTTP_BASE_URL to pick a cloud
	...
	// latency from a self-hosted Cordy deployment to feishu.cn.
...
func (c HTTPClientConfig) withDefaults() HTTPClientConfig {
	...
	// non-empty BaseURL (CORDY_LARK_HTTP_BASE_URL, or an httptest URL
...
// keyed by app_id so a single Cordy server reuses Lark's
...
// cfg.BaseURL (CORDY_LARK_HTTP_BASE_URL, or an httptest URL in tests)
...
func bindingPromptTemplate(bindURL string) (string, error) {
	doc := map[string]any{
		...
		"header": map[string]any{
			"template": "blue",
			"title":    map[string]any{"tag": "plain_text", "content": "Cordy"},
		},
		"elements": []any{
			map[string]any{
				"tag": "div",
