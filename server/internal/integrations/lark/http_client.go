...
const (
	...
	// latency from a self-hosted Orvilo deployment to feishu.cn.
...
// keyed by app_id so a single Orvilo server reuses Lark's
...
func bindingPromptTemplate(bindURL string) (string, error) {
	doc := map[string]any{
		...
		"header": map[string]any{
			"template": "blue",
			"title":    map[string]any{"tag": "plain_text", "content": "Orvilo"},
		},
		"elements": []any{
			map[string]any{
				"tag": "div",
				"text": map[string]any{
					"tag":     "lark_md",
					"content": "你还没有绑定 Orvilo 账户。点击下方按钮完成绑定后即可使用此 Agent。",
				},
			},
			map[string]any{
				"tag": "action",
