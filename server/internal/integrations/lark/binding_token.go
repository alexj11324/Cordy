...
type InstallerBindParams struct {
	WorkspaceID    pgtype.UUID
	InstallationID pgtype.UUID
	CordyUserID  pgtype.UUID // the installer's Cordy account
...
// identity is the supplied cordyUserID (taken from the session by
...
//     (installation, open_id), pointing at a DIFFERENT Cordy user.
...
func (s *BindingTokenService) RedeemAndBind(ctx context.Context, raw string, cordyUserID pgtype.UUID) (RedeemedBindingToken, error) {
	...
	isMember, err := qtx.IsWorkspaceMember(ctx, row.WorkspaceID, cordyUserID)
	if err != nil {
	...
	if err != nil {
		...
		// cordy_user_id differs from ours, so the WHERE clause on
...
// (installation_id, lark_open_id) when cordy_user_id matches (the
...
func (s *BindingTokenService) BindInstallerTx(ctx context.Context, qtx *ChannelStore, p InstallerBindParams) error {
	...
	isMember, err := q.IsWorkspaceMember(ctx, p.WorkspaceID, p.CordyUserID)
	if err != nil {
...
// open_id) pair and points at a different Cordy user. Account
