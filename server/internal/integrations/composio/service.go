// Package composio is the Stage 2 business-integration glue between Cordy and
// the standalone Composio SDK (server/pkg/composio). It owns Cordy semantics:
...
import (
	...
	sdk "github.com/cordy-ai/cordy/server/pkg/composio"
...
type Config struct {
	...
	// trailing slash (e.g. "https://cordy.ai"). The Composio callback
	...
	// browser redirect (e.g. "https://cordy.ai"). May be empty, in which
...
//
// The composio_user_id sent to Composio is the Cordy user id verbatim — the
...
func (s *Service) CompleteCallback(ctx context.Context, state, status, connectedAccountID string) (string, error) {
	...
	// composio_user_id == cordy user id invariant) and was created under the
