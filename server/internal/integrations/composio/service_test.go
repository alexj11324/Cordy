...
func TestBeginConnect_MappingAndState(t *testing.T) {
	...
	// composio_user_id == orvilo user id
	...
	if !strings.HasPrefix(cb, "https://orvilo.aspectlylabs.com"+callbackPath+"?state=") {
...
func TestCallbackRedirect(t *testing.T) {
	t.Parallel()
	svc := newTestService(t, &fakeSDK{}, newFakeStore())
	if got := svc.CallbackRedirect("notion", true); got != "https://orvilo.aspectlylabs.com/settings?tab=integrations&connected=notion" {
		t.Errorf("success redirect = %q", got)
	}
	if got := svc.CallbackRedirect("notion", false); got != "https://orvilo.aspectlylabs.com/settings?tab=integrations&error=composio_connect_failed" {
