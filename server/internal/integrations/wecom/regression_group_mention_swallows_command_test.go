...
// mention is delivered as part of the message text: "@Cordy Bot /issue 登录
...
// "@Cordy" and decide this is ordinary prose. Every slash command in every
...
// TestAMultiWordBotNameDoesNotSwallowTheCommand: a bot named "Cordy Bot" is
// mentioned as "@Cordy Bot", and cutting the mention at the first space
...
func TestAMultiWordBotNameDoesNotSwallowTheCommand(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"two-word name", "Cordy Bot", "@Cordy Bot /new 重新分析这批数据", "/new 重新分析这批数据"},
		...
		{"a colleague's mention before ours", "Cordy Bot", "@李雷 @Cordy Bot /new 分析", "/new 分析"},
...
func TestAMultiWordBotNameReachesTheParser(t *testing.T) {
	t.Parallel()
	msg := dispatchGroupMessage(t, "Cordy Bot", "@Cordy Bot /issue 登录失败")
	cmd, ok := engine.ParseIssueCommand(msg.CommandText)
...
func TestAConfiguredNameDoesNotManufactureACommand(t *testing.T) {
	...
	got := stripLeadingMentions(in, "Cordy Bot")
...
// "@Cordy Bot /issue …" again. Same two lines as
...
func TestSessionBinder_AdapterCommandTextWins(t *testing.T) {
	...
	if fb.appendIn.Body != "@Cordy Bot /issue 登录失败" {
