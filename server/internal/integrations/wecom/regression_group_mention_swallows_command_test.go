...
// mention is delivered as part of the message text: "@Orvilo Bot /issue 登录
...
// "@Orvilo" and decide this is ordinary prose. Every slash command in every
...
// TestAMultiWordBotNameDoesNotSwallowTheCommand: a bot named "Orvilo Bot" is
// mentioned as "@Orvilo Bot", and cutting the mention at the first space
...
func TestAMultiWordBotNameDoesNotSwallowTheCommand(t *testing.T) {
	...
	for _, tc := range []struct {
		...
		{"two-word name", "Orvilo Bot", "@Orvilo Bot /new 重新分析这批数据", "/new 重新分析这批数据"},
		...
		{"a colleague's mention before ours", "Orvilo Bot", "@李雷 @Orvilo Bot /new 分析", "/new 分析"},
...
func TestAMultiWordBotNameReachesTheParser(t *testing.T) {
	t.Parallel()
	msg := dispatchGroupMessage(t, "Orvilo Bot", "@Orvilo Bot /issue 登录失败")
	cmd, ok := engine.ParseIssueCommand(msg.CommandText)
...
func TestAConfiguredNameDoesNotManufactureACommand(t *testing.T) {
	...
	got := stripLeadingMentions(in, "Orvilo Bot")
...
// "@Orvilo Bot /issue …" again. Same two lines as
...
func TestSessionBinder_AdapterCommandTextWins(t *testing.T) {
	...
	if fb.appendIn.Body != "@Orvilo Bot /issue 登录失败" {
