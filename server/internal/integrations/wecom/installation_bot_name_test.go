...
func TestRotatingTheSecretKeepsTheBotName(t *testing.T) {
	...
	if again.BotDisplayName != "Patchbay Bot" {
...
func TestTheBotNameSurvivesTheConfigRoundTrip(t *testing.T) {
	t.Parallel()
	cfg, err := encodeInstallConfig(Installation{BotID: "bot-xyz", BotDisplayName: "Patchbay Bot"})
	if err != nil {
	...
	if out.BotDisplayName != "Patchbay Bot" {
		t.Fatalf("BotDisplayName round trip = %q, want Patchbay Bot", out.BotDisplayName)
	}
}
