...
func TestRotatingTheSecretKeepsTheBotName(t *testing.T) {
	...
	if _, err := svc.Upsert(ctx, botNameParams(wcNameBotA, "Orvilo Bot"), nil); err != nil {
	...
	if again.BotDisplayName != "Orvilo Bot" {
...
func TestTheBotNameSurvivesTheConfigRoundTrip(t *testing.T) {
	t.Parallel()
	cfg, err := encodeInstallConfig(Installation{BotID: "bot-xyz", BotDisplayName: "Orvilo Bot"})
	if err != nil {
	...
	if out.BotDisplayName != "Orvilo Bot" {
		t.Fatalf("BotDisplayName round trip = %q, want Orvilo Bot", out.BotDisplayName)
	}
}
