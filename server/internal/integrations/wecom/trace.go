...
//
// With CORDY_WECOM_TRACE=1 the server records enough to check a real-device
...
// CORDY_WECOM_TRACE; returns what it set so the caller can log it.
...
// a 43-character token, and with a normal CORDY_APP_URL the token's last
...
// could bind that sender's WeCom identity to their own Cordy account before
