...
func mediaBindTestDB(t *testing.T) *pgxpool.Pool {
	...
	if dsn == "" {
		dsn = "postgres://cordy:cordy@localhost:5432/cordy?sslmode=disable"
	}
...
func seedMediaBindFixture(t *testing.T, pool *pgxpool.Pool) mediaBindFixture {
	...
	if err := pool.QueryRow(ctx, `INSERT INTO "user" (name, email) VALUES ($1, $2) RETURNING id`,
		"wecom media bind", fmt.Sprintf("wecom-media-bind-%d@cordy.test", suffix)).Scan(&f.userID); err != nil {
	...
	if err := pool.QueryRow(ctx, `
		...
		VALUES ($1, $2, 'local', 'cordy_daemon', $3) RETURNING id`,
...
// the front of the text run: "@Cordy Bot /issue 登录坏了". main strips that
...
// the parser sees "@Cordy Bot"; derive the command from the resolved body
...
func TestGroupMentionedIssueAfterAnImageIsStillFiled(t *testing.T) {
	issues, body := runMixedIssueThroughTheRouter(t, "Cordy Bot", "group", "GROUP-BIND-1", "@Cordy Bot /issue 登录坏了", false)
	...
	if want := "[Image]\n@Cordy Bot /issue 登录坏了"; body != want {
...
func TestGroupMentionedSpokenIssueAfterAnImageIsStillFiled(t *testing.T) {
	issues, body := runMixedIssueThroughTheRouter(t, "Cordy Bot", "group", "GROUP-BIND-2", "@Cordy Bot /issue 登录坏了", true)
