...
func TestReplyNeedsBindingDoesNotMintBearerLinkInGroup(t *testing.T) {
	...
	if strings.Contains(got.Text, "secret-token") || strings.Contains(got.Text, "orvilo.example") {
...
func TestReplyNeedsBindingInPrivateChatMintsQuotedLink(t *testing.T) {
	...
	r := NewOutboundReplier(OutboundReplierConfig{
		Binding: minter, AppURL: "https://orvilo.example/", APIBase: srv.URL,
	...
	if minter.calls != 1 || !strings.Contains(got.Text, "https://orvilo.example/telegram/bind?token=one-time-token") {
