...
import (
	...
	"github.com/cordy-ai/cordy/server/internal/daemon/processtree"
)
...
type Cache struct {
	root   string // base directory for all caches (e.g. ~/cordy_workspaces/.repos)
	logger *slog.Logger
...
const lastUsedFile = ".cordy_last_used"
...
type WorktreeParams struct {
	...
	// listed as a writable root — on Linux (cordy-ai/cordy#2925) and on the
	// Windows native sandbox (cordy-ai/cordy#6449).
...
const (
	isolatedCheckoutConfigKey   = "cordy.checkout-mode"
	isolatedCheckoutConfigValue = "isolated"
	isolatedCacheRemoteName     = "cordy-cache"
)
...
func (c *Cache) createOrUpdateIsolatedCheckoutContext(ctx context.Context, barePath, repoURL, checkoutPath, branchName, baseRef string) (string, error) {
	...
	if _, err := os.Stat(checkoutPath); err == nil {
		return "", fmt.Errorf("checkout path already exists and is not a Cordy isolated checkout: %s", checkoutPath)
...
// deleteStaleAgentBranches prunes branches left by earlier Cordy tasks while
...
// cordyHookMarker is a sentinel comment embedded in every prepare-commit-msg
...
const cordyHookMarker = "# cordy:prepare-commit-msg:co-authored-by"
...
// treats a hook as Cordy-owned if its content contains ANY of these
// substrings. The list deliberately includes the legacy comment that the
// daemon used before cordyHookMarker existed, so disabling the toggle on
...
var daemonInstalledHookSignatures = []string{
	cordyHookMarker,
	"# Installed by the Cordy daemon.",
}

// prepareCommitMsgHook is the prepare-commit-msg hook script that appends a
// Co-authored-by trailer for the Cordy Agent to every commit message.
...
// a Co-authored-by trailer for the Cordy Agent. The hook is installed in the
...
// installed by the Cordy daemon (current or any previously released
