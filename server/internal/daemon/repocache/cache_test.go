...
func TestCreateWorktreeContextCancelsRunningGitProcessTree(t *testing.T) {
	...
	body := "#!/bin/sh\n(sleep 0.5; echo leaked > \"$CORDY_TEST_GIT_MARKER\") &\nwait\n"
	if err := os.WriteFile(script, []byte(body), 0o755); err != nil {
		t.Fatal(err)
	}
	t.Setenv("PATH", binDir+string(os.PathListSeparator)+os.Getenv("PATH"))
	t.Setenv("CORDY_TEST_GIT_MARKER", marker)
...
// invocation Windows Codex tasks now use (cordy-ai/cordy#6449) and proves
...
// for the Cordy Agent to every commit made in the worktree.
func TestCreateWorktreeInstallsCoAuthoredByHook(t *testing.T) {
	...
	expectedTrailer := "Co-authored-by: cordy-agent <github@cordy.ai>"
...
// path: a bare cache that already carries the Cordy prepare-commit-msg hook
...
func TestCreateWorktreeRemovesCoAuthoredByHookWhenDisabled(t *testing.T) {
	...
	commitMsg := string(out)
	if strings.Contains(commitMsg, "Co-authored-by: cordy-agent") {
...
// prepare-commit-msg hook that does NOT include the cordyHookMarker
// sentinel — only the older `# Installed by the Cordy daemon.` comment.
...
func TestCreateWorktreeRemovesLegacyCoAuthoredByHook(t *testing.T) {
	...
	// previous daemon release (no cordyHookMarker line). Keeping a
...
`
	...
	if commitMsg := string(out); strings.Contains(commitMsg, "Co-authored-by: cordy-agent") {
...
// without the Cordy marker (e.g. one a user added manually) must be left
