...
// same machine shares one UUID stored at `~/.patchbay/daemon.id`. Profile
...
// LegacyDaemonUUIDs scans `~/.patchbay/profiles/*/daemon.id` and returns every
...
// explicitly pins PATCHBAY_DAEMON_ID to the hostname itself, there's nothing
