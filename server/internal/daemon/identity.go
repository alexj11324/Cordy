...
// same machine shares one UUID stored at `~/.cordy/daemon.id`. Profile
...
// LegacyDaemonUUIDs scans `~/.cordy/profiles/*/daemon.id` and returns every
...
// explicitly pins CORDY_DAEMON_ID to the hostname itself, there's nothing
