...
func TestClient_RenewToken_PostsToCorrectEndpoint(t *testing.T) {
	...
	srv := httptest.NewServer(http.HandlerFunc(func(w http.ResponseWriter, r *http.Request) {
		...
		if got := r.Header.Get("Authorization"); got != "Bearer ovy_abc" {
			t.Errorf("expected Bearer ovy_abc, got %q", got)
		}
...
func TestTryRenewToken_SurfacesReloginWarningOn401(t *testing.T) {
	...
	if !strings.Contains(out, "orvilo login") {
		t.Fatalf("401 warning must tell the user to run 'orvilo login', got: %s", out)
	}
}
...
// and the operator would never see a "run orvilo login" hint.
func TestPreflightAuth_RenewsBeforeWorkspaceSyncOnExpiredToken(t *testing.T) {
	...
	var buf bytes.Buffer
	d := &Daemon{client: NewClient(srv.URL), logger: captureLogger(&buf)}
	d.client.SetToken("ovy_already_revoked")
	...
	if !strings.Contains(out, "orvilo login") {
		t.Fatalf("expected the actionable 'run orvilo login' hint in the WARN, got: %s", out)
	}
}
