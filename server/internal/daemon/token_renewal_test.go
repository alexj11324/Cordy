...
func TestTryRenewToken_SurfacesReloginWarningOn401(t *testing.T) {
	...
	if !strings.Contains(out, "cordy login") {
		t.Fatalf("401 warning must tell the user to run 'cordy login', got: %s", out)
	}
}
...
// and the operator would never see a "run cordy login" hint.
func TestPreflightAuth_RenewsBeforeWorkspaceSyncOnExpiredToken(t *testing.T) {
	...
	if !strings.Contains(out, "cordy login") {
		t.Fatalf("expected the actionable 'run cordy login' hint in the WARN, got: %s", out)
	}
}
