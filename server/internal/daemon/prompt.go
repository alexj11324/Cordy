...
func sessionContinuityNoticeFor(task Task) string {
	...
	// WeCom, DingTalk) reads it back via `cordy chat history`; Slack alone
...
func buildActiveSiblingRunsBlock(currentIssueID string, runs []ActiveSiblingRunData) string {
	...
	fmt.Fprintf(&b, " (`cordy issue comment list %s --roots-only --summary --compact --output json`)", currentIssueID)
	b.WriteString(" and inspect relevant siblings with the `run-messages` commands below — coordinate with existing work instead of opening a second PR. For writes that only record ownership or status of work already underway, use `--no-start` on `cordy issue assign`/`update`/`status`.\n\n")
	for _, run := range runs {
		...
		fmt.Fprintf(&b, "; inspect: `cordy issue run-messages %s`\n", run.TaskID)
	}
	b.WriteString("\n")
	return b.String()
}
...
func buildPromptBody(task Task, provider string) string {
	...
	var b strings.Builder
	b.WriteString("You are running as a local coding agent for a Cordy workspace.\n\n")
	...
	fmt.Fprintf(&b, "Start by running `cordy issue get %s --output json` to understand your task, then complete it.\n", task.IssueID)
	fmt.Fprintf(&b, "For comment history, follow the rule in your runtime workflow file (assignment-triggered tasks treat the read as mandatory). Scan the threads first with `cordy issue comment list %s --roots-only --summary --compact --output json`, then expand only what matters with `--thread <thread-id> --tail 30`. For `--since` incremental polling, pagination, and folding, see `cordy issue comment list --help`.\n", task.IssueID)
	return b.String()
}

// buildQuickCreatePrompt constructs a prompt for quick-create tasks. The
// user typed a single natural-language sentence in the create-issue modal;
// the agent's job is to translate it into one `cordy issue create` CLI
...
// call `cordy issue get` or attempt to comment — there's nothing to read
// or reply to.
func buildQuickCreatePrompt(task Task) string {
	var b strings.Builder
	b.WriteString("You are running as a quick-create assistant for a Cordy workspace.\n\n")
	b.WriteString("A user captured the following input via the quick-create modal. There is NO existing issue. Your job is to create a well-formed issue from this input with a single `cordy issue create` command.\n\n")
	...
	b.WriteString("- Run exactly one `cordy issue create --output json` invocation. Do not retry for any reason — even on non-zero exit. The issue may already exist; another attempt would create a duplicate.\n")
	...
	b.WriteString("- Do NOT call `cordy issue get` or `cordy issue comment add` — there is no issue to query or comment on.\n")
...
func buildCommentPrompt(task Task, provider string) string {
	...
	if task.TriggerCommentContent != "" {
		...
		if len(task.CoalescedComments) > 0 {
			...
			fmt.Fprintf(&b, "\nIf you need the surrounding discussion for any of them, fetch its thread with `cordy issue comment list %s --thread <thread-id> --tail 30 --compact --output json` using the thread id shown above.\n\n", task.IssueID)
			...
			if task.NewCommentsSince != "" {
				fmt.Fprintf(&b, "Start with `cordy issue comment list %s --since %s --compact --output json`. Treat that as a candidate window, not a guarantee — it also carries unrelated comments, and a retried run can carry ids older than the window. Check every id above against the result.\n\n",
					task.IssueID, task.NewCommentsSince)
			}
			fmt.Fprintf(&b, "Fetch each id you still need directly: `cordy issue comment list %s --thread <comment-id> --tail 30 --compact --output json`. `--thread` accepts a reply id, not just a thread root, so you do not need to know which thread the comment lives in. If it is older than those 30 replies, page back with the `Next reply cursor` values (`--before` / `--before-id`) until it appears. Do not finish this turn until every id above is accounted for.\n\n",
				task.IssueID)
		}
		if taskIsSquadLeader(task) {
			fmt.Fprintf(&b, "⚠️ **Squad leader no_action rule:** If you decide no action is needed, call `cordy squad activity %s no_action --reason \"...\"` and EXIT. DO NOT post any comment — not even one that says \"no action needed\" or \"exiting silently\". The squad activity call records your decision; a comment is redundant noise.\n\n", task.IssueID)
		}
	}
	fmt.Fprintf(&b, "Start by running `cordy issue get %s --output json` to understand your task, then decide how to proceed.\n\n", task.IssueID)
	...
	if hint := execenv.BuildNewCommentsHint(task.IssueID, task.TriggerCommentID, task.TriggerThreadID, task.NewCommentsSince, task.NewCommentCount); hint != "" {
		...
		fmt.Fprintf(&b, "Read the discussion: scan with `cordy issue comment list %s --roots-only --summary --compact --output json`, then expand what matters with `--thread <thread-id> --tail 30`.\n\n", task.IssueID)
	}
...
func buildChatPrompt(task Task) string {
	...
	if task.ChatIntro {
		var b strings.Builder
		b.WriteString("You are running as a chat assistant for a Cordy workspace.\n")
	...
	// like "what did you just talk about" sends it to read Cordy instead of the
	// channel conversation. A web-only chat session gets no such block — its
	// history is the Cordy chat_session the agent already resumes.
	//
	// The history half: `cordy chat history` is served by handler/chat_history.go,
	...
	// `cordy chat thread` (thread expansion); the transcript surfaces have no
	...
	// "its history lives in the channel, NOT in Cordy" for every channel type
	// contradicted the very next line on a transcript surface, which tells the
	// agent Cordy stored it and hands it the command to read it back. An agent
	...
	if task.ChatChannelType != "" {
		...
		fmt.Fprintf(&b, "You are operating inside a %s conversation — not the Cordy web app. Never look in Cordy issues or comments for this conversation.\n", platform)
		if task.ChatChannelType == execenv.ChannelTypeSlack {
			fmt.Fprintf(&b, "This conversation and its history live in %s, NOT in Cordy. The message below may be only what triggered you. Read the conversation with:\n", platform)
			b.WriteString("- `cordy chat history --output json` — the channel overview: recent top-level messages, each thread tagged with a `thread_id` and `reply_count`. It does NOT expand thread contents.\n")
			b.WriteString("- `cordy chat thread [<thread_id>] --output json` — read one thread's messages; omit the id to read the thread you are in, or pass a `thread_id` from the overview to read a specific thread.\n")
			if task.ChatInThread {
				b.WriteString("You were @mentioned inside a thread: start with `cordy chat thread` to read it; if you need the wider channel, run `cordy chat history` and open a specific thread with `cordy chat thread <thread_id>`.\n")
			} else {
				b.WriteString("You were @mentioned at the channel top level: start with `cordy chat history` to see the channel, then read a specific thread's contents with `cordy chat thread <thread_id>`.\n")
			}
			...
			fmt.Fprintf(&b, "The conversation happens in %s, and Cordy stores a transcript of it. The message below may be only what triggered you — read it back with `cordy chat history` when you need earlier context that is not below.\n", platform)
		} else {
			fmt.Fprintf(&b, "This conversation and its history live in %s, NOT in Cordy, and Cordy has no history reader for it. Work from the context already provided to you below — no command can fetch more of this conversation. If you genuinely need earlier context that is not here, ask the user for it rather than guessing.\n", platform)
		}
	...
	// have expired. `cordy attachment download <id>` re-signs at click
	...
	if len(task.ChatMessageAttachments) > 0 {
		...
		b.WriteString("Use `cordy attachment download <id>` to fetch each file locally before referring to it.\n")
		b.WriteString("When creating an issue that should preserve one of these attachments, pass `--attachment-id <id>` to `cordy issue create` in addition to keeping the attachment markdown inline.\n")
	}
	...
	// Cordy chat reply on every surface; what differs is whether anything
	...
	case execenv.ChannelCarriesFiles(task.ChatChannelType, task.ChatChannelDeliversFiles):
		fmt.Fprintf(&b, "\nTo include a file or image you produced in your reply, run `cordy attachment upload <local-path>`. It binds to your reply and Cordy sends it into the %s conversation as a separate message right after your text — there is no way to place it inline, so write your reply to read correctly with the file arriving after it.\n", channelDisplayName(task.ChatChannelType))
	default:
		fmt.Fprintf(&b, "\nThis reply is delivered to %s as text. You cannot attach a file to it: `cordy attachment upload` binds to a Cordy chat reply, which this is not. If you produce a file, describe it in words — never write its local path as a link, and never upload it and then write as though it arrived.\n", channelDisplayName(task.ChatChannelType))
	}
	return b.String()
}
...
func buildAutopilotPrompt(task Task) string {
	...
	if task.AutopilotID != "" {
		fmt.Fprintf(&b, "Start by running `cordy autopilot get %s --output json` if you need the full autopilot configuration, then complete the instructions above.\n", task.AutopilotID)
	} else {
...
// promoted to leader and handed the leader rules (mandatory `cordy squad
