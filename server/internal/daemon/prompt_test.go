...
func TestBuildQuickCreatePromptRules(t *testing.T) {
	...
	mustContain := []string{
		...
		// auto-subscribe path fires (cordy issue create has no --subscriber flag)
		"CC exception",
...
func TestBuildChatPromptChannelAwareness(t *testing.T) {
	t.Run("slack-backed prompt teaches both read commands", func(t *testing.T) {
		...
		for _, want := range []string{"Slack", "NOT in Cordy", "cordy chat history", "cordy chat thread", "Do NOT narrate"} {
			if !strings.Contains(out, want) {
	...
	t.Run("top-level mention starts with history", func(t *testing.T) {
		...
		if !strings.Contains(out, "top level: start with `cordy chat history`") {
	...
	t.Run("in-thread mention starts with thread", func(t *testing.T) {
		...
		if !strings.Contains(out, "inside a thread: start with `cordy chat thread`") {
	...
	t.Run("web-only session has no channel block", func(t *testing.T) {
		...
		if strings.Contains(out, "cordy chat history") {
			t.Fatalf("web-only chat prompt should not mention channel history, got:\n%s", out)
		}
	})

	// A transcript surface must not be told its history is "NOT in Cordy" and
	// then handed a Cordy command to read that history. The claim used to be
	...
	for _, channelType := range []string{
		...
		t.Run(channelType+" transcript prompt does not contradict itself", func(t *testing.T) {
			...
			if strings.Contains(out, "NOT in Cordy") {
				t.Errorf("transcript surface told its history is NOT in Cordy, then told to read it from Cordy\n--- output ---\n%s", out)
			}
			// The useful half of the original sentence must survive: the agent
			// still must not go hunting through issues and comments.
			if !strings.Contains(out, "Never look in Cordy issues or comments") {
...
//   - history: `cordy chat history` is injected for Slack (live channel) and
//     for every surface that persists a transcript (Feishu, WeCom, DingTalk);
//     `cordy chat thread` is Slack-only. handler/chat_history.go reads the
...
func TestBuildChatPromptTwoLayerChannelPolicy(t *testing.T) {
	...
	// `cordy attachment upload` on purpose — to state that it does not apply
	...
	const uploadGuidance = "run `cordy attachment upload <local-path>`"
	const historyGuidance = "cordy chat history"
	...
	cases := []struct {
		...
		},
		{
			...
			wantPhrases: []string{
				"Feishu/Lark",
				"read it back with `cordy chat history`",
...
func TestBuildChatPromptFeishuIgnoresChatInThread(t *testing.T) {
	...
	if strings.Contains(out, "cordy chat thread") {
		t.Errorf("feishu prompt must not teach `cordy chat thread` (no thread reader)\n--- output ---\n%s", out)
	}
}
...
func TestBuildPromptDefaultScansRootsFirst(t *testing.T) {
	...
	if strings.Contains(out, "cordy issue comment list issue-default-1 --output json") {
...
func TestBuildPromptWarnsAboutActiveSiblingRuns(t *testing.T) {
	...
	if strings.Contains(out, "cordy issue runs") {
...
func TestBuildPromptNewCommentsHint(t *testing.T) {
	...
	if !strings.Contains(out, "cordy issue comment list "+issueID+" --thread thread-root-1 --since "+since+" --compact --output json") {
	...
	if strings.Contains(out, "cordy issue comment list "+issueID+" --since "+since+" --output json") {
...
func TestBuildPromptColdStartThreadRead(t *testing.T) {
	...
	if !strings.Contains(out, "cordy issue comment list "+issueID+" --thread thread-root-1 --tail 30 --compact --output json") {
	...
	if strings.Contains(out, "cordy issue comment list "+issueID+" --roots-only --summary --output json") {
...
func TestBuildCommentPromptCoalescedIDsOnlyFallback(t *testing.T) {
	...
	t.Run("with since anchor", func(t *testing.T) {
		...
		want := "cordy issue comment list issue-fallback-1 --since 2026-08-03T06:00:00Z --compact --output json"
		if !strings.Contains(out, want) {
...
func TestBuildCommentPromptCrossThreadFansOutReplies(t *testing.T) {
	...
	if strings.Contains(out, "cordy issue comment add") {
...
func TestChatChannelDeliversFilesDefaultsOffAcrossVersions(t *testing.T) {
	...
	out := buildChatPrompt(task)
	if strings.Contains(out, "run `cordy attachment upload <local-path>`") {
	...
	if !strings.Contains(buildChatPrompt(delivering), "run `cordy attachment upload <local-path>`") {
