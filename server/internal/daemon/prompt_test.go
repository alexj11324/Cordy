...
func TestBuildQuickCreatePromptRules(t *testing.T) {
	...
	mustContain := []string{
		...
		// auto-subscribe path fires (orvilo issue create has no --subscriber flag)
		"CC exception",
		...
		// fixed prefix.
		"orvilo issue create --output json",
		"JSON response",
		"identifier",
...
func TestBuildQuickCreatePromptTeamDefaultsToTeam(t *testing.T) {
	...
	if !strings.Contains(out, teamName) {
		t.Errorf("buildQuickCreatePrompt with TeamID should mention the team name %q, got:\n%s", teamName, out)
	}
	// And the prompt must explicitly call out the team-vs-leader rule
	// so the agent does not silently regress to "default to YOURSELF".
	mustContain := []string{
		"picker TEAM",
		"running on the team's behalf",
		"do not assign it to your own agent UUID",
	}
	for _, s := range mustContain {
		if !strings.Contains(out, s) {
			t.Errorf("buildQuickCreatePrompt with TeamID missing %q\n--- output ---\n%s", s, out)
		}
	}
}
...
// TestBuildPromptTeamLeaderNoActionFailureFallback locks the escape hatch added
// in MUL-6622 / GH #7487. The comment prohibition is conditional on the
// `team activity` call succeeding — the server only rejects a leader comment
...
func TestBuildPromptTeamLeaderNoActionFailureFallback(t *testing.T) {
	...
	for _, want := range []string{
		...
		if !strings.Contains(out, want) {
			t.Errorf("team leader no_action rule must contain %q, got:\n%s", want, out)
		}
	}
}

// TestBuildPromptTeamLeaderNoActionForMemberTrigger verifies that the
// team leader no_action prohibition is injected in the per-turn prompt
...
func TestBuildPromptTeamLeaderNoActionForMemberTrigger(t *testing.T) {
	task := Task{
		IssueID:               "issue-123",
		TriggerCommentID:      "comment-456",
		TriggerCommentContent: "LGTM",
	...
	if !strings.Contains(out, "Team leader no_action rule") {
		t.Errorf("buildCommentPrompt must inject team leader no_action rule for member-triggered comments, got:\n%s", out)
	}
	if !strings.Contains(out, "DO NOT post any comment") {
		t.Errorf("buildCommentPrompt must contain DO NOT post prohibition for member-triggered team leader, got:\n%s", out)
	}
}

// TestBuildPromptTeamLeaderNoActionForAgentTrigger verifies the rule also
// fires for agent-triggered comments (the original path that already worked).
func TestBuildPromptTeamLeaderNoActionForAgentTrigger(t *testing.T) {
	task := Task{
		IssueID:               "issue-123",
		TriggerCommentID:      "comment-456",
	...
	out := BuildPrompt(task, "claude")
	if !strings.Contains(out, "Team leader no_action rule") {
		t.Errorf("buildCommentPrompt must inject team leader no_action rule for agent-triggered comments, got:\n%s", out)
	}
}

// TestTaskIsTeamLeaderReadsProtocolFields pins the role signal to the wire
// fields a current server sets when (and only when) it injects a team-leader
...
// promoted to team leader. The "legacy" rows are the inverse — reading the
...
func TestTaskIsTeamLeaderReadsProtocolFields(t *testing.T) {
	t.Parallel()

	// What an agent's Instructions look like once the server has appended the
	// briefing — and what an ordinary agent that merely writes about teams
	...
	cases := []struct {
		...
		// is_leader_task / team_id are authoritative ---
		{
		...
		{
			name: "current: quick-create routed through a team picker",
			task: Task{LeaderRoleResolved: true, TeamID: "5f7f7c12-b579-4c6d-aaa0-8ae1d7e72b61", Agent: &AgentData{Instructions: briefed}},
			want: true,
		},
		{
		...
		{
			// Briefing withheld by the claim's defensive gate (team deleted /
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			t.Parallel()
			if got := taskIsTeamLeader(tc.task); got != tc.want {
				t.Fatalf("taskIsTeamLeader(%s) = %v, want %v", tc.name, got, tc.want)
			}
		})
	}
}
...
// happens to document a "## Team Operating Protocol" section in its own
// instructions must get the ordinary comment prompt — no team activity
...
func TestBuildPromptProtocolHeadingInInstructionsIsNotALeader(t *testing.T) {
	...
	for _, banned := range []string{
		...
		if strings.Contains(out, banned) {
			t.Fatalf("ordinary agent prompt leaked team-leader rule %q\n---\n%s", banned, out)
		}
	}
...
// upgraded daemon that trusted the fields alone would demote a real team
...
func TestBuildChatPromptChannelAwareness(t *testing.T) {
	t.Run("slack-backed prompt teaches both read commands", func(t *testing.T) {
		...
		for _, want := range []string{"Slack", "NOT in Orvilo", "orvilo chat history", "orvilo chat thread", "Do NOT narrate"} {
			if !strings.Contains(out, want) {
	...
	t.Run("top-level mention starts with history", func(t *testing.T) {
		...
		if !strings.Contains(out, "top level: start with `orvilo chat history`") {
	...
	t.Run("in-thread mention starts with thread", func(t *testing.T) {
		...
		if !strings.Contains(out, "inside a thread: start with `orvilo chat thread`") {
	...
	t.Run("web-only session has no channel block", func(t *testing.T) {
		...
		if strings.Contains(out, "orvilo chat history") {
...
//   - history: `orvilo chat history` is injected for Slack (live channel) and
//     for every surface that persists a transcript (Feishu, WeCom, DingTalk);
//     `orvilo chat thread` is Slack-only. handler/chat_history.go reads the
...
func TestBuildChatPromptTwoLayerChannelPolicy(t *testing.T) {
	...
	// `orvilo attachment upload` on purpose — to state that it does not apply
	...
	const uploadGuidance = "run `orvilo attachment upload <local-path>`"
	const historyGuidance = "orvilo chat history"
	...
	cases := []struct {
		...
		},
		{
			...
			wantPhrases: []string{
				"Feishu/Lark",
				"read it back with `orvilo chat history`",
...
func TestBuildChatPromptFeishuIgnoresChatInThread(t *testing.T) {
	...
	if strings.Contains(out, "orvilo chat thread") {
		t.Errorf("feishu prompt must not teach `orvilo chat thread` (no thread reader)\n--- output ---\n%s", out)
	}
}
...
func TestBuildPromptDefaultScansRootsFirst(t *testing.T) {
	...
	for _, s := range []string{
		"orvilo issue comment list issue-default-1 --roots-only --summary --compact --output json",
		"--since",
	} {
		if !strings.Contains(out, s) {
	...
	if strings.Contains(out, "orvilo issue comment list issue-default-1 --output json") {
...
func TestBuildPromptWarnsAboutActiveSiblingRuns(t *testing.T) {
	...
	for _, want := range []string{
		...
		"MUL-6000",
		"task-existing",
		"orvilo issue comment list issue-target --roots-only --summary --compact --output json",
		"orvilo issue run-messages task-existing",
		"--no-start",
	} {
		if !strings.Contains(out, want) {
	...
	if strings.Contains(out, "orvilo issue runs") {
...
// TestBuildPromptNonTeamLeaderNoRule verifies that non-team-leader agents
// do NOT get the team leader no_action rule injected.
func TestBuildPromptNonTeamLeaderNoRule(t *testing.T) {
	task := Task{
		IssueID:               "issue-123",
		TriggerCommentID:      "comment-456",
		TriggerCommentContent: "LGTM",
		...
		TriggerAuthorName:     "Bohan",
		Agent: &AgentData{
			Instructions: "Some instructions without the team marker",
		},
	}
	out := BuildPrompt(task, "claude")
	if strings.Contains(out, "Team leader no_action rule") {
		t.Errorf("buildCommentPrompt must NOT inject team leader no_action rule for non-team-leader agents, got:\n%s", out)
	}
}
...
func TestBuildPromptNewCommentsHint(t *testing.T) {
	...
	if !strings.Contains(out, "orvilo issue comment list "+issueID+" --thread thread-root-1 --since "+since+" --compact --output json") {
	...
	if strings.Contains(out, "orvilo issue comment list "+issueID+" --since "+since+" --output json") {
...
func TestBuildPromptColdStartThreadRead(t *testing.T) {
	...
	if !strings.Contains(out, "orvilo issue comment list "+issueID+" --thread thread-root-1 --tail 30 --compact --output json") {
	...
	if strings.Contains(out, "orvilo issue comment list "+issueID+" --roots-only --summary --output json") {
...
func TestBuildCommentPromptCoalescedIDsOnlyFallback(t *testing.T) {
	...
	t.Run("with since anchor", func(t *testing.T) {
		...
		want := "orvilo issue comment list issue-fallback-1 --since 2026-08-03T06:00:00Z --compact --output json"
		if !strings.Contains(out, want) {
...
func assertBoundedIDOnlyFallback(t *testing.T, out string) {
	...
	for _, want := range []string{
		"orvilo issue comment list issue-fallback-1 --thread <comment-id> --tail 30 --compact --output json",
		"accepts a reply id",
		"Next reply cursor",
		"--before-id",
...
func TestBuildCommentPromptCrossThreadFansOutReplies(t *testing.T) {
	...
	if strings.Contains(out, "orvilo issue comment add") {
...
func TestChatChannelDeliversFilesDefaultsOffAcrossVersions(t *testing.T) {
	...
	out := buildChatPrompt(task)
	if strings.Contains(out, "run `orvilo attachment upload <local-path>`") {
	...
	if !strings.Contains(buildChatPrompt(delivering), "run `orvilo attachment upload <local-path>`") {
