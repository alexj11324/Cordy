...
// subagents. The Patchbay daemon currently models only the parent Codex
...
// lifecycle state with drain/cancel primitives, or the Patchbay runtime
...
// PatchbayCodexMultiAgentEnv is the env var users can set to keep Codex
...
func codexMultiAgentEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(PatchbayCodexMultiAgentEnv))
...
// renderPatchbayMultiAgentBlock returns the daemon-managed multi-agent
// block. The body uses `multi_agent = false` when injected inside a
// `[features]` table, and `features.multi_agent = false` otherwise.
func renderPatchbayMultiAgentBlock(inFeaturesTable bool) string {
...
func injectManagedBlockIntoFeaturesTable(content string) string {
	block := renderPatchbayMultiAgentBlock(true)
...
func ensureCodexMultiAgentConfig(configPath string, logger *slog.Logger) error {
	...
	if hasRootFeaturesTable(existing) {
		...
		block := renderPatchbayMultiAgentBlock(false)
		if existing == "" {
			updated = block
		} else {
			updated = block + "\n" + existing
