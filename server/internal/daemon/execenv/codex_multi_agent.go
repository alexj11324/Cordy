...
const (
	patchbayMultiAgentBeginMarker = "# BEGIN patchbay-managed multi-agent (do not edit; regenerated by daemon)"
	patchbayMultiAgentEndMarker   = "# END patchbay-managed multi-agent"
)
...
var multiAgentBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(patchbayMultiAgentBeginMarker) +
		`.*?^` + regexp.QuoteMeta(patchbayMultiAgentEndMarker) + `\n*`)
...
func renderOrviloMultiAgentBlock(inFeaturesTable bool) string {
	...
	b.WriteString(patchbayMultiAgentBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(patchbayMultiAgentEndMarker)
	b.WriteString("\n")
	return b.String()
}
