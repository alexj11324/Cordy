...
// subagents. The Cordy daemon currently models only the parent Codex
...
// lifecycle state with drain/cancel primitives, or the Cordy runtime
...
//
// Users who explicitly want Codex native subagents inside a Cordy task
// (and accept the lifecycle risk) can keep the feature enabled by setting
// `CORDY_CODEX_MULTI_AGENT=1` in the daemon environment.
//
// Layout note
//
...
// CordyCodexMultiAgentEnv is the env var users can set to keep Codex
...
const CordyCodexMultiAgentEnv = "CORDY_CODEX_MULTI_AGENT"

// cordyMultiAgentBeginMarker / cordyMultiAgentEndMarker delimit the
// multi-agent-specific managed block. Kept separate from the sandbox
// block so each can evolve and migrate independently.
const (
	cordyMultiAgentBeginMarker = "# BEGIN cordy-managed multi-agent (do not edit; regenerated by daemon)"
	cordyMultiAgentEndMarker   = "# END cordy-managed multi-agent"
)
...
var multiAgentBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(cordyMultiAgentBeginMarker) +
		`.*?^` + regexp.QuoteMeta(cordyMultiAgentEndMarker) + `\n*`)
...
func codexMultiAgentEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(CordyCodexMultiAgentEnv))
...
// renderCordyMultiAgentBlock returns the daemon-managed multi-agent
// block. The body uses `multi_agent = false` when injected inside a
// `[features]` table, and `features.multi_agent = false` otherwise.
func renderCordyMultiAgentBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(cordyMultiAgentBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(cordyMultiAgentEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectManagedBlockIntoFeaturesTable(content string) string {
	block := renderCordyMultiAgentBlock(true)
...
//
// When CORDY_CODEX_MULTI_AGENT is set to a truthy value, the function is
...
func ensureCodexMultiAgentConfig(configPath string, logger *slog.Logger) error {
	if codexMultiAgentEnabled() {
		if logger != nil {
			logger.Info("codex multi-agent: leaving Codex native multi-agent untouched per CORDY_CODEX_MULTI_AGENT",
	...
	if hasRootFeaturesTable(existing) {
		...
		block := renderCordyMultiAgentBlock(false)
		if existing == "" {
			updated = block
		} else {
			updated = block + "\n" + existing
