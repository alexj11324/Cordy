...
// subagents. The Orvilo daemon currently models only the parent Codex
...
// lifecycle state with drain/cancel primitives, or the Orvilo runtime
...
//
// Users who explicitly want Codex native subagents inside an Orvilo task
...
// OrviloCodexMultiAgentEnv is the env var users can set to keep Codex
...
const OrviloCodexMultiAgentEnv = "ORVILO_CODEX_MULTI_AGENT"

// orviloMultiAgentBeginMarker / orviloMultiAgentEndMarker delimit the
// multi-agent-specific managed block. Kept separate from the sandbox
// block so each can evolve and migrate independently.
const (
	orviloMultiAgentBeginMarker = "# BEGIN orvilo-managed multi-agent (do not edit; regenerated by daemon)"
	orviloMultiAgentEndMarker   = "# END orvilo-managed multi-agent"
)
...
var multiAgentBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(orviloMultiAgentBeginMarker) +
		`.*?^` + regexp.QuoteMeta(orviloMultiAgentEndMarker) + `\n*`)
...
func codexMultiAgentEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(OrviloCodexMultiAgentEnv))
...
// renderOrviloMultiAgentBlock returns the daemon-managed multi-agent
// block. The body uses `multi_agent = false` when injected inside a
// `[features]` table, and `features.multi_agent = false` otherwise.
func renderOrviloMultiAgentBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(orviloMultiAgentBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(orviloMultiAgentEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectManagedBlockIntoFeaturesTable(content string) string {
	block := renderOrviloMultiAgentBlock(true)
...
func ensureCodexMultiAgentConfig(configPath string, logger *slog.Logger) error {
	...
	if hasRootFeaturesTable(existing) {
		...
		block := renderOrviloMultiAgentBlock(false)
		if existing == "" {
			updated = block
		} else {
			updated = block + "\n" + existing
