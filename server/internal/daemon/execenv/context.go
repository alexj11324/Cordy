...
import (
	...
	skillpkg "github.com/cordy-ai/cordy/server/internal/skill"
...
// sandbox strips all CORDY_* env vars before invoking `cordy`.
const TaskContextMarkerRelPath = ".cordy/daemon_task_context.json"
...
const TaskContextMarkerManagedBy = "cordy-daemon-task"
...
// {workspacesRoot}/.cordy/daemon_task_context.json.
//
...
// cwd. A sandboxed subprocess that lost every CORDY_* env var and escaped
...
func writeTaskContextMarker(workDir string, ctx TaskContextForEnv, manifest *sidecarManifest) error {
	...
	if err := recordMkdirAll(dir, 0o755, manifest); err != nil {
		return fmt.Errorf("create .cordy dir: %w", err)
	}
...
// writeProjectResources writes .cordy/project/resources.json into the
...
//
// manifest, when non-nil, is populated with the .cordy/project chain
...
func writeProjectResources(workDir string, ctx TaskContextForEnv, manifest *sidecarManifest) error {
	...
	dir := filepath.Join(workDir, ".cordy", "project")
	if err := recordMkdirAll(dir, 0o755, manifest); err != nil {
		return err
	}
	...
	if err := recordWriteFile(filepath.Join(dir, "resources.json"), data, 0o644, manifest); err != nil {
		// .cordy/project/resources.json is Cordy-owned and a
...
//
// When a Cordy skill's natural slug collides with a user-installed
// skill at the same path, we allocate a collision-free sibling slug
// (e.g. `issue-review-cordy`) and write there instead. Provider-native
...
// skill entirely (which would silently drop a Cordy skill the agent
// expects to see).
...
func renderIssueContext(provider string, ctx TaskContextForEnv) string {
	...
	fmt.Fprintf(&b, "Run `cordy issue get %s --output json` to fetch the full issue details.\n\n", ctx.IssueID)
...
func renderAutopilotContext(ctx TaskContextForEnv) string {
	...
	b.WriteString("## Quick Start\n\n")
	b.WriteString("This is a run-only autopilot task with no assigned issue. Do not run `cordy issue get` unless the autopilot instructions explicitly ask you to create or update an issue.\n\n")
	if ctx.AutopilotID != "" {
