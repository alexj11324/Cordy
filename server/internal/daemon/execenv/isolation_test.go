...
// exercises the same stdin/stdout protocol as the real cordy helper.
			...
			ProjectResources: []ProjectResourceForEnv{
				{
					...
					ResourceRef:  json.RawMessage(`{"url":"https://github.com/cordy-ai/cordy"}`),
				},
			},
		},
	}, logger)
...
func TestPreparationHelperRoundTripsProjectResources(t *testing.T) {
	...
	data, err := os.ReadFile(filepath.Join(env.WorkDir, ".cordy", "project", "resources.json"))
	if err != nil {
	...
	if resource.ID != "resource-helper-project-resource" ||
		...
		ref.URL != "https://github.com/cordy-ai/cordy" ||
		resource.Label != "Cordy" {
