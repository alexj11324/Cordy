...
// runtimeMarkerBegin and runtimeMarkerEnd delimit the Cordy-managed brief
...
const (
	runtimeMarkerBegin = "<!-- BEGIN CORDY-RUNTIME (auto-managed; do not edit) -->"
	runtimeMarkerEnd   = "<!-- END CORDY-RUNTIME -->"
...
// writeRuntimeConfigFile writes the Cordy runtime brief to path without
...
// locateMarkerBlock finds the [start, end) byte range of the Cordy marker
...
//
//   - User content carries a stray `<!-- END CORDY-RUNTIME -->` (e.g. a
...
// CleanupRuntimeConfig excises the Cordy marker block from the runtime
...
// of Cordy runs without ever touching a single non-managed byte.
//
...
// the user inside the same directory after a Cordy task would pick up
...
// agent about the Cordy runtime environment and available CLI tools.
//
