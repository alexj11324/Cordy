...
// runtimeMarkerBegin and runtimeMarkerEnd delimit the Orvilo-managed brief
...
// writeRuntimeConfigFile writes the Orvilo runtime brief to path without
...
// locateMarkerBlock finds the [start, end) byte range of the Orvilo marker
...
// CleanupRuntimeConfig excises the Orvilo marker block from the runtime
...
// of Orvilo runs without ever touching a single non-managed byte.
//
...
// the user inside the same directory after an Orvilo task would pick up
...
// agent about the Orvilo runtime environment and available CLI tools.
//
