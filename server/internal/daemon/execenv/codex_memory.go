...
// `ORVILO_CODEX_MEMORY=1` in the daemon environment. The long-term
...
const PatchbayCodexMemoryEnv = "ORVILO_CODEX_MEMORY"
...
//
// When ORVILO_CODEX_MEMORY is set to a truthy value, the function is a
...
func ensureCodexMemoryConfig(configPath string, logger *slog.Logger) error {
	if codexMemoryEnabled() {
		if logger != nil {
			logger.Info("codex memory: leaving Codex native memory untouched per ORVILO_CODEX_MEMORY",
