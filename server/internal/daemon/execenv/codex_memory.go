...
// — users cannot audit or edit the contents from any Patchbay UI.
//
// This conflicts with the Patchbay daemon's context model. Patchbay
...
//     unrelated host-project context into Patchbay tasks. The reproduction
//     in github.com/patchbay-ai/patchbay#3130 saw Raw Memories from
//     `D:\Project\MoHaYu\WowChat` (a host-local project) injected into a
//     brand-new Patchbay issue's first Codex turn.
//
...
//
// Users who explicitly want Codex native memory inside a Patchbay task
// (and accept the leak risk) can keep the feature enabled by setting
// `PATCHBAY_CODEX_MEMORY=1` in the daemon environment. The long-term
// answer for durable agent context is a Patchbay-owned, user-visible,
...
// PatchbayCodexMemoryEnv is the env var users can set to keep Codex
...
const PatchbayCodexMemoryEnv = "PATCHBAY_CODEX_MEMORY"

const (
	patchbayMemoryFeatureBeginMarker = "# BEGIN patchbay-managed memory-feature (do not edit; regenerated by daemon)"
	patchbayMemoryFeatureEndMarker   = "# END patchbay-managed memory-feature"
	patchbayMemoryConfigBeginMarker  = "# BEGIN patchbay-managed memory-config (do not edit; regenerated by daemon)"
	patchbayMemoryConfigEndMarker    = "# END patchbay-managed memory-config"
)
...
var memoryFeatureBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(patchbayMemoryFeatureBeginMarker) +
		`.*?^` + regexp.QuoteMeta(patchbayMemoryFeatureEndMarker) + `\n*`)

var memoryConfigBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(patchbayMemoryConfigBeginMarker) +
		`.*?^` + regexp.QuoteMeta(patchbayMemoryConfigEndMarker) + `\n*`)
...
func codexMemoryEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(PatchbayCodexMemoryEnv))
...
// renderPatchbayMemoryFeatureBlock returns the daemon-managed memory-feature
// block. The body uses `memories = false` when injected inside a
// `[features]` table, and `features.memories = false` otherwise.
func renderPatchbayMemoryFeatureBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(patchbayMemoryFeatureBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(patchbayMemoryFeatureEndMarker)
	b.WriteString("\n")
	return b.String()
}

// renderPatchbayMemoryConfigBlock returns the daemon-managed memory-config
// block. The body uses bare keys when injected inside a `[memories]` table
// and dotted-key form otherwise.
func renderPatchbayMemoryConfigBlock(inMemoriesTable bool) string {
	var b strings.Builder
	b.WriteString(patchbayMemoryConfigBeginMarker)
	b.WriteString("\n")
	if inMemoriesTable {
	...
	b.WriteString(patchbayMemoryConfigEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectMemoryFeatureBlockIntoFeaturesTable(content string) string {
	return injectAfterHeader(content, rootFeaturesTableHeaderRe, renderPatchbayMemoryFeatureBlock(true))
}
...
func injectMemoryConfigBlockIntoMemoriesTable(content string) string {
	return injectAfterHeader(content, rootMemoriesTableHeaderRe, renderPatchbayMemoryConfigBlock(true))
}
...
//
// When PATCHBAY_CODEX_MEMORY is set to a truthy value, the function is a
...
func ensureCodexMemoryConfig(configPath string, logger *slog.Logger) error {
	if codexMemoryEnabled() {
		if logger != nil {
			logger.Info("codex memory: leaving Codex native memory untouched per PATCHBAY_CODEX_MEMORY",
	...
	if !hasFeatures {
		prepend.WriteString(renderPatchbayMemoryFeatureBlock(false))
	}
	if !hasMemories {
		prepend.WriteString(renderPatchbayMemoryConfigBlock(false))
	}
