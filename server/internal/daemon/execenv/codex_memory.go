...
// — users cannot audit or edit the contents from any Cordy UI.
//
// This conflicts with the Cordy daemon's context model. Cordy
...
//     unrelated host-project context into Cordy tasks. The reproduction
//     in github.com/cordy-ai/cordy#3130 saw Raw Memories from
//     `D:\Project\MoHaYu\WowChat` (a host-local project) injected into a
//     brand-new Cordy issue's first Codex turn.
//
...
//
// Users who explicitly want Codex native memory inside a Cordy task
// (and accept the leak risk) can keep the feature enabled by setting
// `CORDY_CODEX_MEMORY=1` in the daemon environment. The long-term
// answer for durable agent context is a Cordy-owned, user-visible,
...
// CordyCodexMemoryEnv is the env var users can set to keep Codex
...
const CordyCodexMemoryEnv = "CORDY_CODEX_MEMORY"

const (
	cordyMemoryFeatureBeginMarker = "# BEGIN cordy-managed memory-feature (do not edit; regenerated by daemon)"
	cordyMemoryFeatureEndMarker   = "# END cordy-managed memory-feature"
	cordyMemoryConfigBeginMarker  = "# BEGIN cordy-managed memory-config (do not edit; regenerated by daemon)"
	cordyMemoryConfigEndMarker    = "# END cordy-managed memory-config"
)
...
var memoryFeatureBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(cordyMemoryFeatureBeginMarker) +
		`.*?^` + regexp.QuoteMeta(cordyMemoryFeatureEndMarker) + `\n*`)

var memoryConfigBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(cordyMemoryConfigBeginMarker) +
		`.*?^` + regexp.QuoteMeta(cordyMemoryConfigEndMarker) + `\n*`)
...
func codexMemoryEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(CordyCodexMemoryEnv))
...
// renderCordyMemoryFeatureBlock returns the daemon-managed memory-feature
// block. The body uses `memories = false` when injected inside a
// `[features]` table, and `features.memories = false` otherwise.
func renderCordyMemoryFeatureBlock(inFeaturesTable bool) string {
	var b strings.Builder
	b.WriteString(cordyMemoryFeatureBeginMarker)
	b.WriteString("\n")
	if inFeaturesTable {
	...
	b.WriteString(cordyMemoryFeatureEndMarker)
	b.WriteString("\n")
	return b.String()
}

// renderCordyMemoryConfigBlock returns the daemon-managed memory-config
// block. The body uses bare keys when injected inside a `[memories]` table
// and dotted-key form otherwise.
func renderCordyMemoryConfigBlock(inMemoriesTable bool) string {
	var b strings.Builder
	b.WriteString(cordyMemoryConfigBeginMarker)
	b.WriteString("\n")
	if inMemoriesTable {
	...
	b.WriteString(cordyMemoryConfigEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
func injectMemoryFeatureBlockIntoFeaturesTable(content string) string {
	return injectAfterHeader(content, rootFeaturesTableHeaderRe, renderCordyMemoryFeatureBlock(true))
}
...
func injectMemoryConfigBlockIntoMemoriesTable(content string) string {
	return injectAfterHeader(content, rootMemoriesTableHeaderRe, renderCordyMemoryConfigBlock(true))
}
...
//
// When CORDY_CODEX_MEMORY is set to a truthy value, the function is a
...
func ensureCodexMemoryConfig(configPath string, logger *slog.Logger) error {
	if codexMemoryEnabled() {
		if logger != nil {
			logger.Info("codex memory: leaving Codex native memory untouched per CORDY_CODEX_MEMORY",
	...
	if !hasFeatures {
		prepend.WriteString(renderCordyMemoryFeatureBlock(false))
	}
	if !hasMemories {
		prepend.WriteString(renderCordyMemoryConfigBlock(false))
	}
