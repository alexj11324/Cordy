...
// — users cannot audit or edit the contents from any Orvilo UI.
//
// This conflicts with the Orvilo daemon's context model. Orvilo
...
//     unrelated host-project context into Orvilo tasks. The reproduction
//     in github.com/orvilo-ai/orvilo#3130 saw Raw Memories from
//     `D:\Project\MoHaYu\WowChat` (a host-local project) injected into a
//     brand-new Orvilo issue's first Codex turn.
//
...
//
// Users who explicitly want Codex native memory inside an Orvilo task
...
// answer for durable agent context is an Orvilo-owned, user-visible,
...
// OrviloCodexMemoryEnv is the env var users can set to keep Codex
...
const OrviloCodexMemoryEnv = "ORVILO_CODEX_MEMORY"
...
func codexMemoryEnabled() bool {
	raw := strings.TrimSpace(os.Getenv(OrviloCodexMemoryEnv))
...
// renderOrviloMemoryFeatureBlock returns the daemon-managed memory-feature
// block. The body uses `memories = false` when injected inside a
// `[features]` table, and `features.memories = false` otherwise.
func renderOrviloMemoryFeatureBlock(inFeaturesTable bool) string {
...
// renderOrviloMemoryConfigBlock returns the daemon-managed memory-config
// block. The body uses bare keys when injected inside a `[memories]` table
// and dotted-key form otherwise.
func renderOrviloMemoryConfigBlock(inMemoriesTable bool) string {
	var b strings.Builder
...
func injectMemoryFeatureBlockIntoFeaturesTable(content string) string {
	return injectAfterHeader(content, rootFeaturesTableHeaderRe, renderOrviloMemoryFeatureBlock(true))
}
...
func injectMemoryConfigBlockIntoMemoriesTable(content string) string {
	return injectAfterHeader(content, rootMemoriesTableHeaderRe, renderOrviloMemoryConfigBlock(true))
}
...
func ensureCodexMemoryConfig(configPath string, logger *slog.Logger) error {
	...
	if !hasFeatures {
		prepend.WriteString(renderOrviloMemoryFeatureBlock(false))
	}
	if !hasMemories {
		prepend.WriteString(renderOrviloMemoryConfigBlock(false))
	}
