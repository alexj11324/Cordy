...
// `no such host` errors when calling out (for example, `cordy issue get`
// hitting the Cordy API). See upstream issue openai/codex#10390.
//
...
//     danger-full-access on the daemon user's real HOME. Landlock could enforce
//     workspace-write here, but enforcing it meant redirecting HOME/XDG per task
//     and seeding a hand-maintained allowlist of credential paths back into it,
//     which left every un-listed host CLI (aws, kube, gcloud, glab, rclone, …)
//     unconfigured inside tasks and could not converge. The containment it
//     bought was also narrower than it looked: workspace-write restricts writes
//     only — reads and network were already unrestricted, so credentials under
...
//     rejects non-safe mutation commands "by policy" — e.g. `cordy issue
...
//     danger-full-access so the Cordy CLI can reach the API.
...
// cordyManagedBeginMarker / cordyManagedEndMarker delimit the block the
...
const (
	cordyManagedBeginMarker = "# BEGIN cordy-managed (do not edit; regenerated by daemon)"
	cordyManagedEndMarker   = "# END cordy-managed"
)

// renderCordyManagedBlock produces the managed block for the given policy.
//
...
//     other table (e.g. `[permissions.cordy]`), a bare `sandbox_mode = ...`
...
// at the top of the file (see upsertCordyManagedBlock), avoids both traps.
func renderCordyManagedBlock(policy codexSandboxPolicy) string {
	var b strings.Builder
	b.WriteString(cordyManagedBeginMarker)
	b.WriteString("\n")
	b.WriteString(fmt.Sprintf("sandbox_mode = %q\n", policy.Mode))
	if policy.Mode == "workspace-write" {
		b.WriteString(fmt.Sprintf("sandbox_workspace_write.network_access = %t\n", policy.NetworkAccess))
	}
	b.WriteString(cordyManagedEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
var managedBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(cordyManagedBeginMarker) +
		`.*?^` + regexp.QuoteMeta(cordyManagedEndMarker) + `\n*`)

// upsertCordyManagedBlock returns the config content with the cordy-managed
...
// `[permissions.cordy]` or `[profiles.foo]`. Combined with the dotted-key
// form used by renderCordyManagedBlock, this means the managed block neither
// leaks into nor inherits from any surrounding table scope.
func upsertCordyManagedBlock(content string, policy codexSandboxPolicy) string {
	// Drop any previously written managed block (wherever it sits).
	content = managedBlockRe.ReplaceAllString(content, "")
	block := renderCordyManagedBlock(policy)
...
// ensureCodexSandboxConfig writes the cordy-managed sandbox block into the
...
func ensureCodexSandboxConfig(configPath string, policy codexSandboxPolicy, detectedVersion string, logger *slog.Logger) error {
	...
	updated := upsertCordyManagedBlock(existing, policy)
