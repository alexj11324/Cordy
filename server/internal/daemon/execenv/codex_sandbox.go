...
// `no such host` errors when calling out (for example, `orvilo issue get`
...
//     rejects non-safe mutation commands "by policy" — e.g. `orvilo issue
...
// orviloManagedBeginMarker / orviloManagedEndMarker delimit the block the
...
const (
	orviloManagedBeginMarker = "# BEGIN orvilo-managed (do not edit; regenerated by daemon)"
	orviloManagedEndMarker   = "# END orvilo-managed"
)
...
//     other table (e.g. `[permissions.orvilo]`), a bare `sandbox_mode = ...`
...
var managedBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(orviloManagedBeginMarker) +
		`.*?^` + regexp.QuoteMeta(orviloManagedEndMarker) + `\n*`)

// upsertOrviloManagedBlock returns the config content with the orvilo-managed
...
// `[permissions.orvilo]` or `[profiles.foo]`. Combined with the dotted-key
...
// ensureCodexSandboxConfig writes the orvilo-managed sandbox block into the
