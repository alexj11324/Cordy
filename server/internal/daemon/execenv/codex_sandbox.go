...
//     rejects non-safe mutation commands "by policy" — e.g. `patchbay issue
...
// patchbayManagedBeginMarker / patchbayManagedEndMarker delimit the block the
...
const (
	patchbayManagedBeginMarker = "# BEGIN patchbay-managed (do not edit; regenerated by daemon)"
...
//     other table (e.g. `[permissions.patchbay]`), a bare `sandbox_mode = ...`
...
func renderPatchbayManagedBlock(policy codexSandboxPolicy) string {
	...
	b.WriteString(patchbayManagedEndMarker)
	b.WriteString("\n")
	return b.String()
}
...
// ensureCodexSandboxConfig writes the patchbay-managed sandbox block into the
