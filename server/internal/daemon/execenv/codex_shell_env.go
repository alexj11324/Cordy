...
const (
	...
	cordyShellEnvBeginMarker = "# BEGIN cordy-managed shell-environment (do not edit; regenerated by daemon)"
	cordyShellEnvEndMarker   = "# END cordy-managed shell-environment"
)

var shellEnvBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(cordyShellEnvBeginMarker) +
		`.*?^` + regexp.QuoteMeta(cordyShellEnvEndMarker) + `\n*`)
...
func renderCordyShellEnvBlock(includeOnly []string) (string, error) {
	...
	b.WriteString(cordyShellEnvBeginMarker)
	b.WriteByte('\n')
	b.WriteString("[")
	...
	b.WriteByte('\n')
	b.WriteString(cordyShellEnvEndMarker)
	b.WriteByte('\n')
	return b.String(), nil
}
...
func EnsureCodexShellEnvPolicyConfig(configPath string, includeOnly []string, logger *slog.Logger) error {
	...
	block, err := renderCordyShellEnvBlock(includeOnly)
	if err != nil {
		return err
	}
