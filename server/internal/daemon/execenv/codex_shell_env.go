...
// daemon-spawned Codex process can have ORVILO_TOKEN while `orvilo issue ...`
...
const (
	...
	orviloShellEnvBeginMarker = "# BEGIN orvilo-managed shell-environment (do not edit; regenerated by daemon)"
	orviloShellEnvEndMarker   = "# END orvilo-managed shell-environment"
)

var shellEnvBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(orviloShellEnvBeginMarker) +
		`.*?^` + regexp.QuoteMeta(orviloShellEnvEndMarker) + `\n*`)
...
func renderOrviloShellEnvBlock(includeOnly []string) (string, error) {
	...
	b.WriteString(orviloShellEnvBeginMarker)
	b.WriteByte('\n')
	b.WriteString("[")
	...
	b.WriteByte('\n')
	b.WriteString(orviloShellEnvEndMarker)
	b.WriteByte('\n')
	return b.String(), nil
}
...
func EnsureCodexShellEnvPolicyConfig(configPath string, includeOnly []string, logger *slog.Logger) error {
	...
	block, err := renderOrviloShellEnvBlock(includeOnly)
	if err != nil {
		return err
	}
