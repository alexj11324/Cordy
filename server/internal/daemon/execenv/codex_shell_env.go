...
const (
	...
	patchbayShellEnvBeginMarker = "# BEGIN patchbay-managed shell-environment (do not edit; regenerated by daemon)"
	patchbayShellEnvEndMarker   = "# END patchbay-managed shell-environment"
)

var shellEnvBlockRe = regexp.MustCompile(
	`(?ms)^` + regexp.QuoteMeta(patchbayShellEnvBeginMarker) +
		`.*?^` + regexp.QuoteMeta(patchbayShellEnvEndMarker) + `\n*`)
...
func renderPatchbayShellEnvBlock(includeOnly []string) (string, error) {
	...
	b.WriteString(patchbayShellEnvBeginMarker)
	b.WriteByte('\n')
	b.WriteString("[")
	...
	b.WriteByte('\n')
	b.WriteString(patchbayShellEnvEndMarker)
	b.WriteByte('\n')
	return b.String(), nil
}
...
func EnsureCodexShellEnvPolicyConfig(configPath string, includeOnly []string, logger *slog.Logger) error {
	...
	block, err := renderPatchbayShellEnvBlock(includeOnly)
	if err != nil {
		return err
	}
