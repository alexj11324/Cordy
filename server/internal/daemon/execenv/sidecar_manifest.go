...
const sidecarManifestFile = ".patchbay_sidecar_manifest.json"
...
// baseSlug: the bare slug first, then `-patchbay`, then numbered variants.
//
...
func skillSlugCandidate(baseSlug string, attempt int) string {
	...
	case attempt <= 0:
		return baseSlug
	case attempt == 1:
		return baseSlug + "-patchbay"
	default:
		return fmt.Sprintf("%s-patchbay-%d", baseSlug, attempt)
	}
}
...
// (.agent_context/, .patchbay/, .claude/skills/, .github/skills/,
...
// repository, so "left on disk" means a marker that disables every patchbay
...
// issue-review-patchbay. A managed skill directory is platform-owned — the
