...
const sidecarManifestFile = ".orvilo_sidecar_manifest.json"
...
//     .orvilo/project/resources.json) the caller swallows the error
...
// collision we append `-orvilo`, then `-orvilo-2`, `-orvilo-3`,
...
// baseSlug: the bare slug first, then `-orvilo`, then numbered variants.
//
...
func skillSlugCandidate(baseSlug string, attempt int) string {
	...
	case attempt <= 0:
		return baseSlug
	case attempt == 1:
		return baseSlug + "-orvilo"
	default:
		return fmt.Sprintf("%s-orvilo-%d", baseSlug, attempt)
	}
}
...
// (.agent_context/, .orvilo/, .claude/skills/, .github/skills/,
...
// repository, so "left on disk" means a marker that disables every orvilo
...
// issue-review-orvilo. A managed skill directory is platform-owned — the
