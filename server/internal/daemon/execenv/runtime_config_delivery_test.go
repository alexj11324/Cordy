...
func TestBriefSurfaceDeliveryPolicy(t *testing.T) {
	...
	cases := map[string]struct {
		...
		"comment": {
			mustHave: []string{"`--attachment <path>` to `cordy issue comment add`"},
			mustNot:  []string{"cordy attachment upload"},
		},
		"assignment": {
			mustHave: []string{"`--attachment <path>` to `cordy issue comment add`"},
			mustNot:  []string{"cordy attachment upload"},
		...
		"chat_direct": {
			mustHave: []string{"`cordy attachment upload <local-path>`"},
		...
		"quickcreate": {
			mustHave: []string{"your stdout is text-only", "`cordy issue create` call itself via `--attachment <path>`"},
			mustNot:  []string{"cordy attachment upload"},
		},
	}
