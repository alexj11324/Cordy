...
import (
	...
	"github.com/cordy-ai/cordy/server/internal/runtimeapps"
)
...
func writeHeader(b *strings.Builder) {
	b.WriteString("# Cordy Agent Runtime\n\n")
	b.WriteString("You are a coding agent in the Cordy platform. Use the `cordy` CLI to interact with the platform.\n\n")
}
...
// preserved: each `cordy issue …` command name, all three `comment add`
...
// create/update tasks" intro phrase, and `cordy issue comment add
// --help`.
//
...
func writeAvailableCommands(b *strings.Builder, ctx TaskContextForEnv) {
	...
	b.WriteString("Prefer `--output json` for structured data. The default brief lists only the core agent loop and common issue create/update tasks; for everything else run `cordy --help` or `cordy <command> --help`.\n\n")
	...
	b.WriteString("### Core\n")
	b.WriteString("- `cordy issue get <id> --output json` — full issue.\n")
	b.WriteString("- `cordy issue comment list <issue-id> [--roots-only] [--summary] [--thread <comment-id> [--tail N] | --recent N] [--since <RFC3339>] --output json` — thread-aware comment reads. Bound a wide read with `--roots-only --summary` (roots plus `reply_count` / `last_activity_at`, clipped bodies); bound a deep one with `--thread <id> --tail N`; add `--compact` to any JSON read to drop echoed/null/bookkeeping fields. Careful with `--recent N`: it caps THREADS, not comments, and can return the whole history on a small issue. Resolved-thread folding, paging cursors, and full flag semantics: `--help`.\n")
	...
	b.WriteString("- `cordy issue update <id> [--title X] [--description-file <path>] [--priority X] [--status X] [--assignee X] [--parent <issue-id>] [--stage N] [--project <project-id>] [--due-date <YYYY-MM-DD>] [--no-start]` — update fields; pass `--parent \"\"` to clear parent.\n")
	...
	b.WriteString("- `cordy issue assign <id> (--to X | --to-id <uuid> | --unassign) [--no-start]` — change ownership. On assign/update/status, `--no-start` records the change without starting another run — use it when the work is already underway.\n")
	b.WriteString("- `cordy issue status <id> <status> [--no-start]` — flip status (todo / in_progress / in_review / done / blocked / backlog / cancelled).\n")
	b.WriteString("- `cordy issue children <id> [--output json]` — list a parent's sub-issues grouped by stage.\n")
	b.WriteString("- `cordy issue comment add <issue-id> [--content \"...\" | --content-file <path> | --content-stdin] [--parent <comment-id>] [--attachment <path>]` — post a comment. Agent-authored bodies MUST use `--content-file`; see `## Comment Formatting` for why. `cordy issue comment add --help` for full flags.\n")
	b.WriteString("- `cordy issue metadata list <issue-id> [--output json]` — list KV metadata.\n")
	b.WriteString("- `cordy issue metadata set <issue-id> --key <k> --value <v> [--type string|number|bool]` — pin or overwrite a key.\n")
	b.WriteString("- `cordy issue metadata delete <issue-id> --key <k>` — remove a key.\n")
	b.WriteString("- `cordy repo checkout <url> [--ref <branch-or-sha>]` — repository checkout on a dedicated branch.\n\n")
	...
	if ctx.IsSquadLeader {
		...
		b.WriteString("- `cordy squad member set-role <squad-id> --member-id <id> --member-type <agent|member> --role <role> [--output json]` — change role in place (use this instead of remove+add).\n\n")
	}
}
...
// every CLI other than `cordy issue create`, so listing more would just
// tempt the model to bend the guardrail.
func writeAvailableCommandsQuickCreate(b *strings.Builder) {
	b.WriteString("## Available Commands\n\n")
	b.WriteString("**Use `--output json` for structured data.** For anything beyond `issue create`, run `cordy --help` or `cordy <command> --help`.\n\n")
	b.WriteString("`--output json` writes JSON to stdout; confirmations and warnings go to stderr. Do not merge them (`2>&1`) into anything that parses the output — that makes a write that SUCCEEDED look like it failed and invites a duplicate retry.\n\n")
	b.WriteString("### Core\n")
...
func writeRepositories(b *strings.Builder, ctx TaskContextForEnv) {
	...
	b.WriteString("Available in this workspace — `cordy repo checkout <url> [--ref <branch-or-sha>]` to fetch (creates a repository checkout on a dedicated branch).\n\n")
...
func writeProjectContext(b *strings.Builder, ctx TaskContextForEnv) {
	...
	if len(ctx.ProjectResources) > 0 {
		b.WriteString("Project resources (also written to `.cordy/project/resources.json`):\n\n")
		for _, r := range ctx.ProjectResources {
			fmt.Fprintf(b, "- %s\n", formatProjectResource(r))
		}
		b.WriteString("\nResources are pointers — open them only when relevant to the task. ")
		b.WriteString("For `github_repo` resources, use `cordy repo checkout <url>` to fetch the code. Add `--ref <branch-or-sha>` when a task or handoff names an exact revision.\n\n")
	} else {
...
func writeIssueMetadata(b *strings.Builder) {
	...
	b.WriteString("- **Write on exit.** Only what a future run will actually re-read — short values, never secrets or long content. Overwrite or `cordy issue metadata delete` stale keys. Full write discipline: the `cordy-working-on-issues` skill.\n\n")
}
...
//   - Slack: the conversation lives in the channel and `cordy chat history` /
//     `cordy chat thread` can fetch it — see buildChatPrompt, which hands the
...
//     Cordy's chat_message table and `cordy chat history` reads it back —
...
//
// Only a surface whose conversation Cordy never stored (so there is nothing
...
// whose conversation Cordy never stored and cannot read back. Every current
// chat surface (web chat, Feishu, WeCom, DingTalk, Slack) persists a transcript
// that `cordy chat history` can fetch, so no surface routes here today — it
...
func writeWorkflowChat(b *strings.Builder) {
	...
	b.WriteString("- You have full access to the `cordy` CLI to look up issues, workspace info, members, agents, etc.\n")
	b.WriteString("- If asked about issues, use `cordy issue list --output json` or `cordy issue get <id> --output json`\n")
	b.WriteString("- If asked about the workspace, use `cordy workspace get --output json`\n")
	b.WriteString("- If asked to perform actions (create issues, update status, etc.), use the appropriate CLI commands\n")
	b.WriteString("- If the task requires code changes, use `cordy repo checkout <url>` to get the code first. Use `--ref <branch-or-sha>` when you need an exact revision\n")
...
func writeWorkflowQuickCreate(b *strings.Builder) {
	b.WriteString("**This task was triggered by quick-create.** There is NO existing Cordy issue. Follow the field and output rules in the user message you just received; ignore the default assignment-task workflow.\n\n")
	b.WriteString("Hard guardrails (apply even if the user message is missing):\n")
	b.WriteString("- Run exactly one `cordy issue create` invocation, then exit.\n")
	b.WriteString("- Do NOT call `cordy issue get`, `cordy issue status`, or `cordy issue comment add` for this task — there is no issue to query, transition, or comment on. The platform writes the user's success/failure inbox notification automatically based on whether `cordy issue create` succeeded.\n")
...
const AutopilotIssueCommandsGuard = "Do not run `cordy issue get`, `cordy issue comment add`, or `cordy issue status` for this run unless the autopilot instructions explicitly tell you to create or update an issue"
...
func writeWorkflowAutopilot(b *strings.Builder, ctx TaskContextForEnv) {
	b.WriteString("**This task was triggered by an Autopilot in run-only mode.** There is no assigned Cordy issue for this run.\n\n")
	...
	if ctx.AutopilotID != "" {
		fmt.Fprintf(b, "- Run `cordy autopilot get %s --output json` if you need the full autopilot configuration\n", ctx.AutopilotID)
	}
...
func writeWorkflowIssue(b *strings.Builder, ctx TaskContextForEnv) {
	...
	b.WriteString("1. Read the issue (`cordy issue get`) to understand the context — its JSON already carries the issue's `metadata` bag (empty `{}` is normal), so no separate metadata read is needed. What to look for: `## Issue Metadata`.\n")
...
// cordy-working-on-issues built-in skill: the semantics are only needed at
...
func writeSubIssueCreation(b *strings.Builder) {
	...
	b.WriteString("`--status todo` starts an agent-assigned child immediately; `--status backlog` parks it for later promotion; `--stage <N>` groups children into ordered stages. Before creating sub-issues, read the `cordy-working-on-issues` skill — it covers serial chains, promotion, and stage wake semantics.\n\n")
}
...
//
// The index itself stays because it is the one skill listing Cordy controls.
...
func writeAttachments(b *strings.Builder) {
	...
	b.WriteString("Fetch issue/comment attachments via the authenticated CLI (`cordy attachment --help`); never open Cordy resource URLs directly.\n")
...
// writeAlwaysUseCLI emits the "must go through the cordy CLI" guardrail
// (compressed).
func writeAlwaysUseCLI(b *strings.Builder) {
	b.WriteString("## Important: Always Use the `cordy` CLI\n\n")
	b.WriteString("Access Cordy platform resources only through the `cordy` CLI — never `curl` / `wget`. For anything the CLI doesn't cover, post a comment mentioning the workspace owner rather than working around it.\n\n")
}
...
func writeOutput(b *strings.Builder, kind taskKind, ctx TaskContextForEnv) {
	...
	case kindQuickCreate:
		b.WriteString("This is a quick-create task. There is NO existing issue to comment on. Your final stdout is captured automatically and the platform writes the user's success/failure inbox notification based on whether `cordy issue create` succeeded.\n\n")
		b.WriteString("- Do NOT call `cordy issue comment add` — the issue you just created has no conversation context for this run.\n")
		b.WriteString("- Print exactly one final line: `Created <identifier-or-id>: <title>` after a successful `cordy issue create`, using the created issue's `identifier` from JSON output (fall back to its `id`; never assume a workspace issue prefix such as `MUL-`).\n")
		...
		b.WriteString("**Delivering files here:** your stdout is text-only. A file that belongs to the new issue goes on the `cordy issue create` call itself via `--attachment <path>`; never put its path in the description or in your stdout line.\n")
	case kindChat:
		...
		//
		// `attachment upload` binds a file to the Cordy chat reply whatever
		...
		if ctx.ChatChannelType != "" {
			fmt.Fprintf(b, "**Delivering files here:** whether Cordy can push a file you produce into this %s conversation depends on how this deployment is configured, so it is stated per turn rather than here: the per-turn user message tells you, every turn. Follow what it says about files, and never report a file as delivered unless it told you how to deliver one.\n", ChannelDisplayName(ctx.ChatChannelType))
		} else {
			b.WriteString("**Delivering files here:** run `cordy attachment upload <local-path>` — it binds the file to your reply and it renders as an attachment card. That command is the ONLY way a file reaches the user; a path written into your reply text is not.\n")
		}
	default:
		if ctx.IsSquadLeader {
			b.WriteString("⚠️ **Final results MUST be delivered via `cordy issue comment add`** — unless your outcome is `no_action`. When you evaluate a trigger and decide no action is needed, calling `cordy squad activity <issue-id> no_action --reason \"...\"` alone is sufficient; you MUST exit without posting any comment. DO NOT post a comment that announces no_action, acknowledges another agent, or says you are exiting silently — such comments are noise. For all other outcomes (`action`, `failed`), a comment is still mandatory.\n\n")
		} else {
			b.WriteString("⚠️ **Final results MUST be delivered via `cordy issue comment add`.** The user does NOT see your terminal output or run logs — only comments on the issue.\n\n")
		}
		...
		b.WriteString("**Delivering files here:** pass `--attachment <path>` to `cordy issue comment add` (repeatable) — the only way a screenshot or artifact reaches the reader.\n")
	}
	b.WriteString("\n")
