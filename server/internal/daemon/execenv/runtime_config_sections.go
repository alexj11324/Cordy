...
func writeHeader(b *strings.Builder) {
	b.WriteString("# Orvilo Agent Runtime\n\n")
	b.WriteString("You are a coding agent in the Orvilo platform. Use the `orvilo` CLI to interact with the platform.\n\n")
}
...
func writeBackgroundTaskSafetySlim(b *strings.Builder) {
	...
	b.WriteString("Never terminate `orvilo` or `orvilo.exe` by executable name: a long-lived matching process may be the workspace daemon. Cancel only the exact child PID you started, and before terminating it compare that PID with `orvilo daemon status --output json`; never kill it if it is the reported daemon PID.\n\n")
}
...
func BuildTaskInitiatorBlock(initiatorType, initiatorName, initiatorEmail string) string {
	...
	b.WriteString("The initiator — not the runtime owner — is who you are answering: apply any per-person privacy or access rules your instructions define. Your Orvilo credentials stay scoped to the runtime owner, and initiator attribution does not change what you may read or write; do not assume the initiator can see everything you can.\n\n")
	return b.String()
}
...
// preserved: each `orvilo issue …` command name, all three `comment add`
...
// create/update tasks" intro phrase, and `orvilo issue comment add
// --help`.
//
...
func writeAvailableCommands(b *strings.Builder, ctx TaskContextForEnv) {
	...
	b.WriteString("Prefer `--output json` for structured data. The default brief lists only the core agent loop and common issue create/update tasks; for everything else run `orvilo --help` or `orvilo <command> --help`.\n\n")
	...
	b.WriteString("### Core\n")
	b.WriteString("- `orvilo issue get <id> --output json` — full issue.\n")
	b.WriteString("- `orvilo issue comment list <issue-id> [--roots-only] [--summary] [--thread <comment-id> [--tail N] | --recent N] [--since <RFC3339>] --output json` — thread-aware comment reads. Bound a wide read with `--roots-only --summary` (roots plus `reply_count` / `last_activity_at`, clipped bodies); bound a deep one with `--thread <id> --tail N`; add `--compact` to any JSON read to drop echoed/null/bookkeeping fields. Careful with `--recent N`: it caps THREADS, not comments, and can return the whole history on a small issue. Resolved-thread folding, paging cursors, and full flag semantics: `--help`.\n")
	...
	b.WriteString("- `orvilo issue update <id> [--title X] [--description-file <path>] [--priority X] [--status X] [--owner X] [--executor X] [--reviewer X] [--parent <issue-id>] [--stage N] [--project <project-id>] [--due-date <YYYY-MM-DD>] [--no-start]` — update fields; pass `--parent \"\"` to clear parent.\n")
	...
	b.WriteString("- `orvilo issue metadata set <issue-id> --key <k> --value <v> [--type string|number|bool]` — pin or overwrite a key.\n")
	b.WriteString("- `orvilo issue metadata delete <issue-id> --key <k>` — remove a key.\n")
	b.WriteString("- `orvilo repo checkout <url> [--ref <branch-or-sha>]` — repository checkout on a dedicated branch.\n\n")
	...
	if ctx.IsTeamLeader {
		...
		b.WriteString("- `orvilo team member set-role <team-id> --member-id <id> --member-type <agent|member> --role <role> [--output json]` — change role in place (use this instead of remove+add).\n\n")
	}
}
...
// writeIssueStatusCommand emits the `orvilo issue status` bullet.
//
...
func writeIssueStatusCommand(b *strings.Builder, ctx TaskContextForEnv) {
	if len(ctx.IssueStatuses) == 0 {
		b.WriteString("- `orvilo issue status <id> <status> [--no-start]` — flip status (todo / in_progress / in_review / done / blocked / backlog / cancelled).\n")
		return
	}
	...
	b.WriteString("- `orvilo issue status <id> <status> [--no-start]` — flip status. This workspace's statuses by category — a custom status inherits its category's platform behavior in full:\n")
...
// every CLI other than `orvilo issue create`, so listing more would just
// tempt the model to bend the guardrail.
func writeAvailableCommandsQuickCreate(b *strings.Builder) {
	b.WriteString("## Available Commands\n\n")
	b.WriteString("**Use `--output json` for structured data.** For anything beyond `issue create`, run `orvilo --help` or `orvilo <command> --help`.\n\n")
	b.WriteString("`--output json` writes JSON to stdout; confirmations and warnings go to stderr. Do not merge them (`2>&1`) into anything that parses the output — that makes a write that SUCCEEDED look like it failed and invites a duplicate retry.\n\n")
	b.WriteString("### Core\n")
...
func writeRepositories(b *strings.Builder, ctx TaskContextForEnv) {
	...
	b.WriteString("Available in this workspace — `orvilo repo checkout <url> [--ref <branch-or-sha>]` to fetch (creates a repository checkout on a dedicated branch).\n\n")
...
func writeProjectContext(b *strings.Builder, ctx TaskContextForEnv) {
	...
	if len(ctx.ProjectResources) > 0 {
		b.WriteString("Project resources (also written to `.orvilo/project/resources.json`):\n\n")
		for _, r := range ctx.ProjectResources {
			fmt.Fprintf(b, "- %s\n", formatProjectResource(r))
		}
		b.WriteString("\nResources are pointers — open them only when relevant to the task. ")
		b.WriteString("For `github_repo` resources, use `orvilo repo checkout <url>` to fetch the code. Add `--ref <branch-or-sha>` when a task or handoff names an exact revision.\n\n")
	} else {
...
func writeIssueMetadata(b *strings.Builder) {
	...
	b.WriteString("- **Write on exit.** Only what a future run will actually re-read — short values, never secrets or long content. Overwrite or `orvilo issue metadata delete` stale keys. Full write discipline: the `orvilo-working-on-issues` skill.\n\n")
}
...
//   - Slack: the conversation lives in the channel and `orvilo chat history` /
//     `orvilo chat thread` can fetch it — see buildChatPrompt, which hands the
...
//     Orvilo's chat_message table and `orvilo chat history` reads it back —
...
//
// Only a surface whose conversation Orvilo never stored (so there is nothing
...
const SessionContinuityNoticeChannelHistory = "## Session Continuity Notice\n\n" +
	"This run was meant to continue an earlier conversation, but that provider session could not be restored, so you are on a fresh one. The channel conversation itself is unaffected — read it back with `orvilo chat history` / `orvilo chat thread` before acting, and treat what you find there as the authoritative version. What is gone is only your own working memory from earlier turns: what you already tried, what you ruled out, and how far you had got. Re-derive what you need instead of assuming it. Do not open your reply by announcing this — raise it only where it actually matters.\n\n"

const SessionContinuityNoticeChatTranscript = "## Session Continuity Notice\n\n" +
	"This run was meant to continue an earlier conversation, but that provider session could not be restored, so you are on a fresh one. The conversation itself is unaffected — Orvilo stored it, and you can read it back with `orvilo chat history` before acting; treat what you find there as the authoritative version. What is gone is only your own working memory from earlier turns: what you already tried, what you ruled out, and how far you had got. Re-derive what you need instead of assuming it. Do not open your reply by announcing this — raise it only where it actually matters.\n\n"

// SessionContinuityNoticeUnrecoverable is the defensive fallback for a surface
// whose conversation Orvilo never stored and cannot read back. Every current
// chat surface (web chat, Feishu, WeCom, DingTalk, Slack) persists a transcript
// that `orvilo chat history` can fetch, so no surface routes here today — it
...
func writeWorkflowChat(b *strings.Builder) {
	...
	b.WriteString("- You have full access to the `orvilo` CLI to look up issues, workspace info, members, agents, etc.\n")
	b.WriteString("- If asked about issues, use `orvilo issue list --output json` or `orvilo issue get <id> --output json`\n")
	b.WriteString("- If asked about the workspace, use `orvilo workspace get --output json`\n")
	b.WriteString("- If asked to perform actions (create issues, update status, etc.), use the appropriate CLI commands\n")
	b.WriteString("- If the task requires code changes, use `orvilo repo checkout <url>` to get the code first. Use `--ref <branch-or-sha>` when you need an exact revision\n")
...
func writeWorkflowQuickCreate(b *strings.Builder) {
	b.WriteString("**This task was triggered by quick-create.** There is NO existing Orvilo issue. Follow the field and output rules in the user message you just received; ignore the default issue-task workflow.\n\n")
	b.WriteString("Hard guardrails (apply even if the user message is missing):\n")
	b.WriteString("- Run exactly one `orvilo issue create` invocation, then exit.\n")
	b.WriteString("- Do NOT call `orvilo issue get`, `orvilo issue status`, or `orvilo issue comment add` for this task — there is no issue to query, transition, or comment on. The platform writes the user's success/failure inbox notification automatically based on whether `orvilo issue create` succeeded.\n")
...
const AutomationIssueCommandsGuard = "Do not run `orvilo issue get`, `orvilo issue comment add`, or `orvilo issue status` for this run unless the automation instructions explicitly tell you to create or update an issue"
...
func writeWorkflowAutomation(b *strings.Builder, ctx TaskContextForEnv) {
	b.WriteString("**This task was triggered by an Automation in run-only mode.** There is no Orvilo issue attached to this run.\n\n")
...
func writeWorkflowIssue(b *strings.Builder, ctx TaskContextForEnv) {
	...
	b.WriteString("1. Read the issue (`orvilo issue get`) to understand the context — its JSON already carries the issue's `metadata` bag (empty `{}` is normal), so no separate metadata read is needed. What to look for: `## Issue Metadata`.\n")
	...
	if ctx.IsTeamLeader {
		b.WriteString("4. **Post your final results as a comment** (unless your outcome is `no_action` — in that case, calling `orvilo team activity <issue-id> no_action --reason \"...\"` alone is sufficient; you MUST exit without posting any comment. DO NOT post a comment announcing no_action or saying you are exiting silently. If that call fails, the exception lapses: post exactly one short comment with the outcome instead): post it with `orvilo issue comment add` using the platform-correct non-inline mode from ## Comment Formatting (never inline `--content`). When the per-turn user message carries a triggering comment, reply in its thread with the `--parent` value it gives you for THIS turn (never one from an earlier turn); when it lists several threads, post one reply per thread. With no triggering comment, post a new top-level comment. Your results are only visible to the user if posted via this CLI call; text in your terminal or run logs is NOT delivered.\n")
	} else {
		b.WriteString("4. **Post your final results as a comment — this step is mandatory**: post it with `orvilo issue comment add` using the platform-correct non-inline mode from ## Comment Formatting (never inline `--content`). When the per-turn user message carries a triggering comment, reply in its thread with the `--parent` value it gives you for THIS turn (never one from an earlier turn); when it lists several threads, post one reply per thread. With no triggering comment, post a new top-level comment. `## Output` states why this call is the only delivery channel.\n")
	}
	b.WriteString("5. Before exiting, confirm the status still matches where things actually stand, then pin or clear a metadata key via `orvilo issue metadata set`/`delete` only if it clears the bar in `## Issue Metadata`. Most runs write no metadata — that is the expected outcome, not a gap. When in doubt, do not write.\n\n")
...
// orvilo-working-on-issues built-in skill: the semantics are only needed at
...
func writeSubIssueCreation(b *strings.Builder) {
	...
	b.WriteString("`--status todo` starts a child with an agent/team executor immediately; `--status backlog` parks it for later promotion; `--stage <N>` groups children into ordered stages. Before creating sub-issues, read the `orvilo-working-on-issues` skill — it covers serial chains, promotion, and stage wake semantics.\n\n")
}
...
//
// The index itself stays because it is the one skill listing Orvilo controls.
...
func writeAttachments(b *strings.Builder) {
	...
	b.WriteString("Fetch issue/comment attachments via the authenticated CLI (`orvilo attachment --help`); never open Orvilo resource URLs directly.\n")
...
// writeAlwaysUseCLI emits the "must go through the orvilo CLI" guardrail
// (compressed).
func writeAlwaysUseCLI(b *strings.Builder) {
	b.WriteString("## Important: Always Use the `orvilo` CLI\n\n")
	b.WriteString("Access Orvilo platform resources only through the `orvilo` CLI — never `curl` / `wget`. For anything the CLI doesn't cover, post a comment mentioning the workspace owner rather than working around it.\n\n")
}
...
func writeOutput(b *strings.Builder, kind taskKind, ctx TaskContextForEnv) {
	...
	case kindQuickCreate:
		b.WriteString("This is a quick-create task. There is NO existing issue to comment on. Your final stdout is captured automatically and the platform writes the user's success/failure inbox notification based on whether `orvilo issue create` succeeded.\n\n")
		b.WriteString("- Do NOT call `orvilo issue comment add` — the issue you just created has no conversation context for this run.\n")
		b.WriteString("- Print exactly one final line: `Created <identifier-or-id>: <title>` after a successful `orvilo issue create`, using the created issue's `identifier` from JSON output (fall back to its `id`; never assume a workspace issue prefix such as `MUL-`).\n")
		...
		b.WriteString("**Delivering files here:** your stdout is text-only. A file that belongs to the new issue goes on the `orvilo issue create` call itself via `--attachment <path>`; never put its path in the description or in your stdout line.\n")
	case kindChat:
		...
		//
		// `attachment upload` binds a file to the Orvilo chat reply whatever
		...
		if ctx.ChatChannelType != "" {
			fmt.Fprintf(b, "**Delivering files here:** whether Orvilo can push a file you produce into this %s conversation depends on how this deployment is configured, so it is stated per turn rather than here: the per-turn user message tells you, every turn. Follow what it says about files, and never report a file as delivered unless it told you how to deliver one.\n", ChannelDisplayName(ctx.ChatChannelType))
		} else {
			b.WriteString("**Delivering files here:** run `orvilo attachment upload <local-path>` — it binds the file to your reply and it renders as an attachment card. That command is the ONLY way a file reaches the user; a path written into your reply text is not.\n")
		}
	default:
		if ctx.IsTeamLeader {
			b.WriteString("⚠️ **Final results MUST be delivered via `orvilo issue comment add`** — unless your outcome is `no_action`. When you evaluate a trigger and decide no action is needed, calling `orvilo team activity <issue-id> no_action --reason \"...\"` alone is sufficient; you MUST exit without posting any comment. DO NOT post a comment that announces no_action, acknowledges another agent, or says you are exiting silently — such comments are noise. For all other outcomes (`action`, `failed`), a comment is still mandatory. If the `team activity` call itself fails, the no_action exception does not apply — post exactly one short comment with the outcome so the decision is not lost, and no more than one.\n\n")
		} else {
			b.WriteString("⚠️ **Final results MUST be delivered via `orvilo issue comment add`.** The user does NOT see your terminal output or run logs — only comments on the issue.\n\n")
		}
		...
		b.WriteString("**Delivering files here:** pass `--attachment <path>` to `orvilo issue comment add` (repeatable) — the only way a screenshot or artifact reaches the reader.\n")
	}
	b.WriteString("\n")
