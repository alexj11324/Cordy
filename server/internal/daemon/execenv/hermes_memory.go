...
// agent writes back) in `<HERMES_HOME>/memories/`. Orvilo has to take over
...
// hermesMemoryStoreRoot is the directory under the daemon's Orvilo profile dir
...
// agent to key on, or the Orvilo profile dir cannot be resolved. The daemon
// marks the returned path in-use for the task's duration so
// PruneHermesMemoryStores never reclaims it mid-mount.
//
// daemonProfile namespaces by Orvilo profile for free: profile dirs are already
...
//     filesystems — the workspaces root and the Orvilo profile dir can be on
...
// Orvilo profile dir), so it must actively replace a link left by a previous
