...
// agent writes back) in `<HERMES_HOME>/memories/`. Cordy has to take over
...
//
// This file makes memory an agent-scoped store Cordy owns:
//
//	<cordy profile dir>/hermes-state/<agent>/<hermes profile>/memories
//
...
// hermesMemoryStoreRoot is the directory under the daemon's Cordy profile dir
...
// agent to key on, or the Cordy profile dir cannot be resolved. The daemon
// marks the returned path in-use for the task's duration so
// PruneHermesMemoryStores never reclaims it mid-mount.
//
// daemonProfile namespaces by Cordy profile for free: profile dirs are already
...
//     filesystems — the workspaces root and the Cordy profile dir can be on
...
// Cordy profile dir), so it must actively replace a link left by a previous
