...
// agent writes back) in `<HERMES_HOME>/memories/`. Patchbay has to take over
...
//
// This file makes memory an agent-scoped store Patchbay owns:
//
...
// hermesMemoryStoreRoot is the directory under the daemon's Patchbay profile dir
...
// agent to key on, or the Patchbay profile dir cannot be resolved. The daemon
// marks the returned path in-use for the task's duration so
// PruneHermesMemoryStores never reclaims it mid-mount.
//
// daemonProfile namespaces by Patchbay profile for free: profile dirs are already
...
//     filesystems — the workspaces root and the Patchbay profile dir can be on
...
// Patchbay profile dir), so it must actively replace a link left by a previous
