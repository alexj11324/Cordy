...
func prepareCodexHomeWithOpts(codexHome string, opts CodexHomeOptions, logger *slog.Logger) error {
	...
	// `missing field path` and bails out of `thread/start`. Cordy writes the
	...
	// so cross-task and cross-workspace context leaks (cordy#3130) cannot
...
// enumerated by a plain `codex` run, keeping Cordy task history out of the
// user's own thread list.
const codexSessionStoreRoot = "cordy-sessions"
...
// shared home's cordy-sessions root that have not been touched within
