...
const legacyStatusLine = "- `orvilo issue status <id> <status> [--no-start]` — flip status (todo / in_progress / in_review / done / blocked / backlog / cancelled).\n"
...
func TestBriefStatusCatalogRendered(t *testing.T) {
	...
	for _, want := range []string{
		"- `orvilo issue status <id> <status> [--no-start]` — flip status. This workspace's statuses by category — a custom status inherits its category's platform behavior in full:\n",
