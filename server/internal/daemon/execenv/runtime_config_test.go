...
func TestBriefHasNoParentNotificationGuidance(t *testing.T) {
	...
	for _, ctx := range cases {
		...
		for _, banned := range []string{
			...
			"orvilo issue comment add <parent-id>",
			"with NO `--parent`",
			...
			"`orvilo issue status <this-issue-id> in_review`",
...
// `orvilo issue status <this-issue-id> in_review` command would fire on every
...
func TestStatusRuleIsFactJudgmentAtBothMoments(t *testing.T) {
	...
	if strings.Contains(out, "`orvilo issue status <this-issue-id> in_review`") {
	...
	for _, banned := range []string{
		...
		"Before step 3, run `orvilo issue status",
...
func TestColdCommentsHintPointsAtTriggeringThread(t *testing.T) {
	...
	if !strings.Contains(hint, "orvilo issue comment list "+issueID+" --thread thread-root-1 --tail 30 --compact --output json") {
...
func TestSessionContinuityNoticeLivesOutsideBrief(t *testing.T) {
	...
	if !strings.Contains(SessionContinuityNoticeChatTranscript, "orvilo chat history") {
...
func TestIssueWorkflowHonorsAgentIdentity(t *testing.T) {
	...
	for _, banned := range []string{
		"4. Run `orvilo issue status " + issueID + " in_progress`\n",
		"5. Follow your Skills and Agent Identity to complete the task (write code, investigate, etc.)",
		"8. When done, run `orvilo issue status " + issueID + " in_review`\n",
	} {
		if strings.Contains(out, banned) {
...
func TestChatOutputDoesNotRequireIssueComment(t *testing.T) {
	...
	for _, banned := range []string{
		...
		"do not call `orvilo issue comment add`",
...
func TestMultiThreadReplyInstructionsFanOut(t *testing.T) {
	...
	for _, banned := range []string{
		...
		"orvilo issue comment add",           // embedded example commands
