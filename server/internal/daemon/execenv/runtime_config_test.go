	...
	for _, tc := range cases {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			for _, want := range []string{
				...
				// cordy-working-on-issues skill. The brief keeps a one-line
				...
				"read the `cordy-working-on-issues` skill",
			} {
...
func TestBriefHasNoParentNotificationGuidance(t *testing.T) {
	...
	for _, ctx := range cases {
		...
		for _, banned := range []string{
			...
			"cordy issue comment add <parent-id>",
			"with NO `--parent`",
			...
			"`cordy issue status <this-issue-id> in_review`",
...
// `cordy issue status <this-issue-id> in_review` command would fire on every
...
func TestStatusRuleIsFactJudgmentAtBothMoments(t *testing.T) {
	...
	if strings.Contains(out, "`cordy issue status <this-issue-id> in_review`") {
	...
	for _, banned := range []string{
		...
		"Before step 3, run `cordy issue status",
...
func TestColdCommentsHintPointsAtTriggeringThread(t *testing.T) {
	...
	if !strings.Contains(hint, "cordy issue comment list "+issueID+" --thread thread-root-1 --tail 30 --compact --output json") {
...
func TestSessionContinuityNoticeLivesOutsideBrief(t *testing.T) {
	...
	if !strings.Contains(SessionContinuityNoticeChatTranscript, "cordy chat history") {
...
func TestIssueWorkflowHonorsAgentIdentity(t *testing.T) {
	...
	for _, banned := range []string{
		"4. Run `cordy issue status " + issueID + " in_progress`\n",
		"5. Follow your Skills and Agent Identity to complete the task (write code, investigate, etc.)",
		"8. When done, run `cordy issue status " + issueID + " in_review`\n",
	} {
		if strings.Contains(out, banned) {
...
func TestChatOutputDoesNotRequireIssueComment(t *testing.T) {
	...
	for _, banned := range []string{
		...
		"do not call `cordy issue comment add`",
...
func TestWriteRuntimeConfigFileCreatesMissingFile(t *testing.T) {
	...
	const brief = "# Cordy Agent Runtime\n\nbrief body line"
...
func TestWriteRuntimeConfigFileReplacesExistingBlock(t *testing.T) {
	...
	const newBrief = "## New Cordy brief\n\nfresh body"
...
func TestWriteRuntimeConfigFileIgnoresStrayEndMarkerBeforeBegin(t *testing.T) {
	...
	// real block authored by an earlier Cordy run below.
	const userDoc = "# Repo CLAUDE.md\n\nExample of what Cordy writes:\n" +
		runtimeMarkerEnd + "\n\n# Real config below\n"
	original := userDoc +
		runtimeMarkerBegin + "\nFIRST BRIEF\n" + runtimeMarkerEnd + "\n"
	if err := os.WriteFile(path, []byte(original), 0o644); err != nil {
...
// `codex` run started by the user after a Cordy task must see the same
// file the user wrote.
...
func TestMultiThreadReplyInstructionsFanOut(t *testing.T) {
	...
	for _, banned := range []string{
		...
		"cordy issue comment add",            // embedded example commands
...
func TestBriefSkillsListIsNamesOnly(t *testing.T) {
	...
	ctx := TaskContextForEnv{
		...
		AgentSkills: []SkillContextForEnv{
			{
				Name:        "PR Review",
				Description: "Use when reviewing a pull request for the Cordy project.",
				Content:     "---\nname: pr-review\n---\n\nbody",
			},
		},
	}
