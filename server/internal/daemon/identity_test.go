...
func TestEnsureDaemonID_Persists(t *testing.T) {
	...
	path := filepath.Join(home, ".patchbay", "daemon.id")
...
func TestEnsureDaemonID_SharedAcrossProfiles(t *testing.T) {
	...
	// only source of truth is ~/.patchbay/daemon.id.
	profileFile := filepath.Join(home, ".patchbay", "profiles", "staging", "daemon.id")
...
func TestEnsureDaemonID_PromotesPreChangeProfileFile(t *testing.T) {
	...
	profileDir := filepath.Join(home, ".patchbay", "profiles", "staging")
	...
	data, err := os.ReadFile(filepath.Join(home, ".patchbay", "daemon.id"))
	if err != nil {
...
func TestLegacyDaemonUUIDs_ScansProfileDirs(t *testing.T) {
	...
	for name, id := range map[string]string{"prod": uuidA, "desktop-patchbay": uuidB} {
		dir := filepath.Join(home, ".patchbay", "profiles", name)
		if err := os.MkdirAll(dir, 0o755); err != nil {
	...
	corruptDir := filepath.Join(home, ".patchbay", "profiles", "corrupt")
