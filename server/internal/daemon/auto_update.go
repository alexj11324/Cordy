...
// ParseSelfVersion pulls the version out of `cordy --version` output, whose
// first line is rendered by cmd/cordy's version template:
//
...
//
// Exported solely so cmd/cordy can pin that contract from the producing side:
...
func ParseSelfVersion(raw string) string {
	...
	if fields := strings.Fields(line); len(fields) >= 2 && fields[0] == "cordy" {
		return fields[1]
	}
	return line
}
...
// different cordy binary". It runs two independent checks on one goroutine,
...
// (--no-auto-update / CORDY_DAEMON_AUTO_UPDATE=false), when the server is
...
func (d *Daemon) autoUpdateLoop(ctx context.Context) {
	...
	if reloadEnabled {
		d.logger.Info("auto-reload: watching the cordy binary on disk",
...
// trySelfReload restarts the daemon when the cordy binary on disk no longer
// reports the version compiled into this process.
//
// This is the out-of-band half of self-update. `brew upgrade cordy`, a
...
// has its own switch (--no-auto-reload / CORDY_DAEMON_AUTO_RELOAD=false /
// disable_auto_reload) rather than riding on CORDY_DAEMON_AUTO_UPDATE.
//
...
func (d *Daemon) trySelfReload(ctx context.Context) {
	...
	reason := fmt.Sprintf("cordy binary on disk reports %s, running %s", onDisk, d.cfg.CLIVersion)
...
// setReloadPending records that a cordy version change is confirmed but the
