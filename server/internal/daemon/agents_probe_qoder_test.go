...
// rescuing an operator-pinned CORDY_QODER_PATH. A pinned absolute path that
...
func TestProbeAgentCLIs_QoderPinnedPathStaysHardMiss(t *testing.T) {
	...
	t.Setenv("PATH", "")
	t.Setenv("CORDY_QODER_PATH", "/nonexistent/pinned/qodercli")

	if entry, ok := probeAgentCLIs()["qoder"]; ok {
		t.Errorf("pinned-but-missing CORDY_QODER_PATH resolved to %q, want a hard miss", entry.Path)
	}
}
