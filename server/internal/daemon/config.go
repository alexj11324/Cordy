...
import (
	...
	"github.com/cordy-ai/cordy/server/internal/cli"
...
const (
	...
	// hard ceiling for cost/resource control can set CORDY_AGENT_TIMEOUT.
	...
	// Set CORDY_AGENT_IDLE_WATCHDOG=0 to disable.
	...
	// that stuck-tool case (MUL-3064). Set CORDY_AGENT_TOOL_WATCHDOG=0 to
	...
	DefaultGCCompletedTaskTTLCloud        = 14 * 24 * time.Hour // 14 days — Cordy Cloud bounds completed issue-task env retention by default; see defaultGCCompletedTaskTTL
...
// CORDY_GC_ARTIFACT_PATTERNS to extend the list per deployment.
...
type Config struct {
	...
	DeviceName                     string
	RuntimeName                    string
	CLIVersion                     string                // cordy CLI version (e.g. "0.1.13")
	...
	WorkspacesRoot                 string                // base path for execution envs (default: ~/cordy_workspaces)
	...
	GCCompletedTaskTTL             time.Duration         // fully clean inactive issue-task envs completed at least this long ago, regardless of parent issue status (default: 14d on Cordy Cloud, 0/disabled elsewhere; local_directory envs are never fully removed)
	...
	GCCodexSessionTTL              time.Duration         // reclaim a per-issue Codex session store (~/.codex/cordy-sessions/<agent>/<issue>) untouched for at least this long, so a done/abandoned issue's conversation history does not accumulate forever (default: 14d, set 0 to disable)
	...
	AutoUpdateEnabled              bool                  // periodically check for a newer CLI release and self-update when idle (default: true on Cordy Cloud, false on self-host)
	AutoUpdateCheckInterval        time.Duration         // how often the auto-update loop polls for a new release (default: 6h)
	AutoReloadEnabled              bool                  // restart when the cordy binary on disk no longer matches the running version (default: true for CLI-launched daemons)
	PollInterval                   time.Duration
	HeartbeatInterval              time.Duration
	AgentTimeout                   time.Duration
	CodexSemanticInactivityTimeout time.Duration
	// CodexFirstTurnNoProgressTimeout is an explicit override for the Codex
	// first-turn no-progress ceiling (CORDY_CODEX_FIRST_TURN_TIMEOUT). 0 means
	...
	// written by `cordy runtime profile set-path`. appendProfileRuntimes
...
func LoadConfig(overrides Overrides) (Config, error) {
	...
	rawServerURL := envOrDefault("CORDY_SERVER_URL", DefaultServerURL)
	...
	//
	//   - CORDY_OPENCLAW_PATH: read by probe() via envOrDefault for the
	...
	//   - CORDY_OPENCLAW_CLI_TIMEOUT: read by execenv when it sets the
	//     deadline on each `openclaw config ...` call during task prep.
	//
	// Precedence is "env wins over config wins over default" — same shape
	// users already get with CORDY_OPENCLAW_PATH today. We achieve it with
	...
	claudeArgs, err := shellArgsFromEnv("CORDY_CLAUDE_ARGS")
	if err != nil {
		return Config{}, err
	}
	codexArgs, err := shellArgsFromEnv("CORDY_CODEX_ARGS")
	if err != nil {
		return Config{}, err
	}
	codebuddyArgs, err := shellArgsFromEnv("CORDY_CODEBUDDY_ARGS")
	if err != nil {
		return Config{}, err
	}
	qwenArgs, err := shellArgsFromEnv("CORDY_QWEN_ARGS")
	if err != nil {
		return Config{}, err
	}
	qwenpawArgs, err := shellArgsFromEnv("CORDY_QWENPAW_ARGS")
	if err != nil {
		return Config{}, err
	}
	...
	pollInterval, err := durationFromEnv("CORDY_DAEMON_POLL_INTERVAL", DefaultPollInterval)
	if err != nil {
		return Config{}, err
	}
	...
	heartbeatInterval, err := durationFromEnv("CORDY_DAEMON_HEARTBEAT_INTERVAL", DefaultHeartbeatInterval)
	if err != nil {
		return Config{}, err
	}
	...
	agentTimeout, err := durationFromEnv("CORDY_AGENT_TIMEOUT", DefaultAgentTimeout)
	if err != nil {
		return Config{}, err
	}
	...
	codexSemanticInactivityTimeout, err := durationFromEnv("CORDY_CODEX_SEMANTIC_INACTIVITY_TIMEOUT", DefaultCodexSemanticInactivityTimeout)
	if err != nil {
		return Config{}, err
	}
	...
	codexFirstTurnNoProgressTimeout, err := durationFromEnv("CORDY_CODEX_FIRST_TURN_TIMEOUT", 0)
	if err != nil {
		return Config{}, err
	}
	...
	if codexFirstTurnNoProgressTimeout > 0 {
		...
		if codexFirstTurnNoProgressTimeout >= effectiveSemanticTimeout {
			slog.Warn("CORDY_CODEX_FIRST_TURN_TIMEOUT is greater than or equal to the semantic-inactivity timeout; the effective first-turn wait is truncated to the semantic timeout and the model-catalog startup retry is disabled. Because the semantic timer is armed first and equal durations do not deterministically favour the first-turn deadline, set CORDY_CODEX_SEMANTIC_INACTIVITY_TIMEOUT strictly above CORDY_CODEX_FIRST_TURN_TIMEOUT (with some margin) to preserve it.",
	...
	codexHandshakeTimeout, err := durationFromEnv("CORDY_CODEX_HANDSHAKE_TIMEOUT", DefaultCodexHandshakeTimeout)
	if err != nil {
		return Config{}, err
	}
	...
	// CORDY_AGENT_IDLE_WATCHDOG=0 disables the per-task idle watchdog. We
	...
	agentIdleWatchdog, err := durationFromEnv("CORDY_AGENT_IDLE_WATCHDOG", DefaultAgentIdleWatchdog)
	if err != nil {
		return Config{}, err
	}
	// CORDY_OPENCODE_IDLE_WATCHDOG narrows the no-message window for
	// OpenCode's streamed model responses. Zero removes the provider-specific
	// override and falls back to CORDY_AGENT_IDLE_WATCHDOG; positive values
	// cannot extend the global bound, and the global zero still disables the
	// whole mechanism.
	openCodeIdleWatchdog, err := durationFromEnv("CORDY_OPENCODE_IDLE_WATCHDOG", DefaultOpenCodeIdleWatchdog)
	if err != nil {
		return Config{}, err
	}

	// CORDY_AGENT_TOOL_WATCHDOG=0 disables the in-flight-tool backstop; any
	// positive duration overrides DefaultAgentToolWatchdog.
	agentToolWatchdog, err := durationFromEnv("CORDY_AGENT_TOOL_WATCHDOG", DefaultAgentToolWatchdog)
	if err != nil {
		return Config{}, err
	}

	maxConcurrentTasks, err := intFromEnv("CORDY_DAEMON_MAX_CONCURRENT_TASKS", DefaultMaxConcurrentTasks)
	if err != nil {
		return Config{}, err
	}
	...
	// Callers may still pin a specific id via CORDY_DAEMON_ID or the
	// override field (e.g. for tests or embedded environments).
	daemonID := strings.TrimSpace(os.Getenv("CORDY_DAEMON_ID"))
	...
	// — e.g. ~/.cordy/daemon.id (default) plus ~/.cordy/profiles/<x>/
	...
	// the user explicitly pins CORDY_DAEMON_ID=<hostname>, or when the
	// canonical id was itself promoted from a pre-change profile file).
	legacyDaemonIDs = filterLegacyIDs(legacyDaemonIDs, daemonID)

	deviceName := envOrDefault("CORDY_DAEMON_DEVICE_NAME", host)
	if overrides.DeviceName != "" {
		deviceName = overrides.DeviceName
	}

	runtimeName := envOrDefault("CORDY_AGENT_RUNTIME_NAME", DefaultRuntimeName)
	if overrides.RuntimeName != "" {
		runtimeName = overrides.RuntimeName
	}

	// Workspaces root: override > env > default (~/cordy_workspaces or ~/cordy_workspaces_<profile>)
	...
	keepEnv := os.Getenv("CORDY_KEEP_ENV_AFTER_TASK") == "true" || os.Getenv("CORDY_KEEP_ENV_AFTER_TASK") == "1"

	// GC config: env > defaults
	gcEnabled := true
	if v := os.Getenv("CORDY_GC_ENABLED"); v == "false" || v == "0" {
		gcEnabled = false
	}
	gcInterval, err := durationFromEnv("CORDY_GC_INTERVAL", DefaultGCInterval)
	if err != nil {
		return Config{}, err
	}
	gcTTL, err := durationFromEnv("CORDY_GC_TTL", DefaultGCTTL)
	if err != nil {
		return Config{}, err
	}
	gcCompletedTaskTTL, err := durationFromEnv("CORDY_GC_COMPLETED_TASK_TTL", defaultGCCompletedTaskTTL(serverBaseURL))
	if err != nil {
		return Config{}, err
	}
	gcOrphanTTL, err := durationFromEnv("CORDY_GC_ORPHAN_TTL", DefaultGCOrphanTTL)
	if err != nil {
		return Config{}, err
	}
	gcArtifactTTL, err := durationFromEnv("CORDY_GC_ARTIFACT_TTL", DefaultGCArtifactTTL)
	if err != nil {
		return Config{}, err
	}
	gcCodexSessionTTL, err := durationFromEnv("CORDY_GC_CODEX_SESSION_TTL", DefaultGCCodexSessionTTL)
	if err != nil {
		return Config{}, err
	}
	gcHermesMemoryTTL, err := durationFromEnv("CORDY_GC_HERMES_MEMORY_TTL", DefaultGCHermesMemoryTTL)
	if err != nil {
		return Config{}, err
	}
	gcHermesSessionTTL, err := durationFromEnv("CORDY_GC_HERMES_SESSION_TTL", DefaultGCHermesSessionTTL)
	if err != nil {
		return Config{}, err
	}
	gcRepoTTL, err := durationFromEnv("CORDY_GC_REPO_TTL", DefaultGCRepoTTL)
	if err != nil {
		return Config{}, err
	}
	gcRepoMaintenanceEnabled := boolFromEnv("CORDY_GC_REPO_MAINTENANCE_ENABLED", true)
	gcArtifactPatterns := patternsFromEnv("CORDY_GC_ARTIFACT_PATTERNS", DefaultGCArtifactPatterns)

	// Auto-update config: default -> env override -> CLI override.
	//
	// Default is opt-in on Cordy Cloud (api.cordy.ai) and opt-out for
	...
	// Operators on either side can flip the default with CORDY_DAEMON_AUTO_UPDATE.
	autoUpdateEnabled := boolFromEnv("CORDY_DAEMON_AUTO_UPDATE", isOfficialCloudServer(serverBaseURL))
	if overrides.DisableAutoUpdate {
		autoUpdateEnabled = false
	}
	autoUpdateInterval, err := durationFromEnv("CORDY_DAEMON_AUTO_UPDATE_INTERVAL", DefaultAutoUpdateCheckInterval)
	if err != nil {
		return Config{}, err
	}
	...
	autoReloadEnabled := boolFromEnv("CORDY_DAEMON_AUTO_RELOAD", true)
...
// officialCloudHost is the hostname of Cordy's hosted cloud. It's the only
// origin we treat as "official" for the auto-update default — staging,
// preview, and any future *.cordy.ai subdomains are deliberately excluded
// so they inherit the safer self-host default until explicitly opted in.
const officialCloudHost = "api.cordy.ai"

// isOfficialCloudServer reports whether the resolved server base URL points
// at Cordy's hosted cloud. Used to pick defaults that are safe on
...
// with it. On Cordy Cloud that trade is ours to make — we operate the nodes,
...
// give up, so it stays disabled until they set CORDY_GC_COMPLETED_TASK_TTL
...
func NormalizeServerBaseURL(raw string) (string, error) {
	...
	if err != nil {
		return "", fmt.Errorf("invalid CORDY_SERVER_URL: %w", err)
	}
	switch u.Scheme {
	case "ws":
		u.Scheme = "http"
	...
	case "http", "https":
	default:
		return "", fmt.Errorf("CORDY_SERVER_URL must use ws, wss, http, or https")
	}
	if u.Path == "/ws" {
		u.Path = ""
	}
...
const TaskWorkspacesRootEnv = "CORDY_TASK_WORKSPACES_ROOT"
...
// CORDY_WORKSPACES_ROOT env > default ($HOME/cordy_workspaces, or
// $HOME/cordy_workspaces_<profile> for a named profile). Read-only callers
// (e.g. `cordy daemon disk-usage`) use this directly so they pick the same
...
func ResolveWorkspacesRoot(profile, override string) (string, error) {
	root := strings.TrimSpace(os.Getenv("CORDY_WORKSPACES_ROOT"))
	if override != "" {
		root = override
	}
	if root == "" {
		...
		if err != nil {
			return "", fmt.Errorf("resolve home directory: %w (set CORDY_WORKSPACES_ROOT to override)", err)
		}
		if profile != "" {
			root = filepath.Join(home, "cordy_workspaces_"+profile)
		} else {
			root = filepath.Join(home, "cordy_workspaces")
		}
	}
...
func ArtifactPatternsFromEnv() []string {
	return patternsFromEnv("CORDY_GC_ARTIFACT_PATTERNS", DefaultGCArtifactPatterns)
}
...
// When ~/.cordy/hooks shadows a real agent binary, skip that hooks directory:
...
func resolveAgentExecutablePath(cmd string) (string, error) {
	...
	if isInCordyHooksDir(resolved) {
		if unshadowed, err := lookPathExcludingCordyHooks(cmd); err == nil {
...
// probe() order in LoadConfig: exec.LookPath (with the ~/.cordy/hooks
...
func reresolveAgentCommand(cmd string) (string, bool) {
	...
	// this — an operator-pinned CORDY_*_PATH that no longer exists should
...
func lookPathExcludingCordyHooks(cmd string) (string, error) {
	for _, dir := range filepath.SplitList(os.Getenv("PATH")) {
		if dir == "" {
			dir = "."
		}
		if isCordyHooksDir(dir) {
			continue
		}
...
func isInCordyHooksDir(path string) bool {
	if path == "" {
		return false
	}
	return isCordyHooksDir(filepath.Dir(path))
}

func isCordyHooksDir(dir string) bool {
	home, err := os.UserHomeDir()
	if err != nil || home == "" {
		return false
	}
	return samePathDir(dir, filepath.Join(home, ".cordy", "hooks"))
}
...
// before any CORDY_*_PATH override is applied. Kept in sync with the
...
//     keep the script readable. Custom CORDY_*_PATH values never reach this
...
//     name is kept,
//  6. if the resolved path lives in ~/.cordy/hooks, searches the same
...
func buildLoginShellResolveScript(names []string) string {
	...
	b.WriteString("  hc=\"\"\n")
	b.WriteString("  if [ -n \"${HOME:-}\" ]; then hd=\"$HOME/.cordy/hooks\"; hc=$(cd \"$hd\" 2>/dev/null && pwd -P) || hc=\"\"; fi\n")
...
//
//   - The three vars touched (CORDY_OPENCLAW_PATH, OPENCLAW_STATE_DIR,
//     CORDY_OPENCLAW_CLI_TIMEOUT) are
...
func applyOpenclawOverride(oc *cli.OpenClawOverride) {
	...
	if oc.BinaryPath != "" {
		if _, set := os.LookupEnv("CORDY_OPENCLAW_PATH"); !set {
			_ = os.Setenv("CORDY_OPENCLAW_PATH", oc.BinaryPath)
		}
	}
	if oc.StateDir != "" {
