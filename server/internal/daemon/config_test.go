...
func TestPatternsFromEnv_DefaultsWhenUnset(t *testing.T) {
	t.Setenv("CORDY_GC_ARTIFACT_PATTERNS", "")
	defaults := []string{"node_modules", ".next", ".turbo"}
	got := patternsFromEnv("CORDY_GC_ARTIFACT_PATTERNS", defaults)
...
func TestLoadConfig_CompletedTaskTTLDefaultsDisabledOnSelfHostAndReadsEnv(t *testing.T) {
	...
	t.Setenv("CORDY_GC_COMPLETED_TASK_TTL", "")
	...
	t.Setenv("CORDY_GC_COMPLETED_TASK_TTL", "36h")
	cfg, err = LoadConfig(overrides)
	if err != nil {
	...
	t.Setenv("CORDY_GC_COMPLETED_TASK_TTL", "not-a-duration")
	if _, err := LoadConfig(overrides); err == nil || !strings.Contains(err.Error(), "CORDY_GC_COMPLETED_TASK_TTL") {
...
func TestLoadConfig_CompletedTaskTTLDefaultsBoundedOnOfficialCloud(t *testing.T) {
	...
	t.Setenv("CORDY_GC_COMPLETED_TASK_TTL", "0")
	cfg, err = LoadConfig(overrides)
	if err != nil {
...
func TestDefaultGCCompletedTaskTTLOnlyBoundsOfficialCloudHost(t *testing.T) {
	...
	cases := []struct {
		...
		{"official cloud", "https://api.cordy.ai", DefaultGCCompletedTaskTTLCloud},
		{"official cloud with port and path", "https://API.Cordy.AI:443/api", DefaultGCCompletedTaskTTLCloud},
		...
		{"staging", "https://api-staging.cordy.ai", DefaultGCCompletedTaskTTLSelfHost},
		{"self-host", "https://cordy.example.com", DefaultGCCompletedTaskTTLSelfHost},
...
func TestRepoMaintenanceKillSwitchDefaultsOnAndCanDisable(t *testing.T) {
	t.Setenv("CORDY_GC_REPO_MAINTENANCE_ENABLED", "")
	if !boolFromEnv("CORDY_GC_REPO_MAINTENANCE_ENABLED", true) {
		t.Fatal("repo maintenance kill switch should default to enabled")
	}

	t.Setenv("CORDY_GC_REPO_MAINTENANCE_ENABLED", "false")
	if boolFromEnv("CORDY_GC_REPO_MAINTENANCE_ENABLED", true) {
...
func TestPatternsFromEnv_DropsSeparatorBearingEntries(t *testing.T) {
	t.Setenv("CORDY_GC_ARTIFACT_PATTERNS", "node_modules, .next ,foo/bar, ../etc, ,target")
	got := patternsFromEnv("CORDY_GC_ARTIFACT_PATTERNS", nil)
...
func TestIsOfficialCloudServer(t *testing.T) {
	for _, tc := range []struct {
		...
		{"canonical cloud with trailing slash stripped", "https://api.cordy.ai/", true},
		{"canonical cloud case-insensitive", "https://API.Cordy.AI", true},
		{"cloud over plain http (unusual but match host)", "http://api.cordy.ai", true},
		...
		{"third-party host is self-host", "https://cordy.example.com", false},
		// Staging / preview / future subdomains deliberately follow the
		// safer self-host default until explicitly opted in.
		{"cordy.ai apex is not the api host", "https://cordy.ai", false},
		{"staging subdomain is self-host", "https://staging.cordy.ai", false},
		{"preview subdomain is self-host", "https://api-preview.cordy.ai", false},
...
func stageFakeAgent(t *testing.T) string {
	...
	t.Setenv("PATH", binDir)
	t.Setenv("CORDY_DAEMON_ID", "11111111-1111-1111-1111-111111111111")
	...
	t.Setenv("CORDY_DAEMON_AUTO_UPDATE", "")
	return binDir
}
...
func TestLoadConfig_DiscoversQwenCode(t *testing.T) {
	...
	t.Setenv("SHELL", "/usr/bin/fish")
	t.Setenv("CORDY_QWEN_MODEL", "qwen3.8-max-preview")
	t.Setenv("CORDY_QWEN_ARGS", "--verbose --foo=bar")
...
func TestLoadConfig_SkipsCordyHooksShadowingAgentBinaries(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("POSIX shell not available on Windows")
	}
	...
	home := t.TempDir()
	t.Setenv("HOME", home)
	hooksDir := filepath.Join(home, ".cordy", "hooks")
...
func TestLoadConfig_SkipsCordyHooksFromLoginShellFallback(t *testing.T) {
	if runtime.GOOS == "windows" {
		t.Skip("POSIX shell not available on Windows")
	}
	sh := "/bin/sh"
...
func TestLoadConfig_CodexHandshakeTimeout(t *testing.T) {
	stageFakeAgent(t)
	t.Setenv("CORDY_CODEX_HANDSHAKE_TIMEOUT", "")
	...
	t.Setenv("CORDY_CODEX_HANDSHAKE_TIMEOUT", "47s")
	...
	t.Setenv("CORDY_CODEX_HANDSHAKE_TIMEOUT", "0")
	cfg, err = LoadConfig(Overrides{
		ServerURL:      "http://localhost:8080",
		WorkspacesRoot: t.TempDir(),
	})
...
// CORDY_CODEX_FIRST_TURN_TIMEOUT resolution (GH #3262 / #5959): unset and an
...
func TestLoadConfig_CodexFirstTurnNoProgressTimeout(t *testing.T) {
	stageFakeAgent(t)
	t.Setenv("CORDY_CODEX_FIRST_TURN_TIMEOUT", "")
	...
	t.Setenv("CORDY_CODEX_FIRST_TURN_TIMEOUT", "30m")
	cfg, err = LoadConfig(Overrides{
		ServerURL:      "http://localhost:8080",
		WorkspacesRoot: t.TempDir(),
	})
	...
	t.Setenv("CORDY_CODEX_FIRST_TURN_TIMEOUT", "0")
	cfg, err = LoadConfig(Overrides{
		ServerURL:      "http://localhost:8080",
		WorkspacesRoot: t.TempDir(),
	})
...
// edge (cordy-eve review on #6753): because the semantic-inactivity timer is
...
func TestLoadConfig_CodexFirstTurnTimeoutEqualToSemanticWarns(t *testing.T) {
	...
	const warnNeedle = "CORDY_CODEX_FIRST_TURN_TIMEOUT is greater than or equal to the semantic-inactivity timeout"
	...
	loadWithLoggedWarnings := func(t *testing.T, semantic, firstTurn string) string {
		...
		t.Setenv("CORDY_CODEX_SEMANTIC_INACTIVITY_TIMEOUT", semantic)
		t.Setenv("CORDY_CODEX_FIRST_TURN_TIMEOUT", firstTurn)
...
func TestLoadConfig_OpenCodeIdleWatchdog(t *testing.T) {
	stageFakeAgent(t)
	t.Setenv("CORDY_OPENCODE_IDLE_WATCHDOG", "")
	...
	t.Setenv("CORDY_OPENCODE_IDLE_WATCHDOG", "7m")
	cfg, err = LoadConfig(Overrides{
		ServerURL:      "http://localhost:8080",
		WorkspacesRoot: t.TempDir(),
	})
	...
	t.Setenv("CORDY_OPENCODE_IDLE_WATCHDOG", "0")
	cfg, err = LoadConfig(Overrides{
		ServerURL:      "http://localhost:8080",
		WorkspacesRoot: t.TempDir(),
	})
...
// daemon pointed at Cordy's hosted cloud keeps the historical opt-in
...
func TestLoadConfig_AutoUpdateDefault_CloudOn(t *testing.T) {
	...
	if !cfg.AutoUpdateEnabled {
		t.Fatalf("AutoUpdateEnabled = false for Cordy Cloud server, want true")
	}
}
...
func TestLoadConfig_AutoUpdateEnv_ForcesOnForSelfHost(t *testing.T) {
	stageFakeAgent(t)
	t.Setenv("CORDY_DAEMON_AUTO_UPDATE", "true")
	cfg, err := LoadConfig(Overrides{
		ServerURL:      "http://localhost:8080",
		WorkspacesRoot: t.TempDir(),
	})
	...
	if !cfg.AutoUpdateEnabled {
		t.Fatalf("AutoUpdateEnabled = false after explicit CORDY_DAEMON_AUTO_UPDATE=true, want true")
	}
}
...
func TestLoadConfig_AutoUpdateEnv_ForcesOffForCloud(t *testing.T) {
	stageFakeAgent(t)
	t.Setenv("CORDY_DAEMON_AUTO_UPDATE", "false")
	cfg, err := LoadConfig(Overrides{
		ServerURL:      "https://api.cordy.ai",
		WorkspacesRoot: t.TempDir(),
	})
	if err != nil {
		t.Fatalf("LoadConfig: %v", err)
	...
	if cfg.AutoUpdateEnabled {
		t.Fatalf("AutoUpdateEnabled = true after explicit CORDY_DAEMON_AUTO_UPDATE=false, want false")
	}
}
...
func TestLoadConfig_AutoReload_DefaultsOnEvenForSelfHost(t *testing.T) {
	stageFakeAgent(t)
	t.Setenv("CORDY_DAEMON_AUTO_UPDATE", "")
	t.Setenv("CORDY_DAEMON_AUTO_RELOAD", "")
	cfg, err := LoadConfig(Overrides{
		ServerURL:      "http://localhost:8080",
		WorkspacesRoot: t.TempDir(),
	})
...
func TestLoadConfig_AutoReload_NotGatedOnAutoUpdateEnv(t *testing.T) {
	...
	if !cfg.AutoReloadEnabled {
		t.Fatalf("CORDY_DAEMON_AUTO_UPDATE=false disabled auto-reload; the two switches are independent")
	}
}
...
func TestLoadConfig_AutoReload_OffSwitches(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			stageFakeAgent(t)
			t.Setenv("CORDY_DAEMON_AUTO_RELOAD", tc.env)
			overrides := tc.overrides
			overrides.ServerURL = "https://api.cordy.ai"
...
// resolvable via the daemon's PATH (or pinned to an explicit CORDY_*_PATH),
...
func TestLoadConfig_UsesCodexDesktopAppBundleFallback(t *testing.T) {
	...
	t.Setenv("SHELL", filepath.Join(t.TempDir(), "fish"))
	t.Setenv("CORDY_DAEMON_ID", "11111111-1111-1111-1111-111111111111")
	t.Setenv("CORDY_CODEX_MODEL", "gpt-5")
	pinNonCodexAgentsToMissingPaths(t)
...
// Cordy must resolve the bundled CLI under ChatGPT.app (and prefer it over
...
func TestLoadConfig_CodexDesktopFallbackDoesNotOverrideExplicitPath(t *testing.T) {
	...
	t.Setenv("SHELL", filepath.Join(t.TempDir(), "fish"))
	t.Setenv("CORDY_DAEMON_ID", "11111111-1111-1111-1111-111111111111")
	t.Setenv("CORDY_CODEX_PATH", filepath.Join(t.TempDir(), "missing-codex"))
	pinNonCodexAgentsToMissingPaths(t)
	fakeClaude := filepath.Join(t.TempDir(), "claude")
	if err := os.WriteFile(fakeClaude, []byte("#!/bin/sh\nexit 0\n"), 0o755); err != nil {
		t.Fatalf("write fake claude: %v", err)
	}
	t.Setenv("CORDY_CLAUDE_PATH", fakeClaude)
	...
	if got, ok := cfg.Agents["codex"]; ok {
		t.Fatalf("explicit missing CORDY_CODEX_PATH should not fall back to Desktop bundle, got %#v", got)
	}
}
...
func TestApplyOpenclawOverride_DoesNothingWhenNil(t *testing.T) {
	...
	t.Setenv("CORDY_OPENCLAW_PATH", "/before/openclaw")
	t.Setenv("OPENCLAW_STATE_DIR", "/before/state")

	applyOpenclawOverride(nil)

	if got := os.Getenv("CORDY_OPENCLAW_PATH"); got != "/before/openclaw" {
		t.Errorf("CORDY_OPENCLAW_PATH mutated: got %q, want /before/openclaw", got)
	}
...
func TestApplyOpenclawOverride_SetsBothWhenEnvUnset(t *testing.T) {
	t.Setenv("CORDY_OPENCLAW_PATH", "")
	t.Setenv("OPENCLAW_STATE_DIR", "")
	os.Unsetenv("CORDY_OPENCLAW_PATH")
	...
	if got := os.Getenv("CORDY_OPENCLAW_PATH"); got != "/from/config/openclaw" {
		t.Errorf("CORDY_OPENCLAW_PATH: got %q, want /from/config/openclaw", got)
	}
...
// CORDY_OPENCLAW_PATH already in their environment must not see the
...
func TestApplyOpenclawOverride_EnvWinsOverConfig(t *testing.T) {
	...
	t.Setenv("CORDY_OPENCLAW_PATH", "/from/env/openclaw")
	...
	if got := os.Getenv("CORDY_OPENCLAW_PATH"); got != "/from/env/openclaw" {
		t.Errorf("CORDY_OPENCLAW_PATH: env should win, got %q want /from/env/openclaw", got)
	}
...
// must not have their CORDY_OPENCLAW_PATH discovery path forcibly
...
func TestApplyOpenclawOverride_PartialFields_OnlySetsConfigured(t *testing.T) {
	...
	applyOpenclawOverride(&cli.OpenClawOverride{
		StateDir: "/from/config/state",
		// BinaryPath intentionally empty — must NOT call Setenv("CORDY_OPENCLAW_PATH", "")
	})

	if _, set := os.LookupEnv("CORDY_OPENCLAW_PATH"); set {
		t.Errorf("CORDY_OPENCLAW_PATH should remain unset when BinaryPath is empty; got %q", os.Getenv("CORDY_OPENCLAW_PATH"))
	}
	if got := os.Getenv("OPENCLAW_STATE_DIR"); got != "/from/config/state" {
		t.Errorf("OPENCLAW_STATE_DIR: got %q, want /from/config/state", got)
	}
...
func TestLoadConfig_BackendOverrides_MalformedConfigFileNonFatal(t *testing.T) {
	...
	cfgDir := filepath.Join(homeDir, ".cordy")
