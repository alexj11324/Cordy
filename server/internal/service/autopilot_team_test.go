...
func TestAutopilotTeamAttribution(t *testing.T) {
	...
	tests := []struct {
		...
		{"team assignee returns team id", db.Autopilot{AssigneeType: "team", AssigneeID: id}, id},
		{"team with invalid id returns zero", db.Autopilot{AssigneeType: "team", AssigneeID: pgtype.UUID{}}, pgtype.UUID{}},
		{"unset type defaults to non-team", db.Autopilot{AssigneeID: id}, pgtype.UUID{}},
	}
	for _, tc := range tests {
		t.Run(tc.name, func(t *testing.T) {
			got := autopilotTeamAttribution(tc.ap)
...
func TestFormatAdmissionReason(t *testing.T) {
	tests := []struct {
		...
		{"team archived", db.Autopilot{AssigneeType: "team"}, "agent is archived", "team leader agent is archived"},
		{"agent no runtime", db.Autopilot{AssigneeType: "agent"}, "agent has no runtime bound", "assignee agent has no runtime bound"},
		{"team no runtime", db.Autopilot{AssigneeType: "team"}, "agent has no runtime bound", "team leader agent has no runtime bound"},
