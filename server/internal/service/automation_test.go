...
func TestAutomationErrorType(t *testing.T) {
	cases := map[string]string{
		...
		"unexpected":                   "automation_error",
	}

	for reason, want := range cases {
		if got := automationErrorType(reason); got != want {
			t.Fatalf("automationErrorType(%q) = %q, want %q", reason, got, want)
		}
	}
}

func TestTaskFailureReasonForAutomationRun(t *testing.T) {
	cases := []struct {
		name string
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			if got := taskFailureReasonForAutomationRun(tc.task); got != tc.want {
				t.Fatalf("taskFailureReasonForAutomationRun() = %q, want %q", got, tc.want)
			}
		})
	}
}
...
func TestBuildIssueDescription_NoTriggerPayload(t *testing.T) {
	s := &AutomationService{}
	ap := db.Automation{Description: pgtype.Text{String: "do the thing", Valid: true}}
	...
	if !strings.Contains(got.String, "Automation run triggered at") {
...
func TestBuildIssueDescription_UsesTriggerTimezone(t *testing.T) {
	...
	if !strings.Contains(got.String, "Automation run triggered at 2026-05-26 09:00 Asia/Tokyo") {
...
func TestBuildIssueDescription_InvalidTriggerTimezoneFallsBackToUTC(t *testing.T) {
	...
	if !strings.Contains(got.String, "Automation run triggered at 2026-05-26 00:00 UTC") {
...
func TestBuildIssueDescription_WithWebhookPayload(t *testing.T) {
	s := &AutomationService{}
	ap := db.Automation{Description: pgtype.Text{String: "watch PRs", Valid: true}}
	...
	run := db.AutomationRun{Source: "webhook", TriggerPayload: payload, TriggeredAt: pgtype.Timestamptz{Time: time.Now().UTC(), Valid: true}}
	...
	idxItalic := strings.Index(got.String, "*Automation run triggered")
...
func TestBuildIssueDescription_WebhookSourceMissingEnvelope(t *testing.T) {
	...
	s := &AutomationService{}
	ap := db.Automation{Description: pgtype.Text{String: "thing", Valid: true}}
...
func TestBuildIssueDescription_NonWebhookSourceWithPayloadIgnored(t *testing.T) {
	...
	s := &AutomationService{}
	ap := db.Automation{Description: pgtype.Text{String: "thing", Valid: true}}
	run := db.AutomationRun{Source: "manual", TriggerPayload: []byte(`{"event":"x.y"}`), TriggeredAt: pgtype.Timestamptz{Time: time.Now().UTC(), Valid: true}}
...
// TestInterpolateTemplate covers the three behaviours that real automation
...
func TestInterpolateTemplate(t *testing.T) {
	s := &AutomationService{}
	run := db.AutomationRun{TriggeredAt: pgtype.Timestamptz{Time: time.Now().UTC(), Valid: true}}
	...
	cases := []struct {
	...
	}{
		{
			...
			ap:     db.Automation{Title: "fallback", IssueTitleTemplate: pgtype.Text{String: "probe — {{date}}", Valid: true}},
			expect: "probe — " + today,
		},
		{
		...
		{
			name:   "empty template falls back to automation title",
			ap:     db.Automation{Title: "fallback title", IssueTitleTemplate: pgtype.Text{Valid: false}},
			expect: "fallback title",
		},
		{
			name:   "template without placeholder is returned verbatim",
			ap:     db.Automation{Title: "fallback", IssueTitleTemplate: pgtype.Text{String: "static title", Valid: true}},
			expect: "static title",
		},
	}
	for _, tc := range cases {
