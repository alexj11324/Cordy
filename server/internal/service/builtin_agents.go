...
// PatrickSystemKey marks the workspace's built-in Chief of Staff agent. It is the
...
// PatrickDefaultName is the name the agent is created with. Owners may rename it;
// nothing server-side keys off the name, and the prompt is templated on
// whatever the current name is.
const PatrickDefaultName = "Patrick"

// patrickNamePlaceholder is substituted in the embedded prompt with the agent's
// current display name.
const patrickNamePlaceholder = "{{AGENT_NAME}}"
...
// "You are Patrick" here would contradict it the moment an owner renames the
// agent.
//
//go:embed builtin_agents/patrick/INSTRUCTIONS.md
var patrickSystemInstructions string

// patrickWorkspaceNotesSection introduces the workspace's own additions and states
...
const patrickWorkspaceNotesSection = `## Workspace notes
...
// PatrickSystemInstructions returns the product-owned half of Patrick's prompt for an
...
func PatrickSystemInstructions(displayName string) string {
	name := strings.TrimSpace(displayName)
	if name == "" {
		name = PatrickDefaultName
	}
	return strings.ReplaceAll(
		strings.TrimRight(patrickSystemInstructions, "\n"),
		patrickNamePlaceholder,
		name,
	)
}

// ComposePatrickInstructions layers the workspace's notes under the product-owned
// system instructions. workspaceNotes is agent.instructions — the only half a
// workspace can write.
func ComposePatrickInstructions(displayName, workspaceNotes string) string {
	system := PatrickSystemInstructions(displayName)
	notes := strings.TrimSpace(workspaceNotes)
	if notes == "" {
		return system
	}
	return system + "\n\n" + patrickWorkspaceNotesSection + "\n\n" + notes
}
