...
type automationQuotaFixture struct {
	...
	resetAt       time.Time
	policyLimit   int
	createRunArgs db.CreateAutomationRunParams
}

type automationQuotaMetricsRecorder struct {
	mu        sync.Mutex
	decisions map[string]int
}

func (m *automationQuotaMetricsRecorder) RecordAutomationQuotaDecision(action, source, result string) {
	m.mu.Lock()
	defer m.mu.Unlock()
	if m.decisions == nil {
...
func (m *automationQuotaMetricsRecorder) count(action, source, result string) int {
	m.mu.Lock()
	defer m.mu.Unlock()
	return m.decisions[action+"\x00"+source+"\x00"+result]
}

func newAutomationQuotaFixture(t *testing.T, action entitlement.Action, limit int) *automationQuotaFixture {
	t.Helper()
	pool := newResolveOriginatorPool(t)
	q := db.New(pool)
	workspaceIDString, publisherID, agentID, issueIDString := seedAttributionFixture(t, pool)
	automationIDString, _ := seedRunOnlyAutomation(t, pool, workspaceIDString, agentID, publisherID)
	...
	fixture := &automationQuotaFixture{
		pool: pool, queries: q, stub: stub, workspace: workspace,
		workspaceID: workspaceID, automationID: util.MustParseUUID(automationIDString),
		...
		policyLimit: limit,
		createRunArgs: db.CreateAutomationRunParams{
			AutomationID: util.MustParseUUID(automationIDString), Source: "api", Status: "running",
		},
	}
	fixture.service = &AutomationService{
		Queries: q, TxStarter: pool, Bus: events.New(), Entitlements: stub,
	}
	fixture.setPolicy(action, periodStart, periodEnd, limit)
	t.Cleanup(func() {
		pool.Exec(context.Background(), `DELETE FROM automation_quota_reservation WHERE workspace_id = $1`, workspaceID)
		pool.Exec(context.Background(), `DELETE FROM automation_quota_period WHERE workspace_id = $1`, workspaceID)
	})
	return fixture
}

func (f *automationQuotaFixture) setPolicy(action entitlement.Action, start, end time.Time, limit int) {
	f.stub.Set(f.workspace, entitlement.GateAutomationRuns, entitlement.Decision{
...
func TestAutomationQuotaDisabledDoesNotReadQuotaTables(t *testing.T) {
	...
	t.Run("off", func(t *testing.T) {
		stub := entitlementtest.New()
		stub.Set(workspace, entitlement.GateAutomationRuns, entitlement.Decision{
			Gate: entitlement.Gate{Action: entitlement.ActionOff},
		})
		svc := &AutomationService{Entitlements: stub} // Queries intentionally nil
		usage, err := svc.AutomationQuotaUsage(ctx, workspaceID)
	...
	t.Run("malformed", func(t *testing.T) {
		...
		svc := &AutomationService{Entitlements: stub} // missing interval; Queries intentionally nil
		usage, err := svc.AutomationQuotaUsage(ctx, workspaceID)
		if err != nil || usage.Enabled {
...
func TestAutomationQuotaRejectsUnknownExecutionSource(t *testing.T) {
	fixture := newAutomationQuotaFixture(t, entitlement.ActionEnforce, 1)
	_, _, err := fixture.service.createAutomationRunWithQuota(
		context.Background(), fixture.workspaceID, "manul", "invalid-source", fixture.createRunArgs,
	)
	if err == nil || !strings.Contains(err.Error(), "invalid automation execution source") {
		t.Fatalf("unknown source error = %v, want application validation", err)
	}
}

func TestAutomationQuotaEnforcesBoundaryAndFinalizesIdempotently(t *testing.T) {
	pool := newResolveOriginatorPool(t)
	ctx := context.Background()
	q := db.New(pool)
	...
	automationIDString, _ := seedRunOnlyAutomation(t, pool, workspaceIDString, agentID, publisherID)
	workspaceID := util.MustParseUUID(workspaceIDString)
	automationID := util.MustParseUUID(automationIDString)
	t.Cleanup(func() {
		pool.Exec(context.Background(), `DELETE FROM automation_quota_reservation WHERE workspace_id = $1`, workspaceID)
		pool.Exec(context.Background(), `DELETE FROM automation_quota_period WHERE workspace_id = $1`, workspaceID)
	})
	...
	limit := 2
	stub := entitlementtest.New()
	stub.Set(uuid.UUID(workspaceID.Bytes), entitlement.GateAutomationRuns, entitlement.Decision{
		Gate: entitlement.Gate{
	...
	svc := &AutomationService{Queries: q, TxStarter: pool, Bus: events.New(), Entitlements: stub}
	params := db.CreateAutomationRunParams{
		AutomationID: automationID, Source: "api", Status: "running",
	}

	runs := make([]db.AutomationRun, 0, limit)
	for i, key := range []string{"boundary-1", "boundary-2"} {
		run, _, err := svc.createAutomationRunWithQuota(ctx, workspaceID, "api", key, params)
		if err != nil {
			t.Fatalf("admission %d: %v", i+1, err)
		}
		runs = append(runs, run)
	}
	reused, wasReused, err := svc.createAutomationRunWithQuota(ctx, workspaceID, "api", "boundary-1", params)
	...
	_, _, err = svc.createAutomationRunWithQuota(ctx, workspaceID, "api", "boundary-3", params)
	...
	if _, err := settleAutomationQuota(ctx, q, runs[0].QuotaReservationID, true); err != nil {
		t.Fatalf("consume: %v", err)
	}
	if _, err := settleAutomationQuota(ctx, q, runs[0].QuotaReservationID, true); err != nil {
		t.Fatalf("duplicate consume: %v", err)
	}
	if _, err := settleAutomationQuota(ctx, q, runs[1].QuotaReservationID, false); err != nil {
	...
	if _, err := settleAutomationQuota(ctx, q, runs[0].QuotaReservationID, false); err != nil {
...
func TestAutomationQuotaConcurrentAdmissionNeverExceedsLimit(t *testing.T) {
	const attempts = 25
	const limit = 5
	fixture := newAutomationQuotaFixture(t, entitlement.ActionEnforce, limit)
	ctx := context.Background()
	...
	usage, err := fixture.service.AutomationQuotaUsage(ctx, fixture.workspaceID)
...
func TestAutomationQuotaObserveToEnforceAndOffFinalization(t *testing.T) {
	fixture := newAutomationQuotaFixture(t, entitlement.ActionObserve, 1)
	metrics := &automationQuotaMetricsRecorder{}
	fixture.service.QuotaMetrics = metrics
	ctx := context.Background()
	runs := make([]db.AutomationRun, 0, 2)
	for i := 0; i < 2; i++ {
		run, _, err := fixture.service.createAutomationRunWithQuota(
	...
	for _, run := range runs {
		if _, err := fixture.service.failAutomationRun(ctx, db.UpdateAutomationRunFailedParams{
	...
	if _, _, err := fixture.service.createAutomationRunWithQuota(
...
func TestAutomationQuotaConsumedCreateIssueCannotBeRefunded(t *testing.T) {
	fixture := newAutomationQuotaFixture(t, entitlement.ActionEnforce, 2)
	ctx := context.Background()

	cancelled, _, err := fixture.service.createAutomationRunWithQuota(
		ctx, fixture.workspaceID, "api", "consumed-then-cancelled", fixture.createRunArgs,
	)
	if err != nil {
		t.Fatalf("admit cancelled run: %v", err)
	}
	if _, err := settleAutomationQuota(ctx, fixture.queries, cancelled.QuotaReservationID, true); err != nil {
	...
	deleted, _, err := fixture.service.createAutomationRunWithQuota(
		ctx, fixture.workspaceID, "api", "consumed-then-deleted", fixture.createRunArgs,
	)
	if err != nil {
		t.Fatalf("admit deleted run: %v", err)
	}
	deleted, err = fixture.queries.UpdateAutomationRunIssueCreated(ctx, db.UpdateAutomationRunIssueCreatedParams{
		ID: deleted.ID, IssueID: fixture.issueID,
	})
	if err != nil {
		t.Fatalf("link issue-created run: %v", err)
	}
	if _, err := settleAutomationQuota(ctx, fixture.queries, deleted.QuotaReservationID, true); err != nil {
		t.Fatalf("consume deleted run: %v", err)
	}
	if err := fixture.service.FailAutomationRunsByIssue(ctx, fixture.issueID); err != nil {
...
func TestAutomationQuotaTerminalUpdateNeedsNoTransactionStarter(t *testing.T) {
	...
	serviceWithoutTx := &AutomationService{Queries: fixture.queries}
	updated, err := serviceWithoutTx.completeAutomationRun(ctx, db.UpdateAutomationRunCompletedParams{ID: run.ID})
	if err != nil {
...
func TestAutomationQuotaSettlementStaysInOriginalPeriod(t *testing.T) {
	fixture := newAutomationQuotaFixture(t, entitlement.ActionEnforce, 1)
	ctx := context.Background()
	run, _, err := fixture.service.createAutomationRunWithQuota(
	...
	if _, err := fixture.service.completeAutomationRun(ctx, db.UpdateAutomationRunCompletedParams{ID: run.ID}); err != nil {
		t.Fatalf("complete after period rollover: %v", err)
	}

	first, err := fixture.queries.GetAutomationQuotaPeriod(ctx, db.GetAutomationQuotaPeriodParams{
	...
	second, err := fixture.service.AutomationQuotaUsage(ctx, fixture.workspaceID)
...
func TestAutomationQuotaWorkspaceIsolation(t *testing.T) {
	first := newAutomationQuotaFixture(t, entitlement.ActionEnforce, 1)
	second := newAutomationQuotaFixture(t, entitlement.ActionEnforce, 1)
	ctx := context.Background()
	if _, _, err := first.service.createAutomationRunWithQuota(
		ctx, first.workspaceID, "api", "isolated", first.createRunArgs,
	); err != nil {
		t.Fatalf("first workspace admission: %v", err)
	}
	_, _, err := first.service.createAutomationRunWithQuota(
	...
	if _, _, err := second.service.createAutomationRunWithQuota(
...
func TestAutomationQuotaReconcilerSettlesTerminalOnlyOnce(t *testing.T) {
	fixture := newAutomationQuotaFixture(t, entitlement.ActionEnforce, 2)
	ctx := context.Background()
	terminal, _, err := fixture.service.createAutomationRunWithQuota(
		ctx, fixture.workspaceID, "api", "reconcile-terminal", fixture.createRunArgs,
	)
	if err != nil {
		t.Fatalf("admit terminal run: %v", err)
	}
	active, _, err := fixture.service.createAutomationRunWithQuota(
	...
	if _, err := fixture.queries.UpdateAutomationRunCompleted(ctx, db.UpdateAutomationRunCompletedParams{ID: terminal.ID}); err != nil {
	...
	for i := 0; i < 2; i++ {
		go func() {
			settled, err := fixture.service.ReconcileAutomationQuotaReservations(
	...
	activeAfter, err := fixture.queries.GetAutomationRun(ctx, active.ID)
...
func TestAutomationQuotaReconcilerReleasesOnlyAbandonedManualOrAPIRuns(t *testing.T) {
	fixture := newAutomationQuotaFixture(t, entitlement.ActionEnforce, 4)
	ctx := context.Background()
	create := func(source, key string) db.AutomationRun {
		t.Helper()
	...
	if _, err := fixture.queries.CreateAutomationTask(ctx, db.CreateAutomationTaskParams{
		...
		TriggerEvidenceKind:  pgtype.Text{String: "automation_run", Valid: true},
		TriggerEvidenceRefID: taskBackedAPI.ID,
	}); err != nil {
		t.Fatalf("create unlinked task evidence: %v", err)
	}
	if _, err := fixture.pool.Exec(ctx, `
		UPDATE automation_quota_reservation
	...
	manualAfter, err := fixture.queries.GetAutomationRun(ctx, staleManual.ID)
	if err != nil || manualAfter.Status != "failed" {
		t.Fatalf("stale manual run = %+v, %v; want failed", manualAfter, err)
	}
	for name, run := range map[string]db.AutomationRun{
		"durable webhook": staleWebhook,
		"task-backed api": taskBackedAPI,
		"fresh api":       freshAPI,
	} {
		after, err := fixture.queries.GetAutomationRun(ctx, run.ID)
...
func TestAutomationQuotaIdempotencyRecoversOrphanedReservation(t *testing.T) {
	fixture := newAutomationQuotaFixture(t, entitlement.ActionEnforce, 1)
	ctx := context.Background()
	first, _, err := fixture.service.createAutomationRunWithQuota(
		ctx, fixture.workspaceID, "api", "orphaned-key", fixture.createRunArgs,
	)
	if err != nil {
		t.Fatalf("first admission: %v", err)
	}
	if _, err := fixture.pool.Exec(ctx, `DELETE FROM automation_run WHERE id = $1`, first.ID); err != nil {
		t.Fatalf("remove quota-linked run: %v", err)
	}
	recovered, reused, err := fixture.service.createAutomationRunWithQuota(
		ctx, fixture.workspaceID, "api", "orphaned-key", fixture.createRunArgs,
	)
...
func TestAutomationQuotaSchedulePersistsSkippedRun(t *testing.T) {
	pool := newResolveOriginatorPool(t)
	ctx := context.Background()
	q := db.New(pool)
	workspaceIDString, publisherID, agentID, _ := seedAttributionFixture(t, pool)
	...
	automation, err := q.GetAutomation(ctx, util.MustParseUUID(automationIDString))
	if err != nil {
		t.Fatalf("load automation: %v", err)
	}
	var triggerID pgtype.UUID
	if err := pool.QueryRow(ctx, `
		INSERT INTO automation_trigger (automation_id, kind, enabled, cron_expression, timezone)
		VALUES ($1, 'schedule', true, '0 * * * *', 'UTC') RETURNING id`, automation.ID).Scan(&triggerID); err != nil {
	...
	run, err := svc.DispatchAutomationForPlan(ctx, automation, triggerID, "schedule", nil, start.Add(time.Minute))
	if err != nil {
