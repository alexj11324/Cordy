...
// CORDY_LLM_API_KEY / CORDY_LLM_BASE_URL, and matches how chat auto-titling
// already degrades.
...
// so it inherited the Cordy runtime brief's identity and drifted toward
...
func (s *TaskService) GenerateChatQuickActionsForTask(ctx context.Context, task db.AgentTaskQueue, origin ChatQuickActionsOrigin) error {
	...
	raw, err := s.QuickActions.GenerateJSON(ctx,
		"", // deployment default: CORDY_LLM_DEFAULT_MODEL, else llm.FallbackModel
