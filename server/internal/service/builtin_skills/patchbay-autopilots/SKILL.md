---
name: patchbay-autopilots
description: "Use when creating, updating, inspecting, triggering, or debugging a Patchbay autopilot (scheduled, webhook, or manual)."
...
```bash
patchbay autopilot list --output json
patchbay autopilot get <autopilot-id> --output json
patchbay autopilot runs <autopilot-id> --output json
```
...
patchbay autopilot list --output json
patchbay autopilot get <autopilot-id> --output json
patchbay autopilot create --title "<title>" --description "<task prompt>" --agent <agent-name-or-id> --mode create_issue|run_only --output json
patchbay autopilot update <autopilot-id> --status active|paused --output json
patchbay autopilot runs <autopilot-id> --output json
patchbay autopilot trigger-add <autopilot-id> --kind schedule --cron "0 9 * * *" --timezone Asia/Shanghai --output json
patchbay autopilot trigger-add <autopilot-id> --kind webhook --label "ci" --output json
patchbay autopilot trigger <autopilot-id> --output json
patchbay autopilot trigger-rotate-url <autopilot-id> <trigger-id> --yes --output json
```
...
1. `patchbay autopilot get <id> --output json` — status, mode, assignee, triggers.
2. `patchbay autopilot runs <id> --output json` — run status and failure reason.
3. If assigned to a squad, inspect the squad: `patchbay squad get <squad-id> --output json`; execution goes to the leader.
