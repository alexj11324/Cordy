...
to import a specific skill into the current Orvilo workspace.
...
A skill is installed for Orvilo only when it exists in the current workspace's
...
environment, not the Orvilo workspace DB, so Orvilo cannot manage or bind it.
...
Incorrect (bypasses Orvilo):

```bash
npx skills add https://skills.sh/owner/repo/skill
```

The skill may exist locally, but Orvilo cannot manage it as a workspace skill.
