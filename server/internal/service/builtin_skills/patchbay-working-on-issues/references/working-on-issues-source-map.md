...
## `patchbay issue pull-requests` — read PR links from Patchbay

| Behavior | File:line | Drifted from |
|---|---|---|
| CLI command `pull-requests <id>` (alias `prs`) | `server/cmd/patchbay/cmd_issue.go:105` | `:104` |
| `runIssuePullRequests` handler | `server/cmd/patchbay/cmd_issue.go:507` | new citation |
| Calls `GET /api/issues/<id>/pull-requests` | `server/cmd/patchbay/cmd_issue.go:522` | `:522` (unchanged) |
...
| Behavior | Source |
|---|---|
| `issue assign --no-start`, `issue update --no-start`, and `issue status --no-start` send `suppress_run=true` | `server/cmd/patchbay/cmd_issue.go` (`runIssueAssign`, `runIssueUpdate`, `runIssueStatus`) |
...
| `--stage` on `issue create` / `issue update` | `server/cmd/patchbay/cmd_issue.go:328,350` |
...
| Behavior | File:line |
|---|---|
| `patchbay issue metadata set <issue-id> --key --value [--type]` | `server/cmd/patchbay/cmd_issue_metadata.go:80,109-111` |
| `patchbay issue metadata delete <issue-id> --key` | `server/cmd/patchbay/cmd_issue_metadata.go:93,113` |
...
| Behavior | File:line |
|---|---|
| `patchbay property list/get/create/update/archive/unarchive` | `server/cmd/patchbay/cmd_property.go` |
| `patchbay issue property list/set/unset` (name→id translation) | `server/cmd/patchbay/cmd_property.go` (`encodeIssuePropertyValue`) |
...
```bash
cd server
grep -n 'pull-requests <id>'                 cmd/patchbay/cmd_issue.go
