...
description: "Use when creating, inspecting, or debugging a Orvilo agent definition via the `orvilo agent` CLI or POST /api/agents. Not for assigning issues to agents that already exist, and not for runtime task prompts."
...
This is the contract for Orvilo's agent-creation path: what the create entry
...
| `visibility` | `agent.visibility` | — | access control; defaults to `private`; gates who can read/route a private agent (e.g. a private team leader) — NOT the runtime prompt |
...
`orvilo workspace mcp list|add|update|remove`). At claim time every library
entry in that workspace is folded into every agent's payload — there is no
per-agent `agent_mcp_server` opt-in on that path.

Assignment commands (`orvilo agent mcp add|disable|remove`) still mutate
