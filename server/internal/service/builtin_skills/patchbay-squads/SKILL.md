---
name: patchbay-squads
description: "Use when creating, inspecting, updating, assigning to, or debugging a Patchbay squad, including how leader routing picks who runs."
...
patchbay squad get <squad-id> --output json
patchbay squad member list <squad-id> --output json
...
A Patchbay squad is a workspace routing and coordination object.
...
```bash
patchbay squad list --output json
patchbay squad get <squad-id> --output json
patchbay squad create --name <name> --leader <agent-name-or-id> --output json
patchbay squad update <squad-id> --instructions "<leader coordination policy>" --output json
patchbay squad delete <squad-id>
```
...
```bash
patchbay squad member list <squad-id> --output json
patchbay squad member add <squad-id> --member-id <id> --type agent|member --role <role> --output json
patchbay squad member remove <squad-id> --member-id <id> --type agent|member
patchbay squad member set-role <squad-id> --member-id <id> --member-type agent|member --role <role> --output json
```

Squad leader evaluation command:

```bash
patchbay squad activity <issue-id> action|no_action|failed --reason "<why>" --output json
```
...
For squad leader tasks, Patchbay appends a squad leader briefing to the leader
...
- recording squad activity with `patchbay squad activity`;
