...
This file records source evidence for `patchbay-squads/SKILL.md`.
...
```text
server/cmd/patchbay/cmd_squad.go
```
...
```bash
patchbay squad list
patchbay squad get <squad-id>
patchbay squad create
patchbay squad update <squad-id>
patchbay squad delete <squad-id>
patchbay squad activity <issue-id> <outcome>

patchbay squad member list <squad-id>
patchbay squad member add <squad-id>
patchbay squad member remove <squad-id>
...
server/cmd/patchbay/cmd_squad.go                   # runSquadActivity ~459
...
- parent status is not auto-advanced by the barrier: the system comment asks the
  ...
  `patchbay issue status <parent-id> in_review`. The Squad Operating Protocol's
