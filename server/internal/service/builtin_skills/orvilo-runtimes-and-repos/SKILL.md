...
The daemon injects a task-scoped `mat_` credential for Orvilo API commands and a private task-local Orvilo configuration root. Inside that managed task context:

- API commands such as `issue list`, `issue get`, and `issue runs` use the injected task identity and never fall back to the daemon Owner's saved Orvilo profile.
- `config show` and `config set` operate only on task-local Orvilo state. A missing task config root fails closed.
