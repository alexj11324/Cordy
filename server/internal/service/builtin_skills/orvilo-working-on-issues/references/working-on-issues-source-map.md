...
|---|---|---|
| CLI command `pull-requests <id>` (alias `prs`) | `server/cmd/orvilo/cmd_issue.go:105` | `:104` |
| `runIssuePullRequests` handler | `server/cmd/orvilo/cmd_issue.go:507` | new citation |
| Calls `GET /api/issues/<id>/pull-requests` | `server/cmd/orvilo/cmd_issue.go:522` | `:522` (unchanged) |
...
| Behavior | Source |
|---|---|
| `issue assign --no-start`, `issue update --no-start`, and `issue status --no-start` send `suppress_run=true` | `server/cmd/orvilo/cmd_issue.go` (`runIssueAssign`, `runIssueUpdate`, `runIssueStatus`) |
...
| `--value` name / email / id → `member:<uuid>` resolution (same member-only lookup used by owner resolution) | `server/cmd/orvilo/cmd_property.go` (`resolveActorPropertyRef`, `memberOnlyKinds`) |
...
```bash
cd server
grep -n 'pull-requests <id>'                 cmd/orvilo/cmd_issue.go
