---
name: cordy-squads
description: "Use when creating, inspecting, updating, assigning to, or debugging a Cordy squad, including how leader routing picks who runs."
...
cordy squad get <squad-id> --output json
cordy squad member list <squad-id> --output json
cordy issue comment list <issue-id> --roots-only --summary --output json
cordy issue comment list <issue-id> --thread <thread-id> --tail 30 --output json
```
...
```bash
cordy squad --help
cordy squad member --help
cordy issue update --help
...
A Cordy squad is a workspace routing and coordination object.
...
```bash
cordy squad list --output json
cordy squad get <squad-id> --output json
cordy squad create --name <name> --leader <agent-name-or-id> --output json
cordy squad update <squad-id> --instructions "<leader coordination policy>" --output json
cordy squad delete <squad-id>
```
...
```bash
cordy squad member list <squad-id> --output json
cordy squad member add <squad-id> --member-id <id> --type agent|member --role <role> --output json
cordy squad member remove <squad-id> --member-id <id> --type agent|member
cordy squad member set-role <squad-id> --member-id <id> --member-type agent|member --role <role> --output json
```

Squad leader evaluation command:

```bash
cordy squad activity <issue-id> action|no_action|failed --reason "<why>" --output json
```
...
```bash
cordy issue get <issue-id> --output json
cordy issue update <issue-id> --help
cordy issue comment list <issue-id> --roots-only --summary --output json
cordy issue comment add <issue-id> --help
```
...
For squad leader tasks, Cordy appends a squad leader briefing to the leader
...
members carry no skills segment. Builtin `cordy-*` skills are not listed —
...
- recording squad activity with `cordy squad activity`;
...
- Getting the leader briefing does NOT imply status authority. A squad
  ...
  delegation rules yes, `cordy issue status` no.
