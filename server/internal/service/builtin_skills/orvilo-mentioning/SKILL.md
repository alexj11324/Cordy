...
This skill states WHAT a mention link does in the Orvilo backend, traced to
...
at a project (see the orvilo-projects-and-resources skill); everything else in
...
It acts on two types only: the `team` branch resolves the team and adds its
...
contract above: only `agent` and `team` mentions enqueue work.
...
- **A private agent you cannot invoke:** blocked — the mention path gates on
  ...
  ungated — see the orvilo-teams skill.)
...
One nuance for automation (MUL-4857): when an UNATTRIBUTED automation run (a
schedule/webhook dispatch has no human originator, so the A2A gate has no human
to key on) delegates by `@mention` while working on the issue that automation
created, the invoke gate falls back to the **automation creator** as the effective
invoking user — the same principal that admitted the first dispatch. So a mid-run
`@agent` / `@team` delegation fires exactly when the automation creator could
...
delegating run's own task is the one working on that automation issue (author ==
task agent, `task.issue_id` == this issue), so a run doing work elsewhere can
never borrow another automation creator's authority by commenting on its issue.
The same authority carries the plain assigned-team-leader wake (a worker's
result comment on the automation issue can still wake the leader), and it survives
...
editing an agent's comment — CLEARS it. So editing an old automation comment from
