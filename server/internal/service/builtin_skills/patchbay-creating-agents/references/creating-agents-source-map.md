...
## CLI entry points — `server/cmd/patchbay/cmd_agent.go`
...
## Copy command — `server/cmd/patchbay/cmd_agent_copy.go`
...
| Codex catalog projection | `thinking.go` `parseCodexModelCatalog` / `annotateCodexExplicitStandardServiceTier` | Hidden models are excluded; visible model, reasoning, and `service_tiers` metadata are preserved. Codex 0.133.0+ entries also carry `supports_explicit_standard_service_tier=true`; older daemon responses omit the additive field, which the UI treats as false |
...
| Dynamic Codex token gate | `thinking.go` `IsKnownThinkingValue` | Server persistence accepts syntactically safe Codex/OpenCode/Kimi and ACP-catalog tokens so new catalog values do not require a Patchbay release; Pi instead uses its fixed seven-token CLI vocabulary. Exact support remains a daemon-local per-model check. |
...
| Service-tier validation | `thinking.go` `ValidateServiceTier` | Empty inherits local config. `default` requires the daemon-discovered explicit-standard capability and is model-independent; alternative tiers require the explicit Codex model to advertise the exact id, so an empty model fails closed because config.toml is unknown |
