...
// Set by the cordy binary at init() so the package doesn't depend on the
...
// APIClient is a REST client for the Cordy server API.
...
// reading a capped slice of the body. Every Cordy API helper funnels its
...
// It can be overridden with the CORDY_HTTP_TIMEOUT environment variable
...
// httpTimeout returns the HTTP client timeout, honoring CORDY_HTTP_TIMEOUT.
...
func httpTimeout() time.Duration {
	v := strings.TrimSpace(os.Getenv("CORDY_HTTP_TIMEOUT"))
	if v == "" {
...
// which honors CORDY_HTTP_TIMEOUT) plus a small grace margin, so a
...
// hardcoded duration so the deadline always respects CORDY_HTTP_TIMEOUT.
...
// CLI never hands an attachment URL to a native loader — `cordy attachment
