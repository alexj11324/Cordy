...
// context that truncated a longer ORVILO_HTTP_TIMEOUT.
func TestCommandContextNotTruncatedBelowHTTPTimeout(t *testing.T) {
	...
	t.Setenv("ORVILO_HTTP_TIMEOUT", "400ms")
...
// ORVILO_HTTP_TIMEOUT and the AtLeast floor.
func TestAPITimeoutRespectsEnv(t *testing.T) {
	...
	t.Run("AtLeast floor wins when larger", func(t *testing.T) {
		t.Setenv("ORVILO_HTTP_TIMEOUT", "10s")
	...
	t.Run("AtLeast budget wins when larger than floor", func(t *testing.T) {
		t.Setenv("ORVILO_HTTP_TIMEOUT", "120s")
	...
	// responsive, but it must still honor ORVILO_HTTP_TIMEOUT (it is not a
