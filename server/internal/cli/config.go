...
const (
	defaultCLIConfigPath = ".cordy/config.json"

	// TaskConfigRootEnv points daemon-managed CLI invocations at a private,
	// per-task Cordy config directory. It is deliberately Cordy-specific:
	...
	TaskConfigRootEnv = "CORDY_TASK_CONFIG_ROOT"
)
...
type CLIConfig struct {
	...
	// CORDY_DAEMON_DEVICE_NAME env, this field, os.Hostname().
	...
	// CORDY_AGENT_RUNTIME_NAME env, this field, the built-in
	// DefaultRuntimeName.
	...
	// --workspaces-root flag, CORDY_WORKSPACES_ROOT env, this field, the
	...
	// CORDY_DAEMON_MAX_CONCURRENT_TASKS env, this field, default.
	...
	// CORDY_DAEMON_POLL_INTERVAL env, this field, DefaultPollInterval.
	...
	// flag, CORDY_DAEMON_HEARTBEAT_INTERVAL env, this field,
	...
	// explicit 0), CORDY_AGENT_TIMEOUT env, this field,
	// DefaultAgentTimeout.
	...
	// CORDY_CODEX_SEMANTIC_INACTIVITY_TIMEOUT env, this field,
	...
	// CORDY_CODEX_HANDSHAKE_TIMEOUT env, this field,
	...
	// already resolves to enabled on Cordy Cloud. Absent / false means
	// "let env/default decide". Resolution precedence:
	// --no-auto-update flag, CORDY_DAEMON_AUTO_UPDATE=false env, this
	...
	// --auto-update-interval flag, CORDY_DAEMON_AUTO_UPDATE_INTERVAL
	...
	// cordy binary that was replaced on disk out of band. Single-direction
	...
	// --no-auto-reload flag, CORDY_DAEMON_AUTO_RELOAD=false env, this
	...
	// `cordy runtime profile set-path`; the daemon prefers it over the
...
//
//	BinaryPath: CORDY_OPENCLAW_PATH (env)  > backends.openclaw.binary_path > PATH lookup
//	StateDir:   OPENCLAW_STATE_DIR (env)     > backends.openclaw.state_dir   > OpenClaw's built-in default (~/.openclaw)
//	CLITimeout: CORDY_OPENCLAW_CLI_TIMEOUT (env) > backends.openclaw.cli_timeout > built-in default
//
// The StateDir env var here is OpenClaw's own OPENCLAW_STATE_DIR — NOT a new
// CORDY_OPENCLAW_STATE_DIR. Rationale: OpenClaw already honors its own env
...
// OpenClaw's own env, CORDY_OPENCLAW_STATE_DIR can be layered on top
...
type OpenClawOverride struct {
	...
	//
	// Resolution mirrors the fields above: CORDY_OPENCLAW_CLI_TIMEOUT (env) >
...
// An empty profile returns the default path (~/.cordy/config.json).
// A named profile returns ~/.cordy/profiles/<name>/config.json.
...
func CLIConfigPathForProfile(profile string) (string, error) {
	root, taskLocal, err := cordyConfigRoot()
	if err != nil {
	...
	return filepath.Join(root, ".cordy", "profiles", profile, "config.json"), nil
}

// ProfileDir returns the base directory for a profile's state files (pid, log).
// An empty profile returns ~/.cordy/. A named profile returns ~/.cordy/profiles/<name>/.
...
func ProfileDir(profile string) (string, error) {
	...
	if profile == "" {
		...
		return filepath.Join(root, ".cordy"), nil
	}
	if taskLocal {
		return filepath.Join(root, "profiles", profile), nil
	}
	return filepath.Join(root, ".cordy", "profiles", profile), nil
}

func cordyConfigRoot() (root string, taskLocal bool, err error) {
...
func validateTaskLocalProfile(profile string) error {
	...
	if profile == "." || profile == ".." || filepath.IsAbs(profile) || strings.ContainsAny(profile, `/\\`) || filepath.Clean(profile) != profile {
		return fmt.Errorf("invalid task-local Cordy profile name %q", profile)
	}
	return nil
}
