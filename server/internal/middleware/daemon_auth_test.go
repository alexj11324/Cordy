...
// daemon traffic today uses ovy_ PATs (mdt_ minting isn't wired up yet), so
...
func TestDaemonAuth_PATCacheHit(t *testing.T) {
	...
	const rawToken = "ovy_daemon_pat_cache_hit_test"
	hash := auth.HashToken(rawToken)
...
// through to the ovy_/JWT paths (an mcn_ string would never match a
