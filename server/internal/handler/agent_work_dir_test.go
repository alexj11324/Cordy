...
func TestRelativeWorkDir(t *testing.T) {
	...
	tests := []struct {
		...
		},
		{
			...
			workDir:  "/Users/alice/cordy_workspaces/" + wsID + "/" + taskSeg + "/workdir",
			wsID:     wsID,
			taskID:   taskID,
		...
		},
		{
			...
			workDir:  "/Users/alice/cordy_workspaces/" + wsID + "/" + taskSeg,
			wsID:     wsID,
			taskID:   taskID,
		...
		},
		{
			...
			workDir:  "/Users/df007df/code/work/projects/cordy/foo",
			wsID:     wsID,
			taskID:   taskID,
			expected: "code/work/projects/cordy/foo",
		},
		{
		...
		},
		{
			...
			workDir:  `C:\Users\alice\cordy_workspaces\` + wsID + `\` + taskSeg + `\workdir`,
			wsID:     wsID,
			taskID:   taskID,
			expected: wsID + "/" + taskSeg + "/workdir",
		},
		{
			...
			wsID:     "",
			taskID:   taskID,
			expected: "cordy_workspaces/" + wsID + "/" + taskSeg + "/workdir",
		},
		{
...
func TestTaskToResponseDerivesPrivateDurableWorkDir(t *testing.T) {
	response := taskToResponse(db.AgentTaskQueue{
		DurableWorkDir: pgtype.Text{
			String: "/Users/alice/repos/cordy",
			Valid:  true,
		},
	}, "")

	if response.DurableWorkDir != "/Users/alice/repos/cordy" {
		t.Fatalf("durable_work_dir = %q, want absolute clipboard value", response.DurableWorkDir)
	}
	if response.RelativeDurableWorkDir != "repos/cordy" {
