...
const onboardingAssistantDescription = "Built-in workspace assistant. Answers Orvilo questions and runs CLI operations."
...
// Orvilo Helper agent created by this shim. Pre-v3 desktop submits a
...
const onboardingAssistantInstructions = `You are Orvilo Helper, the built-in AI assistant for this Orvilo workspace. Your role is to help any member use Orvilo better — answer questions, give advice, and execute workspace operations on their behalf.
...
const onboardingIssueDescription = `Welcome to Orvilo.

This is your guided first run. Orvilo Helper is the executor for this issue and will help you try the core workflow:

1. Read Orvilo Helper's first comment.
2. Reply with something you want to build, fix, write, or plan.
3. @mention Orvilo Helper when you want it to continue.
...
//
// Creates or reuses one "Orvilo Helper" agent on the supplied runtime,
...
func (h *Handler) BootstrapOnboardingNoRuntime(w http.ResponseWriter, r *http.Request) {
	...
	if foundIssue {
		...
		issue, err = qtx.CreateIssue(r.Context(), db.CreateIssueParams{
			...
			Status:        "todo",
			Priority:      "high",
			OwnerType:     pgtype.Text{String: "member", Valid: true},
			OwnerID:       parseUUID(userID),
			CreatorType:   "member",
			CreatorID:     parseUUID(userID),
			ParentIssueID: emptyUUID,
...
func enNoRuntimeIssueDescription() string {
	return strings.Join([]string{
		...
		"",
		"When the runtime is connected, you can create Orvilo Helper for a guided first run.",
	}, "\n")
}
...
func zhNoRuntimeIssueDescription() string {
	return strings.Join([]string{
		...
		"",
		"运行时连上后，你就可以创建 Orvilo Helper，开始一次有智能体参与的上手引导。",
	}, "\n")
}
