...
import (
	...
	"github.com/cordy-ai/cordy/server/internal/issueguard"
...
const (
	onboardingAssistantName       = "Cordy Helper"
	onboardingIssueTitle          = "Start here: learn Cordy with Cordy Helper"
	onboardingAgentCreationSource = "cordy_helper"
...
const onboardingAssistantDescription = "Built-in workspace assistant. Answers Cordy questions and runs CLI operations."
...
// Cordy Helper agent created by this shim. Pre-v3 desktop submits a
...
const onboardingAssistantInstructions = `You are Cordy Helper, the built-in AI assistant for this Cordy workspace. Your role is to help any member use Cordy better — answer questions, give advice, and execute workspace operations on their behalf.
...
Your toolbox is the ` + "`cordy`" + ` CLI. It's already on your PATH and authenticated as the workspace owner.

Your full capability surface = whatever ` + "`cordy --help`" + ` shows. Run ` + "`cordy --help`" + ` first, then ` + "`cordy <command> --help`" + ` for any subcommand; use ` + "`--output json`" + ` for structured data. The CLI is your manifest — never invent commands or flags.
...
const onboardingIssueDescription = `Welcome to Cordy.

This is your guided first run. Cordy Helper is assigned to this issue and will help you try the core workflow:

1. Read Cordy Helper's first comment.
2. Reply with something you want to build, fix, write, or plan.
3. @mention Cordy Helper when you want it to continue.
...
//
// Creates or reuses one "Cordy Helper" agent on the supplied runtime,
...
func enNoRuntimeIssueDescription() string {
	return strings.Join([]string{
		"Welcome to Cordy.",
		"",
		"Agents need a runtime before they can execute work. You can still use Cordy as a lightweight project-management workspace while you install one.",
		"",
		"## Try Cordy first",
		"",
		...
		"",
		"Full guide: https://cordy.ai/docs/install-agent-runtime",
		"",
		...
		"   which codex",
		"   codex --version",
		"5. Restart the Cordy daemon:",
		"   cordy daemon restart",
		...
		"",
		"When the runtime is connected, you can create Cordy Helper for a guided first run.",
	}, "\n")
}
...
func zhNoRuntimeIssueDescription() string {
	return strings.Join([]string{
		"欢迎来到 Cordy。",
		"",
		"智能体需要先连上运行时才能执行工作。运行时还没准备好时，你也可以先把 Cordy 当作轻量项目管理工具体验起来。",
		"",
		"## 先体验项目管理功能",
		"",
		"运行时安装前，你可以先做这些事：",
		...
		"## 安装第一个 Agent 运行时",
		"",
		"完整文档：https://cordy.ai/docs/install-agent-runtime",
		"",
		"中文用户建议先装 Kimi CLI：",
		"",
		...
		"",
		"运行时连上后，你就可以创建 Cordy Helper，开始一次有智能体参与的上手引导。",
	}, "\n")
}
