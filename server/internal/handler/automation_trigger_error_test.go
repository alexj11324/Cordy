...
// TestAutomationQuotaManualAndWebhookEnforcement.
func TestTriggerAutomation_InternalFailureDoesNotEchoError(t *testing.T) {
	...
	stub.Set(uuid.MustParse(testWorkspaceID), entitlement.GateAutomationRuns, entitlement.Decision{
	...
	dbfx.Insert(t, "automation_quota_reservation", testutil.Cols{
		...
		"source":               "manual",
		"idempotency_key":      "manual:" + automationID + ":" + idempotencyKey,
		"state":                "consumed",
	})

	req := testutil.WithHeaders(
		newRequest("POST", "/api/automations/"+automationID+"/trigger", nil),
		"Idempotency-Key", idempotencyKey,
	)
	res := testutil.Call(t, testHandler.TriggerAutomation, withURLParam(req, "id", automationID)).
		Want(http.StatusInternalServerError)

	if got := res.Map()["error"]; got != "failed to trigger automation" {
