...
func TestMentionAgent_RejectsCrossWorkspaceAgentUUID(t *testing.T) {
	...
	// Patchbay's mention format is markdown-linked: [@Name](mention://agent/<uuid>).
...
func TestShouldEnqueueOnComment_PrivateAgentGate(t *testing.T) {
	...
	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			got := testHandler.shouldEnqueueExecutorFallback(ctx, issue, tc.actorType, tc.actorID, commentTriggerComputeOptions{})
			if got != tc.want {
