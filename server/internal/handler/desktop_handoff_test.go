...
func TestDesktopHandoffInitiateAcceptsWorktreeCallbackProtocols(t *testing.T) {
	...
	for _, protocol := range []string{"patchbay", "patchbay-canary-5718c47b86bf9ece", "patchbay-staging-5718c47b86bf9ece"} {
		binding.CallbackProtocol = protocol
		if !validateDesktopHandoffInitiate(binding) {
			t.Fatalf("owned protocol %q rejected", protocol)
		}
	}
	for _, protocol := range []string{"evil-app", "patchbay-canary", "patchbay-canary-01zp-25", "patchbay-staging"} {
		binding.CallbackProtocol = protocol
		if validateDesktopHandoffInitiate(binding) {
