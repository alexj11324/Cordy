...
// from GitHub issue patchbay-ai/patchbay#3264: a PR that mentions three issues
...
func TestWebhook_MergedPR_ChildWithParent_NotifiesParent(t *testing.T) {
	...
	// Parent has no executor in this fixture, so the routing mentions stay
	// absent. Behavior for executor-bearing parents is covered in
	// issue_child_done_test.go (MUL-2538 Option C).
	for _, banned := range []string{"mention://agent/", "mention://member/", "mention://team/"} {
		if strings.Contains(content, banned) {
			t.Errorf("system comment must not include %q mention (parent has no executor), got: %s", banned, content)
		}
	}
}
