...
import (
	...
	"github.com/cordy-ai/cordy/server/internal/channelmedia"
...
// translates the prompt into a `cordy issue create` invocation in the
...
// the agent's `cordy issue create` invocation passes `--project <uuid>`
...
// metadata column is JSONB on the wire; the daemon stores the cordy CLI
...
func (h *Handler) BatchUpdateIssues(w http.ResponseWriter, r *http.Request) {
	...
	// count match reality. See cordy-ai/cordy#1660.
