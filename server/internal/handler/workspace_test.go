...
VALUES ($1, 123456789, 'cordy-ai', 'cordy', 3366, 987654321, 'abc123', 15368, 'completed', now())
`, wsID)
	githubPRID := dbfx.Insert(t, "github_pull_request", testutil.Cols{
		...
		"state":           "open",
		"html_url":        "https://github.com/cordy-ai/cordy/pull/5265",
...
`, triggerName).Scan(&definition)
		if !strings.Contains(definition, "cordy.workspace_teardown") {
...
func TestWorkspaceTeardownModeDoesNotLeakIntoOrdinaryDeletes(t *testing.T) {
	...
	if _, err := tx.Exec(ctx, `SELECT set_config('cordy.workspace_teardown', 'on', true)`); err != nil {
		_ = tx.Rollback(ctx)
	...
	if err := conn.QueryRow(ctx, `SELECT current_setting('cordy.workspace_teardown', true)`).Scan(&teardownMode); err != nil {
...
`, wsID, testUserID)
	...
	t.Run("normalizes valid repos", func(t *testing.T) {
		req := newRequest("PATCH", "/api/workspaces/"+wsID, map[string]any{
			"repos": []map[string]any{
				{
					"url":         "  https://github.com/cordy-ai/cordy.git  ",
				...
				{
					"url": "git@github.com:cordy-ai/cordy-cloud.git",
				},
			},
		})
		req = withURLParam(req, "id", wsID)
		...
		if repos[0].URL != "https://github.com/cordy-ai/cordy.git" || repos[0].Description != "main monorepo" {
			t.Fatalf("first repo not normalized: %+v", repos[0])
		}
		if repos[1].URL != "git@github.com:cordy-ai/cordy-cloud.git" {
...
`, wsID, testUserID)
	...
	targetEmail := fmt.Sprintf("revocation-%s@cordy.ai", slug)
...
`, fx.WorkspaceID, fx.AgentID, appID, testUserID).Scan(&installID)
	...
	dbfx.Exec(t, `
INSERT INTO channel_user_binding (workspace_id, cordy_user_id, installation_id, channel_type, channel_user_id)
...
`, wsID, testUserID)
	...
	targetUserID := dbfx.Insert(t, "user", testutil.Cols{
		...
		"email": "revocation-no-runtimes@cordy.ai",
	})
	t.Cleanup(func() {
		_, _ = testPool.Exec(context.Background(), `DELETE FROM workspace WHERE id = $1`, wsID)
		_, _ = testPool.Exec(context.Background(), `DELETE FROM "user" WHERE id = $1`, targetUserID)
