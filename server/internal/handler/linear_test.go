...
func TestValidateLinearWebhookBindsFreshTimestampAndDelivery(t *testing.T) {
	...
	if err != nil || event.WebhookID != "hook" || delivery != "delivery" {
		t.Fatalf("event=%+v delivery=%q err=%v", event, delivery, err)
	}
	request.Header.Set("Linear-Timestamp", "1700000000001")
	if _, _, err = validateLinearWebhook("secret", request.Header, body, now); err == nil {
		t.Fatal("timestamp mismatch was accepted")
	}
	request.Header.Set("Linear-Timestamp", "1700000000000")
	if _, _, err = validateLinearWebhook("secret", request.Header, body, now.Add(61*time.Second)); err == nil {
