...
// automation service skipped the run (e.g. runtime offline); the delivery
// row itself records `dispatched` and links the skipped run via
// automation_run_id, because from the ingress's perspective we DID hand
// the payload to the automation machinery.
const (
...
// WebhookEnvelope is the canonical shape stored in automation_run.trigger_payload
...
// HandleAutomationWebhook is the public entry point for webhook-triggered
// automations. It runs OUTSIDE the authenticated route group: the bearer
...
//
// Response shapes:
//   - 200 {"status":"accepted",  "delivery_id", "run_id", "automation_id", "trigger_id"}
...
func (h *Handler) HandleAutomationWebhook(w http.ResponseWriter, r *http.Request) {
	...
	// 4. Cross-check automation/workspace consistency BEFORE we persist the
	...
	if err != nil {
		...
		slog.Error("webhook: automation lookup failed",
			"error", err,
	...
	if dup {
		...
		runID := delivery.AutomationRunID
		if !runID.Valid {
			run, runErr := h.Queries.GetAutomationRunByWebhookDelivery(r.Context(), delivery.ID)
			switch {
			case runErr == nil:
				runID = run.ID
	...
	// 9. Trigger disabled / automation paused / archived → ignored. We return
	...
	run, err := h.AutomationService.AdmitAutomationWebhookDelivery(
		r.Context(),
		automation,
		trigRow.ID,
		envelopeBytes,
	...
	if err != nil {
		...
		writeError(w, http.StatusInternalServerError, "failed to admit automation")
		return
	}
...
// skipped to) an automation_run. Same response-capture as the terminal path.
...
// once per finalise call so the dashboard counts line up with automation_run
// volume.
