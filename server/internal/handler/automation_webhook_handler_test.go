...
func createWebhookTestAutomation(t *testing.T, agentID, status, mode string) string {
	t.Helper()
	var apID string
	if err := testPool.QueryRow(context.Background(), `
...
func createWebhookTriggerViaHandler(t *testing.T, automationID string) AutomationTriggerResponse {
	t.Helper()
	w := httptest.NewRecorder()
	req := newRequest("POST", "/api/automations/"+automationID+"/triggers", map[string]any{
		"kind": "webhook",
	})
	req = withURLParam(req, "id", automationID)
	testHandler.CreateAutomationTrigger(w, req)
	if w.Code != http.StatusCreated {
		t.Fatalf("CreateAutomationTrigger: expected 201, got %d body=%s", w.Code, w.Body.String())
	}
...
// republishes the automation's rule version with the acting member as publisher,
...
func TestCreateTrigger_RepublishesRuleVersionAtomically(t *testing.T) {
	...
	// The test automation is inserted directly (no v1), so no rule version exists yet.
	if _, err := testHandler.Queries.GetActiveAutomationRuleVersion(ctx, verParams); err == nil {
	...
	ver, err := testHandler.Queries.GetActiveAutomationRuleVersion(ctx, verParams)
	if err != nil {
	...
	testHandler.CreateAutomationTrigger(w, req)
	if w.Code != http.StatusCreated {
		t.Fatalf("schedule CreateAutomationTrigger: got %d body=%s", w.Code, w.Body.String())
	}
	ver2, err := testHandler.Queries.GetActiveAutomationRuleVersion(ctx, verParams)
	if err != nil {
...
func createWebhookTriggerWithFilters(t *testing.T, automationID string, filters []WebhookEventFilter) AutomationTriggerResponse {
	t.Helper()
	w := httptest.NewRecorder()
	req := newRequest("POST", "/api/automations/"+automationID+"/triggers", map[string]any{
		"kind":          "webhook",
...
func TestWebhookHandler_AllowsDeclaredEvent(t *testing.T) {
	...
	if delivery.Status != deliveryStatusDispatched || !delivery.AutomationRunID.Valid {
		t.Fatalf("expected worker dispatch, got status=%s run=%v", delivery.Status, delivery.AutomationRunID.Valid)
	}
}
...
func TestCreateWebhookTrigger_EventFiltersRoundTripAsJSONArray(t *testing.T) {
	...
	w := httptest.NewRecorder()
	req := newRequest("POST", "/api/automations/"+apID+"/triggers", body)
	req = withURLParam(req, "id", apID)
	testHandler.CreateAutomationTrigger(w, req)
	if w.Code != http.StatusCreated {
	...
	var typed AutomationTriggerResponse
...
func TestUpdateWebhookTrigger_ExplicitEmptyArrayClearsFilters(t *testing.T) {
	...
	w := httptest.NewRecorder()
	req := newRequest("PATCH", "/api/automations/"+apID+"/triggers/"+created.ID, map[string]any{
		"event_filters": []any{},
	})
	req = withURLParams(req, "id", apID, "triggerId", created.ID)
	testHandler.UpdateAutomationTrigger(w, req)
	if w.Code != http.StatusOK {
		t.Fatalf("expected 200, got %d body=%s", w.Code, w.Body.String())
	}
	var updated AutomationTriggerResponse
	...
	row, err := testHandler.Queries.GetAutomationTrigger(context.Background(), parseUUID(created.ID))
	if err != nil {
		t.Fatalf("load: %v", err)
	}
	env := WebhookEnvelope{
...
func TestCreateAutomationTrigger_RejectsInvalidEventFilter(t *testing.T) {
...
func TestCreateAutomationTrigger_RejectsEventFiltersOnSchedule(t *testing.T) {
...
func postWebhook(t *testing.T, token string, body any, headers map[string]string) *httptest.ResponseRecorder {
	...
	req := httptest.NewRequest("POST", "/api/webhooks/automations/"+token, &buf)
	...
	w := httptest.NewRecorder()
	testHandler.HandleAutomationWebhook(w, req)
	return w
}
...
func TestCreateWebhookTrigger_PublicURLAffectsResponse(t *testing.T) {
	...
	if !strings.HasPrefix(*respURL.WebhookURL, "https://app.example/api/webhooks/automations/") {
...
func TestWebhookHandler_DisabledTriggerReturnsIgnored(t *testing.T) {
	...
	if _, err := testHandler.Queries.UpdateAutomationTrigger(context.Background(), db.UpdateAutomationTriggerParams{
...
func TestWebhookHandler_PausedAutomationReturnsIgnored(t *testing.T) {
	...
	var resp map[string]any
	json.Unmarshal(w.Body.Bytes(), &resp)
	if resp["reason"] != "automation_paused" {
		t.Fatalf("reason: %v", resp["reason"])
	}
}
...
func TestWebhookHandler_ActiveDispatchesRunWithPayload(t *testing.T) {
	...
	if got := uuidToString(delivery.AutomationRunID); got != runID {
		t.Fatalf("delivery run_id mismatch: got %q want %q", got, runID)
	}

	// Validate the persisted run carries the normalized envelope.
	run, err := testHandler.Queries.GetAutomationRun(context.Background(), parseUUID(runID))
	if err != nil {
	...
	trigRow, err := testHandler.Queries.GetAutomationTrigger(context.Background(), parseUUID(trig.ID))
	if err != nil {
...
func TestWebhookHandler_ActiveSkippedRunReturnsLegacyContract(t *testing.T) {
	...
	delivery := processQueuedWebhookDelivery(t, deliveryID)
	if delivery.Status != deliveryStatusDispatched || uuidToString(delivery.AutomationRunID) != runID {
		t.Fatalf("skipped run was not linked to delivery: status=%s run_id=%s", delivery.Status, uuidToString(delivery.AutomationRunID))
	}
}
...
func TestWebhookHandler_GitHubHeaderInferredEvent(t *testing.T) {
	...
	delivery := processQueuedWebhookDelivery(t, requireAcceptedWebhookResponse(t, w))
	runID := uuidToString(delivery.AutomationRunID)
	run, err := testHandler.Queries.GetAutomationRun(context.Background(), parseUUID(runID))
	if err != nil {
		t.Fatalf("load run: %v", err)
	}
...
func TestRotateWebhookToken_ReplacesOldToken(t *testing.T) {
	...
	w := httptest.NewRecorder()
	req := newRequest("POST", fmt.Sprintf("/api/automations/%s/triggers/%s/rotate-webhook-token", apID, trig.ID), nil)
	req = withURLParams(req, "id", apID, "triggerId", trig.ID)
	testHandler.RotateAutomationTriggerWebhookToken(w, req)
	if w.Code != http.StatusOK {
		t.Fatalf("rotate: expected 200, got %d body=%s", w.Code, w.Body.String())
	}
	var rotated AutomationTriggerResponse
...
func TestWebhookHandler_ArchivedAutomationReturnsIgnored(t *testing.T) {
	agentID := createWebhookTestAgent(t, "WebhookArchived Agent")
	apID := createWebhookTestAutomation(t, agentID, "active", "run_only")
	trig := createWebhookTriggerViaHandler(t, apID)

	if _, err := testPool.Exec(context.Background(),
		`UPDATE automation SET status = 'archived' WHERE id = $1`, apID); err != nil {
	...
	var resp map[string]any
	json.Unmarshal(w.Body.Bytes(), &resp)
	if resp["status"] != "ignored" || resp["reason"] != "automation_archived" {
		t.Fatalf("expected ignored/automation_archived, got %#v", resp)
	}
}
...
func TestWebhookHandler_IPRateLimitReturns429BeforeDBLookup(t *testing.T) {
	...
	post := func(token string) int {
		req := httptest.NewRequest("POST", "/api/webhooks/automations/"+token,
...
func TestWebhookHandler_DBErrorOnTokenLookupReturns500(t *testing.T) {
	...
	// automation_webhook.go must show `errors.Is(err, pgx.ErrNoRows)` —
...
func TestCreateAutomationTrigger_RejectsAPIKind(t *testing.T) {
...
func TestCreateAutomationTrigger_RejectsWebhookWithTimezone(t *testing.T) {
...
func TestUpdateAutomationTrigger_RejectsCronExpressionOnWebhookKind(t *testing.T) {
	// Round-2 should-fix: UpdateAutomationTrigger must mirror create-path
	...
	w := httptest.NewRecorder()
	req := newRequest("PATCH", "/api/automations/"+apID+"/triggers/"+trig.ID, map[string]any{
		"cron_expression": "0 0 * * *",
	})
	req = withURLParams(req, "id", apID, "triggerId", trig.ID)
	testHandler.UpdateAutomationTrigger(w, req)
...
func TestUpdateAutomationTrigger_RejectsTimezoneOnWebhookKind(t *testing.T) {
...
func TestUpdateAutomationTrigger_AcceptsEnabledAndLabelOnWebhookKind(t *testing.T) {
...
func TestGetAutomationRun_ReturnsFullPayload(t *testing.T) {
	...
	reqList := newRequest("GET", "/api/automations/"+apID+"/runs", nil)
	reqList = withURLParam(reqList, "id", apID)
	testHandler.ListAutomationRuns(wList, reqList)
	...
	reqDetail := newRequest("GET", "/api/automations/"+apID+"/runs/"+runID, nil)
	reqDetail = withURLParams(reqDetail, "id", apID, "runId", runID)
	testHandler.GetAutomationRun(wDetail, reqDetail)
...
// NOTE: the cross-workspace paranoia branch in automation_webhook.go
// (uuidToString(automation.WorkspaceID) != uuidToString(trigRow.AutomationWorkspaceID))
// is defense-in-depth against a TOCTOU race between the joined token
// lookup and the follow-up GetAutomation read. It is not reachable from
// any valid SQL state — the two reads compute against the same
// automation.workspace_id column — and would require a mock-able
