...
// channel table-of-contents (`cordy chat history`), Thread reads one thread's
// messages (`cordy chat thread [id]`). Both are scoped server-side to the
...
// GetChatChannelHistory serves `cordy chat history` — the channel overview:
// recent top-level messages, each thread tagged with its id + reply count (no
// thread contents). The agent drills into a thread with `cordy chat thread`.
//
...
// paging contract. It backs `cordy chat history` for sessions with no IM
// channel (web chat, Feishu, WeCom, DingTalk), whose history lives only in
// Cordy — there is no platform to read back. It pages through the same
...
// GetChatThread serves `cordy chat thread [id]` — one thread's messages. With
