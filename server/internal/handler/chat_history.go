...
// channel table-of-contents (`orvilo chat history`), Thread reads one thread's
// messages (`orvilo chat thread [id]`). Both are scoped server-side to the
...
// GetChatChannelHistory serves `orvilo chat history` — the channel overview:
// recent top-level messages, each thread tagged with its id + reply count (no
// thread contents). The agent drills into a thread with `orvilo chat thread`.
//
...
// paging contract. It backs `orvilo chat history` for sessions with no IM
...
// GetChatThread serves `orvilo chat thread [id]` — one thread's messages. With
...
// ovy_ PAT leaves X-Actor-Source empty and does NOT strip a client-forged
