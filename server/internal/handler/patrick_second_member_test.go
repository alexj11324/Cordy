...
// workspace. Patrick is per workspace but sessions are per member, so the second
...
func createPatrickAs(t *testing.T, userID string, body any) *httptest.ResponseRecorder {
	t.Helper()
	req := withChatTestWorkspaceCtx(t, newRequestAs(userID, "POST", "/api/agents/patrick", body))
	w := httptest.NewRecorder()
	testHandler.CreatePatrickAgent(w, req)
	return w
}

func decodePatrick(t *testing.T, w *httptest.ResponseRecorder) patrickAgentResponse {
	t.Helper()
	var resp patrickAgentResponse
	if err := json.Unmarshal(w.Body.Bytes(), &resp); err != nil {
		t.Fatalf("decode patrick response: %v (body %s)", err, w.Body.String())
	}
	return resp
}

// TestStartPatrickOnboarding_AllowsAnyMemberNotJustTheOwner is the race the
// CreatePatrickAgent advisory lock exists to survive, followed one call further.
//
// Uniqueness worked: the second member got the first member's Patrick. But
// StartPatrickOnboarding then required the agent owner, so the member who lost
// the race received a valid agent, opened a valid session, and could not start
// onboarding at all — a 403 with no way forward.
func TestStartPatrickOnboarding_AllowsAnyMemberNotJustTheOwner(t *testing.T) {
	if testHandler == nil {
	...
	owner := createPatrick(t, map[string]any{
	...
	if joined.Code != http.StatusOK {
		t.Fatalf("second member: expected the existing Patrick (200), got %d: %s", joined.Code, joined.Body.String())
	}
	resp := decodePatrick(t, joined)
	if resp.SystemKey != service.PatrickSystemKey {
		t.Fatalf("second member did not get the workspace Patrick: %+v", resp)
	}
...
// TestCreatePatrickAgent_RecoversFromAPartialBootstrap is the retry path for the
...
func TestCreatePatrickAgent_RecoversFromAPartialBootstrap(t *testing.T) {
	if testHandler == nil {
		t.Skip("database not available")
	}
	cleanupPatrick(t)
	...
	first := decodePatrick(t, createPatrick(t, map[string]any{
	...
	if retryResp.ID != first.ID {
		t.Fatalf("retry must reuse the same Patrick, got %s then %s", first.ID, retryResp.ID)
	}
}

// TestCreatePatrickAgent_SessionIsPerMemberAndStable covers the other half: the
...
// language-dependent. It is now (workspace, member, Patrick) server-side.
func TestCreatePatrickAgent_SessionIsPerMemberAndStable(t *testing.T) {
	if testHandler == nil {
		t.Skip("database not available")
	}
	cleanupPatrick(t)
	...
	if err := testPool.QueryRow(context.Background(),
		...
		t.Fatalf("load Patrick session origin: %v", err)
	}
	if !explicitlyCreated {
		t.Fatal("Patrick onboarding session must be marked as an explicit first-party Chat")
	}
	t.Cleanup(func() {
		testPool.Exec(context.Background(), `DELETE FROM chat_session WHERE creator_id = $1`, testUserID)
	})
	...
	second := createPatrick(t, map[string]any{
		"runtime_id": runtimeID, "language": "zh", "session_title": "开始使用 Patrick",
	})
	if second.Code != http.StatusOK {
		t.Fatalf("second call: expected 200, got %d: %s", second.Code, second.Body.String())
	}
	secondResp := decodePatrick(t, second)
	...
	if otherResp.ID != firstResp.ID {
		t.Fatalf("both members must share one Patrick, got %s and %s", firstResp.ID, otherResp.ID)
	}
