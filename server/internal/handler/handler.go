...
import (
	...
	composio "github.com/cordy-ai/cordy/server/internal/integrations/composio"
	"github.com/cordy-ai/cordy/server/internal/integrations/dingtalk"
	"github.com/cordy-ai/cordy/server/internal/integrations/ghsnapshot"
...
type Config struct {
	...
	// CORDY_VCS_SECRET_KEY is set. It is the product boundary: the feature is
	// intended for self-hosted Cordy only (where Cordy and the Git instance
	...
	// CORDY_VCS_INTEGRATION_ENABLED; the self-host compose defaults it on.
	VCSIntegrationEnabled bool
	// PublicURL is the absolute base URL the API is reachable at from the
	// public internet, with no trailing slash (e.g. "https://cordy.ai").
	...
	// CORDY_TRUSTED_PROXIES env var (comma-separated CIDRs, e.g.
	...
	//   - LLMAPIKey       -> CORDY_LLM_API_KEY
	//   - LLMBaseURL       -> CORDY_LLM_BASE_URL (OpenAI or any compatible gateway)
	//   - LLMDefaultModel  -> CORDY_LLM_DEFAULT_MODEL (used when a request omits `model`)
	//   - LLMMaxRetries    -> CORDY_LLM_MAX_RETRIES (transport retry budget)
	LLMAPIKey       string
	LLMBaseURL      string
	LLMDefaultModel string
	// LLMMaxRetries is the parsed CORDY_LLM_MAX_RETRIES budget. nil means
...
type Handler struct {
	...
	// (CORDY_LARK_SECRET_KEY) is unset; the corresponding HTTP
	...
	// CORDY_LARK_SECRET_KEY is set; tests that need a no-op
	...
	// (Feishu when CORDY_LARK_SECRET_KEY is set). The router does NOT
	...
	// + app tokens (MUL-3666). Nil unless CORDY_SLACK_SECRET_KEY is set.
	...
	// SlackHistory backs the agent-facing `cordy chat history` command: it
	...
	// "link your Cordy account" prompt sent to first-time WeCom users
	...
	// each bot's token. Nil unless CORDY_TELEGRAM_SECRET_KEY is set.
	...
	// either "run `cordy attachment upload`" or "describe the file in words"
	...
	// CORDY_VCS_SECRET_KEY is unset; the connect/webhook handlers return 503
...
func New(queries *db.Queries, txStarter txStarter, hub *realtime.Hub, bus *events.Bus, emailService *service.EmailService, store storage.Storage, cfSigner *auth.CloudFrontSigner, analyticsClient analytics.Client, cfg Config, daemonHubs ...*daemonws.Hub) *Handler {
	...
	// backs auto-titling. A deployment with no CORDY_LLM_* configuration gets
