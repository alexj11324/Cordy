...
func seedStatusCategoryFixture(t *testing.T) (projectID, customKey string) {
	...
	if _, err := testPool.Exec(ctx, `
		...
		VALUES
			($1, 'cat-a', $2,           'none', 'member', $3, 1, $4,     $5, 'agent', (SELECT id FROM agent WHERE workspace_id = $1 LIMIT 1)),
			($1, 'cat-b', $2,           'none', 'member', $3, 2, $4 + 1, $5, 'agent', (SELECT id FROM agent WHERE workspace_id = $1 LIMIT 1)),
			($1, 'cat-c', 'in_review',  'none', 'member', $3, 3, $4 + 2, $5, 'agent', (SELECT id FROM agent WHERE workspace_id = $1 LIMIT 1)),
