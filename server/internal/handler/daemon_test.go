...
// `cordy issue comment add` call) left the user staring at a "Completed"
// badge with no reply.
...
func createEphemeralUser(t *testing.T, label string) string {
	t.Helper()
	email := fmt.Sprintf("membership-cache-%s-%s@cordy.ai", label, uuid.NewString())
