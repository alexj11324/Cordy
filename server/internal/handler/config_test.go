...
func TestGetConfigIncludesRuntimeAuthConfig(t *testing.T) {
	...
	t.Setenv("CORDY_PUBLIC_URL", "https://api.example.com/")
	t.Setenv("CORDY_APP_URL", "https://app.example.com/")
...
func TestGetConfigUsesAppURLForSameOriginDaemonSetup(t *testing.T) {
	t.Setenv("CORDY_PUBLIC_URL", "")
	t.Setenv("CORDY_APP_URL", "https://cordy.internal.example/")
	...
	if cfg.DaemonServerURL != "https://cordy.internal.example" {
		t.Fatalf("daemon_server_url: want same-origin URL, got %q", cfg.DaemonServerURL)
	}
	if cfg.DaemonAppURL != "https://cordy.internal.example" {
...
func TestGetConfigUsesFrontendOriginForSameOriginDaemonSetup(t *testing.T) {
	t.Setenv("CORDY_PUBLIC_URL", "")
	t.Setenv("CORDY_APP_URL", "")
	t.Setenv("FRONTEND_ORIGIN", "https://cordy.internal.example/")
...
func TestGetConfigOmitsOfficialCloudDaemonSetup(t *testing.T) {
	t.Setenv("CORDY_PUBLIC_URL", "https://api.cordy.ai")
	t.Setenv("CORDY_APP_URL", "")
	t.Setenv("FRONTEND_ORIGIN", "https://cordy.ai")
...
// frontend is cordy.ai, but the deployment does not set CORDY_PUBLIC_URL to
// the api host. Previously this fell through to the same-origin branch and
// emitted daemon_server_url=https://cordy.ai, which the dialog turned into
// `cordy setup self-host --server-url https://cordy.ai` — pointing the
...
// setup URLs are omitted and the dialog falls back to `cordy setup`.
...
// cloud frontend when it is configured through CORDY_APP_URL.
func TestGetConfigOmitsCloudDaemonSetupForConfiguredAppURL(t *testing.T) {
	t.Setenv("CORDY_PUBLIC_URL", "")
	t.Setenv("CORDY_APP_URL", "https://cordy.ai")
...
func TestURLHostEqualsCanonicalizesCommonHostForms(t *testing.T) {
	tests := []struct {
		...
		{name: "full URL", raw: "https://api.cordy.ai", want: true},
		{name: "bare host", raw: "api.cordy.ai", want: true},
		{name: "host port", raw: "api.cordy.ai:8080", want: true},
		{name: "trailing dot", raw: "https://api.cordy.ai.", want: true},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if got := urlHostEquals(tt.raw, "api.cordy.ai"); got != tt.want {
...
func TestGetConfigExposesServerVersion(t *testing.T) {
	...
	t.Setenv("CORDY_APP_URL", "https://cordy.self-hosted.example")
	t.Setenv("FRONTEND_ORIGIN", "")
...
// suppressed on the managed cloud (frontend host cordy.ai) even when the
...
func TestGetConfigOmitsServerVersionOnOfficialCloud(t *testing.T) {
	...
	// Official cloud: frontend host cordy.ai -> version omitted.
	t.Setenv("CORDY_APP_URL", "https://cordy.ai")
	t.Setenv("FRONTEND_ORIGIN", "")
	w := httptest.NewRecorder()
	testHandler.GetConfig(w, req)
