...
// worker_agent_comment_wakes_team_leader case, which uses empty
...
func TestCreateComment_WorkerAgentCommentWakesTeamLeader_MUL4015(t *testing.T) {
...
// pins the dedup behavior: when the team leader ALREADY has a queued or
...
// TestCreateComment_WorkerAgentCommentWakesPrivateTeamLeader_MUL4015 pins
...
// production shape when the assigning member ALSO owns the team's leader.
//
// The failure mode is:
//
//   - Member M owns team leader L (private) and worker W (private).
//   - M assigns the issue to the team → L's task carries originator=M.
...
func TestCreateComment_WorkerAgentCommentWakesPrivateTeamLeader_MUL4015(t *testing.T) {
	if testHandler == nil || testPool == nil {
		t.Skip("database not available")
	}
	ctx := context.Background()
	...
	`, testWorkspaceID, testUserID, teamID).Scan(&issueID); err != nil {
		t.Fatalf("create private team issue: %v", err)
	}
	t.Cleanup(func() {
		testPool.Exec(context.Background(), `DELETE FROM agent_task_queue WHERE issue_id = $1`, issueID)
		testPool.Exec(context.Background(), `DELETE FROM comment WHERE issue_id = $1`, issueID)
	...
	if err := testPool.QueryRow(ctx, `
		INSERT INTO agent_task_queue (agent_id, runtime_id, issue_id, status, is_leader_task, originator_user_id, accountable_user_id, team_id)
		VALUES ($1, $2, $3, 'running', TRUE, $4, $4, $5)
		RETURNING id
	`, leaderID, leaderRuntimeID, issueID, testUserID, teamID).Scan(&leaderTaskID); err != nil {
		t.Fatalf("seed leader task: %v", err)
	}
