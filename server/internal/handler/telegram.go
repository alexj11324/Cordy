...
// Telegram user id carried by the token to the logged-in Patchbay user. The
...
func (h *Handler) RedeemTelegramBindingToken(w http.ResponseWriter, r *http.Request) {
	...
	if err != nil {
		...
		case errors.Is(err, telegram.ErrBindingAlreadyAssigned):
			writeError(w, http.StatusConflict, "this Telegram account is already bound to a different Patchbay user")
