...
// These tests drive the REAL automation update handlers to prove the substantive /
// cosmetic edit boundary that decides when trigger_owner responsibility transfers
// (MUL-4302; Elon must-fix). They deliberately go through UpdateAutomation /
// UpdateAutomationTrigger rather than calling SetAutomationTriggerPublisher* directly,
...
func seedTransferMember(t *testing.T, label string) string {
	...
	if err := testPool.QueryRow(ctx, `INSERT INTO "user" (name, email) VALUES ($1, $2) RETURNING id`,
		label, fmt.Sprintf("%s-%d@patchbay.test", label, time.Now().UnixNano())).Scan(&uid); err != nil {
...
func triggerPublisherMember(t *testing.T, triggerID string) string {
	t.Helper()
	row, err := testHandler.Queries.GetAutomationTrigger(context.Background(), parseUUID(triggerID))
	if err != nil {
...
func patchAutomation(t *testing.T, apID string, body map[string]any) {
	t.Helper()
	w := httptest.NewRecorder()
	req := newRequest("PATCH", "/api/automations/"+apID+"?workspace_id="+testWorkspaceID, body)
...
func patchTrigger(t *testing.T, apID, triggerID string, body map[string]any) {
	t.Helper()
	w := httptest.NewRecorder()
	req := newRequest("PATCH", "/api/automations/"+apID+"/triggers/"+triggerID, body)
	req = withURLParams(req, "id", apID, "triggerId", triggerID)
	testHandler.UpdateAutomationTrigger(w, req)
	if w.Code != http.StatusOK {
		t.Fatalf("UpdateAutomationTrigger: expected 200, got %d: %s", w.Code, w.Body.String())
	}
}

// TestUpdateAutomation_PromptEditTransfersAllTriggers: the automation description IS the
...
func TestUpdateAutomation_PromptEditTransfersAllTriggers(t *testing.T) {
	...
	patchAutomation(t, apID, map[string]any{"description": "Rewritten prompt: do the new thing"})
...
// TestUpdateAutomation_CosmeticTitleEditDoesNotTransfer: the title is a display label,
// not an instruction — editing it alone must NOT move responsibility.
func TestUpdateAutomation_CosmeticTitleEditDoesNotTransfer(t *testing.T) {
	...
	patchAutomation(t, apID, map[string]any{"title": "Renamed automation"})
...
// TestUpdateAutomationTrigger_SubstantiveEditTransfersOnlyThatTrigger: a cron/enabled
...
func TestUpdateAutomationTrigger_SubstantiveEditTransfersOnlyThatTrigger(t *testing.T) {
...
// TestUpdateAutomationTrigger_LabelOnlyEditDoesNotTransfer: label is cosmetic, so a
...
func TestUpdateAutomationTrigger_LabelOnlyEditDoesNotTransfer(t *testing.T) {
...
// TestUpdateAutomationTrigger_NoOpEditDoesNotTransfer: a PATCH that re-sends the trigger's
// current values changes nothing, so responsibility must stay put.
func TestUpdateAutomationTrigger_NoOpEditDoesNotTransfer(t *testing.T) {
