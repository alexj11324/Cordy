...
// MUL-6691 / GH #7563. A schedule/webhook automation run carries no top-of-chain
// human originator by design (MUL-4302), so canInvokeAgent fails closed for a
// private agent. MUL-4857 restored the automation's authority for ONE action —
// creating a child under the automation's own issue — which left the reported
// flow broken in two places:
//
//   - a run_only leader has no automation issue at all, so its top-level
...
// runOnlyAutomationFixture is the run_only shape: a member-created automation with
// a live run, and a dispatched leader task that carries automation attribution
...
// stampAutomationAttribution gives a task the attribution a production automation
// enqueue writes (MUL-4302): an accountable human, no originator, and the source
// label that proves automation lineage. The raw fixtures insert bare task rows.
func stampAutomationAttribution(t *testing.T, taskID, accountableUserID, source string) {
	t.Helper()
	dbfx.Exec(t, `
...
func TestCreateIssue_RunOnlyAutomationLeaderAssignsPrivateWorker(t *testing.T) {
	...
	t.Run("sub-issue under an issue the run created is admitted", func(t *testing.T) {
		...
		var created IssueResponse
		testutil.Call(t, testHandler.CreateIssue,
			automationChildIssueRequest(t, "agent", workerID, parentID, "todo", fx.LeaderAgentID, fx.LeaderTaskID),
		).Want(http.StatusCreated).JSON(&created)
		cleanupAutomationChildIssue(t, created.ID)
	})
		...
		cases := []struct {
		...
		}{
			{
				...
				setup: func(t *testing.T, workerID, ownerID, _ string) (string, string) {
					fx := newRunOnlyAutomationFixture(t, workerID, ownerID, testutil.Cols{"status": "completed"})
			...
			},
			{
				...
				setup: func(t *testing.T, workerID, ownerID, _ string) (string, string) {
					fx := newRunOnlyAutomationFixture(t, workerID, ownerID, testutil.Cols{"originator_source": "owner_fallback"})
					return fx.LeaderAgentID, fx.LeaderTaskID
				},
			},
			{
				...
				setup: func(t *testing.T, workerID, ownerID, _ string) (string, string) {
					fx := newRunOnlyAutomationFixture(t, workerID, ownerID, testutil.Cols{"originator_source": "delegation"})
					return fx.LeaderAgentID, fx.LeaderTaskID
				},
			},
			{
				...
				setup: func(t *testing.T, workerID, ownerID, _ string) (string, string) {
					fx := newRunOnlyAutomationFixture(t, workerID, ownerID, testutil.Cols{
			...
			},
			{
				...
				setup: func(t *testing.T, workerID, ownerID, _ string) (string, string) {
					...
					fx := newRunOnlyAutomationFixture(t, workerID, ownerID, testutil.Cols{"accountable_user_id": outsiderID})
					return fx.LeaderAgentID, fx.LeaderTaskID
				},
			},
			{
			...
			{
				// No automation run behind the task at all.
				name: "task belongs to no automation run",
				setup: func(t *testing.T, workerID, ownerID, _ string) (string, string) {
					fx := newRunOnlyAutomationFixture(t, workerID, ownerID, testutil.Cols{"automation_run_id": nil})
					return fx.LeaderAgentID, fx.LeaderTaskID
				},
			},
			{
				// The run's automation lives in another workspace.
				name: "automation run belongs to another workspace",
				setup: func(t *testing.T, workerID, ownerID, _ string) (string, string) {
					fx := newRunOnlyAutomationFixture(t, workerID, ownerID)
					foreignWorkspaceID := dbfx.Workspace(t, "MUL-6691 Foreign", "mul6691-foreign-"+workerID[:8])
					dbfx.Exec(t, `UPDATE automation SET workspace_id = $1 WHERE id = $2`, foreignWorkspaceID, fx.AutomationID)
					return fx.LeaderAgentID, fx.LeaderTaskID
				},
			},
			{
...
func TestUpdateIssue_AutomationLeaderAssignsPrivateWorker(t *testing.T) {
	if testHandler == nil || testPool == nil {
		t.Skip("database not available")
	}
	...
	// Review must-fix 2: automationDelegationAuthority performs no liveness or
	...
	// strict rule must NOT be rescued by the automation's creator having rights.
	t.Run("creator fallback does not reach the assign verb", func(t *testing.T) {
		cases := []struct {
			...
			// The automation creator (ownerID) owns the target throughout, so any
	...
	t.Run("run_only run cannot assign an issue created by a sibling run", func(t *testing.T) {
		...
		sibling := newRunOnlyAutomationFixture(t, workerID, ownerID)
...
func TestBatchUpdateIssues_AutomationAuthorityIsPerIssue(t *testing.T) {
	if testHandler == nil || testPool == nil {
		t.Skip("database not available")
	}
