...
// failure before the user actually has to re-run `orvilo login`.
...
//
// Only ovy_ PATs may be renewed: a cookie/JWT session has no PAT row to
...
func (h *Handler) RenewCurrentPersonalAccessToken(w http.ResponseWriter, r *http.Request) {
	...
	if rawToken == "" || rawToken == authHeader || !strings.HasPrefix(rawToken, "ovy_") {
