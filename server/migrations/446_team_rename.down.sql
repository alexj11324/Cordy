-- Revert the team rename. The migration is the symmetric rollback; each
-- rename is idempotent, so the operation is safe to retry if a previous
-- rollback partially completed.

ALTER INDEX IF EXISTS idx_activity_log_team_no_action_task
    RENAME TO idx_activity_log_squad_no_action_task;
...
WHERE action = 'squad_leader_evaluated' AND details ? 'team_id';
...
WHERE description LIKE '%mention://team/%' OR description LIKE '%@team%'
   OR instructions LIKE '%mention://team/%' OR description LIKE '%@team%';
