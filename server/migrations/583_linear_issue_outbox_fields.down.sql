-- Restore the function owned by migration 559. The trigger itself remains
-- installed until 559 is rolled back, so rolling back only this migration must
-- leave the already-applied schema at its previous behaviour.
