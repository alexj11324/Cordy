-- Rename the automation feature's persisted schema and discriminator values
-- to drop the legacy "autopilot" vocabulary. Historical migrations stay
-- immutable; this file moves any database that has already run them onto the
-- names the current Go mainline owns.
--
-- No FK or cascade is added. Existing historical FKs are renamed in place so
-- pg_constraint names match the live tables. Discriminator rewrites are
-- idempotent.
...
ALTER TABLE automation_run RENAME CONSTRAINT autopilot_run_squad_id_fkey TO automation_run_team_id_fkey;
