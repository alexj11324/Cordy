...
test("rate limits auth routes and sanitizes origin headers", async () => {
  ...
  const request = new Request("https://accounts.aspectlylabs.com/v1/desktop/google/attempt", { method: "POST", headers: { "cf-connecting-ip": "192.0.2.1", "x-forwarded-host": "evil.example", "x-orvilo-origin-auth": "attacker" }, body: "{}" });
  ...
  assert.equal(result.status, 200); assert.equal(forwarded.headers.get("x-orvilo-origin-auth"), token); assert.equal(forwarded.headers.get("x-forwarded-host"), "accounts.aspectlylabs.com"); assert.equal(forwarded.headers.get("cf-connecting-ip"), null);
});
