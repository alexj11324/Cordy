...
function response(status, error, extra = {}) { return Response.json({ error }, { status, headers: { "cache-control": "no-store", "x-orvilo-auth-contract-version": "1", ...extra } }); }
...
export default { async fetch(request, env) {
  ...
  const upstream = await fetch(new Request(target, init)); const outgoing = new Headers(upstream.headers); outgoing.set("x-orvilo-accounts-proxy", "canonical-origin"); return new Response(upstream.body, { status: upstream.status, statusText: upstream.statusText, headers: outgoing });
} };
