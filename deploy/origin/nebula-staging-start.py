...
def database_url(password):
    return 'postgres://orvilo:' + quote(password, safe='') + '@postgres:5432/orvilo?sslmode=disable'


def require_ready(response, source_sha):
    if response.status != 200:
        raise ValueError('Application is not ready')
    if response.headers.get('X-Orvilo-Commit') != source_sha:
        ...
        while True:
            ...
            try:
                request = Request(f'http://127.0.0.1:{port}{path}', headers={
                    'Host': 'orvilo-staging.nebula-spaces.com',
...
def main():
    ...
    secrets = json.loads(subprocess.check_output([
        ...
        '--secret=orvilo-nebula-staging', '--project=general-secrets-store',
    ], text=True))
    ...
    compose = ['docker', 'compose', '--project-name', 'orvilo-nebula-staging',
    ...
    network = json.loads(subprocess.check_output([
        'docker', 'network', 'inspect', 'orvilo-nebula-staging_default',
    ], text=True))[0]
    ...
    runtime = Path('/run/orvilo-nebula-staging')
    runtime.mkdir(mode=0o700, exist_ok=True)
    temporary = runtime / 'origin.tmp'
    fd = os.open(temporary, os.O_WRONLY | os.O_CREAT | os.O_TRUNC, 0o600)
    with os.fdopen(fd, 'w') as output:
        output.write(f'proxy_set_header X-Orvilo-Origin-Auth "{origin_token}";\n')
