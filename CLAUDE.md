...
- The Pro `@reui` registry is configured in `packages/ui/components.json`; add items with `pnpm ui:add @reui/<name>` and answer `n` to every overwrite prompt so local component customizations survive. `pnpm ui:add` loads `REUI_LICENSE_KEY` from `~/.orvilo/reui.env` (or `$XDG_CONFIG_HOME/orvilo/reui.env`) and writes gitignored `packages/ui/.env.local` for shadcn interpolation. Never write the key into a tracked repo file.
- ReUI ships source, not a dependency: keep directly adopted source under `packages/ui/components/reui/` and compositions under `packages/views/<domain>/`. Preserve the source DOM, class names, geometry, spacing, and interaction styling; adapt only imports, types, product data, i18n, and required package boundaries.
- Product-authored components use design tokens and semantic classes and avoid hardcoded colors. Their font sizes come from the role-named `--text-*` scale in `packages/ui/styles/tokens.css` (`text-caption`, `text-body`, `text-title`, …). Directly adopted ReUI source is exempt from that product-only scale so its visual contract stays intact.
