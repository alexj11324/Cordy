...
ALTER TABLE automation RENAME CONSTRAINT autopilot_priority_check TO automation_priority_check;
ALTER TABLE automation RENAME CONSTRAINT autopilot_status_check TO automation_status_check;
ALTER TABLE automation RENAME CONSTRAINT autopilot_execution_mode_check TO automation_execution_mode_check;
ALTER TABLE automation RENAME CONSTRAINT autopilot_concurrency_policy_check TO automation_concurrency_policy_check;
ALTER TABLE automation RENAME CONSTRAINT autopilot_created_by_type_check TO automation_created_by_type_check;
...
ALTER TABLE automation_trigger RENAME CONSTRAINT autopilot_trigger_kind_check TO automation_trigger_kind_check;
ALTER TABLE automation_trigger RENAME CONSTRAINT autopilot_trigger_provider_check TO automation_trigger_provider_check;
...
ALTER TABLE automation_run RENAME CONSTRAINT autopilot_run_source_check TO automation_run_source_check;
...
ALTER TABLE automation_subscriber RENAME CONSTRAINT autopilot_subscriber_user_type_check TO automation_subscriber_user_type_check;
ALTER TABLE automation_collaborator RENAME CONSTRAINT autopilot_collaborator_pkey TO automation_collaborator_pkey;
ALTER TABLE automation_collaborator RENAME CONSTRAINT autopilot_collaborator_user_type_check TO automation_collaborator_user_type_check;
ALTER TABLE automation_quota_period RENAME CONSTRAINT autopilot_quota_period_pkey TO automation_quota_period_pkey;
ALTER TABLE automation_quota_period RENAME CONSTRAINT autopilot_quota_period_used_count_check TO automation_quota_period_used_count_check;
ALTER TABLE automation_quota_period RENAME CONSTRAINT autopilot_quota_period_reserved_count_check TO automation_quota_period_reserved_count_check;
ALTER TABLE automation_quota_period RENAME CONSTRAINT autopilot_quota_period_check TO automation_quota_period_check;
...
ALTER TABLE automation_quota_reservation RENAME CONSTRAINT autopilot_quota_reservation_state_check TO automation_quota_reservation_state_check;
ALTER TABLE automation_quota_reservation RENAME CONSTRAINT autopilot_quota_reservation_check TO automation_quota_reservation_check;
...
COMMENT ON TABLE automation_rule_version IS
    'Append-only snapshot of Automation rule publishes (PB-4302 §3.4). One row per substantive publish (create / enable / resume / trigger-condition / target / instructions change), recording the publisher + effective-config summary. Dispatch resolves the latest row for an Automation as the run''s rule_owner accountable human. No FK, no cascade.';
COMMENT ON COLUMN automation_trigger.published_by_type IS
    'Actor type of the trigger''s current responsible publisher for this Automation: member | agent. Set to the creator at creation and re-stamped to the editor on any substantive edit governing this trigger. Consumed only for attribution (source=trigger_owner) — never authorization. NULL on pre-migration triggers (PB-4302).';
COMMENT ON COLUMN automation_trigger.published_by_id IS
    'The member/agent currently responsible for this Automation trigger''s effective config (creator, then last substantive editor). For a member this is the accountable human of runs the trigger fires (source=trigger_owner). No FK, app-layer integrity. NULL on pre-migration triggers, which degrade to rule_owner (PB-4302).';
...
ALTER INDEX IF EXISTS idx_autopilot_collaborator_user RENAME TO idx_automation_collaborator_user;
...
-- Migration 359 attached uq_autopilot_quota_period_scope to the primary-key
-- constraint, so the backing index is already named autopilot_quota_period_pkey.
