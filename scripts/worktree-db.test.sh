...
cat >"$local_env" <<'EOF'
POSTGRES_DB=cordy_feature_123
POSTGRES_USER=cordy
DATABASE_URL=postgres://cordy:cordy@localhost:5432/cordy_feature_123?sslmode=disable
EOF
...
require_contains "$docker_log" \
  "compose exec -T postgres dropdb --username cordy --maintenance-db postgres --if-exists --force -- cordy_feature_123"
require_contains "$output" "Dropped database 'cordy_feature_123'."
...
DATABASE_URL=postgres://cordy:cordy@localhost:5432/postgres
EOF
if printf 'y\n' | PATH="$stub_dir:$PATH" DOCKER_LOG="$docker_log" \
...
cat >"$main_env" <<'EOF'
POSTGRES_DB=cordy
DATABASE_URL=postgres://cordy:cordy@localhost:5432/cordy
EOF
if printf 'y\n' | PATH="$stub_dir:$PATH" DOCKER_LOG="$docker_log" \
...
POSTGRES_DB=cordy_worktree_456
POSTGRES_USER=cordy
DATABASE_URL=postgres://cordy:cordy@localhost:5432/cordy_worktree_456?sslmode=disable
EOF
...
require_contains "$docker_log" \
  "compose exec -T postgres dropdb --username cordy --maintenance-db postgres --if-exists --force -- cordy_worktree_456"
