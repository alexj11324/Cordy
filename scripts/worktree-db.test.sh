...
cat >"$local_env" <<'EOF'
POSTGRES_DB=orvilo_feature_123
POSTGRES_USER=orvilo
DATABASE_URL=postgres://orvilo:orvilo@localhost:5432/orvilo_feature_123?sslmode=disable
EOF
...
require_contains "$docker_log" \
  "compose exec -T postgres dropdb --username orvilo --maintenance-db postgres --if-exists --force -- orvilo_feature_123"
require_contains "$output" "Dropped database 'orvilo_feature_123'."
...
DATABASE_URL=postgres://orvilo:orvilo@localhost:5432/postgres
EOF
if printf 'y\n' | PATH="$stub_dir:$PATH" DOCKER_LOG="$docker_log" \
...
cat >"$main_env" <<'EOF'
POSTGRES_DB=orvilo
DATABASE_URL=postgres://orvilo:orvilo@localhost:5432/orvilo
EOF
if printf 'y\n' | PATH="$stub_dir:$PATH" DOCKER_LOG="$docker_log" \
...
POSTGRES_DB=orvilo_worktree_456
POSTGRES_USER=orvilo
DATABASE_URL=postgres://orvilo:orvilo@localhost:5432/orvilo_worktree_456?sslmode=disable
EOF
...
require_contains "$docker_log" \
  "compose exec -T postgres dropdb --username orvilo --maintenance-db postgres --if-exists --force -- orvilo_worktree_456"
