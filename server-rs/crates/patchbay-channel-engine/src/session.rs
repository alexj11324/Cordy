...
use patchbay_channel::{ChatType, MediaRef, MsgType};
use patchbay_db::dbid;
use patchbay_db::queries::attachment::{create_attachment, link_attachments_to_chat_message};
...
use patchbay_db::queries::workspace::lock_workspace_for_chat_session_create;
...
pub struct ChatSession {
    pool: PgPool,
    channel_type: patchbay_channel::Type,
...
impl ChatSession {
    ...
    pub fn new(pool: PgPool, channel_type: patchbay_channel::Type, titles: SessionTitles) -> Self {
        Self {
            pool,
            channel_type,
            titles,
    ...
    async fn clear_media_pending<'e, E: sqlx::Executor<'e, Database = sqlx::Postgres>>(
        ...
        patchbay_db::queries::chat::clear_chat_message_channel_media_pending(
            executor,
            input.message_id,
            input.session_id,
        )
    ...
    async fn bind_media_refs_inner(
        ...
        if let Some(issue_id) = input.issue_id {
            ...
            for media in &created {
                let block = patchbay_util::channel_media::block(
                    &media.id.to_string(),
                    &media.filename,
...
pub use patchbay_db::queries::channel::record_channel_media_pending_object;
