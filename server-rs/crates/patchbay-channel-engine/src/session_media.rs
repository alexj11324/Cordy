...
use patchbay_channel::MediaRef;
use uuid::Uuid;

#[cfg(test)]
use patchbay_util::channel_media as channelmedia;
...
// patchbay_util::channel_media (single source of truth with Go's
...
mod tests {
    ...
    fn inline_markdown_escapes_label_and_detects_image() {
        use patchbay_channel::{MediaRef, MsgType};
        let img = MediaRef {
    ...
    fn block_path_documented() {
        // Issue-side rendering intentionally routes through
        // patchbay_util::channel_media::block (same as Go channelmedia.Block).
