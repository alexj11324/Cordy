...
pub(crate) fn daemon_context_from_headers(headers: &HeaderMap) -> Option<DaemonContext> {
    let workspace_id = headers
        .get(patchbay_middleware::daemon_auth::DAEMON_WORKSPACE_HEADER)
        .and_then(|value| value.to_str().ok())
        .map(str::to_string);
    let daemon_id = headers
        .get(patchbay_middleware::daemon_auth::DAEMON_ID_HEADER)
        .and_then(|value| value.to_str().ok())
        .map(str::to_string);
...
impl<'a> Access<'a> {
    ...
    async fn get_member(&self, user_id: &str, ws_id: &str) -> Option<patchbay_db::models::Member> {
...
async fn require_daemon_task_access_with_workspace(
    ...
    daemon_ctx: Option<DaemonContext>,
    task_id: &str,
) -> Result<(patchbay_db::models::AgentTaskQueue, String), Response> {
...
async fn access_get_member(
    ...
    user_id: &str,
    ws_id: &str,
) -> Option<patchbay_db::models::Member> {
    let user = Uuid::parse_str(user_id).ok()?;
    let ws = Uuid::parse_str(ws_id).ok()?;
...
async fn merge_legacy_runtime(
    ...
    use patchbay_db::queries::runtime as rq;
    let mut tx = state
        .pool
        .begin()
        .await
...
impl DaemonRpcProcessor {
    ...
    fn identity_headers(identity: &ClientIdentity) -> Result<HeaderMap, RpcHandlerError> {
        ...
        if identity.daemon_id.is_empty() {
            ...
            insert(
                &mut headers,
                patchbay_middleware::daemon_auth::DAEMON_WORKSPACE_HEADER,
                &identity.primary_workspace_id(),
            )?;
            insert(
                &mut headers,
                patchbay_middleware::daemon_auth::DAEMON_ID_HEADER,
                &identity.daemon_id,
            )?;
        }
        insert(
        ...
        Ok(_) => {
            if let Some(trigger) = survivors.pop() {
                ...
                if let Err(e) = replayed {
                    if !patchbay_service::task_service::pending_slot_taken_err(&e) {
...
/// Enriched finalization: mints the task token AND the MCP daemon credential
...
async fn finalize_claim_enriched_full(
    ...
    let receipt = state
        ...
        .finalize_task_claim(
            task,
            patchbay_service::task_service::CreateTaskToken {
                token_hash: patchbay_auth::jwt::hash_token(&token_str),
                task_id: task.id,
                agent_id: task.agent_id,
                workspace_id,
...
fn claim_requires_daemon_mcp_token(payload: &Value) -> bool {
...
async fn resolve_task_skill_bundles(
    ...
    for r in &req.skills {
        ...
        if r.source == patchbay_service::skill_bundle::SOURCE_PLUGIN && found.hash != r.hash {
    ...
    if patchbay_task_failure::context_exhausted_completion(&req.output) {
        ...
        return fail_task_impl(
            ...
            FailBody {
                ...
                failure_reason: patchbay_task_failure::Reason::AGENT_CONTEXT_OVERFLOW.to_string(),
...
async fn report_task_usage(
    ...
    for u in &req.usage {
        ...
        if let Err(e) = patchbay_db::queries::task_usage::upsert_task_usage(
            &state.pool,
            task_uuid,
            &provider,
            &u.model,
...
pub(crate) fn task_message_payload(
    m: &patchbay_db::models::TaskMessage,
    ...
    payload.insert(
        "created_at".into(),
        Value::String(patchbay_util::rfc3339_nano(m.created_at)),
    );
...
async fn report_task_messages(
    ...
    for msg in &req.messages {
        ...
        let content = patchbay_service::redact::text(&msg.content);
        let output = patchbay_service::redact::text(&msg.output);
        let input = msg.input.as_ref().map(|m| {
            sanitize_json_for_postgres(Value::Object(patchbay_service::redact::input_map(m)))
        });
        ...
        .await
        {
            Ok(Some(created)) => {
                if !workspace_id.is_empty() {
                    state.bus.publish(&patchbay_events::Event {
                        event_type: patchbay_protocol::EVENT_TASK_MESSAGE.to_string(),
...
fn split_contribution_id(contribution: &str) -> Option<(String, String)> {
    let trimmed = contribution.strip_prefix(patchbay_remotemcp::PLUGIN_CONTRIBUTION_PREFIX)?;
...
async fn invoke_agent_plugin_hook(
    ...
    let (result, outcome) = patchbay_service::plugin_agent_tools::invoke_agent_hook(
        service,
...
async fn resolve_plugin_mcp_credential(
    ...
    match patchbay_service::plugin_mcp_transport::mcp_hook_credential(
        &state.plugins.pool,
...
async fn report_local_skill_import_result(
    ...
    if !is_overwrite {
        match patchbay_db::queries::skill::get_skill_by_workspace_and_name(
            &state.pool,
            rt.workspace_id,
            sanitized_name.trim(),
        )
    ...
    let persisted = if is_overwrite {
        ...
        let target = match patchbay_db::queries::skill::get_skill_in_workspace(
            &mut *tx,
            target_id,
            rt.workspace_id,
        )
        ...
        if let Err(e) =
            patchbay_db::queries::skill::delete_skill_files_by_skill(&mut *tx, target.id).await
        {
            return fail_import(
                store,
                request_id.trim(),
        ...
        match patchbay_db::queries::skill::update_skill(
            &mut *tx,
            target.id,
            Some(&sanitized_name),
        ...
        match patchbay_db::queries::skill::create_skill(
            &mut *tx,
            rt.workspace_id,
            &sanitized_name,
    ...
    for (path, file_content) in &files {
        match patchbay_db::queries::skill::upsert_skill_file(
            &mut *tx,
            persisted.id,
            path,
            file_content,
    ...
    state.bus.publish(&patchbay_events::Event {
        event_type: if is_overwrite {
            patchbay_protocol::EVENT_SKILL_UPDATED
        } else {
            patchbay_protocol::EVENT_SKILL_CREATED
        }
        .to_string(),
        workspace_id: rt.workspace_id.to_string(),
...
fn plugin_error_response(err: &patchbay_service::plugin::PluginError, fallback: &str) -> Response {
    use patchbay_service::plugin::PluginErrorKind as K;
...
mod tests {
    ...
    #[test]
    fn daemon_mcp_token_covers_plugin_only_claims() {
        assert!(claim_requires_daemon_mcp_token(&json!({
            "plugin_hook_tools": [{"name": "plugin.run"}],
        })));
        assert!(claim_requires_daemon_mcp_token(&json!({
            "remote_mcp_connections": [{"endpoint": "https://mcp.example"}],
        })));
        assert!(!claim_requires_daemon_mcp_token(&json!({
            "plugin_hook_tools": [],
