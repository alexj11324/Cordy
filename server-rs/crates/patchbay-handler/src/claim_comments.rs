...
pub async fn build_coalesced_comment_data(
    ...
    for id in ids {
        ...
        let comment =
            match patchbay_db::queries::comment::get_comment_in_workspace(pool, *id, workspace_id)
                .await
            {
                Ok(Some(c)) => c,
                Ok(None) => continue,
        ...
        match comment.author_type.as_str() {
            "agent" => {
                if let Ok(Some(a)) =
                    patchbay_db::queries::agent::get_agent(pool, comment.author_id).await
                {
            ...
            "member" => {
                if let Ok(Some(u)) =
                    patchbay_db::queries::user::get_user(pool, comment.author_id).await
                {
