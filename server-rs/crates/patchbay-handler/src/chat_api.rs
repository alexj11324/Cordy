...
use patchbay_db::models::{ChatMessage, ChatSession};
use patchbay_db::queries::{agent, chat, chat_pinned_agent, member, project, user, workspace};
use patchbay_middleware::workspace::WorkspaceContext;
use patchbay_service::task_service::TaskServiceError;
...
fn dispatch_blocked(
    status: StatusCode,
    reason: patchbay_service::dispatch_reason::ReasonCode,
) -> Response {
    let message = match reason {
        patchbay_service::dispatch_reason::ReasonCode::InvocationNotAllowed => {
            "you don't have permission to use this target"
        }
        patchbay_service::dispatch_reason::ReasonCode::TargetUnavailable => {
            "the target is unavailable"
        }
        patchbay_service::dispatch_reason::ReasonCode::RuntimeOffline => {
            "the target's runtime is offline"
        }
        patchbay_service::dispatch_reason::ReasonCode::RuntimeUnusable => {
            "the target's agent CLI cannot run on its machine"
        }
        patchbay_service::dispatch_reason::ReasonCode::AgentRuntimeRequired => {
...
fn attachment_json(attachment: &patchbay_db::models::Attachment) -> Value {
...
async fn message_attachments(
    ...
    let rows = patchbay_db::queries::attachment::list_attachments_by_chat_message_i_ds(
        &state.pool,
        message_ids,
        workspace_id,
    )
...
async fn update_session(
    ...
    match updated {
        Ok(Some(updated)) => {
            publish_chat(
                &state,
                patchbay_protocol::events::EVENT_CHAT_SESSION_UPDATED,
                &updated,
                json!({
...
async fn set_archived(
    ...
    if input.archived {
        let binding =
            match patchbay_db::queries::channel::get_channel_chat_session_binding_by_session_any(
                &mut *tx, session.id,
            )
            .await
            {
        ...
        if patchbay_db::queries::channel::delete_channel_chat_session_binding_by_session(
            &mut *tx, session.id,
        )
        .await
        .is_err()
...
async fn delete_session(
    ...
    .await
    .is_err()
        || patchbay_db::queries::channel::delete_channel_outbound_card_messages_by_session(
            &mut *tx, session.id,
        )
        .await
        .is_err()
        || chat::delete_chat_draft_restores_by_session(&mut *tx, session.id)
            .await
            .is_err()
        || patchbay_db::queries::agent_builder::delete_agent_builder_draft(&mut *tx, session.id)
            .await
            .is_err()
        || chat::delete_chat_session(&mut *tx, session.id, session.workspace_id)
            .await
            .is_err()
        || patchbay_db::queries::issue_label::delete_agent_label_assignments_by_agent(
            &mut *tx,
            session.agent_id,
        )
        .await
    ...
    publish_chat(
        &state,
        patchbay_protocol::events::EVENT_CHAT_SESSION_DELETED,
        &session,
...
async fn send_message(
    ...
    match patchbay_service::agent_ready::agent_readiness(&state.pool, &target).await {
    ...
    let platform = headers
        ...
        .and_then(|value| value.to_str().ok())
        .unwrap_or_default();
    let analytics_event = patchbay_analytics::events::chat_message_sent(
        &user_id.to_string(),
    ...
    publish_chat(
        &state,
        patchbay_protocol::events::EVENT_CHAT_MESSAGE,
        &session,
        json!({
...
async fn list_messages_page(
    ...
    let cursor = messages.last().map(|message| {
        json!({"created_at": patchbay_util::rfc3339_nano(message.created_at), "id": message.id})
    });
    messages.reverse();
    let mut attachments = match message_attachments(&state, session.workspace_id, &messages).await {
        Ok(grouped) => grouped,
...
async fn mark_read(
    ...
    match chat::mark_chat_session_read(&state.pool, session.id).await {
        Ok(_) => {
            publish_chat(
                &state,
                patchbay_protocol::events::EVENT_CHAT_SESSION_READ,
                &session,
                json!({"chat_session_id": session.id}),
            );
            StatusCode::NO_CONTENT.into_response()
...
async fn list_draft_restores(
    ...
    let attachment_rows = if attachment_ids.is_empty() {
        ...
        match patchbay_db::queries::attachment::list_attachments_by_i_ds(
            &state.pool,
            attachment_ids,
            session.workspace_id,
        )
        ...
        .await
    {
        Ok((message_id, task)) => {
            state.tasks.generate_chat_quick_actions_async(
                task,
                patchbay_service::chat_quick_actions::ChatQuickActionsOrigin::Refresh,
            );
            (
                StatusCode::ACCEPTED,
    ...
    let next_cursor = if messages.len() == limit {
        messages
            ...
            .map(|message| {
                format!(
                    "{}|{}",
                    patchbay_util::rfc3339_nano(message.created_at),
                    message.id
                )
            })
            .unwrap_or_default()
        ...
        messages: messages
            ...
            .map(|message| {
                ...
                HistoryMessage {
                    ...
                    ts: patchbay_util::rfc3339_nano(message.created_at),
                    thread_id: String::new(),
                    reply_count: 0,
...
async fn history(
    ...
    if let Some(reader) = state.slack_history.as_ref() {
        ...
        match reader.channel_overview(session_id, &options).await {
            ...
            Err(error) if error.is::<patchbay_slack::history::ErrNoSlackSession>() => {}
            Err(error) => {
