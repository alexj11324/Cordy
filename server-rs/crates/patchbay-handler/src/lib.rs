//! patchbay-handler — HTTP handler layer (S8).
//!
...
use patchbay_middleware::auth::{auth_middleware, AuthState, IdentityProxyTrust};
use patchbay_middleware::daemon_auth::{daemon_auth_middleware, DaemonAuthState};
...
fn cors_layer() -> CorsLayer {
    ...
    let exposed_headers = [
        ...
        HeaderName::from_static("x-patchbay-next-before"),
        HeaderName::from_static("x-patchbay-next-before-id"),
        HeaderName::from_static("x-cordy-next-before"), // legacy-brand-compat
        HeaderName::from_static("x-cordy-next-before-id"), // legacy-brand-compat
...
/// mount `patchbay_middleware::auth::auth_middleware`).
///
...
pub fn build_router_from_state(state: HandlerState) -> Router {
    ...
    let auth_side_effects: Arc<dyn patchbay_middleware::auth::AuthSideEffectSpawner> = {
    ...
    let authenticated = workspace::authenticated_router()
        ...
        .merge(
            formal_guard(workspace::admin_router().route_layer(middleware::from_fn_with_state(
    ...
    let plugin_action = plugin_action::router().route_layer(middleware::from_fn_with_state(
        auth_state,
        patchbay_middleware::plugin_auth::plugin_auth,
    ));
    ...
    let contact_sales = contact_sales::router().route_layer(middleware::from_fn_with_state(
        patchbay_middleware::ratelimit::RateLimitState::configured(
            state.rate_limit_client.clone(),
            contact_sales_limit,
            60 * 60,
            patchbay_middleware::ratelimit::parse_trusted_proxies(
    ...
    let app = Router::new()
        ...
        .layer(middleware::from_fn(
            patchbay_middleware::request_logger::request_logger,
        ))
        .layer(middleware::from_fn(
            patchbay_middleware::client::client_metadata,
        ))
        .layer(middleware::from_fn(
            patchbay_middleware::request_id::request_id,
        ));

    match http_metrics {
        Some(metrics) => app.layer(middleware::from_fn_with_state(
            metrics,
            patchbay_metrics::http::middleware,
        )),
        None => app,
    }
}
