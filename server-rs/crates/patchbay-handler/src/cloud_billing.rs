...
impl StripeIpLimiter {
    fn from_env() -> Self {
        Self {
            ...
            trusted_proxies: patchbay_middleware::ratelimit::parse_trusted_proxies(
...
fn subscription_context(
    ...
    let enabled = state
        .feature_flags
        .as_deref()
        .is_some_and(patchbay_service::feature_flags::billing_workspace_subscriptions_enabled);
    if !enabled {
...
mod tests {
    ...
    impl patchbay_service::feature_flags::FlagSource for EnabledFlags {
        fn is_enabled(&self, key: &str, default: bool) -> bool {
            if key == patchbay_service::feature_flags::BILLING_WORKSPACE_SUBSCRIPTIONS {
                true
            } else {
                default
            }
    ...
    fn test_state(flags: bool) -> HandlerState {
        ...
        let pat_cache = patchbay_auth::pat_cache::PatCache::disabled();
        if flags {
