...
use patchbay_db::models::{Agent, AgentRuntime, ChatSession, Member};
use patchbay_db::queries::{agent, agent_builder, chat, runtime, workspace};
...
const AGENT_BUILDER_INSTRUCTIONS: &str = r#"You are Patchbay Agent Builder. Help the user design one practical AI agent through a short conversation.
...
async fn create_session(
    ...
    let builder = match agent::create_agent_builder(
        &mut *transaction,
        context.member.workspace_id,
        &format!(".patchbay-agent-builder-{flow_id}"),
