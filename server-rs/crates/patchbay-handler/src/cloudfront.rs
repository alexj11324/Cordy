...
impl CloudFrontSigner {
    pub async fn from_config(config: &patchbay_config::Config) -> anyhow::Result<Option<Self>> {
...
pub fn cloudfront_cookie_expiry(
    ...
    now + chrono::Duration::seconds(patchbay_auth::cookie::auth_token_ttl()).min(MAX_COOKIE_TTL)
}
