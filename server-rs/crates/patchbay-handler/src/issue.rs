...
async fn move_issue(
    ...
    if let Err(message) = normalize_legacy_issue_assignment(&mut fields, "move") {
...
/// Temporary wire compatibility for clients released before the role split.
///
/// The adapter is intentionally kept at the HTTP boundary. It must be removed
/// after all supported desktop clients have adopted owner/executor, with the
/// removal tracked by the API compatibility deprecation issue. No database or
/// domain type should add the old aggregate assignment back.
#[allow(clippy::result_large_err)]
fn normalize_legacy_issue_assignment(
    fields: &mut serde_json::Map<String, Value>,
    operation: &'static str,
) -> Result<bool, &'static str> {
    let Some(legacy_type) = fields.get("assignee_type").cloned() else {
        if fields.contains_key("assignee_id") {
            return Err("assignee_type and assignee_id must be provided together");
        }
        return Ok(false);
    };
    let Some(legacy_id) = fields.get("assignee_id").cloned() else {
        return Err("assignee_type and assignee_id must be provided together");
    };
    let legacy_type = match legacy_type {
        Value::Null => None,
        Value::String(value) => Some(value),
        _ => return Err("invalid assignee_type"),
    };
    let legacy_id = match legacy_id {
        Value::Null => None,
        Value::String(value) => Some(value),
        _ => return Err("invalid assignee_id"),
    };
    if legacy_type.is_some() != legacy_id.is_some() {
        return Err("assignee_type and assignee_id must be provided together");
    }

    let assign = |fields: &mut serde_json::Map<String, Value>,
                  name: &str,
                  value: Value|
     -> Result<(), &'static str> {
        if let Some(existing) = fields.get(name) {
            if existing != &value {
                return Err("legacy assignee conflicts with role fields");
            }
        } else {
    ...
    match (legacy_type.as_deref(), legacy_id.as_deref()) {
        (None, None) => {
            assign(fields, "owner_type", Value::Null)?;
            assign(fields, "owner_id", Value::Null)?;
            assign(fields, "executor_type", Value::Null)?;
            assign(fields, "executor_id", Value::Null)?;
        }
        (Some("member"), Some(id)) => {
            assign(fields, "owner_type", Value::String("member".to_string()))?;
            assign(fields, "owner_id", Value::String(id.to_string()))?;
        }
        (Some(kind @ ("agent" | "team")), Some(id)) => {
            assign(fields, "executor_type", Value::String(kind.to_string()))?;
            assign(fields, "executor_id", Value::String(id.to_string()))?;
        }
        (Some(_), Some(_)) => return Err("invalid assignee_type"),
        _ => return Err("assignee_type and assignee_id must be provided together"),
    }

    fields.remove("assignee_type");
    fields.remove("assignee_id");
    tracing::warn!(
        legacy_issue_assignment_compatibility = true,
        operation,
        "translated legacy issue assignment fields"
    );
    Ok(true)
}
...
async fn update_issue(
    ...
    if let Err(message) = normalize_legacy_issue_assignment(&mut fields, "update") {
        return error_response(StatusCode::BAD_REQUEST, message);
    }
...
async fn batch_update_issues(
    ...
    let mut updates = match root.get("updates").cloned() {
        Some(Value::Object(fields)) => fields,
        _ => return error_response(StatusCode::BAD_REQUEST, "invalid request body"),
    };
    if let Err(message) = normalize_legacy_issue_assignment(&mut updates, "batch_update") {
        return error_response(StatusCode::BAD_REQUEST, message);
    }
    let mutation_keys = [
        "title",
...
struct CreateIssueRequest {
    ...
    /// Legacy aggregate assignment accepted only by this boundary adapter.
    #[serde(default, rename = "assignee_type")]
    legacy_assignee_type: Option<String>,
    /// Legacy aggregate assignment accepted only by this boundary adapter.
    #[serde(default, rename = "assignee_id")]
    legacy_assignee_id: Option<String>,
}

impl CreateIssueRequest {
    #[allow(clippy::result_large_err)]
    fn normalize_legacy_assignment(&mut self) -> Result<bool, &'static str> {
        let legacy_type = self.legacy_assignee_type.take();
        let legacy_id = self.legacy_assignee_id.take();
        let (kind, id) = match (legacy_type, legacy_id) {
            (None, None) => return Ok(false),
            (Some(kind), Some(id)) => (kind, id),
            _ => return Err("assignee_type and assignee_id must be provided together"),
        };
        let (target_type, target_id) = match kind.as_str() {
            "member" => (&mut self.owner_type, &mut self.owner_id),
            "agent" | "team" => (&mut self.executor_type, &mut self.executor_id),
            _ => return Err("invalid assignee_type"),
        };
        if target_type.is_some() || target_id.is_some() {
            if target_type.as_deref() != Some(kind.as_str())
                || target_id.as_deref() != Some(id.as_str())
            {
                return Err("legacy assignee conflicts with role fields");
            }
        } else {
            *target_type = Some(kind.clone());
            *target_id = Some(id);
        }
        tracing::warn!(
            legacy_issue_assignment_compatibility = true,
...
async fn create_issue(
    ...
    if let Err(message) = request.normalize_legacy_assignment() {
        return error_response(StatusCode::BAD_REQUEST, message);
    }
...
async fn validate_owner(
    ...
    if member::get_member_by_user_and_workspace(&state.pool, id, context.member.workspace_id)
        .await
        .ok()
        .flatten()
        .is_none()
...
mod tests {
    ...
    #[test]
    fn legacy_create_assignment_maps_members_to_owner_and_agents_to_executor() {
        let mut member = serde_json::from_value::<CreateIssueRequest>(json!({
            "title": "legacy member",
            "assignee_type": "member",
            "assignee_id": "018f03a0-c4d2-7a37-ae4d-5aa45de12f11"
        }))
        .unwrap();
        assert!(member.normalize_legacy_assignment().unwrap());
        assert_eq!(member.owner_type.as_deref(), Some("member"));
        assert_eq!(
            member.owner_id.as_deref(),
            Some("018f03a0-c4d2-7a37-ae4d-5aa45de12f11")
        );
        assert!(member.executor_type.is_none());

        let mut agent = serde_json::from_value::<CreateIssueRequest>(json!({
            "title": "legacy agent",
            "assignee_type": "agent",
            "assignee_id": "018f03a0-c4d2-7a37-ae4d-5aa45de12f12"
        }))
        .unwrap();
        assert!(agent.normalize_legacy_assignment().unwrap());
        assert_eq!(agent.executor_type.as_deref(), Some("agent"));
        assert_eq!(
            agent.executor_id.as_deref(),
            Some("018f03a0-c4d2-7a37-ae4d-5aa45de12f12")
        );
        assert!(agent.owner_type.is_none());
    }

    #[test]
    fn legacy_update_assignment_maps_and_removes_old_wire_fields() {
        let mut fields = update_object(
            br#"{"assignee_type":"team","assignee_id":"018f03a0-c4d2-7a37-ae4d-5aa45de12f13"}"#,
        )
        .unwrap();
        assert!(normalize_legacy_issue_assignment(&mut fields, "test").unwrap());
        assert_eq!(fields.get("executor_type"), Some(&json!("team")));
        assert_eq!(
            fields.get("executor_id"),
            Some(&json!("018f03a0-c4d2-7a37-ae4d-5aa45de12f13"))
        );
        assert!(!fields.contains_key("assignee_type"));
        assert!(!fields.contains_key("assignee_id"));
    }

    #[test]
    fn legacy_update_assignment_clear_removes_both_roles() {
        let mut fields = update_object(br#"{"assignee_type":null,"assignee_id":null}"#).unwrap();
        normalize_legacy_issue_assignment(&mut fields, "test").unwrap();
        for field in ["owner_type", "owner_id", "executor_type", "executor_id"] {
            assert_eq!(fields.get(field), Some(&Value::Null));
        }
    }

    #[test]
    fn legacy_assignment_conflicts_are_rejected() {
        let mut fields = update_object(
            br#"{"assignee_type":"member","assignee_id":"018f03a0-c4d2-7a37-ae4d-5aa45de12f11","owner_type":"member","owner_id":"018f03a0-c4d2-7a37-ae4d-5aa45de12f12"}"#,
        )
        .unwrap();
        assert_eq!(
            normalize_legacy_issue_assignment(&mut fields, "test"),
            Err("legacy assignee conflicts with role fields")
        );
    }
