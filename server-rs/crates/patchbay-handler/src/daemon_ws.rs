...
pub async fn daemon_ws_handler(
    ...
    upgrade
        .protocols(["daemon"])
        .write_buffer_size(0)
        .max_message_size(patchbay_daemon::hub::MAX_FRAME_READ_BYTES)
        .max_frame_size(patchbay_daemon::hub::MAX_FRAME_READ_BYTES)
...
async fn serve_daemon_socket(hub: Arc<DaemonHub>, identity: ClientIdentity, socket: WebSocket) {
    ...
    let (client, mut rx): (Arc<patchbay_daemon::hub::DaemonClient>, _) =
                    ...
                    match frame {
                        Some(frame) => {
                            if tokio::time::timeout(
                                patchbay_daemon::hub::WRITE_WAIT,
    ...
    loop {
        let msg = match tokio::time::timeout(patchbay_daemon::hub::PONG_WAIT, stream.next()).await {
            Ok(Some(msg)) => msg,
            Ok(None) | Err(_) => break,
        };
        let raw = match msg {
        ...
        if raw.len() > patchbay_daemon::hub::MAX_FRAME_READ_BYTES {
