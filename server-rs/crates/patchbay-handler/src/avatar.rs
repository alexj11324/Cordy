...
fn signing_key() -> [u8; 32] {
    Sha256::digest(format!("avatar-url:{}", patchbay_auth::jwt::jwt_secret()).as_bytes()).into()
}
...
async fn publishable(state: &HandlerState, key: &str) -> bool {
    ...
    let Some(attachment) =
        patchbay_db::queries::attachment::get_attachment_by_id_only(&state.pool, id)
            .await
            .ok()
            .flatten()
    else {
...
mod tests {
    ...
    async fn served_urls_use_loaded_public_url_not_process_env() {
        std::env::set_var("PATCHBAY_PUBLIC_URL", "https://env.example");
        ...
        std::env::remove_var("PATCHBAY_PUBLIC_URL");
    }
}
