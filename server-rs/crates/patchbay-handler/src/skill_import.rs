...
use patchbay_db::models::Skill;
use patchbay_db::queries::skill;
...
async fn github_get(client: &Client, url: String) -> Result<reqwest::Response, ImportError> {
    ...
    let mut request = client
        ...
        .header("User-Agent", "Patchbay-Skill-Importer");
...
async fn resolve_conflict(
    ...
    match strategy {
        ...
        "overwrite" => match overwrite_imported(state, context, &existing, &imported, false).await {
            Ok(response) => { publish_skill(state, context, headers, patchbay_protocol::EVENT_SKILL_UPDATED, &response).await; Json(json!({ "status": "updated", "skill": response })).into_response() }
            Err(error) => {
        ...
        "rename" => {
            for suffix in 2..52 {
                ...
                match create_imported(state, context, &imported, &candidate).await {
                    Ok(response) => { publish_skill(state, context, headers, patchbay_protocol::EVENT_SKILL_CREATED, &response).await; return (StatusCode::CREATED, Json(json!({ "status": "created", "reason": "renamed to avoid an existing skill", "skill": response, "existing_skill": identity }))).into_response(); }
