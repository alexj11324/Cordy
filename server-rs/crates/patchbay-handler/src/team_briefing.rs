...
//! Composes the full system briefing appended to a team leader's Instructions
//! when it claims a task on a team-assigned issue: Operating Protocol (system
//! rules), Team Roster (data, with literal mention markdown), and the team's
//! own Instructions. Byte-stable with the Go text — contract tests on both
//! sides pin the compositions.

use patchbay_db::models::{Agent, Team, TeamMember, User};
use patchbay_db::queries::{agent as agent_q, skill as skill_q, team as team_q, user as user_q};
use sqlx::PgPool;

const TEAM_OPERATING_PROTOCOL_HEADER: &str = r#"## Team Operating Protocol
...
defeats the entire purpose of the team and is a protocol violation.

Your responsibilities, in order:

1. **Read the issue** (title, description, latest comments, acceptance
   criteria) and decide which team member is best suited to do the work.
   Match the task to each member's listed **skills** and role in the Team
...
2. **Delegate by @mention.** Post a single comment on this issue that
   ...
   - Use the exact mention markdown shown in the Team Roster below —
...
/// assigned to THIS team (Go `teamParentStatusOwned`). Only then does the
/// leader own the parent's status arc.
const TEAM_PARENT_STATUS_OWNED: &str = r#"6. **Own the parent issue status.** This issue is assigned to your team,
...
/// `teamParentStatusNotOwned`): an @team mention on an issue owned by someone
...
const TEAM_OPERATING_PROTOCOL_HARD_RULES: &str = r#"Hard rules:
- EVERY delegation MUST use the full mention markdown syntax
  `[@Name](mention://<type>/<UUID>)` exactly as shown in the Team
...
- Do NOT @mention members who don't appear in the Team Roster below;
...
/// Go `teamOperatingProtocolFor`: assembles the protocol, selecting the
/// parent-status responsibility that matches this leader's actual authority
/// over the issue.
fn team_operating_protocol_for(owns_issue_status: bool) -> String {
    let status = if owns_issue_status {
        TEAM_PARENT_STATUS_OWNED
    } else {
        TEAM_PARENT_STATUS_NOT_OWNED
    };
    format!("{TEAM_OPERATING_PROTOCOL_HEADER}\n{status}\n\n{TEAM_OPERATING_PROTOCOL_HARD_RULES}")
}
...
/// Go `loadTeamMemberSkillNames`: one batched query for every non-leader agent
...
async fn load_team_member_skill_names(
    pool: &PgPool,
    members: &[TeamMember],
    leader_id: &str,
    ...
    match skill_q::list_agent_skill_names_by_agent_i_ds(pool, ids).await {
        ...
        Err(e) => {
            tracing::warn!(error = %e, "team roster: load member skill names failed");
            None
        }
    }
}
...
async fn render_member_row(
    ...
    match m.member_type.as_str() {
        ...
        "member" => {
            ...
            // the product). Humans have no Patchbay skills, so no skills segment.
            format_roster_row(
                &name,
                "member (human)",
                role,
...
/// Go `buildTeamRoster`: renders the "## Team Roster" section — a leader
/// self-row plus one row per non-archived member, with literal mention
/// markdown.
async fn build_team_roster(pool: &PgPool, team: &Team) -> String {
    let mut out = String::from("## Team Roster\n\n");

    // Leader self-row. Leaders are always agents (FK enforced in schema).
    let mut leader_name = "Leader".to_string();
    if let Ok(Some(leader)) = agent_q::get_agent(pool, team.leader_id).await {
    ...
    out.push_str(&format!(
        "- {leader_name} — agent — `{}`\n",
        format_mention(&leader_name, "agent", &team.leader_id.to_string())
    ));

    let members = team_q::list_team_members(pool, team.id)
        .await
        .unwrap_or_default();

    let skills_by_agent =
        load_team_member_skill_names(pool, &members, &team.leader_id.to_string()).await;
    ...
    for m in &members {
        ...
        if m.member_type == "agent" && m.member_id == team.leader_id {
            continue;
        }
...
/// Builds the team leader briefing. The returned string contains three
/// sections: Team Operating Protocol (constant), Team Roster (data), and
/// Team Instructions (user-defined, omitted when empty).
///
/// `owns_issue_status` must be true only when the issue this task is bound to
/// is assigned to this very team — it keeps status authority from leaking
/// along with the roster.
pub async fn build_team_leader_briefing(
    pool: &PgPool,
    team: &Team,
    owns_issue_status: bool,
) -> String {
    let mut out = team_operating_protocol_for(owns_issue_status);
    out.push_str("\n\n");
    out.push_str(&build_team_roster(pool, team).await);

    let trimmed = team.instructions.trim();
    if !trimmed.is_empty() {
        out.push_str("\n\n## Team Instructions (");
...
mod tests {
    ...
    fn protocol_selects_parent_status_section() {
        let owned = team_operating_protocol_for(true);
        assert!(owned.starts_with("## Team Operating Protocol\n"));
