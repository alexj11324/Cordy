...
use patchbay_db::models::{IssueLabel, Skill, SkillFile};
use patchbay_db::queries::{issue_label, skill};
...
async fn delete(
    ...
    let (actor_type, actor_id, _) = crate::issue::mutation_actor(&state, &context, &headers).await;
    state.bus.publish(&skill_event(
        patchbay_protocol::EVENT_SKILL_DELETED,
        value.workspace_id,
        &actor_type,
        actor_id,
...
async fn attach_label(
    ...
    state.bus.publish(&patchbay_events::Event {
        event_type: patchbay_protocol::EVENT_LABEL_UPDATED.into(),
