//! Automation run event listeners.
...
type WorkProcessor = Arc<dyn Fn(AutomationEventWork) -> WorkFuture + Send + Sync>;

/// Keeps Automation runs synchronized with the issue or task created for them.
///
...
pub struct AutomationEventListeners {
    bus: Arc<Bus>,
    processor: WorkProcessor,
    started: AtomicBool,
    accepting_tasks: AtomicBool,
    sender: Mutex<Option<mpsc::UnboundedSender<AutomationEventWork>>>,
}

impl AutomationEventListeners {
    pub fn new(bus: Arc<Bus>, service: Arc<AutomationService>) -> Arc<Self> {
        let processor: WorkProcessor = Arc::new(move |work| {
            let service = service.clone();
            Box::pin(async move {
                if let Err(error) = handle_work(&service, work).await {
                    tracing::error!(%error, "automation event listener failed");
                }
            })
        });
    ...
    pub fn start(
    ...
    ) -> Option<AutomationEventListenersRuntime> {
        ...
        *self
        ...
        Some(AutomationEventListenersRuntime {
    ...
    fn dispatch_issue(&self, event: &Event) {
        ...
        self.enqueue(AutomationEventWork::Issue(issue_id));
    }
    ...
    fn dispatch_task(&self, event: &Event, sync_linked_issue_failure: bool) {
        ...
        self.enqueue(AutomationEventWork::Task {
            task_id,
            sync_linked_issue_failure,
        });
    }

    fn enqueue(&self, work: AutomationEventWork) {
...
async fn handle_work(
    ...
    work: AutomationEventWork,
) -> anyhow::Result<()> {
    match work {
        AutomationEventWork::Issue(issue_id) => {
            if let Some(issue) =
        ...
        AutomationEventWork::Task {
            ...
            if task.automation_run_id.is_some() {
...
pub(crate) async fn handle_event(
    ...
    let work = match event.event_type.as_str() {
        patchbay_protocol::EVENT_ISSUE_UPDATED => {
            terminal_issue_id(event).map(AutomationEventWork::Issue)
        }
        patchbay_protocol::EVENT_TASK_COMPLETED | patchbay_protocol::EVENT_TASK_CANCELLED => {
            task_id(event).map(|task_id| AutomationEventWork::Task {
                task_id,
...
struct AutomationEventWorker {
    receiver: mpsc::UnboundedReceiver<AutomationEventWork>,
    processor: WorkProcessor,
}

impl AutomationEventWorker {
    async fn run(mut self, parent: CancellationToken, listeners: Arc<AutomationEventListeners>) {
        loop {
            tokio::select! {
                biased;
                _ = parent.cancelled() => {
    ...
    async fn process_next(&mut self) -> bool {
            ...
            .await
        {
            ...
            tracing::error!(recovered = %detail, "automation event listener task panicked");
        }
        true
    }
}
...
pub enum AutomationEventShutdownOutcome {
    Stopped,
    Panicked,
    TimedOut,
}

pub struct AutomationEventListenersRuntime {
    listeners: Arc<AutomationEventListeners>,
    task: Option<JoinHandle<()>>,
}

impl AutomationEventListenersRuntime {
    pub async fn shutdown(mut self, timeout: Duration) -> AutomationEventShutdownOutcome {
        self.listeners.stop_accepting_tasks();
        let Some(mut task) = self.task.take() else {
            return AutomationEventShutdownOutcome::Stopped;
        };
        match tokio::time::timeout(timeout, &mut task).await {
            Ok(Ok(())) => AutomationEventShutdownOutcome::Stopped,
            Ok(Err(_)) => AutomationEventShutdownOutcome::Panicked,
            Err(_) => {
                task.abort();
                let _ = task.await;
                AutomationEventShutdownOutcome::TimedOut
            }
        }
    }
}

impl Drop for AutomationEventListenersRuntime {
    fn drop(&mut self) {
        self.listeners.stop_accepting_tasks();
    ...
    async fn worker_processes_admitted_events_in_fifo_order() {
        ...
        let processor: WorkProcessor = Arc::new({
            ...
            move |work| {
                ...
                Box::pin(async move {
                    let kind = match work {
                        AutomationEventWork::Issue(_) => "issue",
                        AutomationEventWork::Task { .. } => "task",
                    };
        ...
        let (sender, receiver) = mpsc::unbounded_channel();
        sender
            .send(AutomationEventWork::Issue(ISSUE_ID.parse().unwrap()))
            .unwrap();
        sender
            .send(AutomationEventWork::Task {
                task_id: TASK_ID.parse().unwrap(),
                sync_linked_issue_failure: true,
            })
            .unwrap();
        drop(sender);
        let mut worker = AutomationEventWorker {
            receiver,
            processor,
        };
