...
use patchbay_db::models::WorkspaceInvitation;
use patchbay_db::queries::invitation::{
    self, ListPendingInvitationsByWorkspaceRow, ListPendingInvitationsForUserRow,
};
use patchbay_db::queries::{member, user, workspace};
...
async fn create(
    ...
    state.bus.publish(&patchbay_events::Event {
        event_type: patchbay_protocol::events::EVENT_INVITATION_CREATED.into(),
    ...
    let event = patchbay_analytics::team_invite_sent(
...
fn acceptance_metric_events(
    ...
    if first_onboarding_completion {
        ...
        events.push(patchbay_analytics::onboarding_completed(
            &user_id.to_string(),
            &workspace_id.to_string(),
            patchbay_analytics::ONBOARDING_PATH_INVITE_ACCEPT,
            &onboarded_at,
...
async fn load_user_and_invitation(
    ...
    user_id: Uuid,
    invitation_id: Uuid,
) -> Result<(patchbay_db::models::User, WorkspaceInvitation), Response> {
...
async fn revoke(
    ...
    state.bus.publish(&patchbay_events::Event {
        event_type: patchbay_protocol::events::EVENT_INVITATION_REVOKED.into(),
...
async fn accept(
    ...
    state.bus.publish(&patchbay_events::Event {
        event_type: patchbay_protocol::events::EVENT_MEMBER_ADDED.into(),
    ...
    state.bus.publish(&patchbay_events::Event {
        event_type: patchbay_protocol::events::EVENT_INVITATION_ACCEPTED.into(),
        workspace_id: workspace_id.clone(),
        actor_type: "member".into(),
        actor_id: user_id.to_string(),
        payload: serde_json::json!({
...
async fn decline(
    ...
    state.bus.publish(&patchbay_events::Event {
        event_type: patchbay_protocol::events::EVENT_INVITATION_DECLINED.into(),
...
mod tests {
    ...
    fn acceptance_metrics_include_first_invite_onboarding_completion() {
        ...
        let onboarded_user = patchbay_db::models::User {
            id: user_id,
            name: "Alex".into(),
