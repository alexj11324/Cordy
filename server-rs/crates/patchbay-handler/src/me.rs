...
async fn patch_onboarding(
            ...
            .is_some_and(|value| !value.is_null() && value != &serde_json::json!({}))
    {
        ...
        record_metric_event(
            &state,
            &patchbay_analytics::onboarding_started(&user_id.to_string(), platform),
        );
    }
    ...
    if after.complete() && !before.complete() {
        record_metric_event(
            &state,
            &patchbay_analytics::onboarding_questionnaire_submitted(
                &user_id.to_string(),
                after.source.clone(),
        ...
        && !before.source_resolved()
    {
        record_metric_event(
            &state,
            &patchbay_analytics::onboarding_source_submitted(
                &user_id.to_string(),
                after.source.clone(),
                after.source_skipped,
                after
...
async fn complete_onboarding(
    ...
    if first_completion {
        let completion_path = match request.completion_path.as_str() {
            ...
            | patchbay_analytics::ONBOARDING_PATH_CLOUD_WAITLIST
            | patchbay_analytics::ONBOARDING_PATH_SKIP_EXISTING
            | patchbay_analytics::ONBOARDING_PATH_INVITE_ACCEPT => request.completion_path.as_str(),
            _ => patchbay_analytics::ONBOARDING_PATH_UNKNOWN,
        };
        ...
        record_metric_event(
            &state,
            &patchbay_analytics::onboarding_completed(
                &user_id.to_string(),
...
async fn join_cloud_waitlist(
    ...
    record_metric_event(
        &state,
        &patchbay_analytics::cloud_waitlist_joined(&user_id.to_string(), reason.is_some()),
    );
    Json(user_response(&state, &updated)).into_response()
}
...
fn record_metric_event(state: &HandlerState, event: &patchbay_analytics::Event) {
