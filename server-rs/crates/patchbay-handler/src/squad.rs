...
use patchbay_db::models::{Autopilot, Squad, SquadMember};
use patchbay_db::queries::{agent, autopilot, member, squad, workspace};
...
async fn create(
    ...
    publish_squad_event(
        &state,
        &context,
        patchbay_protocol::EVENT_SQUAD_CREATED,
        json!({ "squad": &response }),
    );
    let analytics = patchbay_analytics::events::squad_created(
...
fn publish_squad_updated(state: &HandlerState, context: &WorkspaceContext, squad_id: Uuid) {
    publish_squad_event(
        state,
        context,
        patchbay_protocol::EVENT_SQUAD_UPDATED,
...
async fn remove(
    ...
    publish_squad_event(
        &state,
        &context,
        patchbay_protocol::EVENT_SQUAD_DELETED,
