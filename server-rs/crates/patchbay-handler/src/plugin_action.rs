...
use patchbay_service::plugin::{find_hook, PluginError, PluginErrorKind};
use patchbay_service::plugin_action::{
    authorize_plugin_action, build_plugin_context, has_scope, PluginActionCaller,
};
use patchbay_service::plugin_hook::{invoke_hook, HookInvocation};
use patchbay_service::plugin_storage::{
...
const INSTALLATION_HEADER: &str = "x-patchbay-plugin-installation";
const LEGACY_INSTALLATION_HEADER: &str = "x-cordy-plugin-installation"; // legacy-brand-compat
...
fn has_callback_token(headers: &HeaderMap) -> bool {
    patchbay_middleware::plugin_auth::bearer_token(headers).starts_with(CALLBACK_TOKEN_PREFIX)
}
...
pub(crate) fn plugins_enabled(state: &HandlerState) -> bool {
    state
        .feature_flags
        .as_deref()
        .is_some_and(patchbay_service::feature_flags::plugins_v1_enabled)
}
...
async fn caller(
    ...
    let token = patchbay_middleware::plugin_auth::bearer_token(headers);
    if patchbay_middleware::plugin_auth::is_plugin_bearer_token(&token) {
    ...
    let installation_id = headers
        ...
        .or_else(|| headers.get(LEGACY_INSTALLATION_HEADER))
