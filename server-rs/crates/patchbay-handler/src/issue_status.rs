...
async fn create(
    ...
    if !state
        .feature_flags
        .as_deref()
        .is_some_and(patchbay_service::feature_flags::custom_issue_statuses_enabled)
    {
...
mod tests {
    ...
    impl FlagSource for TestFlags {
        fn is_enabled(&self, key: &str, default: bool) -> bool {
            assert_eq!(key, patchbay_service::feature_flags::CUSTOM_ISSUE_STATUSES);
    ...
    async fn write_policy_fails_before_database_access() {
        let pool = PgPoolOptions::new()
            .connect_lazy("postgres://localhost/patchbay-policy-test")
            .expect("lazy pool");
    ...
    async fn production_catalog_contract_is_atomic_and_emits_only_committed_changes() {
        ...
        let events = Arc::new(Mutex::new(Vec::<patchbay_events::Event>::new()));
