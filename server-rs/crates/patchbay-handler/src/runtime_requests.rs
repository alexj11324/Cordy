...
use patchbay_db::queries::{member, runtime};
        ...
        Ok(request) => {
            if let Some(hub) = state.daemon_hub.as_ref() {
                hub.notify_pending_work(
                    &resolved_runtime_id,
                    patchbay_protocol::PENDING_WORK_KIND_MODEL_LIST,
                );
            }
...
async fn revalidate_model_catalog(state: &HandlerState, runtime_id: &str) {
    ...
    if let Some(hub) = state.daemon_hub.as_ref() {
        hub.notify_pending_work(runtime_id, patchbay_protocol::PENDING_WORK_KIND_MODEL_LIST);
    }
}
