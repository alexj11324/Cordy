...
async fn create_autopilot(
    ...
    if let Some(template) = &req.issue_title_template {
        if let Err(message) = patchbay_service::autopilot::validate_issue_title_template(template) {
...
async fn update_autopilot(
    ...
    if let Some(Some(value)) = &template {
        if let Err(m) = patchbay_service::autopilot::validate_issue_title_template(value) {
