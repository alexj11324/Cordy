...
pub fn router() -> Router<HandlerState> {
    Router::new()
        ...
        .route(
            "/api/tasks/{task_id}/execution-provenance",
            post(record_execution_provenance),
        )
        .route(
...
/// Records facts from a checkout performed inside the current task's daemon.
/// The route is deliberately task-token-only: a workspace member or daemon
/// credential cannot choose another task id and submit provenance for it.
async fn record_execution_provenance(
        ...
        != Some("task_token")
    {
        return error_response(
            ...
            "execution provenance requires the task credential",
        );
    }
    let Ok(task_id) = Uuid::parse_str(raw_task_id.trim()) else {
    ...
    let Some(source_task_id) = headers
        ...
        return error_response(StatusCode::FORBIDDEN, "invalid task execution identity");
    };
    if source_task_id != task_id {
        return error_response(
            StatusCode::FORBIDDEN,
            "execution provenance task does not match the task credential",
        );
    }
    let Some(source_agent_id) = headers
        .get("x-agent-id")
        .and_then(|value| value.to_str().ok())
        .and_then(|value| Uuid::parse_str(value).ok())
    else {
    ...
    let task =
            ...
            .await
        {
            ...
            Err(error) => {
                tracing::warn!(%error, %task_id, "failed to load provenance task");
                return error_response(StatusCode::INTERNAL_SERVER_ERROR, "failed to load task");
            }
        };
    if task.agent_id != source_agent_id {
        return error_response(
            StatusCode::FORBIDDEN,
            "execution provenance Agent does not match the task credential",
        );
    }
    ...
    let clean = |value: String| value.replace('\0', "");
    let request = ExecutionProvenanceRequest {
        execution_repo_identity: clean(request.execution_repo_identity),
        execution_workspace: clean(request.execution_workspace),
        execution_head_branch: clean(request.execution_head_branch),
        execution_head_sha: clean(request.execution_head_sha),
        execution_head_state: clean(request.execution_head_state),
    };
