...
use patchbay_db::queries::issue_property::{
...
async fn create(
    ...
    state.bus.publish(&patchbay_events::Event {
        event_type: patchbay_protocol::EVENT_PROPERTY_CREATED.into(),
...
async fn update(
    ...
    let response = from_model(property, 0);
    state.bus.publish(&patchbay_events::Event {
        event_type: patchbay_protocol::EVENT_PROPERTY_UPDATED.into(),
        workspace_id: context.member.workspace_id.to_string(),
        actor_type: "member".into(),
        actor_id: context.member.user_id.to_string(),
        payload: json!({ "property": &response }),
