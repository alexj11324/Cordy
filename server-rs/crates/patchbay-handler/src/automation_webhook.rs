//! Public, bearer-token automation webhook ingress. The raw request is
...
use patchbay_service::automation::AutomationQuotaExceededError;
...
pub fn router() -> Router<HandlerState> {
    Router::new().route("/api/webhooks/automations/{token}", post(webhook))
}
...
async fn duplicate_response(
    state: &HandlerState,
    delivery: patchbay_db::models::WebhookDelivery,
) -> Response {
    let _ = webhook_delivery::bump_webhook_delivery_attempt(&state.pool, delivery.id).await;
    let run_id = match delivery.automation_run_id {
        Some(value) => Some(value),
        None => automation::get_automation_run_by_webhook_delivery(&state.pool, delivery.id)
            .await
            .ok()
            .flatten()
            .map(|run| run.id),
...
async fn webhook(
    ...
    let trigger = match automation::get_webhook_trigger_by_token(&state.pool, Some(&token)).await {
        Ok(Some(value)) => value,
        Ok(None) => {
        ...
        Err(error) => {
            tracing::error!(%error, "automation webhook token lookup failed");
    ...
    let (Some(trigger_id), Some(automation_id), Some(workspace_id)) = (
        trigger.id,
        trigger.automation_id,
        trigger.automation_workspace_id,
    ) else {
        return error_response(StatusCode::NOT_FOUND, "webhook not found");
    };
    let rule = match automation::get_automation(&state.pool, automation_id).await {
        Ok(Some(value)) if value.workspace_id == workspace_id => value,
        Ok(_) => return error_response(StatusCode::NOT_FOUND, "webhook not found"),
        Err(error) => {
            tracing::error!(%error, "automation webhook rule lookup failed");
            return error_response(StatusCode::INTERNAL_SERVER_ERROR, "internal error");
        }
    };
    ...
    .await
    {
        ...
        Err(error) if dedupe_key.is_some() => {
            tracing::debug!(%error, "automation webhook concurrent dedupe collision");
    ...
    let service = patchbay_service::automation::AutomationService::new(
        state.pool.clone(),
        state.bus.clone(),
        state.tasks.clone(),
    );
    let run = match service
        .admit_automation_webhook_delivery(&rule, trigger_id, &envelope, delivery.id)
        .await
    {
        Ok(value) => value,
        Err(error) => {
            if let Some(quota) = error.downcast_ref::<AutomationQuotaExceededError>() {
            ...
            tracing::warn!(%error, delivery_id = %delivery.id, "automation webhook admission failed");
            state.notify_webhook_delivery();
            return error_response(
                StatusCode::INTERNAL_SERVER_ERROR,
                "failed to admit automation",
            );
        }
    };
    let response = if run.status == "skipped" {
        ...
        json!({"status": "accepted", "delivery_id": delivery.id, "run_id": run.id, "automation_id": rule.id, "trigger_id": trigger_id})
    };
...
fn limited_response(
    ...
    gate: &'static str,
    metrics: Option<&patchbay_metrics::BusinessMetrics>,
) -> Option<Response> {
    ...
    tracing::warn!(gate, "automation webhook rate limited");
...
fn webhook_client_ip(
    ...
    patchbay_middleware::ratelimit::client_ip(headers, remote, trusted_proxies)
}
