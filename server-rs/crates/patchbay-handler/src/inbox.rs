...
use patchbay_db::models::InboxItem;
use patchbay_db::queries::{inbox, issue};
...
async fn mutate_item(
    ...
    let (event_type, payload) = match action {
        "read" => (
            patchbay_protocol::EVENT_INBOX_READ,
            json!({ "item_id": item.id, "recipient_id": item.recipient_id }),
        ),
        "unread" => (
            patchbay_protocol::EVENT_INBOX_UNREAD,
            json!({ "item_id": item.id, "recipient_id": item.recipient_id }),
        ),
        "archive" => (
            patchbay_protocol::EVENT_INBOX_ARCHIVED,
            json!({ "item_id": item.id, "issue_id": item.issue_id, "recipient_id": item.recipient_id }),
        ),
        _ => (
            patchbay_protocol::EVENT_INBOX_UNARCHIVED,
            json!({ "item_id": item.id, "issue_id": item.issue_id, "recipient_id": item.recipient_id }),
        ),
    };
...
async fn batch(state: HandlerState, context: WorkspaceContext, action: &'static str) -> Response {
    ...
    let event_type = if action == "read" {
        patchbay_protocol::EVENT_INBOX_BATCH_READ
    } else {
        patchbay_protocol::EVENT_INBOX_BATCH_ARCHIVED
    };
    publish(
        &state,
        &context,
