...
pub const AUTH_COOKIE_NAME: &str = "patchbay_auth";
pub const CSRF_COOKIE_NAME: &str = "patchbay_csrf";
pub const LEGACY_AUTH_COOKIE_NAME: &str = "cordy_auth"; // legacy-brand-compat
pub const LEGACY_CSRF_COOKIE_NAME: &str = "cordy_csrf"; // legacy-brand-compat
...
/// Tombstones the pre-Patchbay cookie pair after a successful migration or
/// logout. Authentication still accepts the old pair during the transition.
pub fn clear_legacy_auth_cookie_values(domain: Option<&str>, secure: bool) -> [String; 2] {
    [
        clear_cookie_value(LEGACY_AUTH_COOKIE_NAME, domain, secure, true),
        clear_cookie_value(LEGACY_CSRF_COOKIE_NAME, domain, secure, false),
    ]
}
...
mod tests {
    ...
    fn clear_cookie_values_match_go_logout_contract() {
        ...
        assert_eq!(
            values[0],
            "patchbay_auth=; Path=/; Domain=.example.com; Expires=Thu, 01 Jan 1970 00:00:00 GMT; Max-Age=0; HttpOnly; Secure; SameSite=Strict"
        );
        assert_eq!(
            values[1],
            "patchbay_csrf=; Path=/; Domain=.example.com; Expires=Thu, 01 Jan 1970 00:00:00 GMT; Max-Age=0; Secure; SameSite=Strict"
        );
        ...
        let legacy = clear_legacy_auth_cookie_values(None, false);
        assert!(legacy[0].starts_with("cordy_auth=;")); // legacy-brand-compat
        assert!(legacy[1].starts_with("cordy_csrf=;")); // legacy-brand-compat
    }
