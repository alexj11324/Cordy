...
//! through [`patchbay_channel::InboundMessage::raw`].
//!
...
pub fn decode_slack_raw(msg: &patchbay_channel::InboundMessage) -> anyhow::Result<SlackRawEvent> {
    if msg.raw.is_null() {
...
mod tests {
    ...
    fn decode_raw_roundtrips() {
        let msg = patchbay_channel::InboundMessage {
            raw: serde_json::json!({
                "team_id": "T1",
                "api_app_id": "A1",
                "event_type": "message",
    ...
    fn decode_raw_rejects_missing_payload() {
        let msg = patchbay_channel::InboundMessage::default();
