...
#[async_trait]
impl patchbay_channel::Channel for SlackChannel {
    fn r#type(&self) -> Type {
...
impl SlackChannel {
    ...
    async fn connect_impl(
        ...
        let tasks = Arc::new(patchbay_channel::RuntimeTasks::new());
...
pub fn register_slack(reg: &patchbay_channel::Registry, deps: ChannelDeps) {
    reg.register(Type(TYPE_SLACK.to_string()), new_slack_factory(deps));
}

pub fn new_slack_factory(deps: ChannelDeps) -> patchbay_channel::Factory {
...
mod tests {
    ...
    fn capabilities_declare_text_and_thread_reply_only() {
        use patchbay_channel::Channel as _;
        let ch = SlackChannel {
            app_id: "A1".into(),
            bot_user_id: "U1".into(),
            socket_mode: true,
    ...
    async fn factory_decodes_config_and_refuses_missing_app_token() {
        ...
        let reg = patchbay_channel::Registry::new();
        register_slack(
            &reg,
            ChannelDeps {
                decrypt: None,
