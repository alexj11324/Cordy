...
//! Slack user id is bound to their Patchbay account. Port of
//!
...
pub enum BindingError {
    ...
    TokenInvalid,
    /// This Slack user id is already bound to a different Patchbay user (account
...
impl BindingTokenService {
    ...
    pub async fn redeem_and_bind(
        ...
        // token.
        match get_member_by_user_and_workspace(&mut *tx, patchbay_user_id, row.workspace_id).await {
            Ok(Some(_)) => {}
            Ok(None) => return Err(BindingError::NotWorkspaceMember.into()),
            Err(e) => return Err(anyhow::anyhow!("check membership: {e:#}")),
        }

        // The ON CONFLICT DO UPDATE WHERE patchbay_user_id = … gating rejects a
