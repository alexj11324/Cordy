...
use patchbay_db::queries::channel::get_channel_installation;
...
impl TypingIndicatorManager {
    ...
    pub fn register(
        ...
        let subscribe = {
            ...
            move |bus: &patchbay_events::Bus, event_type: &str| {
                let me = Arc::clone(&me);
                let decrypt = decrypt.clone();
                let tasks = tasks.clone();
                bus.subscribe(event_type, move |e: &patchbay_events::Event| {
                    me.handle_event(e, decrypt.as_ref(), &tasks);
                });
            }
        };
        subscribe(bus, patchbay_protocol::EVENT_CHAT_DONE);
        subscribe(bus, patchbay_protocol::EVENT_TASK_FAILED);
        subscribe(bus, patchbay_protocol::EVENT_TASK_CANCELLED);
    }

    fn handle_event(
        self: &Arc<Self>,
        e: &patchbay_events::Event,
        decrypt: Option<&DecrypterArc>,
        tasks: &RuntimeTasks,
    ) {
...
/// both.
fn chat_session_id_from_event(e: &patchbay_events::Event) -> Option<Uuid> {
...
mod tests {
    ...
    fn session_id_from_envelope_then_payload() {
        let id = Uuid::now_v7();
        let mk = |session: &str, payload: serde_json::Value| patchbay_events::Event {
