...
//! bot token (`xoxb-`) + app-level token (`xapp-`) into Patchbay. The InstallService
...
pub enum InstallError {
    ...
    /// DIFFERENT Patchbay workspace — it would collide with the
...
impl InstallService {
    ...
    pub async fn persist_install(&self, p: &InstallPersist) -> anyhow::Result<ChannelInstallation> {
        ...
        let current = patchbay_db::queries::channel::list_channel_installations_by_workspace(
    ...
    /// Kept next to the service so adapters do not reach into patchbay-db directly.
