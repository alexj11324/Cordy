...
//! `patchbay attachment upload <path>`, the file lands in object storage, and
...
use patchbay_db::models::Attachment;
...
impl Outbound {
    ...
    pub fn register(self: &Arc<Self>, bus: &patchbay_events::Bus) {
        let me = Arc::clone(self);
        let tasks = self.runtime_tasks.clone();
        bus.subscribe(patchbay_protocol::EVENT_CHAT_DONE, move |event| {
            let me = Arc::clone(&me);
        ...
        let me = Arc::clone(self);
        let tasks = self.runtime_tasks.clone();
        bus.subscribe(patchbay_protocol::EVENT_INBOX_NEW, move |event| {
            let me = Arc::clone(&me);
            let event = event.clone();
            tasks.spawn(async move {
    ...
    async fn process_inbox_event(&self, event: &Event, allow_relay: bool) -> anyhow::Result<()> {
        ...
        let Some(binding) = patchbay_db::queries::channel::find_channel_binding_for_member(
            &self.pool,
            workspace_id,
            recipient_id,
            crate::TYPE_WECOM,
        ...
        let slug = patchbay_db::queries::workspace::get_workspace(&self.pool, workspace_id)
            .await?
...
fn attachment_target(binding: &patchbay_db::models::ChannelChatSessionBinding) -> AttachmentTarget {
    let config =
    ...
    AttachmentTarget {
        ...
        chat_type: crate::ws_frame::aibot_chat_type_from_channel(&patchbay_channel::ChatType(
