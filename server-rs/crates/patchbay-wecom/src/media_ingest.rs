...
use patchbay_channel::message::{InboundMessage as ChannelInboundMessage, MediaRef, MsgType};
...
impl WecomMediaResolver {
    ...
    async fn ingest_one(
        ...
        Ok(MediaRef {
            r#type: patchbay_channel::MsgType(m.kind.clone()),
            storage_key: key,
            storage_url: link,
            filename,
                ...
                Err(e) => {
                    ...
                    if failure == MediaFailure::Blocked {
                        tracing::warn!(
                            ...
                            kind = %m.kind,
                            err = %e,
                            "wecom media ingest refused by the media address guard: the host resolved to a non-public address and was not dialed. If this deployment sits behind a fake-IP proxy, declare its range in PATCHBAY_WECOM_MEDIA_ALLOW_CIDRS; otherwise this is a URL that should not have been sent."
                        );
                    } else {
                        tracing::warn!(
                            installation_id = %inst.id,
...
fn describe_media(
    ...
    let filename = if filename.is_empty() {
        fallback_media_name(
            &wm.msg_id,
            index,
            &patchbay_channel::MsgType(m.kind.clone()),
            &content_type,
        )
    } else {
        filename
