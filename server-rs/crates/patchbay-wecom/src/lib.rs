...
/// here alongside the wecom-specific types (rather than in `patchbay-channel`)
...
/// plus a constructor because [`patchbay_channel::Type`] is an owned newtype.
...
/// Builds the [`patchbay_channel::Type`] this adapter registers under.
pub fn type_wecom() -> patchbay_channel::Type {
    patchbay_channel::Type(TYPE_WECOM.to_string())
}
