...
use patchbay_channel::capability::Capability;
use patchbay_channel::channel::{BuiltChannel, Channel, Config, Factory, FactoryFuture, Type};
use patchbay_channel::handler::InboundHandler;
use patchbay_channel::LeaseGeneration;
...
impl Channel for WecomChannel {
    ...
    async fn connect(&self, ctx: CancellationToken) -> anyhow::Result<()> {
        ...
        drop(cb_tx);
        ping_ctx.cancel();
        if !patchbay_channel::shutdown_join_handles(
    ...
    async fn send(
        &self,
        _out: patchbay_channel::message::OutboundMessage,
    ) -> anyhow::Result<patchbay_channel::message::SendResult> {
...
/// edit.
pub fn register_wecom(reg: &patchbay_channel::Registry, deps: ChannelDeps) {
...
mod tests {
    ...
    use patchbay_channel::message::InboundMessage as ChannelInboundMessage;
    ...
    async fn cancelled_generation_drops_queued_callback_before_dispatch() {
        ...
        dispatch_frame(&ctx, &env, &handler, &sender, "bot-1", "Patchbay")
            .await
            .unwrap();
