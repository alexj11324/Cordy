...
const MARKER_PREFIX: &str = "<!-- patchbay:channel-media:";

fn marker_pattern() -> &'static Regex {
    static PATTERN: std::sync::OnceLock<Regex> = std::sync::OnceLock::new();
    PATTERN.get_or_init(|| {
        // Go: `<!-- patchbay:channel-media:([0-9a-fA-F-]{36}) -->`
        Regex::new(r"<!-- patchbay:channel-media:([0-9a-fA-F-]{36}) -->").unwrap()
    })
}
...
mod tests {
    ...
    fn marker_wraps_id_in_html_comment() {
        assert_eq!(marker(ID), format!("<!-- patchbay:channel-media:{ID} -->"));
    }
    ...
    fn marked_ids_dedupe_and_ignore_invalid() {
        let md = format!(
            "a\n{m1}\n\nb\n{m1}\n\nnot-a-uuid <!-- patchbay:channel-media:zz -->\n{m2}",
            m1 = marker(ID),
