...
pub fn filter_from_values(log_level: Option<&str>, rust_log: Option<&str>) -> String {
    match log_level {
        Some(value) => patchbay_scoped_filter(normalize_level(value)),
        None => rust_log
            .map(str::trim)
            .filter(|value| !value.is_empty())
            .map(str::to_string)
            .unwrap_or_else(|| patchbay_scoped_filter(DEFAULT_LEVEL)),
    }
}

fn patchbay_scoped_filter(level: &str) -> String {
    format!("warn,patchbay={level}")
}
...
mod tests {
    ...
    fn rust_log_is_preserved_when_log_level_is_unset() {
        assert_eq!(
            filter_from_values(None, Some("patchbay=trace,tower=info")),
            "patchbay=trace,tower=info"
        );
        assert_eq!(filter_from_values(None, Some("  ")), "warn,patchbay=debug");
        assert_eq!(filter_from_values(None, None), "warn,patchbay=debug");
    }
