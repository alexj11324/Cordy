...
pub(super) async fn run_repo_checkout(
    ...
    let daemon_port = environment.raw("PATCHBAY_DAEMON_PORT").unwrap_or_default();
    if daemon_port.is_empty() {
        bail!(
            "PATCHBAY_DAEMON_PORT not set (this command is intended to be run by an agent inside a daemon task)"
        );
    }
    let token = environment.raw("PATCHBAY_TOKEN").unwrap_or_default();
    if token.is_empty() {
        bail!("PATCHBAY_TOKEN not set (repo checkout requires the active task credential)");
    }
    let body = serde_json::json!({
        "url":repo_url,
        "workspace_id":environment.raw("PATCHBAY_WORKSPACE_ID").unwrap_or_default(),
        "workdir":environment.current_dir(),
        "ref":checkout_ref.unwrap_or_default(),
        "agent_name":environment.raw("PATCHBAY_AGENT_NAME").unwrap_or_default(),
        "task_id":environment.raw("PATCHBAY_TASK_ID").unwrap_or_default(),
        "checkout_mode":environment.raw("PATCHBAY_REPO_CHECKOUT_MODE").unwrap_or_default().trim(),
        "retry_busy":true
    });
    ...
    let checkout = async {
        loop {
            ...
            let retryable = response
                .headers()
                .get("X-Patchbay-Retryable")
                .or_else(|| response.headers().get("X-Cordy-Retryable")) // legacy-brand-compat
