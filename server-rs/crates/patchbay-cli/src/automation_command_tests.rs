...
#[test]
fn automation_read_parser_matches_go_registry() {
    ...
    .expect("automation trigger CLI");
    let Command::Automation(AutomationArgs {
        command: AutomationCommand::Trigger { id, output },
    }) = trigger.command
    else {
        panic!("expected automation trigger");
    };
    assert_eq!(id, "abcd");
    ...
    let Command::Automation(AutomationArgs {
        ...
        panic!("expected automation runs");
    };
    assert_eq!(id, "abcd");
    assert_eq!(limit, 5);
    assert_eq!(offset, 2);
    ...
    .expect("automation trigger-add CLI");
    let Command::Automation(AutomationArgs {
        command: AutomationCommand::TriggerAdd(args),
    }) = add.command
    else {
        panic!("expected automation trigger-add");
    };
    assert_eq!(args.automation_id, "abcd");
    ...
    .expect("automation trigger-update CLI");
    let Command::Automation(AutomationArgs {
        command: AutomationCommand::TriggerUpdate(args),
    }) = update.command
    else {
        panic!("expected automation trigger-update");
    };
    assert_eq!(args.automation_id, "abcd");
    ...
    let delete = Cli::try_parse_from(["patchbay", "automation", "trigger-delete", "abcd", "beef"])
        .expect("automation trigger-delete CLI");
    assert!(matches!(
        delete.command,
        Command::Automation(AutomationArgs {
            command: AutomationCommand::TriggerDelete { .. }
        })
    ));
    assert_eq!(output, OutputFormat::Json);
    ...
    .expect("automation trigger-rotate-url CLI");
    let Command::Automation(AutomationArgs {
        command: AutomationCommand::TriggerRotateUrl(args),
    }) = rotate.command
    else {
        panic!("expected automation trigger-rotate-url");
    };
    assert_eq!(args.automation_id, "abcd");
    assert_eq!(args.trigger_id, "beef");
    ...
    assert!(Cli::try_parse_from(["patchbay", "automation", "get"]).is_err());
    assert!(Cli::try_parse_from(["patchbay", "automation", "list", "extra"]).is_err());
    ...
    .expect("automation create CLI");
    let Command::Automation(AutomationArgs {
        command: AutomationCommand::Create(args),
    }) = create.command
    else {
        panic!("expected automation create");
    };
    ...
    .expect("automation update CLI");
    let Command::Automation(AutomationArgs {
        command: AutomationCommand::Update(args),
    }) = update.command
    else {
        panic!("expected automation update");
    };
    ...
    let delete = Cli::try_parse_from(["patchbay", "automation", "delete", "abcd"])
        .expect("automation delete CLI");
    let Command::Automation(AutomationArgs {
        command: AutomationCommand::Delete { id },
    }) = delete.command
    else {
        panic!("expected automation delete");
    };
    assert_eq!(id, "abcd");
}

#[tokio::test]
async fn automation_create_resolves_references_and_preserves_go_body() {
    ...
    assert_eq!(
        output.stdout,
        "Automation created: Daily planner (automation-1)\n"
    );
    let body = captured
        .lock()
...
#[tokio::test]
async fn automation_create_rejects_missing_and_invalid_required_values() {
    const AGENT_ID: &str = "11111111-1111-1111-1111-111111111111";
    let home = tempfile::tempdir().expect("temp home");
    let cwd = tempfile::tempdir().expect("temp cwd");
    let mut environment = Environment::for_test(home.path().into(), cwd.path().into());
    ...
    for (argv, expected) in [
        (
            vec!["patchbay", "automation", "create"],
            "--title is required",
        ),
        (
            vec!["patchbay", "automation", "create", "--title", "Daily"],
        ...
        let cli = Cli::try_parse_from(argv).expect("automation create CLI");
...
#[tokio::test]
async fn automation_update_resolves_references_and_patches_only_changed_fields() {
    const AUTOMATION_ID: &str = "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa";
    const AGENT_ID: &str = "11111111-1111-1111-1111-111111111111";
    const PROJECT_ID: &str = "22222222-2222-2222-2222-222222222222";
    const USER_ID: &str = "33333333-3333-3333-3333-333333333333";
    let captured = Arc::new(Mutex::new(None));
    ...
    let app = Router::new()
        ...
        .route(
            "/api/automations/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa",
            patch(move |Json(body): Json<Value>| {
                let captured = Arc::clone(&captured_handler);
                async move {
                    *captured.lock().expect("captured body") = Some(body);
                    Json(serde_json::json!({"id":AUTOMATION_ID,"title":"Updated"}))
                }
            }),
        );
    let listener = TcpListener::bind("127.0.0.1:0").await.expect("bind");
    ...
    assert_eq!(
        output.stdout,
        format!("Automation updated: Updated ({AUTOMATION_ID})\n")
    );
    let body = captured
        .lock()
        .expect("captured body")
...
#[tokio::test]
async fn automation_update_preserves_clear_and_no_change_semantics() {
    ...
    let no_change = Cli::try_parse_from(["patchbay", "automation", "update", ID])
        .expect("automation no-change CLI");
    ...
    .expect("automation subscriber conflict CLI");
...
#[tokio::test]
async fn automation_delete_resolves_prefix_and_reports_title() {
    ...
    let app = Router::new()
        .route(
            ...
            get(|request: Request| async move {
                ...
                Json(serde_json::json!({
                    "automations":[{"id":ID,"title":"Daily planner","status":"active"}],
                    "total":1
                }))
            }),
        )
        .route(
            "/api/automations/abcd0000-1111-2222-3333-444444444444",
    ...
    environment.set("PATCHBAY_SERVER_URL", format!("http://{address}"));
    environment.set("PATCHBAY_WORKSPACE_ID", "workspace-1");
    let cli = Cli::try_parse_from(["patchbay", "automation", "delete", "abcd"])
        .expect("automation delete CLI");
    let output = run_with_input(&cli, &environment, &mut Cursor::new(Vec::<u8>::new()))
        .await
        .expect("delete automation");
    assert_eq!(output.stdout, "Automation Daily planner deleted.\n");
    server.abort();
}

#[tokio::test]
async fn automation_trigger_and_runs_match_go_requests_and_outputs() {
    const ID: &str = "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa";
    let app = Router::new()
        .route(
            "/api/automations/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/trigger",
        ...
        .route(
            "/api/automations/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/runs",
            get(|request: Request| async move {
    ...
    let trigger =
        Cli::try_parse_from(["patchbay", "automation", "trigger", ID, "--output", "table"])
            .expect("automation trigger CLI");
    ...
    assert_eq!(
        output.stdout,
        "Automation triggered: run run-1 (status: queued)\n"
    );
    ...
    .expect("automation runs table CLI");
...
#[tokio::test]
async fn automation_trigger_add_preserves_schedule_and_webhook_semantics() {
    const ID: &str = "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa";
    let captured = Arc::new(Mutex::new(Vec::new()));
    let captured_handler = Arc::clone(&captured);
    let app = Router::new().route(
        "/api/automations/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/triggers",
        post(move |Json(body): Json<Value>| {
            let captured = Arc::clone(&captured_handler);
            async move {
...
#[tokio::test]
async fn automation_trigger_add_rejects_invalid_kind_specific_flags() {
    const ID: &str = "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa";
    let home = tempfile::tempdir().expect("temp home");
    let cwd = tempfile::tempdir().expect("temp cwd");
    let mut environment = Environment::for_test(home.path().into(), cwd.path().into());
    ...
    for (extra, expected) in [
        ...
        let mut argv = vec!["patchbay", "automation", "trigger-add", ID];
        argv.extend(extra);
...
#[tokio::test]
async fn automation_trigger_rotate_url_matches_go_confirmation_request_and_output() {
    const AUTOMATION_ID: &str = "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa";
    const TRIGGER_ID: &str = "bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb";
    let calls = Arc::new(Mutex::new(Vec::<Value>::new()));
    let calls_handler = Arc::clone(&calls);
    let app = Router::new().route(
        "/api/automations/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/triggers/bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb/rotate-webhook-token",
...
#[tokio::test]
async fn automation_trigger_update_and_delete_resolve_prefixes_and_mutate() {
    const AUTOMATION_ID: &str = "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa";
    ...
    let app = Router::new()
        ...
        .route(
            "/api/automations/aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa/triggers/bbbb0000-1111-2222-3333-444444444444",
            patch(move |Json(body): Json<Value>| {
                let captured = Arc::clone(&captured_handler);
                async move {
                    *captured.lock().expect("captured body") = Some(body);
...
#[tokio::test]
async fn automation_trigger_update_rejects_no_changes_before_requests() {
    const AUTOMATION_ID: &str = "aaaaaaaa-aaaa-aaaa-aaaa-aaaaaaaaaaaa";
    const TRIGGER_ID: &str = "bbbbbbbb-bbbb-bbbb-bbbb-bbbbbbbbbbbb";
    let home = tempfile::tempdir().expect("temp home");
    let cwd = tempfile::tempdir().expect("temp cwd");
...
#[tokio::test]
async fn automation_list_matches_go_filter_actor_and_output_semantics() {
    let app = Router::new()
        .route(
            "/api/automations",
            get(|request: Request| async move {
    ...
    environment.set("PATCHBAY_WORKSPACE_ID", "workspace-1");
    environment.set("PATCHBAY_TOKEN", "token-1");
    let cli = Cli::try_parse_from(["patchbay", "automation", "list", "--status", "paused"])
        .expect("automation list CLI");
    let output = run_with_input(&cli, &environment, &mut Cursor::new(Vec::<u8>::new()))
        .await
    ...
    .expect("automation list JSON CLI");
    let output = run_with_input(&json, &environment, &mut Cursor::new(Vec::<u8>::new()))
        .await
        .expect("list automations as JSON");
    let value: Value = serde_json::from_str(&output.stdout).expect("JSON output");
    assert_eq!(value["total"], 1);
    assert_eq!(value["automations"][0]["server_only"], "preserved");
    server.abort();
}

#[tokio::test]
async fn automation_get_resolves_prefix_and_preserves_detail_envelope() {
    const ID: &str = "abcd0000-1111-2222-3333-444444444444";
    let app = Router::new()
        .route(
            "/api/automations",
            get(|request: Request| async move {
                match request.uri().query() {
                    Some("limit=50&workspace_id=workspace-1") => Json(serde_json::json!({
                        "automations":(0..50).map(|index| serde_json::json!({
                    ...
                    Some("limit=50&offset=50&workspace_id=workspace-1") => {
                        Json(serde_json::json!({
                            "automations":[{"id":ID,"title":"Morning triage","status":"active"}],
                            "total":51,
                            "has_more":false
                        }))
                    }
    ...
    let table = Cli::try_parse_from(["patchbay", "automation", "get", "abcd", "--output", "table"])
        .expect("automation get table CLI");
    let output = run_with_input(&table, &environment, &mut Cursor::new(Vec::<u8>::new()))
        .await
        .expect("get automation table");
...
#[tokio::test]
async fn automation_prefix_errors_match_go_resolver_contract() {
    let app = Router::new().route(
        "/api/automations",
        get(|| async {
            Json(serde_json::json!({
    ...
    let short =
        Cli::try_parse_from(["patchbay", "automation", "get", "abc"]).expect("short prefix CLI");
    let error = run_with_input(&short, &environment, &mut Cursor::new(Vec::<u8>::new()))
        .await
        .expect_err("short prefix rejected");
    assert_eq!(
        error.to_string(),
        "resolve automation: resolve automation: expected a full UUID or at least 4 hex characters, got \"abc\""
    );

    let ambiguous = Cli::try_parse_from(["patchbay", "automation", "get", "abcd"])
    ...
    assert!(error
        .to_string()
        .starts_with("resolve automation: ambiguous automation id prefix \"abcd\"; matches:"));
    assert!(error
        .to_string()
