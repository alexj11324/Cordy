...
pub(crate) async fn run_update(
    ...
    let executor = patchbay_daemon::update_executor::UpdateExecutor::detect()
        .await
        .map_err(|error| sanitize_update_error(error.into()))?;
    write_update_progress(std::io::stderr(), "Applying update...\n")?;
    let outcome = executor
        .update_request(
            patchbay_daemon::update_executor::UpdateRequest::latest()
...
pub(crate) fn resolve_update_download_timeout(args: &UpdateArgs) -> Duration {
    args.download_timeout
        .unwrap_or(patchbay_daemon::update_executor::DEFAULT_UPDATE_DOWNLOAD_TIMEOUT)
}
...
fn sanitize_update_error(error: anyhow::Error) -> anyhow::Error {
    let code = error
        .downcast_ref::<patchbay_daemon::update_executor::UpdateExecutorError>()
...
pub(crate) fn render_update_outcome(
    outcome: patchbay_daemon::update_executor::UpdateOutcome,
) -> RunOutput {
    ...
    let latest = outcome
        ...
        .as_deref()
        .map(str::trim)
        .filter(|version| patchbay_daemon::auto_update::is_release_version(version));
    if let Some(version) = latest {
        let _ = writeln!(stderr, "Latest version:  {version}\n");
    }

    match outcome.method {
        patchbay_daemon::update_executor::UpdateInstallMethod::Homebrew => {
            let _ = writeln!(stderr, "Updating via Homebrew...");
        }
        patchbay_daemon::update_executor::UpdateInstallMethod::Direct => {
