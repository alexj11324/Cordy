...
fn autopilot_read_parser_matches_go_registry() {
    ...
    let delete = Cli::try_parse_from(["patchbay", "autopilot", "trigger-delete", "abcd", "beef"])
    ...
    assert!(Cli::try_parse_from(["patchbay", "autopilot", "get"]).is_err());
    assert!(Cli::try_parse_from(["patchbay", "autopilot", "list", "extra"]).is_err());
    ...
    let delete = Cli::try_parse_from(["patchbay", "autopilot", "delete", "abcd"])
...
async fn autopilot_delete_resolves_prefix_and_reports_title() {
    ...
    environment.set("PATCHBAY_SERVER_URL", format!("http://{address}"));
    environment.set("PATCHBAY_WORKSPACE_ID", "workspace-1");
    let cli = Cli::try_parse_from(["patchbay", "autopilot", "delete", "abcd"])
        .expect("autopilot delete CLI");
    let output = run_with_input(&cli, &environment, &mut Cursor::new(Vec::<u8>::new()))
        .await
...
async fn autopilot_trigger_add_rejects_invalid_kind_specific_flags() {
    ...
    for (extra, expected) in [
        ...
        let mut argv = vec!["patchbay", "autopilot", "trigger-add", ID];
        argv.extend(extra);
...
async fn autopilot_list_matches_go_filter_actor_and_output_semantics() {
    ...
    environment.set("PATCHBAY_WORKSPACE_ID", "workspace-1");
    environment.set("PATCHBAY_TOKEN", "token-1");
    let cli = Cli::try_parse_from(["patchbay", "autopilot", "list", "--status", "paused"])
        .expect("autopilot list CLI");
    let output = run_with_input(&cli, &environment, &mut Cursor::new(Vec::<u8>::new()))
        .await
...
async fn autopilot_get_resolves_prefix_and_preserves_detail_envelope() {
    ...
    let table = Cli::try_parse_from(["patchbay", "autopilot", "get", "abcd", "--output", "table"])
    ...
    let json =
        Cli::try_parse_from(["patchbay", "autopilot", "get", ID]).expect("autopilot get JSON CLI");
    let output = run_with_input(&json, &environment, &mut Cursor::new(Vec::<u8>::new()))
        .await
...
async fn autopilot_prefix_errors_match_go_resolver_contract() {
    ...
    let short =
        Cli::try_parse_from(["patchbay", "autopilot", "get", "abc"]).expect("short prefix CLI");
