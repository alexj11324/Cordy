...
impl super::Cli {
    pub fn debug_enabled(&self, environment: &Environment) -> bool {
        self.debug
            || environment.trimmed("PATCHBAY_DEBUG").is_some_and(|value| {
                !matches!(
...
fn message_for(kind: ErrorKind, chinese: bool) -> String {
    let messages = match kind {
        ErrorKind::NetworkTimeout => ("Request timed out: the server did not respond in time. Check your network connection or try again later. You can raise the limit with PATCHBAY_HTTP_TIMEOUT.", "请求超时：服务器未在规定时间内响应。请检查网络连接或稍后重试。可通过 PATCHBAY_HTTP_TIMEOUT 调高超时时间。"),
        ErrorKind::NetworkDns => ("Could not resolve the Patchbay server address. Check your network connection or the --server-url setting.", "无法解析 Patchbay 服务器地址。请检查网络连接或 --server-url 配置。"),
        ErrorKind::NetworkRefused => ("Could not connect to the Patchbay server. Make sure the server address is correct and reachable.", "无法连接到 Patchbay 服务器。请确认服务器地址正确且网络可达。"),
        ErrorKind::NetworkTls => ("Could not establish a secure connection to the Patchbay server (TLS/certificate error). Check your system clock and CA certificates.", "无法与 Patchbay 服务器建立安全连接（TLS/证书错误）。请检查系统时间和 CA 证书。"),
        ErrorKind::NetworkOffline => ("Could not reach the Patchbay server. Check your network connection.", "无法访问 Patchbay 服务器。请检查网络连接。"),
        ErrorKind::AuthRequired => ("Your session has expired or you are not signed in. Run `patchbay login` to sign in again. On a self-hosted or non-OAuth setup, ask your administrator for valid credentials.", "登录已过期或尚未登录。请运行 `patchbay login` 重新登录。自托管或非 OAuth 场景请联系管理员获取有效凭证。"),
        ...
        ErrorKind::Server => ("The Patchbay service is temporarily unavailable (server error). Please try again later; if it persists, contact support. Re-run with --debug to see the raw server response.", "Patchbay 服务暂时不可用（服务器错误）。请稍后重试；若持续出现请联系支持。可加 --debug 查看服务器原始响应。"),
