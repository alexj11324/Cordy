...
pub(super) fn validate_disk_usage_args(
    ...
    if task_context {
        ...
        if environment
            .trimmed(patchbay_daemon::config::TASK_WORKSPACES_ROOT_ENV)
            .is_none()
        {
            bail!(
                "daemon-managed task requires {}",
                patchbay_daemon::config::TASK_WORKSPACES_ROOT_ENV
            );
        }
    }
    Ok(())
...
pub(super) fn limit_disk_usage_report(
    report: &mut patchbay_daemon::diskusage::DiskUsageReport,
    args: &DaemonDiskUsageArgs,
) {
...
pub(super) fn limit_disk_usage_aggregate(
    aggregate: &mut patchbay_daemon::diskusage::AggregateDiskUsageReport,
    args: &DaemonDiskUsageArgs,
) {
...
pub(super) fn resolve_disk_usage_root_for_profile(
    ...
    let configured_root = flag_root
        ...
        .or_else(|| environment.trimmed("PATCHBAY_WORKSPACES_ROOT"))
        .or_else(|| {
            (!config.workspaces_root.trim().is_empty()).then_some(config.workspaces_root.trim())
        })
        .unwrap_or_default();
    patchbay_daemon::config::resolve_workspaces_root(profile, configured_root)
        .context("resolve workspaces root")
}

pub(super) fn enumerate_disk_usage_roots(
    environment: &Environment,
) -> Result<Vec<patchbay_daemon::diskusage::DiskUsageRoot>> {
    let mut roots = Vec::new();
    let default_root = resolve_disk_usage_root_for_profile(environment, "", None)?;
    roots.push(patchbay_daemon::diskusage::DiskUsageRoot {
        profile: String::new(),
        root: default_root,
    });
    ...
    for profile in profiles {
        ...
        roots.push(patchbay_daemon::diskusage::DiskUsageRoot { profile, root });
    }
    Ok(roots)
}
...
pub(super) async fn fill_disk_usage_parent_statuses(
    ...
    let raw_server_url = cli
        .server_url
        .as_deref()
        .or_else(|| environment.trimmed("PATCHBAY_SERVER_URL"))
        .or_else(|| (!config.server_url.trim().is_empty()).then_some(config.server_url.as_str()))
        .unwrap_or(patchbay_daemon::config::DEFAULT_SERVER_URL);
    let Ok(server_url) = patchbay_daemon::config::normalize_server_base_url(raw_server_url) else {
    ...
    let client = Arc::new(patchbay_daemon::client::Client::new(server_url));
    client.set_token(&token);
    let resolver = patchbay_daemon::diskusage::ClientParentStatusResolver::new(client);
    patchbay_daemon::diskusage::resolve_parent_statuses(cancellation, report, &resolver)
        .await
        .is_err()
}
...
{
    let deadline = super::http_timeout(environment.raw("PATCHBAY_HTTP_TIMEOUT"));
    tokio::select! {
