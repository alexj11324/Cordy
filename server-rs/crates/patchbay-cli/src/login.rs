...
pub(crate) async fn run_browser_login(
    ...
    let app_url = (!app_url.trim().is_empty())
        .then_some(app_url)
        .context("No app URL configured. Run 'patchbay setup' first.")?;
...
pub(super) async fn run_login_with_urls(
    ...
    let server_url = server_override
        ...
        .context("No server configured. Run 'patchbay setup' first.")?;
    let app_url = app_override
        .map(str::to_owned)
        .or_else(|| environment.trimmed("PATCHBAY_APP_URL").map(str::to_owned))
    ...
    let token = match args
        ...
        .as_deref()
        .map(str::trim)
        .or_else(|| environment.trimmed("PATCHBAY_TOKEN"))
    {
    ...
    let (workspace_id, workspace_message) = match workspaces {
        ...
        Err(_) => {
            ...
            (
                String::new(),
                "Authenticated, but workspace discovery did not complete; run 'patchbay workspace list' to retry.\n".into(),
            )
        }
    };
