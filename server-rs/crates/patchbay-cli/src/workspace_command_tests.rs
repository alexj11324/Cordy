...
async fn workspace_list_empty_and_missing_auth_match_go_messages() {
    ...
    environment.set("PATCHBAY_SERVER_URL", format!("http://{address}"));
    environment.set("PATCHBAY_TOKEN", "workspace-token");
    let cli = Cli::try_parse_from(["patchbay", "workspace", "list"]).expect("workspace list CLI");
    ...
    environment.set("PATCHBAY_TOKEN", "");
    let error = run_with_input(&cli, &environment, &mut Cursor::new(Vec::<u8>::new()))
        .await
        .expect_err("missing token");
    assert!(error
        .to_string()
        .contains("not authenticated: run 'patchbay login' first"));
    server.abort();
}
...
async fn workspace_get_resolves_slug_but_bypasses_list_for_full_uuid() {
    ...
    for target in ["BETA", workspace_id] {
        let cli = Cli::try_parse_from(["patchbay", "workspace", "get", target, "--output", "json"])
...
fn workspace_reference_reports_ambiguous_and_missing_targets() {
    ...
    assert!(resolve_workspace_reference(&workspaces, "gamma")
        ...
        .to_string()
        .contains("run 'patchbay workspace list'"));
    assert_eq!(
...
async fn workspace_get_without_argument_requires_default_workspace() {
    ...
    let cli = Cli::try_parse_from(["patchbay", "workspace", "get"]).expect("workspace get CLI");
    let error = run_with_input(&cli, &environment, &mut Cursor::new(Vec::<u8>::new()))
        .await
        .expect_err("missing default workspace");
    assert!(error.to_string().contains(
        "workspace ID is required: pass an id/slug/prefix as argument or set PATCHBAY_WORKSPACE_ID"
    ));
}
...
async fn workspace_create_posts_complete_body_without_workspace_scope() {
    ...
    environment.set("PATCHBAY_SERVER_URL", format!("http://{address}"));
    environment.set("PATCHBAY_TOKEN", "workspace-token");
    environment.set("PATCHBAY_WORKSPACE_ID", "must-not-be-sent");
...
fn workspace_create_validates_required_and_safe_input_flags() {
    let missing_name =
        Cli::try_parse_from(["patchbay", "workspace", "create", "--slug", "support-team"])
...
async fn workspace_member_list_and_invite_use_go_http_and_output_contracts() {
    ...
    let mut environment = Environment::for_test(home.path().into(), cwd.path().into());
    environment.set("PATCHBAY_SERVER_URL", format!("http://{address}"));
    environment.set("PATCHBAY_WORKSPACE_ID", workspace_id);
