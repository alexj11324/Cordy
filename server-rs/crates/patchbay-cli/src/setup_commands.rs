...
pub(crate) async fn run_setup<R: Read>(
    ...
    let mut output = if environment.trimmed("PATCHBAY_TOKEN").is_some() {
...
pub(crate) async fn prepare_setup_profile_input(
    ...
    if let Some(token) = environment.trimmed("PATCHBAY_TOKEN") {
    ...
    RFut: std::future::Future<Output = Result<RunOutput>>,
{
    match action {
        ...
        SetupDaemonAction::LeaveRunning { active_task_count } => {
            ...
            let restart_command = "patchbay daemon restart";
            bail!(
    ...
    match &args.command {
        ...
        Some(SetupCommand::SelfHost(options)) => {
            ...
            let app_url = options
                .app_url
                .as_deref()
                .or_else(|| environment.trimmed("PATCHBAY_APP_URL"))
                .or_else(|| {
