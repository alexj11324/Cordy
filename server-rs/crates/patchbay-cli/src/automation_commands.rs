...
use super::{
    automation_webhook_url, format_automation_runs_table, format_automation_table, http_timeout,
    load_automation_agent_names, new_api_client, read_setup_confirmation, required_workspace_id,
    resolve_automation_agent, resolve_automation_id, resolve_automation_subscribers,
    resolve_automation_trigger_id, resolve_current_workspace_id, resolve_project_reference,
    value_string, Cli, Environment, OutputFormat, RunOutput,
};

#[derive(Debug, Deserialize, Serialize)]
struct AutomationListEnvelope {
    automations: Vec<Value>,
    total: i64,
}

pub(super) async fn run_automation_list(
    cli: &Cli,
    ...
    let response: AutomationListEnvelope =
        client.get_json(&path).await.context("list automations")?;
    let stdout = match output {
        OutputFormat::Json => format!("{}\n", serde_json::to_string_pretty(&response)?),
        OutputFormat::Table => {
            let agents =
                load_automation_agent_names(&client, &workspace_id, &response.automations).await;
            format_automation_table(&response.automations, full_id, &agents)
        }
    };
    Ok(RunOutput {
        stdout,
...
pub(super) async fn run_automation_get(
    cli: &Cli,
    environment: &Environment,
    id: &str,
    output: OutputFormat,
    ...
    let (automation_id, _) = resolve_automation_id(&client, &workspace_id, id)
        .await
        .map_err(|error| anyhow::anyhow!("resolve automation: {error:#}"))?;
    let response: Value = client
        .get_json(&format!("/api/automations/{automation_id}"))
        .await
...
pub(super) async fn run_automation_create(
    cli: &Cli,
    environment: &Environment,
    ...
    if !args.subscriber.is_empty() {
        body.insert(
            "subscribers".into(),
            Value::Array(
                resolve_automation_subscribers(&client, &workspace_id, &args.subscriber).await?,
            ),
        );
    }

    let result: Value = client
        .post_json("/api/automations", &body)
        .await
...
pub(super) async fn run_automation_update(
    cli: &Cli,
    environment: &Environment,
    ...
    let client = new_api_client(cli, environment)?;
    let workspace_id = resolve_current_workspace_id(cli, environment);
    let (automation_id, _) = resolve_automation_id(&client, &workspace_id, &args.id)
        .await
        .map_err(|error| anyhow::anyhow!("resolve automation: {error:#}"))?;
    ...
    let result: Value = client
        .patch_json(&format!("/api/automations/{automation_id}"), &body)
        .await
...
pub(super) async fn run_automation_delete(
    cli: &Cli,
    environment: &Environment,
    id: &str,
) -> Result<RunOutput> {
    let client = new_api_client(cli, environment)?;
    let workspace_id = resolve_current_workspace_id(cli, environment);
    let (automation_id, display) = resolve_automation_id(&client, &workspace_id, id)
        .await
        .map_err(|error| anyhow::anyhow!("resolve automation: {error:#}"))?;
    client
        .delete(&format!("/api/automations/{automation_id}"))
        .await
        .context("delete automation")?;
    Ok(RunOutput {
        stdout: format!("Automation {display} deleted.\n"),
        stderr: String::new(),
    })
}

pub(super) async fn run_automation_trigger(
    cli: &Cli,
    environment: &Environment,
    id: &str,
    output: OutputFormat,
    ...
    let run: Value = client
        .post_json(
            &format!("/api/automations/{automation_id}/trigger"),
            &Value::Null,
        )
        .await
        .context("trigger automation")?;
    Ok(RunOutput {
        stdout: match output {
            OutputFormat::Json => format!("{}\n", serde_json::to_string_pretty(&run)?),
            OutputFormat::Table => format!(
                "Automation triggered: run {} (status: {})\n",
                value_string(&run, "id"),
...
#[derive(Debug, Deserialize, Serialize)]
struct AutomationRunsEnvelope {
    runs: Vec<Value>,
    total: i64,
}

pub(super) async fn run_automation_runs(
    cli: &Cli,
    environment: &Environment,
    id: &str,
    limit: i32,
    ...
    let path = if query.is_empty() {
        format!("/api/automations/{automation_id}/runs")
    } else {
        format!("/api/automations/{automation_id}/runs?{query}")
    };
    let response: AutomationRunsEnvelope = client.get_json(&path).await.context("list runs")?;
    Ok(RunOutput {
        stdout: match output {
            OutputFormat::Json => format!("{}\n", serde_json::to_string_pretty(&response)?),
            OutputFormat::Table => format_automation_runs_table(&response.runs),
        },
        stderr: String::new(),
    })
}
pub(super) async fn run_automation_trigger_add(
    cli: &Cli,
    environment: &Environment,
    args: &AutomationTriggerAddArgs,
) -> Result<RunOutput> {
    let client = new_api_client(cli, environment)?;
    ...
    let workspace_id = resolve_current_workspace_id(cli, environment);
    let (automation_id, _) = resolve_automation_id(&client, &workspace_id, &args.automation_id)
        .await
        .map_err(|error| anyhow::anyhow!("resolve automation: {error:#}"))?;
    let result: Value = client
        .post_json(&format!("/api/automations/{automation_id}/triggers"), &body)
        .await
    ...
    let stdout = match args.output {
        ...
        OutputFormat::Table => {
            ...
            if kind == "webhook" {
                if let Some(url) = automation_webhook_url(&result, client.base_url()) {
...
pub(super) async fn run_automation_trigger_update(
    cli: &Cli,
    environment: &Environment,
    args: &AutomationTriggerUpdateArgs,
) -> Result<RunOutput> {
    let client = new_api_client(cli, environment)?;
    let mut body = serde_json::Map::new();
    ...
    let (automation_id, _) = resolve_automation_id(&client, &workspace_id, &args.automation_id)
        .await
        .map_err(|error| anyhow::anyhow!("resolve automation: {error:#}"))?;
    let trigger_id = resolve_automation_trigger_id(&client, &automation_id, &args.trigger_id)
        .await
        .map_err(|error| anyhow::anyhow!("resolve trigger: {error:#}"))?;
    let result: Value = client
        .patch_json(
            &format!("/api/automations/{automation_id}/triggers/{trigger_id}"),
            &body,
        )
        .await
...
pub(super) async fn run_automation_trigger_delete(
    cli: &Cli,
    environment: &Environment,
    automation: &str,
    trigger: &str,
    ...
    let client = new_api_client(cli, environment)?;
    let workspace_id = resolve_current_workspace_id(cli, environment);
    let (automation_id, _) = resolve_automation_id(&client, &workspace_id, automation)
        .await
        .map_err(|error| anyhow::anyhow!("resolve automation: {error:#}"))?;
    let trigger_id = resolve_automation_trigger_id(&client, &automation_id, trigger)
        .await
        .map_err(|error| anyhow::anyhow!("resolve trigger: {error:#}"))?;
    client
        .delete(&format!(
            "/api/automations/{automation_id}/triggers/{trigger_id}"
        ))
        .await
...
pub(super) async fn run_automation_trigger_rotate_url<R: Read>(
    cli: &Cli,
    environment: &Environment,
    args: &AutomationTriggerRotateUrlArgs,
    input: &mut R,
) -> Result<RunOutput> {
    let client = new_api_client(cli, environment)?;
    let workspace_id = resolve_current_workspace_id(cli, environment);
    ...
    let result: Value = client
        .post_json(
            &format!("/api/automations/{automation_id}/triggers/{trigger_id}/rotate-webhook-token"),
            &Value::Null,
        )
        .await
...
pub(super) struct AutomationArgs {
    #[command(subcommand)]
    pub(super) command: AutomationCommand,
}

#[derive(Debug, Subcommand)]
pub(super) enum AutomationCommand {
    #[command(about = "List automations in the workspace")]
    List {
    ...
    #[command(about = "Get automation details (includes triggers)")]
    Get {
        #[arg(value_name = "ID")]
        id: String,
    ...
    #[command(about = "Create a new automation")]
    Create(AutomationCreateArgs),
    #[command(about = "Update an automation")]
    Update(AutomationUpdateArgs),
    #[command(about = "Delete an automation")]
    Delete {
        #[arg(value_name = "ID")]
        id: String,
    },
    #[command(about = "Manually trigger an automation to run once")]
    Trigger {
        #[arg(value_name = "ID")]
        id: String,
        #[arg(long, value_enum, default_value_t = OutputFormat::Json)]
    ...
    #[command(about = "List execution history for an automation")]
    Runs {
        #[arg(value_name = "ID")]
        id: String,
    ...
    #[command(about = "Add a schedule or webhook trigger to an automation")]
    TriggerAdd(AutomationTriggerAddArgs),
    #[command(about = "Update an existing trigger")]
    TriggerUpdate(AutomationTriggerUpdateArgs),
    #[command(about = "Delete a trigger")]
    TriggerDelete {
        #[arg(value_name = "AUTOMATION-ID")]
        automation_id: String,
        #[arg(value_name = "TRIGGER-ID")]
        trigger_id: String,
    },
    #[command(about = "Rotate the webhook URL of a webhook trigger")]
    TriggerRotateUrl(AutomationTriggerRotateUrlArgs),
}

#[derive(Debug, Args)]
pub(super) struct AutomationTriggerAddArgs {
    #[arg(value_name = "AUTOMATION-ID")]
...
#[derive(Debug, Args)]
pub(super) struct AutomationTriggerUpdateArgs {
    #[arg(value_name = "AUTOMATION-ID")]
    pub(super) automation_id: String,
    #[arg(value_name = "TRIGGER-ID")]
...
#[derive(Debug, Args)]
pub(super) struct AutomationTriggerRotateUrlArgs {
    #[arg(value_name = "AUTOMATION-ID")]
    pub(super) automation_id: String,
    #[arg(value_name = "TRIGGER-ID")]
    pub(super) trigger_id: String,
...
#[derive(Debug, Args)]
pub(super) struct AutomationCreateArgs {
    #[arg(long, help = "Automation title (required)")]
    ...
    #[arg(
        ...
        help = "Member subscriber to notify for issues this automation creates (name or user ID; repeatable)"
    )]
...
#[derive(Debug, Args)]
pub(super) struct AutomationUpdateArgs {
    #[arg(value_name = "ID")]
    pub(super) id: String,
    #[arg(long)]
    pub(super) title: Option<String>,
    ...
    pub(super) subscriber: Vec<String>,
    #[arg(long, help = "Remove all automation subscribers")]
