...
struct AutomationResolverEnvelope {
...
pub(super) async fn resolve_automation_trigger_id(
    client: &ApiClient,
    automation_id: &str,
    input: &str,
) -> Result<String> {
    ...
    let Some(prefix) = normalize_uuid_prefix(trimmed) else {
        if trimmed.is_empty() {
            bail!("automation trigger id is required");
        }
        let compact = trimmed.replace('-', "");
        if compact.len() < 4 {
            bail!(
                "resolve automation trigger: expected a full UUID or at least 4 hex characters, got {input:?}"
            );
        }
        bail!(
            "resolve automation trigger: expected a UUID prefix containing only hex characters, got {input:?}"
        );
    };
    let response: Value = client
        .get_json(&format!("/api/automations/{automation_id}"))
        .await
        .map_err(|error| anyhow::anyhow!("resolve automation trigger: {error:#}"))?;
    ...
    match matches.as_slice() {
        [id] => Ok(id.clone()),
        [] => bail!(
            "no automation trigger found matching id prefix {input:?}; run the list command with --full-id to copy the full UUID"
        ),
        _ => bail!(
            "ambiguous automation trigger id prefix {input:?}; matches:\n  {}\nUse more characters or run the list command with --full-id",
            matches.join("\n  ")
        ),
    }
}

pub(super) async fn resolve_automation_agent(
    client: &ApiClient,
    workspace_id: &str,
    input: &str,
) -> Result<String> {
    ...
    if workspace_id.is_empty() {
        bail!(
            "workspace ID is required to resolve agents; use --workspace-id or set PATCHBAY_WORKSPACE_ID"
        );
    }
    let path = format!(
...
pub(super) async fn resolve_automation_subscribers(
    client: &ApiClient,
    workspace_id: &str,
    refs: &[String],
) -> Result<Vec<Value>> {
...
pub(super) async fn resolve_automation_id(
    client: &ApiClient,
    workspace_id: &str,
    input: &str,
    ...
    let trimmed = input.trim();
    if trimmed.is_empty() {
        bail!("automation id is required");
    }
    if is_canonical_uuid(trimmed) {
    ...
    let Some(prefix) = normalize_uuid_prefix(trimmed) else {
        ...
        if compact.len() < 4 {
            bail!(
                "resolve automation: expected a full UUID or at least 4 hex characters, got {input:?}"
            );
        }
        bail!(
            "resolve automation: expected a UUID prefix containing only hex characters, got {input:?}"
        );
    };
    if workspace_id.is_empty() {
        bail!("resolve automation: workspace_id is required to resolve automation id prefixes");
    }
    ...
    loop {
        ...
        let page: AutomationResolverEnvelope = client
            .get_json(&format!("/api/automations?{}", query.finish()))
            .await
            .map_err(|error| anyhow::anyhow!("resolve automation: {error:#}"))?;
        let page_len = page.automations.len();
        let mut added = 0;
        for automation in page.automations {
            let id = value_string(&automation, "id");
            if !id.is_empty() && seen.insert(id.clone()) {
                added += 1;
                let title = value_string(&automation, "title");
    ...
    match matches.as_slice() {
        ...
        [] => bail!(
            "no automation found matching id prefix {input:?}; run the list command with --full-id to copy the full UUID"
        ),
        _ => bail!(
            "ambiguous automation id prefix {input:?}; matches:\n  {}\nUse more characters or run the list command with --full-id",
            matches
                .iter()
                .map(|(id, _)| id.as_str())
                .collect::<Vec<_>>()
...
pub(super) async fn load_automation_agent_names(
    client: &ApiClient,
    workspace_id: &str,
    automations: &[Value],
