...
pub(super) async fn run_issue_comment_list(
    ...
    let next_before = headers
        .get("X-Patchbay-Next-Before")
        .or_else(|| headers.get("X-Cordy-Next-Before")) // legacy-brand-compat
        .and_then(|value| value.to_str().ok())
        .unwrap_or_default();
    let next_before_id = headers
        .get("X-Patchbay-Next-Before-Id")
        .or_else(|| headers.get("X-Cordy-Next-Before-Id")) // legacy-brand-compat
        .and_then(|value| value.to_str().ok())
        .unwrap_or_default();
