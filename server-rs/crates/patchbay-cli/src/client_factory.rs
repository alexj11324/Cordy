...
fn new_api_client_with_options(
    ...
    if task_context && !token.starts_with("mat_") {
        ...
        bail!(
            "agent execution context requires PATCHBAY_TOKEN to be a task-scoped mat_ token{suffix}"
        );
    }
    ...
    if server_url.is_empty() {
        bail!(
            "No server configured. Run 'patchbay setup' first{}.",
            environment.daemon_port_only_context_hint()
        );
    }
    if require_token && token.is_empty() {
        bail!(
            "not authenticated: run 'patchbay login' first{}",
            environment.daemon_port_only_context_hint()
        );
    }
    ...
    ApiClient::new(
        ...
        if include_execution_context {
            environment.raw("PATCHBAY_AGENT_ID").unwrap_or_default()
        } else {
            ""
        }
        .into(),
        if include_execution_context {
            environment.raw("PATCHBAY_TASK_ID").unwrap_or_default()
        } else {
            ""
        }
        .into(),
        http_timeout(environment.raw("PATCHBAY_HTTP_TIMEOUT")),
        CLIENT_VERSION,
    )
}
...
pub(super) fn required_workspace_id(cli: &Cli, environment: &Environment) -> Result<String> {
    ...
    if workspace_id.is_empty() {
        if environment.in_daemon_managed_execution_context() {
            bail!(
                "workspace_id is required: PATCHBAY_WORKSPACE_ID must be set by the daemon in agent execution context (no fallback to user config)"
            );
        }
        bail!(
...
pub(super) fn normalize_api_base_url(raw: &str) -> Result<String> {
    let mut url = Url::parse(raw.trim()).context("invalid PATCHBAY_SERVER_URL")?;
    match url.scheme() {
        "ws" => url
            .set_scheme("http")
        ...
        "wss" => url
            ...
            .map_err(|_| anyhow::anyhow!("set scheme"))?,
        "http" | "https" => {}
        _ => bail!("PATCHBAY_SERVER_URL must use ws, wss, http, or https"),
    }
    if url.path() == "/ws" {
        url.set_path("");
    }
