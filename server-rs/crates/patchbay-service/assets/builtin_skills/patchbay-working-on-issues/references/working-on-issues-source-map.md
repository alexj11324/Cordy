...
- Work Product attachment, execution-branch discovery, and provider snapshots:
  `server-rs/crates/patchbay-handler/src/work_product.rs`,
  `issue_pull_request.rs`, `github.rs`, and `vcs_webhook.rs`.
- Custom properties and metadata validation: `server-rs/crates/patchbay-handler/src/property.rs` and `issue_property_value.rs`.
- Task admission, execution provenance, completion, and failure recovery:
  `server-rs/crates/patchbay-handler/src/task.rs`,
  `server-rs/crates/patchbay-handler/src/daemon.rs`, and
  `server-rs/crates/patchbay-service/src/task_service.rs`.
- Persistence queries: `server-rs/crates/patchbay-db/src/queries/work_product.rs`,
  `issue.rs`, `issue_property.rs`, `github.rs`, and `task_message.rs`.
