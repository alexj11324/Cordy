---
name: patchbay-skill-importing
description: "Use when asked to import or install a specific skill into this Patchbay workspace from a URL or slug. Not for choosing which skill the user needs; never treat a local installer such as `npx skills add` as the final install."
user-invocable: false
allowed-tools: Bash(patchbay *)
---

# Importing skills into Patchbay

Use this skill when the user already provided a skill URL, slug, or a clear intent
to import a specific skill into the current Patchbay workspace.
...
A skill is installed for Patchbay only when it exists in the current workspace's
...
environment, not the Patchbay workspace DB, so Patchbay cannot manage or bind it.
...
`patchbay skill get`), not the import result envelope. Failure modes to report
...
Incorrect (bypasses Patchbay):

```bash
npx skills add https://skills.sh/owner/repo/skill
```

The skill may exist locally, but Patchbay cannot manage it as a workspace skill.
