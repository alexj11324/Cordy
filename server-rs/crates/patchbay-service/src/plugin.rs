...
const HOOK_SIGNING_INFO: &[u8] = b"patchbay-plugin-hook-signature:v1:";
...
fn hook_signing_key(deployment_key: &[u8], installation_id: Uuid) -> Result<[u8; 32], PluginError> {
    if deployment_key.is_empty() {
        return Err(plugin_errf(
            ...
            "hooks are disabled: PATCHBAY_PLUGIN_SECRET_KEY is not configured",
        ));
    }
...
/// Installs from PATCHBAY_PLUGIN_DIR instead of the network. Self-hosted operators
...
pub struct PluginService {
    ...
    pub secrets: Option<patchbay_util::secretbox::SecretBox>,
    /// PATCHBAY_PLUGIN_DIR. Empty disables local sources.
    pub local_dir: String,
    /// PATCHBAY_PLUGIN_DEV_ORIGINS parsed. See [`parse_dev_origins`].
    pub dev_origins: Vec<String>,
    /// Gates which declared contributions this build can actually run.
    pub host: patchbay_plugincontract::Capabilities,
    /// The raw PATCHBAY_PLUGIN_SECRET_KEY, used to derive each installation's
...
impl PluginService {
    ...
    pub fn new_from_env(pool: sqlx::PgPool) -> Self {
        let dev_origins =
            parse_dev_origins(&std::env::var("PATCHBAY_PLUGIN_DEV_ORIGINS").unwrap_or_default());
        Self {
            pool,
            secrets: None,
            local_dir: std::env::var("PATCHBAY_PLUGIN_DIR")
                .unwrap_or_default()
                .trim()
                .to_string(),
            dev_origins,
            host: patchbay_plugincontract::host_capabilities(),
            deployment_key: patchbay_util::secretbox::load_key("PATCHBAY_PLUGIN_SECRET_KEY")
                .unwrap_or_default(),
        }
    }
    ...
    pub async fn fetch_manifest(
        ...
        patchbay_plugincontract::parse_manifest(&raw).map_err(|e| {
    ...
    fn read_local_manifest(&self, name: &str) -> Result<Vec<u8>, PluginError> {
        if self.local_dir.is_empty() {
            return Err(plugin_errf(
                PluginErrorKind::Invalid,
                "local plugin sources require PATCHBAY_PLUGIN_DIR",
            ));
        }
        ...
        if !valid_local_name(name) {
            return Err(plugin_errf(
                PluginErrorKind::Invalid,
                "local plugin source must be a single directory name under PATCHBAY_PLUGIN_DIR",
            ));
        }
        let directory = std::path::Path::new(&self.local_dir).join(name);
        for filename in [
            patchbay_plugincontract::MANIFEST_FILENAME,
            patchbay_plugincontract::LEGACY_MANIFEST_FILENAME,
        ] {
    ...
    pub async fn preview_plugin(
        ...
        let existing = patchbay_db::queries::plugin::get_workspace_plugin_installation_by_key(
            &self.pool,
            workspace_id,
            &key,
        )
    ...
    pub async fn install_plugin(
        ...
        source_url: &str,
        granted_scopes: &[String],
    ) -> Result<patchbay_db::models::PluginInstallation, PluginError> {
        ...
        let Some(existing) = existing else {
            ...
            let installation = patchbay_db::queries::plugin::create_plugin_installation(
                &mut *tx,
                workspace_id,
                &manifest.key,
                source_url,
        ...
        for key in &orphaned_secrets {
            patchbay_db::queries::plugin::delete_plugin_secret(&mut *tx, existing.id, key)
                .await
                .map_err(|e| {
                    PluginError::with_source(
                        PluginErrorKind::Unavailable,
        ...
        let updated = patchbay_db::queries::plugin::update_plugin_installation_manifest(
            &mut *tx,
            existing.id,
            source_url,
            &manifest.version,
    ...
    pub async fn set_config(
        &self,
        installation: &patchbay_db::models::PluginInstallation,
        ...
        for (key, text) in &secrets {
            ...
            if text.is_empty() {
                patchbay_db::queries::plugin::delete_plugin_secret(&mut *tx, installation.id, key)
                    .await
                    .map_err(|e| {
                        PluginError::with_source(
                            PluginErrorKind::Unavailable,
            ...
            patchbay_db::queries::plugin::upsert_plugin_secret(
                &mut *tx,
                installation.id,
                key,
                &ciphertext,
        ...
        let updated = patchbay_db::queries::plugin::update_plugin_installation_config(
            &mut *tx,
            installation.id,
            &encoded,
        )
    ...
    pub async fn set_enabled(
        ...
        patchbay_db::queries::plugin::set_plugin_installation_enabled(
            &self.pool,
            installation.id,
            enabled,
        )
    ...
    pub async fn uninstall(
        ...
        delete_step!(
            patchbay_db::queries::plugin::delete_plugin_storage_by_installation(&mut *tx, id),
            "delete plugin storage"
        );
        delete_step!(
            patchbay_db::queries::plugin::delete_plugin_secrets_by_installation(&mut *tx, id),
        ...
        // swept later.
        delete_step!(
            patchbay_db::queries::plugin::delete_plugin_invocations_by_installation(&mut *tx, id),
        ...
        delete_step!(
            patchbay_db::queries::plugin::delete_plugin_skills_by_installation(&mut *tx, id),
            "delete plugin skills"
        );
        delete_step!(
            patchbay_db::queries::plugin::delete_plugin_installation(&mut *tx, id),
    ...
    pub async fn installation_for_workspace(
        ...
        let installation = patchbay_db::queries::plugin::get_workspace_plugin_installation(
            &self.pool,
            workspace_id,
            parsed,
        )
...
async fn fetch_remote_manifest(source_url: &str) -> Result<Vec<u8>, PluginError> {
    let endpoint = patchbay_remotemcp::validate_public_https_endpoint(source_url, &[], None)
        .await
        .map_err(|e| {
            PluginError::with_source(
                PluginErrorKind::Invalid,
    ...
    let request = http::Request::builder()
        ...
        .body(patchbay_remotemcp::RequestBody::from(Vec::new()))
        .map_err(|e| {
            PluginError::with_source(PluginErrorKind::Invalid, "build manifest request", e)
        })?;
...
fn bound_manifest_bytes(raw: &[u8]) -> Result<Vec<u8>, PluginError> {
    if raw.len() > patchbay_plugincontract::MAX_MANIFEST_SIZE {
        return Err(plugin_errf(
            PluginErrorKind::Invalid,
            format!(
                "plugin manifest exceeds {} bytes",
                patchbay_plugincontract::MAX_MANIFEST_SIZE
            ),
        ));
    }
    Ok(raw.to_vec())
...
mod tests {
    ...
    fn normalize_config_value_rejects_mismatched_values() {
        ...
        let secret_field = field(patchbay_plugincontract::CONFIG_SECRET, &[]);
