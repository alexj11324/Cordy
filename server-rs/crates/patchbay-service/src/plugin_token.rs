...
pub async fn issue_install_token(
    ...
    patchbay_db::queries::plugin::set_plugin_installation_token(
        pool,
        installation_id,
...
pub async fn revoke_install_token(
    ...
    patchbay_db::queries::plugin::set_plugin_installation_token(pool, installation_id, None)
        .await
        .map_err(|e| {
            PluginError::with_source(
                PluginErrorKind::Unavailable,
...
pub async fn authenticate_install_token(
    ...
    let installation = patchbay_db::queries::plugin::get_plugin_installation_by_token_hash(
        pool,
        Some(&hash_token(token)),
    )
    .await;
