...
pub async fn get_storage_value(
    ...
    let row = patchbay_db::queries::plugin::get_plugin_storage_value(
        pool,
        installation_id,
        scope_type,
        scope_id,
...
pub async fn list_storage_keys(
    ...
    let rows = patchbay_db::queries::plugin::list_plugin_storage_keys(
        pool,
        installation_id,
        scope_type,
        scope_id,
...
pub fn enforce_storage_quota(
    usage: patchbay_db::queries::plugin::GetPluginStorageUsageRow,
    value_bytes: usize,
) -> Result<(), PluginError> {
...
pub async fn set_storage_value(
    ...
    validate_storage_key(key)?;
    let usage = patchbay_db::queries::plugin::get_plugin_storage_usage(
        pool,
        installation_id,
        scope_type,
        scope_id,
    ...
    .unwrap_or(patchbay_db::queries::plugin::GetPluginStorageUsageRow {
        key_count: 0,
        total_bytes: 0,
    });
    enforce_storage_quota(usage, value.len())?;
    patchbay_db::queries::plugin::upsert_plugin_storage_value(
        pool,
        installation_id,
        scope_type,
        scope_id,
...
pub async fn delete_storage_value(
    ...
    validate_storage_key(key)?;
    let deleted = patchbay_db::queries::plugin::delete_plugin_storage_value(
        pool,
        installation_id,
        scope_type,
        scope_id,
...
mod tests {
    ...
    fn usage(
        key_count: i64,
        total_bytes: i64,
    ) -> patchbay_db::queries::plugin::GetPluginStorageUsageRow {
        patchbay_db::queries::plugin::GetPluginStorageUsageRow {
            key_count,
            total_bytes,
        }
