...
pub fn validate_key(key: &str) -> Result<String, String> {
    ...
    // `patchbay issue status <id> human_review` is unambiguous to type.
    let valid = {
...
{
    patchbay_db::queries::issue_status::seed_issue_status_entries(executor, workspace_id).await?;
    Ok(())
}
...
impl Resolver {
        ...
        E: Executor<'e, Database = sqlx::Postgres>,
    {
        ...
        if self.categories.is_none() {
            match patchbay_db::queries::issue_status::list_issue_status_entries(
                executor,
                self.workspace_id,
                true,
            )
    ...
    E: Executor<'e, Database = sqlx::Postgres> + Copy,
{
    ...
    let keys = patchbay_db::queries::issue_status::list_issue_status_keys_by_categories(
        executor,
        workspace_id,
        &valid_refs,
    )
...
{
    let entries =
        patchbay_db::queries::issue_status::list_issue_status_entries(executor, workspace_id, true)
            .await?;
