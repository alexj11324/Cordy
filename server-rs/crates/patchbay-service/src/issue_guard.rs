...
pub async fn lock_and_find_active_duplicate(
    ...
    let duplicate = patchbay_db::queries::issue::find_active_duplicate_issue(
        &mut *executor,
        workspace_id,
        project_id,
        parent_issue_id,
...
mod tests {
    ...
    fn duplicate_message_shape_is_stable() {
        let m = duplicate_message("PATCHBAY-12", "Fix bug", "todo");
        assert_eq!(
            m,
            "Active duplicate issue exists: PATCHBAY-12 Fix bug (status: todo). Set allow_duplicate=true or use --allow-duplicate to create another."
        );
    }
    ...
    fn active_duplicate_error_displays_the_message() {
        let e = ActiveDuplicateError {
            id: "0197".into(),
            identifier: "PATCHBAY-12".into(),
            title: "Fix bug".into(),
            status: "todo".into(),
        };
        assert_eq!(
            e.to_string(),
            duplicate_message("PATCHBAY-12", "Fix bug", "todo")
        );
    }
