//! Automation domain service — port of `service/automation.go` +
//! `automation_quota.go`. This slice lands the types, constructor seams and
...
use patchbay_db::models::{Agent, Automation, AutomationRun};
...
use patchbay_db::queries::automation_quota::{
    consume_automation_quota_reservation, create_automation_quota_reservation,
    ensure_automation_quota_period, get_automation_quota_period,
    get_automation_quota_reservation_by_key, increment_automation_quota_blocked,
...
pub const DEFAULT_AUTOMATION_TRIGGER_TIMEZONE: &str = "UTC";

/// Duplicate-suppression window for recently dispatched runs.
pub const AUTOMATION_RECENT_DUPLICATE_WINDOW: Duration = Duration::from_secs(60);

/// The timezone used to render Automation trigger output when a trigger has no
...
pub fn default_trigger_location() -> (Tz, &'static str) {
    (chrono_tz::UTC, DEFAULT_AUTOMATION_TRIGGER_TIMEZONE)
}
...
pub struct AutomationService {
    ...
    pub quota_metrics: Option<Arc<dyn AutomationQuotaMetrics>>,
...
pub async fn record_automation_rule_version(
    executor: &PgPool,
    ap: &Automation,
    ...
    create_automation_rule_version(
        executor,
        ap.id,
        ap.workspace_id,
        published_by_type,
    ...
    .await?
    .ok_or_else(|| anyhow::anyhow!("create automation rule version: no row"))?;
    Ok(())
}
...
pub fn is_run_complete(run: &AutomationRun) -> bool {
...
impl AutomationService {
    /// Returns the agent that will actually execute the automation's work and
...
pub fn automation_error_type(reason: &str) -> &'static str {
...
pub fn automation_actor_id(ap: &Automation) -> String {
...
pub fn automation_run_duration_ms(run: &AutomationRun) -> i64 {
...
impl AutomationService {
    ...
    pub async fn resolve_trigger_timezone(&self, trigger_id: Option<Uuid>) -> String {
        let Some(trigger_id) = trigger_id else {
            return DEFAULT_AUTOMATION_TRIGGER_TIMEZONE.to_string();
        };
        let trigger = match get_automation_trigger(&self.pool, trigger_id).await {
            Ok(Some(t)) => t,
            Ok(None) | Err(_) => {
                tracing::warn!(trigger_id = %trigger_id, "failed to load automation trigger timezone; falling back to UTC");
                return DEFAULT_AUTOMATION_TRIGGER_TIMEZONE.to_string();
            }
        };
        ...
        if timezone.parse::<Tz>().is_err() {
            tracing::warn!(
                ...
                timezone = %timezone,
                "invalid automation trigger timezone; falling back to UTC"
            );
            return DEFAULT_AUTOMATION_TRIGGER_TIMEZONE.to_string();
        }
        timezone.to_string()
...
fn trigger_location(timezone: &str) -> (Tz, String) {
    ...
    let label = if label.is_empty() {
        DEFAULT_AUTOMATION_TRIGGER_TIMEZONE
    } else {
        label
    };
    match label.parse::<Tz>() {
        ...
        Err(_) => (
            chrono_tz::UTC,
            DEFAULT_AUTOMATION_TRIGGER_TIMEZONE.to_string(),
        ),
    }
}

fn automation_run_triggered_at(run: &AutomationRun) -> DateTime<Utc> {
    run.triggered_at
}

pub fn format_automation_run_timestamp(run: &AutomationRun, timezone: &str) -> String {
    let triggered_at = automation_run_triggered_at(run);
...
pub fn format_automation_run_date(run: &AutomationRun, timezone: &str) -> String {
    let triggered_at = automation_run_triggered_at(run);
...
impl AutomationService {
    ...
    pub fn build_issue_description(
        ...
        let triggered_at = format_automation_run_timestamp(run, trigger_timezone);
...
/// substitution logic and the automations docs.
...
/// An empty template is valid (the automation falls back to its own Title).
...
impl AutomationService {
    ...
    pub fn interpolate_template(
        ...
        let trigger_date = format_automation_run_date(run, trigger_timezone);
        token_re()
...
// --- Quota types (automation_quota.go head) ----------------------------------

pub trait AutomationQuotaMetrics: Send + Sync {
    fn record_automation_quota_decision(&self, action: &str, source: &str, result: &str);
}

impl AutomationQuotaMetrics for patchbay_metrics::BusinessMetrics {
    fn record_automation_quota_decision(&self, action: &str, source: &str, result: &str) {
        patchbay_metrics::BusinessMetrics::record_automation_quota_decision(
...
#[derive(Debug, Clone)]
pub struct AutomationQuotaExceededError {
    pub used: i64,
    pub reserved: i64,
    pub limit: i64,
...
impl std::fmt::Display for AutomationQuotaExceededError {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        f.write_str("automation run quota exceeded")
    }
}

impl std::error::Error for AutomationQuotaExceededError {}
...
pub struct AutomationQuotaUsage {
    pub enabled: bool,
    pub action: String,
    pub used: Option<i64>,
...
#[derive(Debug, Clone)]
pub(crate) struct AutomationQuotaPolicy {
    pub action: String,
    pub limit: i64,
...
pub fn valid_automation_execution_source(source: &str) -> bool {
...
/// db.CreateAutomationRunParams minus the columns the quota path injects.
#[derive(Debug, Clone, Default)]
pub struct CreateAutomationRunParams {
...
impl AutomationService {
    fn record_quota_decision(&self, action: &str, source: &str, result: &str) {
        if let Some(m) = &self.quota_metrics {
            m.record_automation_quota_decision(action, source, result);
        }
    }
    ...
    fn run_from_terminal_row(row: UpdateAutomationRunTerminalWithQuotaRow) -> AutomationRun {
        AutomationRun {
            id: row.id.unwrap_or_default(),
            automation_id: row.automation_id.unwrap_or_default(),
    ...
    pub async fn complete_run(
        ...
        let row = update_automation_run_terminal_with_quota(
            &self.pool,
            "completed",
            result,
            None,
        ...
        .await?
        .ok_or_else(|| anyhow::anyhow!("complete automation run: no row"))?;
    ...
    pub async fn fail_automation_run(
        ...
        .await?
        .ok_or_else(|| anyhow::anyhow!("fail automation run: no row"))?;
        Ok(Self::run_from_terminal_row(row))
    }

    pub(crate) async fn skip_automation_run(
        &self,
        run_id: Uuid,
        failure_reason: &str,
        reason_code: Option<&str>,
        ...
        .await?
        .ok_or_else(|| anyhow::anyhow!("skip automation run: no row"))?;
        Ok(Self::run_from_terminal_row(row))
    }

    pub async fn recover_partial_automation_run(&self, run_id: Uuid) -> anyhow::Result<bool> {
        let rows =
            patchbay_db::queries::automation::recover_partial_automation_run(&self.pool, run_id)
                .await?;
...
/// another actor).
pub async fn settle_automation_quota<'e, E>(
    executor: E,
    ...
    E: sqlx::Executor<'e, Database = sqlx::Postgres>,
{
    ...
    let res = if consume {
        consume_automation_quota_reservation(executor, id).await
    } else {
        release_automation_quota_reservation(executor, id).await
    };
    match res {
...
impl AutomationService {
    fn publish_run_done(&self, workspace_id: &str, run: &AutomationRun, status: &str) {
        self.bus.publish(&patchbay_events::Event {
            event_type: patchbay_protocol::EVENT_AUTOMATION_RUN_DONE.to_string(),
    ...
    /// resolved leader for team automations) so per-agent task counts line up
    /// with what daemons report.
    async fn capture_issue_created_from_automation(
        &self,
        ap: &Automation,
        run: &AutomationRun,
    ...
    async fn capture_automation_run_started(
        &self,
        ap: &Automation,
        run: &AutomationRun,
        trigger_source: &str,
    ...
    async fn capture_automation_run_failed(
        &self,
        ap: &Automation,
        run: &AutomationRun,
        trigger_source: &str,
...
impl AutomationService {
    ...
    pub(crate) async fn handle_dispatch_skip(
        ...
        let updated = match self
            .skip_automation_run(run.id, &skip_err.reason, Some(skip_err.code.as_str()))
            .await
        {
            Ok(updated) => updated,
            Err(uerr) => {
        ...
        *run = updated;
        tracing::info!(
            ...
            "automation dispatch skipped post-admission"
        );
        ...
        let _ = update_automation_last_run_at(&self.pool, ap.id).await;
    ...
    pub(crate) async fn fail_run(&self, run_id: Uuid, reason: &str) {
        if let Err(err) = self
            ...
            .await
        {
            tracing::warn!(run_id = %run_id, error = %err, "failed to mark automation run as failed");
        }
    }
}
...
impl AutomationService {
    ...
    pub async fn sync_run_from_issue(
        ...
        let Some(run) = get_automation_run_by_issue(&self.pool, issue.id).await? else {
        ...
        let ws_id = automation.workspace_id.to_string();
        ...
        match effective_status.as_str() {
            "done" | "in_review" => {
                ...
                self.capture_automation_run_completed(&automation, &updated)
                    .await;
    ...
    pub async fn sync_run_from_task(
        ...
        let Some(run_id) = task.automation_run_id else {
            return Ok(());
        };
    ...
    /// origin_type=automation by construction), so one query both identifies
    ...
    pub async fn sync_run_from_linked_issue_task(
        ...
        let Some(run) = get_automation_run_by_issue(&self.pool, issue_id).await? else {
            return Ok(());
        };
        ...
        let updated = self
            .fail_automation_run(run.id, opt_str_reason(&reason), None)
            .await?;
        let source = updated.source.clone();
        self.capture_automation_run_failed(&automation, &updated, &source, &reason)
            .await;
...
pub enum QuotaAdmissionError {
    #[error("invalid automation execution source {0:?}")]
    InvalidSource(String),
    #[error("{0}")]
    Exceeded(#[from] AutomationQuotaExceededError),
    #[error("{0}")]
    Internal(String),
}
...
impl AutomationService {
    ...
    async fn quota_policy(&self, workspace_id: Uuid) -> Option<AutomationQuotaPolicy> {
        let entitlements = self.entitlements.as_ref()?;
        let decision = entitlements.gate_automation_runs(workspace_id).await;
    ...
    pub async fn create_run_with_quota(
    ...
    ) -> Result<(AutomationRun, bool), QuotaAdmissionError> {
        if !valid_automation_execution_source(source) {
        ...
        let Some(policy) = self.quota_policy(workspace_id).await else {
            let run = insert_run(&self.pool, params, Uuid::nil())
                .await
                .map_err(|e| quota_admission_internal("create automation run", e))?
                .ok_or_else(|| {
                    QuotaAdmissionError::Internal("create automation run: no row".into())
                })?;
            return Ok((run, false));
        };
        ...
        let mut period = ensure_automation_quota_period(&mut *tx, workspace_id, Some(ps), Some(pe))
            .await
        ...
        let existing = match get_automation_quota_reservation_by_key(
            &mut *tx,
            workspace_id,
            Some(ps),
            Some(pe),
        ...
        if let Some(existing) = existing {
            match get_automation_run_by_quota_reservation(&mut *tx, existing.id).await {
                Ok(Some(run)) => {
                ...
                Ok(None) if existing.state == "reserved" => {
                    ...
                    settle_automation_quota(&mut *tx, Some(existing.id), false)
                        .await
                        .map_err(|e| {
        ...
        if would_block && policy.action == EntitlementAction::Enforce.as_str() {
            increment_automation_quota_blocked(&mut *tx, source, workspace_id, Some(ps), Some(pe))
                .await
        ...
        let reservation = create_automation_quota_reservation(
            &mut *tx,
            workspace_id,
            Some(ps),
            Some(pe),
        ...
        increment_automation_quota_reserved(&mut *tx, workspace_id, Some(ps), Some(pe))
            .await
    ...
    pub async fn quota_usage(&self, workspace_id: Uuid) -> anyhow::Result<AutomationQuotaUsage> {
        let Some(policy) = self.quota_policy(workspace_id).await else {
            return Ok(AutomationQuotaUsage::default());
        };
        let (used_count, reserved_count, blocked_value) = match get_automation_quota_period(
            &self.pool,
            workspace_id,
        ...
        .await
        {
            ...
            Err(e) => return Err(anyhow::anyhow!("load automation quota usage: {e}")),
        };
        let blocked_counts: std::collections::HashMap<String, i64> = if blocked_value.is_null() {
            std::collections::HashMap::new()
        } else {
            serde_json::from_value(blocked_value)
                .map_err(|e| anyhow::anyhow!("decode automation quota blocked counts: {e}"))?
        };
        Ok(AutomationQuotaUsage {
            enabled: true,
    ...
    pub async fn reconcile_quota_reservations(
        ...
        let reservations = list_recoverable_automation_quota_reservations(
            &self.pool,
        ...
        for reservation in reservations {
            let changed = match get_automation_run_by_quota_reservation(&self.pool, reservation.id)
                .await
            {
                // No run ever linked: an orphaned reservation — release it.
                Ok(None) => settle_automation_quota(&self.pool, Some(reservation.id), false)
                    .await
                ...
                Ok(Some(run)) => match run.status.as_str() {
                    "completed" => settle_automation_quota(&self.pool, Some(reservation.id), true)
                        .await
                        .context("consume completed quota reservation")?,
                    "failed" | "skipped" => {
                        settle_automation_quota(&self.pool, Some(reservation.id), false)
                            .await
                    ...
                    "pending" | "issue_created" | "running"
                        if matches!(run.source.as_str(), "manual" | "api")
                            ...
                            && run.task_id.is_none() =>
                    {
                        self.recover_partial_automation_run(run.id)
                            .await
...
impl AutomationService {
    ...
    pub(crate) async fn should_skip_dispatch(
        ...
        let agent = match self.resolve_leader(ap).await {
            ...
            Err(err) => {
                ...
                tracing::warn!(
                    ...
                    error = %err,
                    "automation admission: failed to resolve leader"
                );
                match err {
        ...
        let verdict = match agent_readiness(&self.pool, &agent).await {
            ...
            Err(err) => {
                tracing::warn!(
                    ...
                    error = %err,
                    "automation admission: failed to load runtime"
                );
                return None;
            }
        };
        if !verdict.ready() {
                ...
                && verdict.availability == AgentAvailability::Waitable
            {
                tracing::info!(
                    ...
                    "automation admission: allowing create_issue dispatch for offline runtime"
                );
            } else {
                return Some((
        ...
        // Invocation gate at the automation layer (PB-3963 / PB-4525): a
    ...
    /// bypass.
    async fn automation_admit_invoke(
        &self,
        ap: &Automation,
        agent: &Agent,
        actor_user_id: Option<Uuid>,
    ...
    /// a manual automation "run now" where the clicker, not the creator, is
    ...
    /// Automation fallback: the automation creator is the admission principal.
    /// Member creators must be workspace members; agent-created automations
    /// count as workspace-internal principals.
    async fn can_creator_invoke_agent(&self, ap: &Automation, agent: &Agent) -> bool {
        ...
        if agent.permission_mode != "public_to" {
            ...
            // branch above passes. Admins and agent-created automations do
            // not bypass.
            return false;
        }
        let Ok(targets) = list_agent_invocation_targets(&self.pool, agent.id).await else {
    ...
    pub(crate) async fn record_skipped_run(
        ...
        match update_automation_run_skipped(&self.pool, run.id, Some(reason), code_str).await {
            ...
            Err(err) => {
                tracing::warn!(
                    run_id = %run.id,
                    error = %err,
                    "failed to set skip reason on automation run"
                );
            }
        }
        ...
        tracing::info!(
            ...
            source = %source,
            reason = %reason,
            "automation dispatch skipped"
        );

        let _ = update_automation_last_run_at(&self.pool, automation.id).await;

        self.publish_run_done(&automation.workspace_id.to_string(), &run, "skipped");
        Ok(run)
    }
}
...
use patchbay_db::queries::automation::{
    create_automation_task, get_automation_in_workspace, list_automation_subscribers,
    update_automation_run_issue_created, update_automation_run_running,
};
...
impl AutomationService {
    ...
    pub async fn dispatch_automation(
        ...
        if let Some((reason, code)) = self.should_skip_dispatch(automation, actor_user_id).await {
            let run = self
                .record_skipped_run(
                    automation,
                    trigger_id,
        ...
        let (mut run, reused) = match self
            .create_run_with_quota(automation.workspace_id, source, idempotency_key, &params)
            .await
        {
            Ok(pair) => pair,
        ...
        self.capture_automation_run_started(automation, &run, source)
            .await;
        self.dispatch_automation_run(automation, trigger_id, source, &mut run, actor_user_id)
            .await
    }
    ...
    pub async fn dispatch_automation_run(
        &self,
        automation: &Automation,
        trigger_id: Uuid,
        source: &str,
        ...
        match automation.execution_mode.as_str() {
            "create_issue" => {
                let trigger_timezone = self.resolve_trigger_timezone(Some(trigger_id)).await;
                if let Err(err) = self
                    .dispatch_create_issue(automation, run, &trigger_timezone, actor_user_id)
                    .await
                {
                    if let Some(code) = self.handle_dispatch_skip(automation, run, &err).await {
                        return Ok(DispatchOutcome {
                            run: run.clone(),
                            reason_code: Some(code),
                        });
                    ...
                    self.capture_automation_run_failed(automation, run, source, &err_text)
                        .await;
                    return Err(anyhow::anyhow!("dispatch create_issue: {err}"));
                }
            }
            "run_only" => {
                if let Err(err) = self.dispatch_run_only(automation, run, actor_user_id).await {
                    if let Some(code) = self.handle_dispatch_skip(automation, run, &err).await {
                        return Ok(DispatchOutcome {
                            run: run.clone(),
                            reason_code: Some(code),
            ...
            other => {
                ...
                self.capture_automation_run_failed(automation, run, source, &msg)
                    .await;
                return Err(anyhow::anyhow!("{msg}"));
            }
        }

        update_automation_last_run_at(&self.pool, automation.id)
            .await
            .ok();

        self.bus.publish(&patchbay_events::Event {
            event_type: patchbay_protocol::EVENT_AUTOMATION_RUN_START.to_string(),
            workspace_id: automation.workspace_id.to_string(),
            actor_type: "system".to_string(),
            actor_id: String::new(),
            payload: serde_json::json!({
                "run_id": run.id.to_string(),
    ...
    /// recent-duplicate guard → counter/position → issue with automation origin
    ...
    async fn dispatch_create_issue(
        ...
        // Refresh the automation row at dispatch time so the current project
        // binding wins over any stale snapshot the caller cached.
        let current_automation = get_automation_in_workspace(&mut *tx, ap.id, ap.workspace_id)
            .await
            .map_err(|e| de("refresh automation", e))?
            .ok_or_else(|| de_msg("refresh automation: not found"))?;
        let project_id = current_automation.project_id.unwrap_or_else(Uuid::nil);

        let guard_window = chrono::Duration::from_std(AUTOMATION_RECENT_DUPLICATE_WINDOW)
            .map_err(|e| de("recent duplicate guard", e))?;
        if let (Some(duplicate), true) =
            crate::issue_guard::lock_and_find_recent_automation_duplicate(
                &mut tx,
                ap.workspace_id,
                Some(ap.id),
            ...
            .map_err(|e| de("recent duplicate guard", e))?
        {
            return Err(DispatchError::Skipped(ErrDispatchSkipped {
                reason: format!("recent duplicate automation issue: {}", duplicate.id),
        ...
        // the human configurer rides origin_type=automation + origin_id.
        ...
        let template_subs = list_automation_subscribers(&mut *tx, ap.id)
            .await
        ...
        // count only fully observable automation issues and closes the crash
        // window where recovery would see an orphan issue without its run.
        let updated_run = update_automation_run_issue_created(&mut *tx, run.id, issue.id)
            .await
            .map_err(|e| de("link run to issue", e))?
            .ok_or_else(|| de_msg("link run to issue: no row"))?;
        *run = updated_run;
        settle_automation_quota(&mut *tx, run.quota_reservation_id, true)
            .await
            .map_err(|e| de("consume quota reservation", e))?;

        tx.commit().await.map_err(|e| de("commit tx", e))?;

        // issue:created drives the existing event chain; for team automations
        ...
        self.capture_issue_created_from_automation(ap, run, &issue, leader.id)
            .await;
        ...
        self.notify_automation_subscribers_on_create(ap, &issue, leader.id, &template_subs)
            .await;
        ...
        tracing::info!(
            ...
            leader_id = %leader.id,
            run_id = %run.id,
            "automation dispatched (create_issue)"
        );
        Ok(())
    }
    ...
    async fn notify_automation_subscribers_on_create(
        &self,
        ap: &Automation,
        issue: &patchbay_db::models::Issue,
        leader_id: Uuid,
        subscribers: &[patchbay_db::models::AutomationSubscriber],
    ) {
        if subscribers.is_empty() {
            return;
        }
        let details = serde_json::json!({
            "automation_id": ap.id.to_string(),
            "reason": "automation",
        });
            ...
            .await
            {
                ...
                Err(err) => {
                    tracing::error!(
                        ...
                        error = %err,
                        "automation subscriber inbox write failed"
                    );
                    continue;
                }
            };
    ...
    async fn dispatch_run_only(
        ...
        let attr = match actor_user_id {
            Some(actor) => crate::attribution::direct_human_run(
                Some(actor),
                crate::attribution::evidence_automation_run(),
                Some(run.id),
            ),
            None => {
        ...
        let task = create_automation_task(
            ...
            // without joining back to automation.
        ...
        .await
        .map_err(|e| de("create automation task", e))?
        .ok_or_else(|| de_msg("create automation task: no row"))?;

        match update_automation_run_running(&self.pool, run.id, task.id).await {
            Ok(Some(updated)) => *run = updated,
            Ok(None) => {}
            Err(err) => {
        ...
        tracing::info!(
            ...
            task_id = %task.id,
            run_id = %run.id,
            "automation dispatched (run_only)"
        );
        Ok(())
    }
...
impl AutomationService {
    ...
    pub async fn admit_automation_webhook_delivery(
        ...
        {
            Ok((run, _)) => {
                self.capture_automation_run_started(automation, &run, "webhook")
                    .await;
                Ok(run)
            }
    ...
    pub async fn dispatch_automation_for_webhook_delivery(
        ...
        if is_run_complete(&run) {
            if automation.execution_mode == "create_issue" && run.issue_id.is_some() {
                self.ensure_webhook_create_issue_task(automation, &run)
                    .await?;
            }
            return Ok(run);
        }
    ...
    pub async fn dispatch_automation_for_plan(
        ...
        match get_automation_run_by_trigger_and_planned(&self.pool, trigger_id, Some(planned_at))
            .await
        {
            Ok(Some(existing)) => {
...
mod dispatch_contract_tests {
    ...
    async fn required_pool() -> PgPool {
        let url = std::env::var("DATABASE_URL")
            .expect("DATABASE_URL is required for automation dispatch contracts");
        PgPoolOptions::new()
            .max_connections(4)
            .connect(&url)
            .await
    ...
    async fn load_run(pool: &PgPool, automation_id: Uuid) -> AutomationRun {
        let id: Uuid = sqlx::query_scalar(
            "INSERT INTO automation_run (automation_id, source, status, trigger_payload) \
    ...
    async fn cleanup_dispatch_rows(pool: &PgPool, workspace_id: Uuid, user_id: Uuid) {
        ...
        sqlx::query(
            "DELETE FROM automation_run WHERE automation_id IN \
        ...
        sqlx::query("DELETE FROM automation_rule_version WHERE workspace_id = $1")
            .bind(workspace_id)
            .execute(pool)
            .await
            .expect("delete automation rule versions");
        for table in ["issue", "automation", "agent", "agent_runtime", "member"] {
    ...
    async fn production_create_issue_dispatch_uses_top_position_and_recent_duplicate_guard() {
        ...
        let workspace_id: Uuid = sqlx::query_scalar(
            "INSERT INTO workspace (name, slug) VALUES ('automation dispatch contract', $1) RETURNING id",
        )
        .bind(format!("automation-dispatch-{}", Uuid::now_v7().simple()))
        .fetch_one(&pool)
        .await
        .expect("create contract workspace");
        let user_id: Uuid = sqlx::query_scalar(
            "INSERT INTO \"user\" (name, email) VALUES ('automation dispatch owner', $1) RETURNING id",
        )
        .bind(format!("automation-dispatch-{}@example.test", workspace_id.simple()))
        .fetch_one(&pool)
        .await
        ...
        let runtime_id: Uuid = sqlx::query_scalar(
            "INSERT INTO agent_runtime \
             ...
             VALUES ($1, $2, 'automation dispatch runtime', 'local', $3, 'online', now()) RETURNING id",
        )
        .bind(workspace_id)
        .bind(format!("automation-dispatch-{workspace_id}"))
        .bind(format!("automation-dispatch-{workspace_id}"))
        .fetch_one(&pool)
        .await
        ...
        let agent_id: Uuid = sqlx::query_scalar(
            "INSERT INTO agent \
             ...
             VALUES ($1, 'automation dispatch agent', 'local', 'idle', $2, $3) RETURNING id",
        )
        .bind(workspace_id)
        .bind(user_id)
        .bind(runtime_id)
        ...
        .fetch_one(&pool)
        .await
        .expect("create contract automation");
        let automation = get_automation(&pool, automation_id)
            .await
            .expect("load contract automation")
            .expect("contract automation exists");
        record_automation_rule_version(&pool, &automation, "member", Some(user_id))
            .await
        ...
        let service = AutomationService::new(pool.clone(), bus, tasks);
        let mut first_run = load_run(&pool, automation_id).await;
        let first = service
        ...
        let mut duplicate_run = load_run(&pool, automation_id).await;
        ...
        let automation_issue_count: i64 = sqlx::query_scalar(
            "SELECT count(*) FROM issue \
             WHERE workspace_id = $1 AND origin_type = 'automation' AND origin_id = $2",
        )
        .bind(workspace_id)
        .bind(automation_id)
        .fetch_one(&pool)
        .await
        .expect("count automation issues after duplicate dispatch");
        assert_eq!(automation_issue_count, 1);
