//! Sustained-failure Automation monitor.
use std::future::Future;
use std::sync::Arc;
use std::time::Duration;

use chrono::{SecondsFormat, Utc};
use patchbay_db::models::{Automation, InboxItem};
use patchbay_db::queries::automation::SelectAutomationsExceedingFailureThresholdRow as Candidate;
use patchbay_db::queries::{agent, automation, inbox, member};
...
use crate::automation::record_automation_rule_version;
...
impl FailureMonitorConfig {
    pub fn from_env() -> Self {
        ...
        if let Some(raw) = env_value("AUTOMATION_FAIL_MONITOR_INTERVAL") {
            config.interval = match parse_go_duration(&raw) {
                ...
                None => {
                    warn_invalid("AUTOMATION_FAIL_MONITOR_INTERVAL", &raw);
                    config.interval
                }
            };
        }
        if let Some(raw) = env_value("AUTOMATION_FAIL_MONITOR_LOOKBACK") {
            match parse_go_duration(&raw) {
                Some(ParsedDuration::Positive(value)) => config.lookback = value,
                _ => warn_invalid("AUTOMATION_FAIL_MONITOR_LOOKBACK", &raw),
            }
        }
        if let Some(raw) = env_value("AUTOMATION_FAIL_MONITOR_STARTUP_DELAY") {
            match parse_go_duration(&raw) {
                ...
                _ => warn_invalid("AUTOMATION_FAIL_MONITOR_STARTUP_DELAY", &raw),
            }
        }
        if let Some(raw) = env_value("AUTOMATION_FAIL_MONITOR_MIN_RUNS") {
            match raw.trim().parse::<i64>().ok().filter(|value| *value > 0) {
                Some(value) => config.min_runs = value,
                None => warn_invalid("AUTOMATION_FAIL_MONITOR_MIN_RUNS", &raw),
            }
        }
        if let Some(raw) = env_value("AUTOMATION_FAIL_MONITOR_FAIL_RATIO") {
            match raw
                .trim()
                .parse::<f64>()
                .ok()
                .filter(|value| *value > 0.0 && *value <= 1.0)
            {
                ...
                None => warn_invalid("AUTOMATION_FAIL_MONITOR_FAIL_RATIO", &raw),
            }
        }
        config
    }
...
impl FailureMonitorMetrics for patchbay_metrics::BusinessMetrics {
    fn record(&self, stage: &'static str, outcome: &'static str) {
        self.record_automation_failure_monitor(stage, outcome);
    }
}
...
#[derive(Clone)]
pub struct AutomationFailureMonitor {
    pool: PgPool,
    bus: Arc<Bus>,
...
impl AutomationFailureMonitor {
    ...
    pub fn start(self, cancel: CancellationToken) -> FailureMonitorRuntime {
        let Some(interval) = self.config.interval else {
            tracing::info!("automation failure monitor: disabled (interval <= 0)");
    ...
    pub async fn run_once(&self, cancel: &CancellationToken) -> SweepReport {
        ...
        let candidates = match self
            .retry_db(cancel, "sweep", || {
                automation::select_automations_exceeding_failure_threshold(
                    &self.pool,
        ...
        if !candidates.is_empty() {
            tracing::info!(
                ...
                "automation failure monitor: candidates"
            );
        }
    ...
    pub async fn process_next(
        ...
        let paused = match self
            .retry_db(cancel, "pause", || {
                automation::system_pause_automation_in_workspace(&self.pool, id, workspace_id)
            })
            .await
        {
            ...
            record_automation_rule_version(&self.pool, &paused, "system", None).await
        {
            ...
            tracing::warn!(%error, automation_id = %paused.id, "automation failure monitor: record rule version failed");
        } else {
        ...
        match self.resolve_recipient(cancel, &paused).await {
            Ok(Some(recipient)) => {
                    ...
                    .await
                {
                    ...
                    tracing::warn!(%error, automation_id = %paused.id, recipient_id = %recipient.id,
                        "automation failure monitor: inbox write failed");
                } else {
            ...
            Err(failure) => {
                side_effect_failures += 1;
                tracing::debug!(error = %failure.message, automation_id = %paused.id,
                    "automation failure monitor: recipient resolution failed");
            }
        }

        self.bus.publish(&Event {
            event_type: patchbay_protocol::EVENT_AUTOMATION_UPDATED.into(),
            workspace_id: paused.workspace_id.to_string(),
            actor_type: "system".into(),
            payload: json!({
                "automation": automation_event_payload(&paused),
    ...
    async fn create_and_publish_inbox(
        ...
        let title = format!("Automation paused: {}", paused.title);
        let body = format!(
            "Auto-paused after {} of {} runs failed ({fail_pct:.1}%) in the last {}. Investigate the failures, fix the root cause, then re-enable from the automation page.",
            candidate.failed_runs,
            candidate.total_runs,
            format_lookback(self.config.lookback),
        );
        let details = json!({
            "automation_id": paused.id.to_string(),
            "automation_title": paused.title,
            "failed_runs": candidate.failed_runs,
            "total_runs": candidate.total_runs,
            "fail_pct": fail_pct,
        ...
        .await?
        .ok_or_else(|| anyhow::anyhow!("create automation paused inbox: no row"))?;
        self.bus.publish(&Event {
...
fn automation_event_payload(value: &Automation) -> Value {
    json!({
