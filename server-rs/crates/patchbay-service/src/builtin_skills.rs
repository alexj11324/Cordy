...
//! The `<name>` directory carries a `patchbay-` prefix so its on-disk slug can
...
mod tests {
    ...
    #[test]
    fn every_skill_has_nonempty_content_and_patchbay_prefix() {
        for s in load_builtin_skills() {
            assert!(
                s.name.starts_with("patchbay-"),
