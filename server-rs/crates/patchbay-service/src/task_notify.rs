...
use patchbay_db::models::{AgentTaskQueue, ChatMessage, Comment, Issue};
...
use patchbay_db::queries::comment::{create_comment, get_thread_root, unresolve_comment};
use patchbay_db::queries::inbox::create_inbox_item;
use patchbay_db::queries::issue::get_issue;
use patchbay_db::queries::issue::get_issue_by_origin;
use patchbay_db::queries::skill::{list_agent_skills, list_skill_files};
...
impl TaskService {
    ...
    pub fn report_progress(
        ...
        self.bus.publish(&patchbay_events::Event {
            event_type: patchbay_protocol::EVENT_TASK_PROGRESS.to_string(),
    ...
    pub(crate) async fn broadcast_chat_done(
        ...
        if let Some(msg) = msg {
            ...
            payload.created_at = patchbay_util::rfc3339_nano(msg.created_at);
            payload.elapsed_ms = msg.elapsed_ms.unwrap_or(0);
        }
        self.bus.publish(&patchbay_events::Event {
            event_type: patchbay_protocol::EVENT_CHAT_DONE.to_string(),
            workspace_id,
            actor_type: "system".to_string(),
            actor_id: String::new(),
            payload: serde_json::to_value(&payload).unwrap_or(serde_json::Value::Null),
    ...
    pub async fn auto_unresolve_thread_on_reply(
        ...
        self.bus.publish(&patchbay_events::Event {
            event_type: patchbay_protocol::EVENT_COMMENT_UNRESOLVED.to_string(),
            workspace_id: workspace_id.to_string(),
...
pub fn issue_to_map(issue: &Issue, issue_prefix: &str) -> serde_json::Value {
    json!({
        ...
        "executor_id": issue.executor_id.map(|u| u.to_string()),
...
/// error as its only output when `patchbay issue create` fails, so this normally
...
impl TaskService {
    ...
    pub(crate) fn publish_quick_create_inbox(
        &self,
        item: &patchbay_db::models::InboxItem,
        workspace_id: &str,
        agent_id: &str,
        issue_status: &str,
    ) {
...
pub fn agent_to_map(a: &patchbay_db::models::Agent) -> serde_json::Value {
    json!({
        "id": a.id.to_string(),
