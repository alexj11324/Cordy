...
use patchbay_plugincontract::{Manifest, Resource, RESOURCE_SKILL};
...
pub(crate) async fn install_skill_resources_in_tx(
    ...
    patchbay_db::queries::plugin::delete_plugin_skills_not_in(&mut *tx, installation.id, &keep)
        .await
        .map_err(|e| {
    ...
    for resource in &resources {
        ...
        if let Err(e) = patchbay_db::queries::plugin::upsert_plugin_skill(
            &mut *tx,
