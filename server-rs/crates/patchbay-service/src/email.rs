...
fn build_invitation_params(
    ...
    SendEmailRequest {
        from: from.to_string(),
        to: vec![to.to_string()],
        subject: format!("{subject_inviter} invited you to {subject_workspace} on Patchbay"),
        html: format!(
            r#"<div style="font-family: sans-serif; max-width: 480px; margin: 0 auto;">
				<h2>You're invited to join {safe_workspace}</h2>
				<p><strong>{safe_inviter}</strong> invited you to collaborate in the <strong>{safe_workspace}</strong> workspace on Patchbay.</p>
...
impl EmailService {
    ...
    pub async fn send_verification_code(&self, to: &str, code: &str) -> anyhow::Result<()> {
        ...
        if !self.smtp_host.is_empty() {
            return self
                .send_smtp(to, "Your Patchbay verification code", &body)
                .await;
        }
        ...
        let req = SendEmailRequest {
            ...
            to: vec![to.to_string()],
            subject: "Your Patchbay verification code".to_string(),
            html: body,
        };
...
mod tests {
    ...
    fn invitation_params_escapes_html_but_not_subject() {
        ...
        assert!(req
            .html
            .contains(r#"href="https://aspectlylabs.com/invite/x""#));
        // Subject is sanitized (control chars stripped, length capped), not
        // HTML-escaped.
        assert_eq!(
            req.subject,
            "Eve <script> invited you to Acme & Co \"inc\" on Patchbay"
        );
