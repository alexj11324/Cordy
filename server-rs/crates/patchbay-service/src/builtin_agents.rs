...
/// and hard deleted when its runtime goes away), and Patrick needs the opposite
/// of all three.
pub const PATRICK_SYSTEM_KEY: &str = "patrick";
...
/// current name is.
pub const PATRICK_DEFAULT_NAME: &str = "Patrick";

/// Substituted in the embedded prompt with the agent's current display name.
const PATRICK_NAME_PLACEHOLDER: &str = "{{AGENT_NAME}}";
...
pub const PATRICK_WORKSPACE_NOTES_SECTION: &str = r#"## Workspace notes
...
/// Returns the product-owned half of Patrick's prompt for an agent displayed
...
pub fn patrick_system_instructions(display_name: &str) -> String {
    let name = display_name.trim();
    let name = if name.is_empty() {
        PATRICK_DEFAULT_NAME
    } else {
        name
    };
    patrick_instructions_md()
        .trim_end_matches('\n')
        .replace(PATRICK_NAME_PLACEHOLDER, name)
}
...
/// write.
pub fn compose_patrick_instructions(display_name: &str, workspace_notes: &str) -> String {
    let system = patrick_system_instructions(display_name);
    ...
    format!(
        "{}\n\n{}\n\n{}",
        system, PATRICK_WORKSPACE_NOTES_SECTION, notes
    )
}

fn patrick_instructions_md() -> &'static str {
    PATRICK_INSTRUCTIONS_MD
}
...
mod tests {
    ...
    fn placeholder_is_substituted_with_display_name() {
        let out = patrick_system_instructions("Atlas");
        assert!(out.contains("You are Atlas,"), "prompt must greet as Atlas");
        assert!(!out.contains("{{AGENT_NAME}}"), "placeholder must be gone");
    }

    #[test]
    fn empty_display_name_falls_back_to_patrick() {
        let out = patrick_system_instructions("   ");
        assert!(out.contains("You are Patrick,"));
    }

    #[test]
    fn embedded_prompt_is_nonempty_and_carries_identity_duties() {
        let md = patrick_instructions_md();
    ...
    fn compose_without_notes_returns_system_only() {
        let system = patrick_system_instructions("Patrick");
        let composed = compose_patrick_instructions("Patrick", "");
        assert_eq!(composed, system);
        // Trailing whitespace-only notes count as empty too.
        assert_eq!(compose_patrick_instructions("Patrick", "   \n\t "), system);
    }

    #[test]
    fn compose_layers_notes_under_workspace_section() {
        let composed = compose_patrick_instructions("Patrick", "Always reply in French.");
        assert!(composed.contains(PATRICK_WORKSPACE_NOTES_SECTION));
        assert!(composed.contains("Always reply in French."));
        // System half comes first.
        let notes_pos = composed.find(PATRICK_WORKSPACE_NOTES_SECTION).unwrap();
