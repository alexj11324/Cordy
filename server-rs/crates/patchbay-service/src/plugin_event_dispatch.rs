    ...
    async fn sweep_once(&self) {
        ...
        if let Ok(cutoff) = cutoff.parse::<chrono::DateTime<chrono::Utc>>() {
            match tokio::time::timeout(
                ...
                patchbay_db::queries::plugin::delete_expired_plugin_invocations(
                    &self.service.pool,
                    Some(cutoff),
                ),
            )
    ...
    S: PluginEventSink + ?Sized + 'static,
{
    ...
    bus.subscribe(PROTOCOL_ISSUE_UPDATED, move |e: &patchbay_events::Event| {
...
mod tests {
    ...
    fn empty_hook() -> Hook {
        Hook {
            ...
            triggers: Vec::new(),
            events: Vec::new(),
            transport: patchbay_plugincontract::HookTransport::default(),
            timeout_ms: 0,
        }
    }
}
