...
const DSH_PROFILE: &str = "patchbay";
        ...
        tokio::spawn(async move {
            ...
            let (run_end, exit, stream) = match outcome {
                DshCompletionOutcome::Completed(completed) => {
                    let (exit, stream) = *completed;
                    (RunEnd::Completed, exit, stream)
                }
...
fn next_request_id() -> String {
    ...
    let nanos = SystemTime::now()
        ...
        .unwrap_or_default();
    format!("patchbay-{}-{}", nanos, sequence)
}
...
mod tests {
    ...
    fn dsh_profile_is_owned_by_the_fixed_prefix_when_already_present() {
        let command =
            RuntimeCommand::new("dsh", vec!["--profile".to_string(), "patchbay".to_string()]);
        assert_eq!(
    ...
    async fn execute_translates_frames_usage_and_result() {
        let (_directory, backend) = fake_backend(
            r##"#!/bin/sh
test "$1" = --profile && test "$2" = patchbay && test "$3" = --stdio || exit 9
...
"##,
    ...
    async fn discovery_reads_models_and_scopes_cache() {
        let (_directory, backend) = fake_backend(
            r##"#!/bin/sh
test "$1" = --profile && test "$2" = patchbay && test "$3" = --list-models || exit 9
