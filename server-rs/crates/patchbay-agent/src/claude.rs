...
fn windows_chromium_fallback_executable() -> Option<PathBuf> {
    if let Some(path) = std::env::var_os("PATCHBAY_CHROME_DEVTOOLS_EXECUTABLE_PATH") {
...
async fn run_claude(
    ...
    let guard = state.saw_async_launch.then_some(
        "claude launched an async background task; Patchbay-managed runs require foreground execution",
    );
    ...
    #[cfg(unix)]
    {
        ...
        if unsafe { libc::geteuid() } == 0 && !sandbox {
            return Err(AgentError::InvalidConfig(
                "Claude Code refuses bypassPermissions under root/sudo privileges. Run the Patchbay daemon as a non-root user, or set IS_SANDBOX=1 if running in a genuine container/sandbox".to_string(),
            ));
        }
    }
    #[cfg(not(unix))]
