...
impl PiBackend {
    ...
    async fn discover_pi_models_rpc(&self) -> io::Result<Vec<Model>> {
        ...
        for request in [
            serde_json::json!({"id":"patchbay-state","type":"get_state"}),
            serde_json::json!({"id":"patchbay-models","type":"get_available_models"}),
        ] {
        ...
        while let Some(line) = reader.next_line().await? {
            ...
            if response.id == "patchbay-state" || response.command == "get_state" {
                state_done = true;
                if response.success {
            ...
            .execute(
                ...
                ExecOptions {
                    ...
                    resume_session_id: std::env::temp_dir()
                        .join("patchbay-pi-test-session.jsonl")
                        .to_string_lossy()
                        .into_owned(),
                    ..ExecOptions::default()
                },
...
while IFS= read -r request; do
  case "$request" in
    *patchbay-state*) printf '%s\n' '{"id":"patchbay-state","type":"response","command":"get_state","success":true,"data":{"model":{"provider":"anthropic","id":"claude-sonnet"},"thinkingLevel":"high"}}' ;;
    *patchbay-models*) printf '%s\n' '{"id":"patchbay-models","type":"response","command":"get_available_models","success":true,"data":{"models":[{"provider":"anthropic","id":"claude-sonnet","name":"Sonnet","reasoning":true,"thinkingLevelMap":{"max":null}}]}}' ; break ;;
  esac
done
cat > /dev/null
"##,
