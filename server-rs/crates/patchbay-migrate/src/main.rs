//! Patchbay database migration runner.
//!
//! Usage: `patchbay-migrate up|down|status` (DATABASE_URL env required).
...
#[derive(Debug, Parser)]
#[command(
    name = "patchbay-migrate",
    about = "Apply, roll back, or check Patchbay database migrations"
)]
struct Args {
...
fn configured_lock_timeout(cli_seconds: Option<u64>) -> anyhow::Result<Duration> {
    let seconds = match cli_seconds {
        ...
        None => match std::env::var("PATCHBAY_MIGRATION_LOCK_TIMEOUT_SECONDS") {
            Ok(raw) => raw.parse::<u64>().with_context(|| {
                format!("parse PATCHBAY_MIGRATION_LOCK_TIMEOUT_SECONDS={raw:?} as seconds")
            })?,
            Err(std::env::VarError::NotPresent) => DEFAULT_LOCK_TIMEOUT_SECONDS,
            Err(std::env::VarError::NotUnicode(_)) => {
                anyhow::bail!("PATCHBAY_MIGRATION_LOCK_TIMEOUT_SECONDS must be valid UTF-8")
            }
        },
    };
    if seconds == 0 {
