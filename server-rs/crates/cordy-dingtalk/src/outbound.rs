...
impl Outbound {
    ...
    /// budget.
    async fn process_detached(&self, e: &cordy_events::Event) {
    ...
    async fn process_event(&self, e: &cordy_events::Event) -> anyhow::Result<()> {
...
fn event_content(e: &cordy_events::Event) -> String {
    let payload = &e.payload;
    if e.event_type == cordy_protocol::events::EVENT_TASK_FAILED {
        if payload
            .get("retry_pending")
...
fn task_and_session_from_event(e: &cordy_events::Event) -> (Option<Uuid>, Option<Uuid>) {
