...
//! AppSecret (client secret) into Cordy. Each channel_installation carries its
...
//! <https://cordy.ai/docs/community-maintained>. That page is the single source
...
/// The [`cordy_channel::Type`] value for this adapter (see [`TYPE_DINGTALK`]).
pub fn channel_type() -> cordy_channel::Type {
    cordy_channel::Type(TYPE_DINGTALK.to_string())
}
...
/// the Router carries on [`cordy_channel_engine::resolvers::ResolvedInstallation`].
/// Go: `inst.Platform.(db.ChannelInstallation)`.
pub(crate) fn db_row_from_platform(
    inst: &cordy_channel_engine::resolvers::ResolvedInstallation,
) -> anyhow::Result<std::sync::Arc<cordy_db::models::ChannelInstallation>> {
    inst.platform
        .clone()
        .downcast::<cordy_db::models::ChannelInstallation>()
