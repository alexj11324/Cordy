...
pub struct LarkOutcomeReplier {
    ...
    /// e.g. https://cordy.example, trailing slash trimmed
    app_url: String,
    /// path component of the binding URL, default "/lark/bind"
    binding_path: String,
}

/// Wires the production replier. `app_url` is the Cordy web app host the user
/// clicks into to redeem the binding token or open an issue. It comes from
/// CORDY_APP_URL and is intentionally separate from CORDY_PUBLIC_URL, which is
...
pub fn new_lark_outcome_replier(cfg: OutcomeReplierConfig) -> Arc<dyn OutcomeReplier> {
    ...
    if cfg.app_url.is_empty() {
        tracing::warn!(
            "lark outcome replier: CORDY_APP_URL not set; binding prompt CTA will not work"
        );
    }
...
/// who haven't configured CORDY_APP_URL) the message still confirms the issue,
...
mod tests {
    ...
    fn issue_texts_compose_confirmation_and_conflict() {
        ...
        res.issue_identifier = "CORDY-41".to_string();
        assert_eq!(
            issue_created_text(&res, "https://cordy.example"),
            "Created CORDY-41\nhttps://cordy.example/issues/CORDY-41"
        );
        ...
        assert_eq!(
            issue_created_text(&res, "https://cordy.example/"),
            "Created CORDY-41 — Fix login\nhttps://cordy.example/issues/CORDY-41"
        );

        // Duplicate wording.
        let dup = DispatchResult {
            issue_identifier: "CORDY-7".to_string(),
            issue_title: "Existing".to_string(),
            ..Default::default()
        };
        assert_eq!(
            issue_duplicate_text(&dup, ""),
            "Not created — active issue CORDY-7 already exists: Existing"
        );
