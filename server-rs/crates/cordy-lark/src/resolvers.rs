...
//! engine's normalized `cordy_channel::InboundMessage` / engine types and the
...
use cordy_channel::InboundMessage as ChannelMessage;
...
) -> cordy_channel_engine::resolvers::ResolverSet {
    let mut set = cordy_channel_engine::resolvers::ResolverSet {
...
pub trait FeishuChatSession: Send + Sync {
    ...
    async fn append_user_message(
        &self,
        input: &cordy_channel_engine::session::AppendInput,
    ) -> anyhow::Result<cordy_channel_engine::resolvers::AppendResult>;
    async fn bind_media_refs(
        &self,
        input: &cordy_channel_engine::session::BindMediaInput,
    ) -> anyhow::Result<()>;
}

#[async_trait]
impl FeishuChatSession for cordy_channel_engine::session::ChatSession {
    async fn ensure_session(
        &self,
        workspace_id: Uuid,
        agent_id: Uuid,
        ...
        cordy_channel_engine::session::ChatSession::ensure_session(
            self,
            &cordy_channel_engine::session::EnsureSessionInput {
                workspace_id,
                agent_id,
                installation_id,
                sender,
    ...
    async fn mark_pending_fresh(&self, session_id: Uuid) -> anyhow::Result<()> {
        cordy_channel_engine::session::ChatSession::mark_pending_fresh(self, session_id).await
    }
    async fn append_user_message(
        &self,
        input: &cordy_channel_engine::session::AppendInput,
    ) -> anyhow::Result<cordy_channel_engine::resolvers::AppendResult> {
        cordy_channel_engine::session::ChatSession::append_user_message(self, input).await
    }
    async fn bind_media_refs(
        &self,
        input: &cordy_channel_engine::session::BindMediaInput,
    ) -> anyhow::Result<()> {
        cordy_channel_engine::session::ChatSession::bind_media_refs(self, input).await
    }
}
...
impl IdentityResolver for FeishuIdentityResolver {
    async fn resolve_sender(
        ...
        let is_member = self
            .store
            .is_workspace_member(inst.workspace_id, binding.cordy_user_id)
            .await?;
        if !is_member {
...
mod tests {
    ...
    fn channel(raw: serde_json::Value, source: cordy_channel::Source) -> ChannelMessage {
        ChannelMessage {
            raw,
            source,
            ..Default::default()
    ...
    fn empty_and_garbage_raw_are_errors() {
        let msg = channel(serde_json::Value::Null, cordy_channel::Source::default());
        assert!(lark_msg_from_raw(&msg).is_err());
        let junk = channel(serde_json::json!(3), cordy_channel::Source::default());
    ...
    fn bot_name_preset_is_not_here_but_dispatch_mapping_keeps_fields() {
        ...
        let res = EngineResult {
            outcome: Some(cordy_channel_engine::resolvers::Outcome::needs_binding()),
            drop_reason: None,
