...
impl FrameDecoder for LarkJsonFrameDecoder {
    ...
    fn decode(
        ...
        // adapter separately downloads and binds the binary as a Cordy
        // attachment.
...
/// matching on it would re-introduce the MUL-2671 routing bug). Only when
...
/// MUL-2671 group-@-mention triage. In a Lark group with several Cordy bots,
    ...
    fn decodes_text_message_and_strips_bot_mention() {
        ...
        let msg = decode_json(
            text_event(
                ...
                serde_json::json!([
                    {"key": "@_user_1", "id": {"open_id": "ou_bot", "union_id": "un_bot"}, "name": "Cordy"},
