...
/// pair and points at a different Cordy user. Account transfer must go
...
impl BindingTokenService {
    ...
    /// DB transaction. The redeemer's identity is the supplied cordy_user_id
    ...
    ///   (installation, open_id), pointing at a DIFFERENT Cordy user. The
    ...
    pub async fn redeem_and_bind(
        ...
        .await
        {
            ...
            // cordy_user_id differs from ours, so the WHERE clause on the ON
...
impl InstallerBinder for BindingTokenService {
    ...
    /// (installation_id, open_id) when cordy_user_id matches (the ON CONFLICT
