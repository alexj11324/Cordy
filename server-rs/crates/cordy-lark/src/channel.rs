...
pub fn channel_message_from_lark(message: LarkInboundMessage) -> InboundMessage {
    ...
    InboundMessage {
        ...
        source: Source {
            channel_type: cordy_channel::Type::feishu(),
...
fn channel_msg_type(message_type: &str) -> cordy_channel::MsgType {
    match message_type {
        "image" => cordy_channel::MsgType::image(),
        "file" => cordy_channel::MsgType::file(),
        "audio" => cordy_channel::MsgType::audio(),
        "media" | "video" => cordy_channel::MsgType::video(),
        "" | "text" | "post" | "merge_forward" | "interactive" => cordy_channel::MsgType::text(),
        _ => cordy_channel::MsgType::unknown(),
    }
}
...
pub fn register_feishu(registry: &cordy_channel::Registry, deps: FeishuChannelDeps) {
    registry.register(cordy_channel::Type::feishu(), feishu_factory(deps));
}
