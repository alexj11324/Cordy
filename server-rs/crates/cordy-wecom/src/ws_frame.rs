...
use cordy_channel::message::{ChatType, InboundMessage as ChannelInboundMessage, MsgType, Source};
...
/// avoid clashing with [`cordy_channel::InboundMessage`]. JSON tags are
/// unchanged.
#[derive(Debug, Clone, Default, PartialEq, Serialize, Deserialize)]
...
pub fn strip_leading_mentions(s: &str, bot_name: &str) -> String {
    ...
    loop {
        ...
        // spaces — "Cordy Bot" is the obvious one — and cutting at the first
...
fn is_issue_command(body: &str) -> bool {
    cordy_channel_engine::parse_issue_command(body).is_some()
}
...
mod tests {
    ...
    fn channel_message_routes_single_and_group() {
        ...
        let group = msg_callback(json!({
            ...
            "text": {"content": "@Cordy Bot /new 重新分析"},
        }));
        let m = channel_message_from_callback("bot", "Cordy Bot", &group, "x", "");
    ...
    fn strip_leading_mentions_whole_name_first() {
        assert_eq!(
            strip_leading_mentions("@Cordy Bot /new x", "Cordy Bot"),
            "/new x"
        );
