...
use cordy_db::queries::{user, verification_code};
use rand::Rng;
...
impl AuthSettings {
    pub fn from_env() -> Self {
        Self {
            ...
            dev_verification_code: env_trimmed("CORDY_DEV_VERIFICATION_CODE"),
...
pub fn public_router(
    auth_limit: cordy_middleware::ratelimit::RateLimitState,
    verify_limit: cordy_middleware::ratelimit::RateLimitState,
) -> Router<HandlerState> {
...
async fn send_code(State(state): State<HandlerState>, body: Bytes) -> Response {
    ...
    match user::get_user_by_email(&state.pool, &email).await {
        Ok(Some(existing)) => {
            if cordy_auth::disabled_users::is_temporarily_disabled_user(
...
async fn complete_login(
    ...
    if is_new {
        let mut event = cordy_analytics::signup(
    ...
    let token = match cordy_auth::jwt::issue_user_jwt(
        &current.id.to_string(),
        &current.email,
        &current.name,
    ) {
...
fn signup_source(headers: &HeaderMap) -> String {
    ...
    let Some(value) = raw_cookie.split(';').find_map(|cookie| {
        ...
        (name == "cordy_signup_source").then_some(value)
    }) else {
        return String::new();
    };
    ...
    fn signup_source_decodes_and_caps_cookie() {
        ...
        headers.insert(
            ...
            HeaderValue::from_static(
                "x=1; cordy_signup_source=%7B%22utm_source%22%3A%22docs%22%7D",
            ),
        );
