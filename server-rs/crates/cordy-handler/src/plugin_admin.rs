...
use cordy_service::plugin_token::{issue_install_token, revoke_install_token};
...
async fn list_invocations(
    ...
    match cordy_db::queries::plugin::list_plugin_invocations(&state.pool, installation.id, 100)
        .await
    {
        Ok(rows) => Json(json!({
...
impl From<cordy_db::models::PluginInvocation> for InvocationResponse {
    fn from(row: cordy_db::models::PluginInvocation) -> Self {
        Self {
            id: row.id.to_string(),
