...
use cordy_db::models::{Issue, VcsConnection, VcsPullRequest};
use cordy_vcs::{CiStatusEvent, EventKind, PullRequestEvent};
...
async fn handle(
    ...
    let connection =
        match cordy_db::queries::vcs::get_vcs_connection_by_id(&state.pool, connection_id).await {
    ...
    let Some(provider) = cordy_vcs::for_kind(&connection.provider) else {
...
fn decrypt_secret(
    secret_box: &cordy_util::secretbox::SecretBox,
    encoded: &str,
...
async fn mirror_pull_request(
    ...
    let pull_request = match cordy_db::queries::vcs::upsert_vcs_pull_request(
        &state.pool,
    ...
    let prefix =
        match cordy_db::queries::workspace::get_workspace(&state.pool, connection.workspace_id)
            .await
        {
    ...
    for identifier in identifiers {
        ...
        match cordy_db::queries::vcs::link_issue_to_vcs_pull_request(
            &state.pool,
            issue.id,
            pull_request.id,
            close_intent,
...
async fn mirror_ci_status(state: &HandlerState, connection: &VcsConnection, event: CiStatusEvent) {
    ...
    if let Err(error) = cordy_db::queries::vcs::upsert_vcs_commit_status(
        &state.pool,
        connection.id,
        &event.sha,
        &event.context,
    ...
    let issue_ids = match cordy_db::queries::vcs::list_issue_i_ds_for_vcspr_head(
        &state.pool,
        connection.id,
        &event.sha,
    )
...
async fn maybe_complete_issue(state: &HandlerState, issue: Issue) {
    ...
    let aggregate = match cordy_db::queries::vcs::get_issue_combined_pull_request_close_aggregate(
        &state.pool,
        issue.id,
    )
    .await
...
async fn lookup_issue(
    ...
    cordy_db::queries::issue::get_issue_by_number(&state.pool, workspace_id, number)
        .await
        .ok()
        .flatten()
}
...
mod tests {
    ...
    fn empty_encrypted_secret_reaches_signature_rejection() {
        let secret_box = cordy_util::secretbox::SecretBox::new(&[7; 32]).expect("secret box");
        let secret = decrypt_secret(&secret_box, "").expect("Go treats empty ciphertext as empty");
        let provider = cordy_vcs::for_kind("forgejo").expect("forgejo provider");
