...
use cordy_db::queries::{agent, agent_invocation_target, chat, task_message};
use cordy_middleware::workspace::WorkspaceContext;
use cordy_service::task_service::{CancelTaskOptions, TaskServiceError};
...
async fn cancel_task(
    ...
    let options = match cancel_options(
        ...
        crate::claim_response::request_has_client_capability(
            &headers,
            cordy_protocol::APP_CAPABILITY_CHAT_DRAFT_RESTORE_V1,
        ),
    ) {
        Ok(options) => options,
