...
use cordy_db::models::VcsConnection;
use cordy_db::queries::vcs;
...
fn require_available(state: &HandlerState) -> Result<(), Response> {
    ...
    if state.vcs_secret_box.is_none() {
        return Err(error_response(
            ...
            "vcs integration not configured (CORDY_VCS_SECRET_KEY unset)",
        ));
    }
    Ok(())
}
...
async fn connect(
    ...
    let Some(provider) = cordy_vcs::for_kind(request.provider.trim()) else {
        return error_response(StatusCode::BAD_REQUEST, "unsupported provider");
    };
    let instance_url = cordy_vcs::normalize_instance_url(&request.instance_url);
    ...
    let account = match provider.validate_token(&instance_url, token).await {
        ...
        Err(error)
            if error
                .downcast_ref::<cordy_vcs::UnauthorizedError>()
                .is_some() =>
        {
            return error_response(
                StatusCode::BAD_REQUEST,
    ...
    .await
    {
        Ok(Some(row)) => {
            ...
            state.bus.publish(&cordy_events::Event {
                event_type: cordy_protocol::EVENT_VCS_CONNECTION_CREATED.into(),
...
async fn remove(
    ...
    match vcs::delete_vcs_connection(&state.pool, connection_id, workspace_id).await {
        Ok(_) => {
            state.bus.publish(&cordy_events::Event {
                event_type: cordy_protocol::EVENT_VCS_CONNECTION_DELETED.into(),
                workspace_id: workspace_id.to_string(),
                actor_type: "system".into(),
