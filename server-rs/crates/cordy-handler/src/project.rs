...
fn runtime_has_worktree(runtime: &cordy_db::models::AgentRuntime) -> bool {
    runtime
        .metadata
        .get("capabilities")
        ...
        .is_some_and(|values| {
            values.iter().any(|value| {
                value.as_str() == Some(cordy_protocol::DAEMON_CAPABILITY_LOCAL_WORKTREE_V1)
            })
        })
}

fn newest_daemon_runtime<'a>(
    runtimes: &'a [cordy_db::models::AgentRuntime],
    daemon_id: &str,
) -> Option<&'a cordy_db::models::AgentRuntime> {
    runtimes
        .iter()
...
fn latest_daemon_cli_version(
    runtimes: &[cordy_db::models::AgentRuntime],
    daemon_id: &str,
) -> String {
    runtimes
        .iter()
...
async fn require_worktree_capability(
    ...
    let runtimes = cordy_db::queries::runtime::list_agent_runtimes(&state.pool, workspace_id)
        .await
        .map_err(|error| {
    ...
    Err((
        ...
        Json(json!({
            "error": format!(
                "local_directory: {:?} is set to parallel (worktree) mode, but the Cordy runtime on that machine does not support it. Update the Cordy app on that machine to the latest version, or keep the resource on in_place.",
...
async fn create(
    ...
    if request.resources.is_empty() {
        ...
        publish_project(
            &state,
            &context,
            cordy_protocol::EVENT_PROJECT_CREATED,
    ...
    for resource in &resources {
        publish_project(
            &state,
            &context,
            cordy_protocol::EVENT_PROJECT_RESOURCE_CREATED,
...
async fn update_resource(
    ...
    publish_project(
        &state,
        &context,
        cordy_protocol::EVENT_PROJECT_RESOURCE_UPDATED,
        json!({ "resource": &response, "project_id": project.id.to_string() }),
    );
    Json(response).into_response()
}
...
mod tests {
    ...
    fn resource_ref_validation_accepts_supported_git_and_absolute_path_forms() {
        ...
        for value in ["/Users/alex/Cordy", r"C:\\code\\Cordy", r"\\server\\share"] {
    ...
    fn newest_runtime_capability_fails_closed_after_downgrade() {
        let now = chrono::Utc::now();
        let runtime = |id: u128, seen, metadata| cordy_db::models::AgentRuntime {
            created_at: now,
            custom_name: None,
        ...
        let runtimes = vec![
            runtime(
                ...
                json!({"capabilities":[cordy_protocol::DAEMON_CAPABILITY_LOCAL_WORKTREE_V1],"cli_version":"0.4.24"}),
            ),
