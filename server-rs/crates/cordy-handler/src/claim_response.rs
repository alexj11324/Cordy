...
use cordy_db::models::{AgentRuntime, AgentTaskQueue};
...
fn worktree_claim_block_reason(
    ...
    for res in resources {
        ...
        return format!(
            "This machine's Cordy runtime does not support parallel (worktree) mode, which {:?} is set to use. \
             Update the Cordy app on that machine to the latest version, then re-run this task. \
...
fn trailing_user_messages(
    msgs: Vec<cordy_db::models::ChatMessage>,
    now: chrono::DateTime<chrono::Utc>,
) -> Vec<cordy_db::models::ChatMessage> {
...
pub(crate) async fn build_claimed_task_response(
    ...
    if let Ok(workspace_uuid) = Uuid::parse_str(runtime_workspace_id) {
        if let Ok(tools) =
            cordy_service::plugin_agent_tools::agent_hook_tools(&state.plugins, workspace_uuid)
                .await
        {
        ...
        if let Ok(connections) = cordy_service::plugin_mcp_transport::agent_mcp_connections(
            &state.plugins.pool,
            workspace_uuid,
        )
        .await
    ...
    let agent = match agent_q::get_agent(&state.pool, task.agent_id).await {
        ...
        Ok(None) => {
            ...
            return Err(fail_claimed_task_before_launch(
                ...
                cordy_task_failure::Reason::INVALID_TASK_IDENTITY,
    ...
    if agent.system_key.as_deref() == Some(cordy_service::builtin_agents::MIKA_SYSTEM_KEY) {
        instructions = cordy_service::builtin_agents::compose_mika_instructions(
            &agent.name,
            &agent.instructions,
        );
    }
    ...
    if use_skill_refs {
        ...
        skills.extend(cordy_service::builtin_skills::load_builtin_skills());
        if !skills.is_empty() {
            ...
            if let Some(rerun_of) = task.rerun_of_task_id {
                ...
                match agent_queries::get_agent_task(&state.pool, rerun_of).await {
                    Ok(Some(src)) if rerun_source_matches_task_scope(task, &src) => {
                        ...
                        let resume_unsafe = cordy_service::task_helpers::resume_unsafe_failure(
    ...
    if let Some(chat_session_id) = task.chat_session_id {
        if let Ok(Some(cs)) = chat_q::get_chat_session(&state.pool, chat_session_id).await {
            ...
            if let Some(project_id) = cs.project_id {
                if let Ok(Some(project)) = cordy_db::queries::project::get_project_in_workspace(
                    &state.pool,
                    project_id,
                    cs.workspace_id,
                )
            ...
            let unanswered: Result<Vec<cordy_db::models::ChatMessage>, _> =
    ...
    // ---- Quick-create tasks --------------------------------------------------
    if has_quick_create {
        if let Some(qc) = cordy_service::task_service::TaskService::parse_quick_create_context(task)
        {
            obj.insert(
