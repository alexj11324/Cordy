...
fn handle_issue_updated(pool: PgPool, bus: Arc<Bus>, event: Event) -> ListenerFuture {
    Box::pin(async move {
        ...
        if flag(&event.payload, "status_changed") {
            ...
            .await;
            let effective =
                cordy_service::issue_status::effective(&pool, fields.workspace_id, &fields.status)
                    .await;
...
async fn notify_issue_subscribers(
    ...
    let effective =
        cordy_service::issue_status::effective(pool, fields.workspace_id, &fields.status).await;
...
async fn archive_task_failures(pool: &PgPool, bus: &Bus, workspace_id: Uuid, issue_id: Uuid) {
    ...
    for (recipient_id, count) in counts {
        bus.publish(&Event {
            event_type: cordy_protocol::EVENT_INBOX_BATCH_ARCHIVED.into(),
