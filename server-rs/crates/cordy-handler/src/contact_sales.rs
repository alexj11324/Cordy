...
async fn create(
    ...
    let count = match cordy_db::queries::contact_sales::count_recent_contact_sales_by_email(
        &state.pool,
    ...
    let inquiry = match cordy_db::queries::contact_sales::create_contact_sales_inquiry(
        &state.pool,
        &request.first_name,
        &request.last_name,
        &request.business_email,
    ...
    let event = cordy_analytics::events::contact_sales_submitted(
...
mod tests {
    ...
    fn validation_canonicalizes_the_go_contract() {
        ...
        assert_eq!(request.goals, "Pilot Cordy");
