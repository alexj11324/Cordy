...
//! - `TxBeginner` + `*db.Queries` → a single sqlx `PgPool`; the patchbay-db
...
use patchbay_db::queries::github_snapshot as dbq;
...
mod tests {
    ...
    async fn seed_pr_at(
        ...
        let pr = patchbay_db::queries::github::upsert_git_hub_pull_request(
            pool,
            ws_id,
            installation_id,
            "o",
