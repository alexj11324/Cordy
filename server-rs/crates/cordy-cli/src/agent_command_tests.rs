...
fn agent_read_parser_matches_go_registry() {
    ...
    let get = Cli::try_parse_from(["cordy", "agent", "get", "agent-123"]).expect("agent get CLI");
    let Command::Agent(AgentArgs {
...
async fn agent_list_and_get_match_go_requests_and_outputs() {
    ...
    let get_json =
        Cli::try_parse_from(["cordy", "agent", "get", "agent-123"]).expect("agent get JSON CLI");
...
async fn agent_update_rejects_no_changes_and_does_not_expose_custom_env() {
    assert!(
        Cli::try_parse_from(["cordy", "agent", "update", "agent-1", "--custom-env", "{}"]).is_err()
    );
    let home = tempfile::tempdir().expect("temp home");
    let cwd = tempfile::tempdir().expect("temp cwd");
    let mut environment = Environment::for_test(home.path().into(), cwd.path().into());
...
async fn agent_lifecycle_and_tasks_match_go_requests_and_outputs() {
    ...
    for (command, expected) in [
        ...
        let cli = Cli::try_parse_from(["cordy", "agent", command, "agent-1", "--output", "table"])
...
async fn agent_mcp_commands_match_go_api_and_redacted_output_contract() {
    ...
    let remove = Cli::try_parse_from(["cordy", "agent", "mcp", "remove", "agent-1", "server-1"])
...
async fn agent_copy_copies_only_portable_same_runtime_fields() {
    ...
    environment.set("CORDY_TOKEN", "token-1");
    let cli =
        Cli::try_parse_from(["cordy", "agent", "copy", "agent-source"]).expect("agent copy CLI");
    let output = run_with_input(&cli, &environment, &mut Cursor::new(Vec::<u8>::new()))
        .await
        .expect("copy agent");
    assert_eq!(
