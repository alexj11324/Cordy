...
pub(super) fn ensure_file_within_workdir(
    ...
    if !candidate.starts_with(&base) {
        let option = if field == "file" {
            "--file".to_string()
        } else {
            format!("--{field}-file")
        };
        bail!(
            "{option} path {:?} resolves outside the current working directory; write agent temp files inside the task workdir (e.g. ./{field}.md) rather than machine-shared paths like /tmp, where another run's stale file can be read by mistake. Pass --allow-external-file to override.",
            file_path,
        );
    }
    Ok(())
