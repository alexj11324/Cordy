...
use clap::ValueEnum;
pub(crate) use config::CliConfig;
...
pub(crate) use agent_command_schema::{
    ...
    AgentSkillsArgs, AgentSkillsCommand, AgentSkillsMutationArgs, AgentUpdateArgs,
};
...
pub(crate) use agent_lifecycle_commands::{run_agent_avatar, run_agent_lifecycle, run_agent_tasks};
pub(crate) use agent_mcp_commands::{run_agent_mcp_list, run_agent_mcp_mutation, AgentMcpAction};
...
use autopilot_mutation_commands::{
    run_autopilot_create, run_autopilot_delete, run_autopilot_update,
};
...
use autopilot_reference_resolver::{
    context_autopilot_resolution, resolve_autopilot_id, resolve_autopilot_trigger_id,
};
...
pub(crate) use chat_command_schema::{
    AttachmentArgs, AttachmentCommand, ChatArgs, ChatCommand, ChatReadArgs,
};
...
use config_mutation_commands::run_config_set;
use config_read_commands::run_config_show;
...
use daemon_launch_inputs::parse_cli_duration;
use daemon_lifecycle_commands::{
    run_daemon_after_setup, run_daemon_restart, run_daemon_start, run_daemon_stop,
};
use daemon_log_commands::{parse_log_lines, run_daemon_logs};
use daemon_profile_discovery::require_known_daemon_profile;
use daemon_status_commands::run_daemon_status;
...
pub(crate) use issue_actor_resolver::{
...
use project_mutation_commands::{run_project_create, run_project_delete, run_project_update};
use project_output::{
    format_project_details_table, format_project_list_table, project_actor_inputs,
};
...
use project_status_commands::run_project_status;
...
pub(crate) use repo_checkout_commands::run_repo_checkout;
pub(crate) use repo_command_schema::{RepoArgs, RepoCommand, RepoMutationArgs, RepoRemoveArgs};
pub(crate) use repo_mutation_commands::{run_repo_add, run_repo_remove};
...
pub(crate) use runtime_mutation_commands::{run_runtime_delete, run_runtime_rename};
...
use runtime_profile_path_commands::{run_runtime_profile_set_path, run_runtime_profile_unset_path};
...
pub(crate) use runtime_update_output::format_runtime_update_result;
...
pub(crate) use setup_profile::setup_daemon_action;
use skill_catalog_commands::{run_skill_import, run_skill_refresh, run_skill_search};
...
use skill_files_commands::{run_skill_files_delete, run_skill_files_list, run_skill_files_upsert};
use skill_mutation_commands::{
    resolve_skill_content_sources, run_skill_create, run_skill_delete, run_skill_update,
};
use skill_read_commands::{run_skill_get, run_skill_list};
use squad_activity_commands::run_squad_activity;
...
use squad_member_commands::{
    run_squad_member_add, run_squad_member_list, run_squad_member_remove, run_squad_member_set_role,
};
use squad_mutation_commands::{run_squad_create, run_squad_delete, run_squad_update};
use squad_read_commands::{run_squad_get, run_squad_list};
...
use workspace_commands::{
    resolve_workspace_arg, run_workspace_get, run_workspace_list, run_workspace_switch,
};
...
use workspace_member_commands::{run_workspace_member_invite, run_workspace_member_list};
use workspace_mutation_commands::{run_workspace_create, run_workspace_update};

#[cfg(test)]
pub(crate) use agent_mcp_commands::agent_mcp_path;
#[cfg(test)]
pub(crate) use auth_commands::display_token_prefix;
#[cfg(test)]
pub(crate) use config_mutation_commands::validate_config_set;
#[cfg(test)]
pub(crate) use config_read_commands::format_config_table;
#[cfg(test)]
pub(crate) use daemon_launch_inputs::ensure_restart_is_background;
#[cfg(test)]
pub(crate) use daemon_log_commands::resolve_daemon_log_path;
#[cfg(test)]
pub(crate) use daemon_log_io::read_daemon_log_tail;
#[cfg(test)]
pub(crate) use daemon_profile_discovery::known_daemon_profiles;
#[cfg(test)]
pub(crate) use daemon_status_commands::resolve_daemon_status_port;
#[cfg(test)]
pub(crate) use daemon_status_output::{format_daemon_status_table, render_daemon_status};
#[cfg(test)]
pub(crate) use disk_usage_output::format_disk_ratio;
#[cfg(test)]
pub(crate) use issue_children_commands::{
    child_stage, format_issue_children_table, group_issue_children,
};
#[cfg(test)]
pub(crate) use issue_comment_add_commands::resolve_issue_comment_content;
#[cfg(test)]
pub(crate) use issue_comment_list_commands::format_issue_comments_table;
#[cfg(test)]
pub(crate) use issue_get_commands::format_issue_get_table;
#[cfg(test)]
pub(crate) use issue_list_commands::{
    build_issue_list_query, build_metadata_filter, issue_list_has_more,
};
#[cfg(test)]
pub(crate) use issue_metadata_input::parse_metadata_value;
#[cfg(test)]
pub(crate) use issue_metadata_output::format_metadata_table;
#[cfg(test)]
pub(crate) use issue_property_commands::{build_issue_property_rows, format_issue_property_rows};
#[cfg(test)]
pub(crate) use issue_pull_request_commands::format_issue_pull_requests_table;
#[cfg(test)]
pub(crate) use issue_reorder_commands::compute_reorder_position;
#[cfg(test)]
pub(crate) use issue_safety::guard_issue_description_local_links;
#[cfg(test)]
pub(crate) use issue_search_commands::format_issue_search_table;
#[cfg(test)]
pub(crate) use issue_subscriber_commands::format_issue_subscribers_table;
#[cfg(test)]
pub(crate) use issue_task_output::{format_issue_run_messages_table, format_issue_runs_table};
#[cfg(test)]
pub(crate) use issue_timeline_filter::{build_timeline_filter, filter_timeline};
#[cfg(test)]
pub(crate) use issue_timeline_output::format_issue_timeline_table;
#[cfg(test)]
pub(crate) use label_commands::{format_label_result, format_workspace_label_table};
#[cfg(test)]
...
#[cfg(test)]
pub(crate) use project_status_commands::{validate_project_status, PROJECT_STATUSES};
#[cfg(test)]
pub(crate) use property_models::{PropertyDefinition, PropertyOption};
#[cfg(test)]
pub(crate) use property_mutation_input::parse_property_options;
#[cfg(test)]
pub(crate) use property_read_commands::{format_property_definitions, resolve_property};
#[cfg(test)]
pub(crate) use repo_checkout_commands::repo_checkout_retry_delay;
#[cfg(test)]
pub(crate) use repo_mutation_commands::{repo_urls, WorkspaceRepo};
#[cfg(test)]
pub(crate) use runtime_update::run_runtime_update_with_policy;
#[cfg(test)]
pub(crate) use setup_commands::{
    confirm_setup_overwrite, prepare_setup_profile, resolve_setup_profile_input,
};
#[cfg(test)]
pub(crate) use setup_profile::setup_callback_host;
#[cfg(test)]
pub(crate) use setup_profile::SetupDaemonAction;
#[cfg(test)]
pub(crate) use skill_catalog_commands::{
    format_skill_import_table, format_skill_search_table, read_skill_archive,
};
#[cfg(test)]
pub(crate) use skill_files_commands::format_skill_files_table;
#[cfg(test)]
pub(crate) use skill_mutation_commands::resolve_skill_content;
#[cfg(test)]
pub(crate) use skill_read_commands::format_skill_list_table;
#[cfg(test)]
pub(crate) use squad_member_commands::render_squad_member_output;
#[cfg(test)]
pub(crate) use squad_read_commands::{
    format_squad_details_table, format_squad_list_table, squad_member_count_display,
};
#[cfg(test)]
...
#[cfg(test)]
pub(crate) use user_commands::{format_user_profile_table, resolve_profile_description};
#[cfg(test)]
pub(crate) use workspace_commands::{
    format_workspace_details_table, format_workspace_table, resolve_workspace_reference,
    WorkspaceSummary,
};
#[cfg(test)]
pub(crate) use workspace_mcp_commands::parse_workspace_mcp_server_config;
#[cfg(test)]
pub(crate) use workspace_member_commands::normalize_workspace_invite_role;
#[cfg(test)]
pub(crate) use workspace_mutation_commands::{
    build_workspace_create_body, build_workspace_update_body,
};
