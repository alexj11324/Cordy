...
async fn skill_list_matches_go_requests_and_table_json_outputs() {
    ...
    let table_cli = Cli::try_parse_from(["cordy", "skill", "list"]).expect("skill list table CLI");
...
async fn skill_get_matches_go_path_headers_and_output_contracts() {
    ...
    let defaults =
        Cli::try_parse_from(["cordy", "skill", "get", "skill-1"]).expect("skill get default CLI");
    let Command::Skill(SkillArgs {
    ...
    let table_cli = Cli::try_parse_from(["cordy", "skill", "get", "skill-1", "--output", "table"])
