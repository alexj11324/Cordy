...
/// a Patchbay user UUID where a Lark open_id is expected.
...
/// Patchbay chat_session via channel_chat_session_binding.
#[derive(Debug, Clone, Default, PartialEq, Eq, Hash, serde::Serialize, serde::Deserialize)]
#[serde(transparent)]
...
/// `patchbay_channel::ChatType` open newtype and exposes the two constructors
/// under lark-flavored names to keep call sites readable.
pub type ChatType = patchbay_channel::message::ChatType;
...
/// separate clouds with distinct hosts; a single Patchbay deployment serves
...
impl DropReason {
    ...
    /// The sender resolved to a Patchbay user, but that user is not a member
