...
//!    expiry. Patchbay renders the QR, the user scans it in the Lark app, walks
...
pub struct RegistrationConfig {
    ...
    /// attribute installs back to Patchbay. Empty defaults to "patchbay".
    pub source: String,
}
...
pub struct BeginResult {
    ...
    /// The verification_uri_complete with Patchbay's `source` telemetry params
...
mod tests {
    ...
    fn config_defaults_match_go_values() {
        ...
        assert_eq!(c.cfg.source, "patchbay");
    ...
    fn begin_response_happy_path_decorates_qr_and_defaults() {
        ...
        assert!(res.qr_code_url.contains("source=go-sdk%2Fpatchbay"));
    ...
    fn qr_decoration_replaces_existing_telemetry_keys() {
        let out =
            decorate_qr_code_url("https://h/qr?token=t&from=web&name=Old", "patchbay", "").unwrap();
        ...
        let out = decorate_qr_code_url("https://h/qr?name=Old", "patchbay", "New").unwrap();
