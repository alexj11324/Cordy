...
use patchbay_db::queries::agent::{get_agent, get_agent_task};
...
impl LarkPatcher {
    ...
    async fn process_event(&self, ctx: &CancellationToken, e: &Event) -> anyhow::Result<()> {
        ...
        if e.event_type == patchbay_protocol::EVENT_TASK_CANCELLED {
        ...
        // Patchbay. Sealed channel tasks own an input batch just like direct
...
fn chat_session_binding_from_row(
    row: patchbay_db::models::ChannelChatSessionBinding,
) -> ChatSessionBinding {
    ChatSessionBinding {
        id: row.id,
