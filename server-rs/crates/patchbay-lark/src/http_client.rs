...
/// Operators do NOT set PATCHBAY_LARK_HTTP_BASE_URL to pick a cloud anymore —
...
/// headroom for cross-region latency from a self-hosted Patchbay deployment to
/// feishu.cn.
...
impl HttpClientConfig {
    fn with_defaults(self) -> Self {
        ...
        // A non-empty base_url (PATCHBAY_LARK_HTTP_BASE_URL, or a mock URL in
...
/// single Patchbay server reuses Lark's tenant_access_token across calls to the
/// same app.
...
impl HttpApiClient {
    ...
    /// (PATCHBAY_LARK_HTTP_BASE_URL, or a mock URL in tests) overrides every
...
pub(crate) fn binding_prompt_template(bind_url: &str) -> anyhow::Result<String> {
    let doc = json!({
        ...
        "header": {
            "template": "blue",
            "title": {"tag": "plain_text", "content": "Patchbay"},
        },
        "elements": [
            {
                "tag": "div",
                "text": {
                    "tag": "lark_md",
                    "content": "你还没有绑定 Patchbay 账户。点击下方按钮完成绑定后即可使用此 Agent。",
                },
            },
            {
                "tag": "action",
...
mod tests {
    ...
    fn binding_prompt_template_is_valid_card_json() {
        let raw = binding_prompt_template("https://patchbay.example/lark/bind?token=x").unwrap();
        let doc: serde_json::Value = serde_json::from_str(&raw).unwrap();
        assert_eq!(doc["header"]["template"], serde_json::json!("blue"));
        assert_eq!(
            doc["elements"][1]["actions"][0]["url"],
            serde_json::json!("https://patchbay.example/lark/bind?token=x")
        );
    }
