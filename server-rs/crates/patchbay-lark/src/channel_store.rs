...
//!   carries over 1:1.
//! - Go's `WithTx` disappears: every query in `patchbay_db::queries` is
...
impl ChannelStore {
    ...
    pub async fn list_lark_installations_missing_bot_union_id_after(
        ...
        installations_from_rows(
            patchbay_db::queries::channel::list_active_channel_installations_missing_bot_union_id_after(
                &self.pool,
                CHANNEL_TYPE_FEISHU,
                created_at,
                id,
    ...
    pub async fn stamp_lark_installation_bot_union_id_if_missing(
        ...
        patchbay_db::queries::channel::set_channel_installation_bot_union_id_if_missing(
            &self.pool,
            id,
            CHANNEL_TYPE_FEISHU,
            bot_union_id,
...
pub async fn create_lark_user_binding_with(
    ...
    .await?
        ...
        // ON CONFLICT ... WHERE patchbay_user_id mismatch → no row (Go:
...
fn installations_from_rows(
    rows: Vec<patchbay_db::models::ChannelInstallation>,
) -> anyhow::Result<Vec<Installation>> {
