    ...
    let token = req
        ...
        .or_else(|| {
            ...
            cookies.split(';').find_map(|pair| {
                ...
                ((name == patchbay_auth::cookie::AUTH_COOKIE_NAME
                    || name == patchbay_auth::cookie::LEGACY_AUTH_COOKIE_NAME)
                    && !value.is_empty())
...
mod tests {
    ...
    fn valid_jwt_identity_is_available_to_outer_logger() {
        let token = encode(
            ...
            &EncodingKey::from_secret(patchbay_auth::jwt::jwt_secret().as_bytes()),
        )
        .unwrap();
        let request = Request::builder()
            .uri("/api/me")
