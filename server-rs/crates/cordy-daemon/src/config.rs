...
/// basenames only; extend via CORDY_GC_ARTIFACT_PATTERNS.
...
pub(crate) fn load_config(
    ...
    let raw_server_url = crate::helpers::env_or_default("CORDY_SERVER_URL", DEFAULT_SERVER_URL);
    ...
    let mut poll_interval =
        crate::helpers::duration_from_env("CORDY_DAEMON_POLL_INTERVAL", DEFAULT_POLL_INTERVAL)?;
    ...
    let mut agent_timeout =
        crate::helpers::duration_from_env("CORDY_AGENT_TIMEOUT", DEFAULT_AGENT_TIMEOUT)?;
    ...
    let codex_first_turn_no_progress_timeout =
        crate::helpers::duration_from_env("CORDY_CODEX_FIRST_TURN_TIMEOUT", Duration::ZERO)?;
    ...
    // CORDY_AGENT_IDLE_WATCHDOG=0 disables; positive duration overrides.
    let agent_idle_watchdog = crate::helpers::duration_from_env(
        "CORDY_AGENT_IDLE_WATCHDOG",
    ...
    // CORDY_AGENT_TOOL_WATCHDOG=0 disables the in-flight-tool backstop.
    let agent_tool_watchdog = crate::helpers::duration_from_env(
        "CORDY_AGENT_TOOL_WATCHDOG",
    ...
    let mut device_name = crate::helpers::env_or_default("CORDY_DAEMON_DEVICE_NAME", &host);
    if !overrides.device_name.is_empty() {
        device_name = overrides.device_name.clone();
    }

    let mut runtime_name =
        crate::helpers::env_or_default("CORDY_AGENT_RUNTIME_NAME", DEFAULT_RUNTIME_NAME);
    ...
    let keep_env = matches!(
        std::env::var("CORDY_KEEP_ENV_AFTER_TASK").as_deref(),
        Ok("true") | Ok("1")
    );
    ...
    let gc_interval = crate::helpers::duration_from_env("CORDY_GC_INTERVAL", DEFAULT_GC_INTERVAL)?;
    let gc_ttl = crate::helpers::duration_from_env("CORDY_GC_TTL", DEFAULT_GC_TTL)?;
    ...
    let gc_orphan_ttl =
        crate::helpers::duration_from_env("CORDY_GC_ORPHAN_TTL", DEFAULT_GC_ORPHAN_TTL)?;
    let gc_artifact_ttl =
        crate::helpers::duration_from_env("CORDY_GC_ARTIFACT_TTL", DEFAULT_GC_ARTIFACT_TTL)?;
    let gc_codex_session_ttl = crate::helpers::duration_from_env(
        "CORDY_GC_CODEX_SESSION_TTL",
        DEFAULT_GC_CODEX_SESSION_TTL,
    )?;
    let gc_hermes_memory_ttl = crate::helpers::duration_from_env(
        "CORDY_GC_HERMES_MEMORY_TTL",
        DEFAULT_GC_HERMES_MEMORY_TTL,
    )?;
    let gc_hermes_session_ttl = crate::helpers::duration_from_env(
        "CORDY_GC_HERMES_SESSION_TTL",
        DEFAULT_GC_HERMES_SESSION_TTL,
    )?;
    let gc_repo_ttl = crate::helpers::duration_from_env("CORDY_GC_REPO_TTL", DEFAULT_GC_REPO_TTL)?;
    let gc_repo_maintenance_enabled =
        crate::helpers::bool_from_env("CORDY_GC_REPO_MAINTENANCE_ENABLED", true);
    ...
    // concerns).
    let mut auto_reload_enabled = crate::helpers::bool_from_env("CORDY_DAEMON_AUTO_RELOAD", true);
...
pub const OFFICIAL_CLOUD_HOST: &str = "api.cordy.ai";
...
pub fn normalize_server_base_url(raw: &str) -> anyhow::Result<String> {
    let mut u = url::Url::parse(raw.trim())
        .map_err(|err| anyhow::anyhow!("invalid CORDY_SERVER_URL: {err}"))?;
    match u.scheme() {
        "ws" => {
            u.set_scheme("http")
...
pub const TASK_WORKSPACES_ROOT_ENV: &str = "CORDY_TASK_WORKSPACES_ROOT";

/// `ResolveWorkspacesRoot`: explicit override > CORDY_WORKSPACES_ROOT env >
/// default ($HOME/cordy_workspaces[_<profile>]).
pub fn resolve_workspaces_root(profile: &str, override_root: &str) -> anyhow::Result<String> {
    ...
    if root.is_empty() {
        ...
        root = if profile.is_empty() {
            format!("{home}/cordy_workspaces")
        } else {
            format!("{home}/cordy_workspaces_{profile}")
        };
    }
...
fn home_dir() -> anyhow::Result<String> {
    std::env::var("HOME")
        ...
        .map_err(|_| {
            anyhow::anyhow!("resolve home directory (set CORDY_WORKSPACES_ROOT to override)")
        })
}
...
/// cannot redirect launches; ~/.cordy/hooks shadowing is skipped to avoid
/// wrapper recursion.
...
/// `isInCordyHooksDir` (config.go:800).
fn is_in_cordy_hooks_dir(path: &str) -> bool {
    if path.is_empty() {
        return false;
    }
    let parent = std::path::Path::new(path)
        ...
        .unwrap_or_default();
    is_cordy_hooks_dir(&parent)
}

/// `isCordyHooksDir` (config.go:807).
fn is_cordy_hooks_dir(dir: &str) -> bool {
    ...
    same_path_dir(dir, &format!("{home}/.cordy/hooks"))
}

/// `OpenclawCLITimeoutEnv` (openclaw_config.go:76).
pub(crate) const OPENCLAW_CLI_TIMEOUT_ENV: &str = "CORDY_OPENCLAW_CLI_TIMEOUT";
...
fn apply_openclaw_override(oc: Option<&OpenClawOverride>) {
    ...
    if !oc.binary_path.is_empty() && std::env::var_os("CORDY_OPENCLAW_PATH").is_none() {
        // SAFETY-free: single-threaded startup window before any backend spawn.
        std::env::set_var("CORDY_OPENCLAW_PATH", &oc.binary_path);
    }
...
mod tests {
    ...
    fn normalize_server_base_url_variants() {
        ...
        assert_eq!(
            normalize_server_base_url("wss://api.cordy.ai/ws").unwrap(),
            "https://api.cordy.ai"
        );
        assert_eq!(
    ...
    fn official_cloud_detection_host_only() {
        assert!(is_official_cloud_server(
            "https://API.CORDY.AI:443/some/path"
        ));
        // Staging subdomains deliberately excluded.
        assert!(!is_official_cloud_server("https://staging.cordy.ai"));
    ...
    fn completed_task_ttl_by_deployment_kind() {
        assert_eq!(
            default_gc_completed_task_ttl("https://api.cordy.ai"),
            DEFAULT_GC_COMPLETED_TASK_TTL_CLOUD
        );
        assert_eq!(
