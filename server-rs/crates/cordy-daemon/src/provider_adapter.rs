        ...
        tokio::select! {
            () = ctx.cancelled(), if terminal.is_none() => {
                ...
                terminal = Some(ExecutionResult {
                    ...
                    session_id: transcript.observed_session_id.clone(),
...
impl TranscriptBatch {
    fn begin_attempt(&mut self) {
        ...
        self.observed_session_id.clear();
    }
    ...
    fn push(&mut self, message: Message) -> Option<String> {
        if message.message_type == MessageType::Status {
            if !message.session_id.is_empty() {
                self.observed_session_id.clone_from(&message.session_id);
            }
            if !self.session_pinned && !self.observed_session_id.is_empty() {
                self.session_pinned = true;
                return Some(self.observed_session_id.clone());
            }
            return None;
        }
