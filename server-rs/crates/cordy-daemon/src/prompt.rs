...
pub(crate) fn session_continuity_notice_for(task: &Task) -> &'static str {
    ...
    // Every other transcript-persisting surface reads back via `cordy chat
...
fn build_active_sibling_runs_block(
    ...
    b.push_str(" and inspect relevant siblings with the `run-messages` commands below — coordinate with existing work instead of opening a second PR. For writes that only record ownership or status of work already underway, use `--no-start` on `cordy issue assign`/`update`/`status`.\n\n");
    for run in runs {
...
pub(crate) fn build_chat_prompt(task: &Task) -> String {
    ...
    if task.chat_channel_type.is_empty() {
        ...
        let platform = channel_display_name(&task.chat_channel_type);
        b.push_str(&format!(
            "\nThis reply is delivered to {platform} as text. You cannot attach a file to it: `cordy attachment upload` binds to a Cordy chat reply, which this is not. If you produce a file, describe it in words — never write its local path as a link, and never upload it and then write as though it arrived.\n"
        ));
    }
    b
}
...
pub(crate) fn build_quick_create_prompt(task: &Task) -> String {
    ...
    b.push_str("A user captured the following input via the quick-create modal. There is NO existing issue. Your job is to create a well-formed issue from this input with a single `cordy issue create` command.\n\n");
...
mod tests {
    ...
    fn handoff_note_assignment_branch() {
        ...
        assert!(out.contains("cordy issue get issue-123"));
    }
    ...
    fn autopilot_branch() {
        ...
        assert!(!out.contains("`cordy issue get"));
    }
