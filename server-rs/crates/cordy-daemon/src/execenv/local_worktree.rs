...
//! - cordySidecarDirNames /
//!   isCordySidecarPath              → CORDY_SIDECAR_DIR_NAMES / is_cordy_sidecar_path
...
async fn commit_identity_args(dir: &str) -> Vec<String> {
    ...
    vec![
        "-c".to_string(),
        "user.name=Cordy Agent".to_string(),
        "-c".to_string(),
        "user.email=agent@cordy.local".to_string(),
    ]
}
...
async fn copy_untracked_files(git_root: &str, worktree_path: &str) -> (usize, usize) {
    ...
    for rel in out.split('\u{0}') {
        ...
        // Never replay Cordy's own sidecars. They are untracked files in the
...
/// cordySidecarDirNames are the directories Prepare writes into a workdir. A
...
const CORDY_SIDECAR_DIR_NAMES: [&str; 2] = [".agent_context", ".cordy"];

/// is_cordy_sidecar_path reports whether a repo-relative path is one of the
...
pub(crate) fn is_cordy_sidecar_path(rel: &str) -> bool {
    rel.split('/')
        .any(|seg| CORDY_SIDECAR_DIR_NAMES.contains(&seg))
}
