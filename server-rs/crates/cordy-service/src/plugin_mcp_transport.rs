...
use cordy_remotemcp::{discover, tool_set_digest, Connection, Tool, PLUGIN_CONTRIBUTION_PREFIX};
...
pub async fn discover_mcp_hook_tools(
    pool: &sqlx::PgPool,
    secrets: Option<&cordy_util::secretbox::SecretBox>,
...
pub async fn approve_mcp_hook_tools(
    ...
    let updated =
        cordy_db::queries::plugin::set_plugin_mcp_approvals(pool, installation.id, &encoded)
            .await
            .map_err(|e| {
                PluginError::new(
                    PluginErrorKind::Unavailable,
...
pub async fn agent_mcp_connections(
    ...
    let installations =
        cordy_db::queries::plugin::list_workspace_plugin_installations(pool, workspace_id)
            .await
            .map_err(|e| {
                PluginError::new(
                    PluginErrorKind::Unavailable,
...
async fn decrypted_secret(
    ...
    let row = cordy_db::queries::plugin::get_plugin_secret(pool, installation_id, key)
        .await
