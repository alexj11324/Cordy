//! Owned lifecycle for the Autopilot quota reservation reconciler.
...
use crate::autopilot::AutopilotService;
use crate::autopilot_failure_monitor::{classify_error, FailureClass, ShutdownOutcome};
...
impl QuotaReconcilerMetrics for cordy_metrics::BusinessMetrics {
    fn record(&self, stage: &'static str, outcome: &'static str) {
        self.record_autopilot_quota_reconciler(stage, outcome);
    }
}
        ...
        loop {
            match self.run_once(&cancel).await {
                ...
                ReconcileOutcome::Failed { ref message, .. } => {
                    tracing::warn!(error = %message, "autopilot quota reconciler failed");
                }
