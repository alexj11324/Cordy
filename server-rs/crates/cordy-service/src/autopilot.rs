...
use cordy_db::models::{Agent, Autopilot, AutopilotRun};
use cordy_db::queries::agent::has_active_task_for_issue;
use cordy_db::queries::agent_invocation_target::list_agent_invocation_targets;
...
use cordy_db::queries::autopilot::{create_autopilot_run, get_autopilot_run_by_quota_reservation};
use cordy_db::queries::autopilot_quota::{
...
impl AutopilotService {
    pub fn new(pool: PgPool, bus: Arc<cordy_events::Bus>, task_svc: Arc<TaskService>) -> Self {
        Self {
            pool,
            bus,
            task_svc,
...
pub fn task_failure_reason_for_run(task: &cordy_db::models::AgentTaskQueue) -> String {
...
impl AutopilotService {
    ...
    pub async fn resolve_leader(
        ...
        match ap.assignee_type.as_str() {
            "" | "agent" => {
                let agent = cordy_db::queries::agent::get_agent(&self.pool, ap.assignee_id)
                    .await
            ...
            "squad" => {
                let squad = cordy_db::queries::squad::get_squad(&self.pool, ap.assignee_id)
                    .await
                ...
                let agent = cordy_db::queries::agent::get_agent(&self.pool, squad.leader_id)
                    .await
...
pub fn new_request_idempotency_key() -> String {
    cordy_db::dbid::new_v7().to_string()
}
...
impl AutopilotService {
    ...
    pub async fn recover_partial_autopilot_run(&self, run_id: Uuid) -> anyhow::Result<bool> {
        let rows =
            cordy_db::queries::autopilot::recover_partial_autopilot_run(&self.pool, run_id).await?;
    ...
    pub async fn fail_runs_by_issue(&self, issue_id: Uuid) -> anyhow::Result<()> {
        cordy_db::queries::autopilot::fail_autopilot_runs_by_issue(&self.pool, issue_id).await?;
        Ok(())
    }
}
...
impl AutopilotService {
    fn publish_run_done(&self, workspace_id: &str, run: &AutopilotRun, status: &str) {
        self.bus.publish(&cordy_events::Event {
            event_type: cordy_protocol::EVENT_AUTOPILOT_RUN_DONE.to_string(),
...
impl AutopilotService {
    ...
    pub async fn sync_run_from_task(&self, task: &cordy_db::models::AgentTaskQueue) {
    ...
    pub async fn sync_run_from_linked_issue_task(&self, task: &cordy_db::models::AgentTaskQueue) {
...
use cordy_db::queries::subscriber::add_issue_subscriber;
use cordy_db::queries::workspace::increment_issue_counter;
...
impl AutopilotService {
    ...
    pub async fn dispatch_autopilot_run(
        ...
        self.bus.publish(&cordy_events::Event {
            event_type: cordy_protocol::EVENT_AUTOPILOT_RUN_START.to_string(),
    ...
    async fn dispatch_create_issue(
        ...
        let issue = cordy_db::queries::issue::create_issue_with_origin(
            &mut *tx,
            ap.workspace_id,
            &title,
        ...
        *run = updated_run;
        ...
        self.bus.publish(&cordy_events::Event {
            event_type: cordy_protocol::EVENT_ISSUE_CREATED.to_string(),
    ...
    async fn notify_autopilot_subscribers_on_create(
        ...
        issue: &cordy_db::models::Issue,
        leader_id: Uuid,
        subscribers: &[cordy_db::models::AutopilotSubscriber],
    ) {
    ...
    async fn get_issue_prefix(&self, workspace_id: Uuid) -> String {
        match cordy_db::queries::workspace::get_workspace(&self.pool, workspace_id).await {
...
use cordy_db::queries::agent::list_tasks_by_issue;
use cordy_db::queries::autopilot::{
