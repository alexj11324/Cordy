...
impl TaskService {
    ...
    pub async fn complete_task(
        ...
        if let Some(issue_id) = task.issue_id {
            ...
            let agent_commented = cordy_db::queries::comment::has_agent_commented_since(
                &self.pool,
                issue_id,
                task.agent_id,
                task.started_at,
            ...
            if !suppress_no_action && !agent_commented {
                if let Ok(payload) = serde_json::from_value::<
                    cordy_protocol::messages::TaskCompletedPayload,
                >(result.clone())
                {
        ...
        if retryable_reason(&failure_reason) {
            if let Ok(Some(parent)) = get_agent_task(&self.pool, task_id).await {
                if retry_eligible(&failure_reason, &parent) {
                    ...
                    match cordy_db::queries::agent::get_agent(&self.pool, parent.agent_id).await {
                        Err(aerr) => {
        ...
        if let Some(source_task_id) = source_task_id {
            ...
            match (issue.assignee_type.as_deref(), issue.assignee_id) {
                ...
                (Some("squad"), Some(squad_assignee)) => {
                    let squad = cordy_db::queries::squad::get_squad(&self.pool, squad_assignee)
                        .await
                        .map_err(|_| {
            ...
            if let Some(issue_id) = t.issue_id {
                if let Ok(Some(issue)) = get_issue(&self.pool, issue_id).await {
                        ...
                        && !retried_issues.contains(&issue_id)
                    {
                        ...
                        match cordy_db::queries::agent::has_active_task_for_issue(
                            &self.pool, issue_id,
                        )
                        .await
                        {
...
impl TaskService {
    ...
    async fn write_chat_completion_outcome_tx(
        ...
        let body_full = cordy_util::unescape_backslash_escapes(&payload.output);
        ...
        let (content, message_kind): (String, Option<&str>) = if !is_empty {
            ...
            (
                ...
                Some(if opening_only {
                    cordy_protocol::CHAT_MESSAGE_KIND_ONBOARDING_OPENING
                } else {
                    cordy_protocol::CHAT_MESSAGE_KIND_MESSAGE
                }),
            )
            ...
            (
                ...
                Some(cordy_protocol::CHAT_MESSAGE_KIND_NO_RESPONSE),
            )
        };
...
async fn lock_chat_session_for_task_write(
    ...
    let _ = cordy_db::queries::chat::lock_chat_session_for_task(&mut *exec, task_id)
        .await
        .map_err(|e| {
...
async fn has_squad_leader_no_action_for_task(pool: &sqlx::PgPool, task: &AgentTaskQueue) -> bool {
    ...
    match cordy_db::queries::activity::has_squad_leader_no_action_evaluation_for_task(
        pool,
        issue_id,
        task.agent_id,
        &task.id.to_string(),
