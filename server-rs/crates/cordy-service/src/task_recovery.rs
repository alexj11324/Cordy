...
use cordy_db::models::{Agent, AgentTaskQueue, Comment, InboxItem, Issue};
...
pub(crate) async fn load_delegated_failure_recover_target(
    ...
    let Some(agent) = no_row_or_err(
        cordy_db::queries::agent::get_agent(&mut *exec, source.agent_id).await,
        "load source agent",
    )?
    else {
        return Ok(None);
...
mod tests {
    ...
    async fn recovery_attempts_exhaust_once_and_never_self_recurse() {
        ...
        let result = async {
            ...
            let captured = events.lock().unwrap_or_else(|poisoned| poisoned.into_inner());
            anyhow::ensure!(captured.iter().any(|event| event.event_type == cordy_protocol::EVENT_INBOX_NEW), "exhaustion inbox event missing");
            Ok::<(), anyhow::Error>(())
        }
        .await;
        let cleanup = rows.cleanup().await;
