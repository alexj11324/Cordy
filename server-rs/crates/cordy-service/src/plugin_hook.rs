...
use cordy_plugincontract::{net_domains, Hook, TRANSPORT_HTTP};
...
async fn recent_invocation_count(
    ...
    match since.parse::<chrono::DateTime<Utc>>() {
        Ok(created_at) => {
            ...
            cordy_db::queries::plugin::count_recent_plugin_invocations(
                pool,
                installation_id,
                hook_key,
                Some(created_at),
...
pub async fn hook_breaker_open(pool: &sqlx::PgPool, installation_id: Uuid, hook_key: &str) -> bool {
    ...
    let failures = match since.parse::<chrono::DateTime<Utc>>() {
        Ok(created_at) => {
            match cordy_db::queries::plugin::count_recent_plugin_failures(
                pool,
                installation_id,
                hook_key,
                Some(created_at),
...
async fn call_hook_endpoint(
    ...
    let endpoint = if is_dev_origin(&service.dev_origins, transport_url) {
        ...
        cordy_remotemcp::validate_public_https_endpoint(transport_url, &domains, None)
            .await
            .map_err(|e| {
                PluginError::with_source(
                    PluginErrorKind::Forbidden,
...
async fn send_hook_request(
    ...
    if is_dev_origin(&service.dev_origins, endpoint.as_str()) {
        ...
        // redirects stay off. (The dev CA from CORDY_PLUGIN_DEV_CA applies to
        ...
        let response = client
            ...
            .header("X-Cordy-Timestamp", timestamp)
            .header(
                "X-Cordy-Signature",
                format!("{HOOK_SIGNATURE_VERSION}={signature}"),
            )
            .header("X-Cordy-Plugin-Installation", uuid_string(installation_id))
            .header("User-Agent", "Cordy-Hooks/1")
            .body(encoded)
            .send()
            .await
            .map_err(|e| {
        ...
        let request = http::Request::builder()
            ...
            .header("X-Cordy-Plugin-Installation", uuid_string(installation_id))
            .header("User-Agent", "Cordy-Hooks/1")
            .body(cordy_remotemcp::RequestBody::from(encoded))
            .map_err(|e| {
                PluginError::with_source(PluginErrorKind::Invalid, "build hook request", e)
            })?;
...
use cordy_plugincontract::CONFIG_SECRET as CONFIG_SECRET_VALUE;
...
async fn record_invocation(
    ...
    let _ = cordy_db::queries::plugin::create_plugin_invocation(
        &service.pool,
