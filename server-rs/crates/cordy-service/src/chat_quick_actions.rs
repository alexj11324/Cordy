...
use cordy_db::models::ChatMessage;
...
pub use cordy_protocol::CHAT_MESSAGE_KIND_MESSAGE;
...
/// from `cordy_protocol` (Go `protocol.ChatQuickAction`).
pub use cordy_protocol::ChatQuickAction;
...
/// CORDY_LLM_API_KEY / CORDY_LLM_BASE_URL).
#[async_trait]
...
pub fn split_chat_quick_actions(
    output: &str,
) -> (String, Vec<cordy_protocol::messages::ChatQuickAction>) {
    ...
    let Ok(candidates) =
        serde_json::from_str::<Vec<cordy_protocol::messages::ChatQuickAction>>(raw)
    else {
...
pub fn sanitize_chat_quick_actions(
    candidates: Vec<cordy_protocol::messages::ChatQuickAction>,
