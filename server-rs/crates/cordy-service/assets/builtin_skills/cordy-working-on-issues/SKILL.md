---
name: cordy-working-on-issues
description: "Use when acting on a Cordy issue beyond what the brief covers: PR linking vs close intent, reading a linked PR's real state, metadata keys, status-change side effects, sub-issue todo vs backlog."
user-invocable: false
allowed-tools: Bash(cordy *), Bash(git *), Bash(gh *)
---

# Working on Cordy issues
...
For building mention links, load `cordy-mentioning` instead — not this skill.
...
is to open or update a PR before posting the final Cordy issue comment, unless
...
```bash
cordy issue pull-request attach <issue-id> --url https://github.com/<owner>/<repo>/pull/<number>
```
...
When a step depends on PR state, query Cordy's link table — do not infer it
from branch names, GitHub search, memory, or `pr_url` metadata (which can be
stale).

```bash
cordy issue pull-requests <issue-id> --output json
```
...
PR yourself (`cordy issue pull-request attach <issue-id> --url …`); that writes a
...
```bash
cordy issue metadata set <issue-id> --key <key> --value <value>
cordy issue metadata delete <issue-id> --key <stale-key>
```
...
- Read what exists before writing: `cordy property list` shows the catalog;
  `cordy issue property list <issue-id>` shows values set on the issue.
- Set values by property name and option name — the CLI translates to ids:

```bash
cordy issue property set <issue-id> --name Environment --value staging
cordy issue property set <issue-id> --name Platforms --value "iOS,Android"
cordy issue property set <issue-id> --name Reviewer --value Bohan
cordy issue property unset <issue-id> --name Environment
```
...
```bash
cordy issue assign <issue-id> --to-id <agent-id> --no-start
cordy issue update <issue-id> --assignee-id <agent-id> --no-start
cordy issue status <issue-id> in_progress --no-start
```
...
```bash
cordy issue create --title "..." --parent <issue-id> --assignee <agent> --status todo
```

Strictly serial children — park later steps, promote one at a time:

```bash
cordy issue create --title "Step 2: ..." --parent <issue-id> --assignee <agent> --status backlog
cordy issue status <child-id> todo   # promote when the previous step is truly done
```
...
cordy issue create --title "Research A" --parent <id> --assignee <agent> --stage 1 --status todo
cordy issue create --title "Research B" --parent <id> --assignee <agent> --stage 1 --status todo
cordy issue create --title "Build"      --parent <id> --assignee <agent> --stage 2 --status backlog
cordy issue create --title "Ship"       --parent <id> --assignee <agent> --stage 3 --status backlog
```
...
```bash
cordy issue children <parent-id>             # sub-issues grouped by stage
cordy issue status <stage-2-child-id> todo   # promote when its deps are met
```
...
cordy issue create --title "Step 2" --parent <issue-id> --assignee <agent> --status todo
cordy issue create --title "Step 3" --parent <issue-id> --assignee <agent> --status todo

# correct — stage them; Stage 1 runs, later stages park and are promoted as
# each stage's barrier closes
cordy issue create --title "Step 1" --parent <issue-id> --assignee <agent> --stage 1 --status todo
cordy issue create --title "Step 2" --parent <issue-id> --assignee <agent> --stage 2 --status backlog
cordy issue create --title "Step 3" --parent <issue-id> --assignee <agent> --stage 3 --status backlog
```
