---
name: cordy-autopilots
description: "Use when creating, updating, inspecting, triggering, or debugging a Cordy autopilot (scheduled, webhook, or manual)."
...
```bash
cordy autopilot list --output json
cordy autopilot get <autopilot-id> --output json
cordy autopilot runs <autopilot-id> --output json
```
...
- `create_issue` creates a Cordy issue, making the run visible as issue state.
...
cordy autopilot list --output json
cordy autopilot get <autopilot-id> --output json
cordy autopilot create --title "<title>" --description "<task prompt>" --agent <agent-name-or-id> --mode create_issue|run_only --output json
cordy autopilot update <autopilot-id> --status active|paused --output json
cordy autopilot runs <autopilot-id> --output json
cordy autopilot trigger-add <autopilot-id> --kind schedule --cron "0 9 * * *" --timezone Asia/Shanghai --output json
cordy autopilot trigger-add <autopilot-id> --kind webhook --label "ci" --output json
cordy autopilot trigger <autopilot-id> --output json
cordy autopilot trigger-rotate-url <autopilot-id> <trigger-id> --yes --output json
```
...
1. `cordy autopilot get <id> --output json` — status, mode, assignee, triggers.
2. `cordy autopilot runs <id> --output json` — run status and failure reason.
3. If assigned to a squad, inspect the squad: `cordy squad get <squad-id> --output json`; execution goes to the leader.
4. Inspect the target agent/runtime: `cordy agent get <agent-id> --output json` and `cordy runtime list --output json`.
