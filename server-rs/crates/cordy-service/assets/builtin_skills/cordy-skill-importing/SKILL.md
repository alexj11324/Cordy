---
name: cordy-skill-importing
description: "Use when asked to import or install a specific skill into this Cordy workspace from a URL or slug. Not for choosing which skill the user needs; never treat a local installer such as `npx skills add` as the final install."
user-invocable: false
allowed-tools: Bash(cordy *)
---

# Importing skills into Cordy

Use this skill when the user already provided a skill URL, slug, or a clear intent
to import a specific skill into the current Cordy workspace.
...
A skill is installed for Cordy only when it exists in the current workspace's
...
```bash
cordy skill import --url <url> --output json              # hosted source
cordy skill import --file <path-to.skill> --output json   # local archive
```
...
environment, not the Cordy workspace DB, so Cordy cannot manage or bind it.

## Supported URL source families

`detectImportSource` accepts these hosts (and `www.` variants). Pass any of these
forms to `cordy skill import --url <url> --output json`:

```bash
cordy skill import --url clawhub.ai/owner/skill --output json
cordy skill import --url skills.sh/owner/repo/skill --output json
cordy skill import --url github.com/owner/repo --output json
cordy skill import --url github.com/owner/repo/tree/main/path/to/skill --output json
cordy skill import --url github.com/owner/repo/blob/main/path/to/SKILL.md --output json
```
...
`cordy skill import --file <path> --output json` imports a skill from a local
...
```bash
cordy skill import --url <url> --output json
```
...
```bash
cordy agent skills add <agent-id> --skill-ids <skill-id> --output json
cordy agent skills list <agent-id> --output json
```

After the final `cordy agent skills list <agent-id> --output json`, verify the
target skill id is present before claiming the skill is available to that agent.

## Additive add vs replace-all set

`cordy agent skills add` is additive: the server inserts the assignments without
clearing existing ones (`AddAgentSkills`).

`cordy agent skills set` is replace-all: the server clears every current
...
Default behavior is safe: `cordy skill import --url <url>` is equivalent to
...
cordy skill import --url https://skills.sh/acme/repo/review-helper --output json

# Replace the existing same-name skill, preserving its ID and agent bindings.
cordy skill import --url https://skills.sh/acme/repo/review-helper --on-conflict overwrite --output json

# Keep the existing skill and import a copy such as review-helper-2.
cordy skill import --url https://skills.sh/acme/repo/review-helper --on-conflict rename --output json

# Batch-friendly behavior: leave the existing skill alone and mark it skipped.
cordy skill import --url https://skills.sh/acme/repo/review-helper --on-conflict skip --output json
```
...
```bash
cordy skill get <skill-id> --output json
```
...
## Updating an already-imported skill: `cordy skill refresh`
...
```bash
cordy skill refresh <skill-id> --output json
```
...
`cordy skill get`), not the import result envelope. Failure modes to report
...
Incorrect (bypasses Cordy):

```bash
npx skills add https://skills.sh/owner/repo/skill
```

The skill may exist locally, but Cordy cannot manage it as a workspace skill.
...
```bash
cordy skill import --url https://skills.sh/owner/repo/skill --output json
```
