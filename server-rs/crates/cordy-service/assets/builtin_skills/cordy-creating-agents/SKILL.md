---
name: cordy-creating-agents
description: "Use when creating, inspecting, or debugging a Cordy agent definition via the `cordy agent` CLI or POST /api/agents. Not for assigning issues to agents that already exist, and not for runtime task prompts."
user-invocable: false
allowed-tools: Bash(cordy *)
---

# Creating Cordy agents

This is the contract for Cordy's agent-creation path: what the create entry
...
```bash
cordy agent get <agent-id> --output json      # full persisted agent record
cordy agent skills list <agent-id> --output json   # current skill bindings
cordy agent env get <agent-id> --output json  # plaintext env (agent owner or ws owner/admin; agents denied)
```
...
deleting its agents (PB-5559). An unbound agent keeps everything it owns and
...
`POST /api/agents` (`cordy agent create`). At task claim time the daemon
...
```bash
cordy agent create --name <name> --runtime-id <runtime-id> \
...
`cordy agent copy <source-agent-id>` forks an existing agent's portable
...
```bash
cordy agent copy <source-agent-id> --name "My Agent (copy)"   # same runtime
cordy agent copy <source-agent-id> --runtime-id <target> --model <model>  # cross-runtime fork
```
...
```bash
cordy agent create --name <name> --runtime-id <runtime-id> --custom-env-stdin --output json
cordy agent create --name <name> --runtime-id <runtime-id> --custom-env-file <0600-json> --output json
```
...
- Reading plaintext values requires the dedicated `GET /api/agents/{id}/env`
  endpoint (`cordy agent env get`). It is gated to the **agent's own human
...
- Writing values after creation does NOT go through `agent update`. The generic
  ...
  `PUT /api/agents/{id}/env` (`cordy agent env set`), which carries the same
...
```bash
cordy agent create --name <name> --runtime-id <runtime-id> --mcp-config-file <0600-json> --output json
cordy agent update <agent-id> --mcp-config-stdin --output json
cordy agent update <agent-id> --mcp-config 'null'   # clears the config
```
...
`cordy workspace mcp list|add|update|remove`). Adding one there gives it to
NO agent — same shape as a workspace skill. It reaches an agent only when
someone assigns it:

```bash
cordy workspace mcp list --output table        # find the server id
cordy agent mcp add <agent-id> <server-id>     # give it to one agent
cordy agent mcp disable <agent-id> <server-id> # stop sending it, keep the assignment
cordy agent mcp remove <agent-id> <server-id>  # take it away
```
...
- `cordy agent create` — inserts a new agent row.
- `cordy agent copy` — inserts a new agent row (a fork of an existing agent);
  the source is left untouched.
- `cordy agent skills add` / `set` — mutate bindings (`set` is destructive:
  it drops bindings not in the new list).
- `cordy agent env set` — overwrites the full `custom_env` map and writes an
  audit row.
