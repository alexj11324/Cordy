...
use cordy_db::models::{ChannelChatSessionBinding, ChannelInstallation};
...
impl History {
    ...
    /// security boundary for `cordy chat thread <id>` (the agent supplies only
    ...
    /// thread, then drills into with `cordy chat thread <id>`. Backs
    /// `cordy chat history`.
...
/// can drill in with `cordy chat thread <id>`.
