...
use cordy_channel::{InboundMessage, OutboundMessage};
use cordy_channel_engine::resolvers::{Outcome, ResolvedInstallation, Result as EngineResult};
...
pub struct OutboundReplierConfig {
    ...
    /// The Cordy web app host the user clicks into to redeem the binding token
    /// (e.g. https://cordy.example). It comes from CORDY_APP_URL (falling back
    /// to FRONTEND_ORIGIN) and is intentionally separate from CORDY_PUBLIC_URL,
...
#[async_trait]
impl cordy_channel_engine::resolvers::OutboundReplier for OutboundReplier {
...
fn member_issue_title(title: &str) -> String {
    let title = cordy_channel::break_markdown_link_adjacency(title);
