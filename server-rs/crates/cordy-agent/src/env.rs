...
//!
//! Daemon-level `CORDY_*` values and Claude runtime markers must not leak into
//! provider-controlled tools. Task-scoped overrides are applied after the
//! ambient filter so an explicit `CORDY_TOKEN` still reaches the child.
...
mod tests {
    ...
    #[test]
    fn strips_inherited_cordy_and_claude_runtime_markers() {
        let ambient = [
            ("PATH", "/usr/bin"),
            ("CORDY_TOKEN", "daemon-secret"),
            ("CORDY_SERVER_URL", "https://daemon.example"),
            ("CLAUDECODE", "1"),
        ...
        let overrides = BTreeMap::from([
            ("CORDY_TOKEN".to_string(), "mat_task".to_string()),
            (
                "CORDY_SERVER_URL".to_string(),
                "https://task.example".to_string(),
            ),
        ]);
        let env = merge_child_env(ambient, &overrides);
        assert_eq!(env.get("PATH").map(String::as_str), Some("/usr/bin"));
        assert_eq!(env.get("CORDY_TOKEN").map(String::as_str), Some("mat_task"));
        assert_eq!(
            env.get("CORDY_SERVER_URL").map(String::as_str),
        ...
        assert!(is_filtered_child_env_key("cordy_token"));
    }
}
