...
struct CodexAttempt {
    session: Session,
    initialize_retry: oneshot::Receiver<bool>,
}
...
async fn run_codex_attempts(
    ...
    for attempt_number in 1..=2 {
        ...
        let retry_safe = initialize_retry.await.unwrap_or(false);
        if !retry_safe || attempt_number == 2 {
        ...
        let retry = tokio::time::sleep(INITIALIZE_RETRY_BACKOFF);
        tokio::pin!(retry);
        tokio::select! {
