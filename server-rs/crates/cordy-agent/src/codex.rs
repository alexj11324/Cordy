...
async fn write_managed_codex_mcp(
    ...
    let object = config.as_object().ok_or_else(|| {
        AgentError::InvalidConfig("codex managed MCP configuration must be an object".to_string())
    })?;
    let servers = object
        .get("mcpServers")
        ...
        .ok_or_else(|| {
            AgentError::InvalidConfig(
                "codex managed MCP configuration must contain an object mcpServers".to_string(),
            )
        })?;
