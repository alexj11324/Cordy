...
use patchbay_daemon::hub::Metrics as DaemonWsMetrics;
...
mod tests {
    ...
    fn daemonws_families_use_counter_and_gauge_types() {
        ...
        registry
            .register(Box::new(DaemonWsCollector::new(&patchbay_daemon::hub::M)))
            .expect("register daemon WebSocket collector");
        let families = registry.gather();
        let active = families
            .iter()
            .find(|family| family.name() == "patchbay_daemonws_active_connections")
            .unwrap();
        assert_eq!(active.get_field_type(), MetricType::GAUGE);
        let connects = families
            .iter()
            .find(|family| family.name() == "patchbay_daemonws_connects_total")
            .unwrap();
        assert_eq!(connects.get_field_type(), MetricType::COUNTER);
        let delivered = families
            .iter()
            .find(|family| family.name() == "patchbay_daemonws_wakeup_delivered_total")
            .unwrap();
