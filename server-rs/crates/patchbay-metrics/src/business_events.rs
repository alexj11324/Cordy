...
use patchbay_analytics::{is_metrics_only, AnalyticsClient, Event};
...
impl BusinessEventMetrics {
    pub(crate) fn new() -> Self {
        Self {
            signup: cvec("patchbay_signup_total", "Total user signups (account creations)."),
            workspace_created: cvec("patchbay_workspace_created_total", "Total workspaces created."),
            team_invite_sent: cvec("patchbay_team_invite_sent_total", "Total workspace invitations sent."),
            team_invite_accepted: cvec(
                "patchbay_team_invite_accepted_total",
                "Total workspace invitations accepted.",
            ),
            onboarding_started: cvec("patchbay_onboarding_started_total", "Total onboarding flows started."),
            ...
            onboarding_completed: cvec("patchbay_onboarding_completed_total", "Total onboarding flows completed."),
            cloud_waitlist_joined: cvec(
                "patchbay_cloud_waitlist_joined_total",
                "Total users that joined the cloud waitlist.",
            ),
            issue_created: cvec("patchbay_issue_created_total", "Total issues created (any source)."),
            chat_message_sent: cvec(
                "patchbay_chat_message_sent_total",
            ...
            runtime_ready: cvec("patchbay_runtime_ready_total", "Total runtimes that reached ready state."),
            runtime_ready_seconds: HistogramVec::new(
                HistogramOpts::new(
                    "patchbay_runtime_ready_seconds",
                    "Time from runtime registration to ready (seconds).",
                )
                .buckets(RUNTIME_READY_BUCKETS.to_vec()),
                metric_labels("patchbay_runtime_ready_seconds"),
            )
            ...
            runtime_offline: cvec("patchbay_runtime_offline_total", "Total runtime offline transitions."),
            ...
            cloud_runtime_request: cvec(
                "patchbay_cloudruntime_request_total",
                "Total outbound cloud runtime requests by op and status bucket.",
            ),
            cloud_runtime_request_duration_secs: HistogramVec::new(
                HistogramOpts::new(
                    "patchbay_cloudruntime_request_duration_seconds",
                    "Outbound cloud runtime request duration (seconds).",
                )
                .buckets(CLOUD_RUNTIME_REQUEST_BUCKETS.to_vec()),
                metric_labels("patchbay_cloudruntime_request_duration_seconds"),
            )
            .expect("valid histogram vec"),
            feedback_submitted: cvec("patchbay_feedback_submitted_total", "Total in-app feedback submissions."),
            contact_sales_submitted: cvec(
                "patchbay_contact_sales_submitted_total",
                "Total contact-sales inquiries submitted.",
            ),
            chat_output_local_path: cvec(
                "patchbay_chat_output_local_path_total",
...
mod tests {
    ...
    fn record_event_pairs_counter_and_skips_metrics_only_capture() {
        ...
        let ev = patchbay_analytics::signup("u1", "a@b.co", "x");
    ...
    fn inc_for_event_dispatch_spot_checks() {
        ...
        let ready = patchbay_analytics::runtime_ready("", "ws", "rt", "dm", "claude", 4500);
