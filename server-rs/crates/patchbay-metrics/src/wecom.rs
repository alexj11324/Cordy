...
mod tests {
    ...
    fn production_registry_registers_every_wecom_counter_once() {
        ...
        let names = registry
            ...
            .filter(|name| name.starts_with("patchbay_wecom_"))
            .collect::<BTreeSet<_>>();
        assert_eq!(
            names,
            BTreeSet::from([
                "patchbay_wecom_auth_failures_total".to_string(),
                "patchbay_wecom_connect_failures_total".to_string(),
                "patchbay_wecom_inbound_callbacks_total".to_string(),
                "patchbay_wecom_inbound_queue_blocked_total".to_string(),
            ])
        );
    }
}
