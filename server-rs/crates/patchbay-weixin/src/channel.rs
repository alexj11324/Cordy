...
impl Channel for WeixinChannel {
    fn r#type(&self) -> patchbay_channel::Type {
        patchbay_channel::Type(crate::TYPE_WEIXIN.to_string())
    }

    async fn connect(&self, ctx: CancellationToken) -> anyhow::Result<()> {
        let mut cursor = patchbay_db::queries::channel::get_channel_receive_cursor(
            &self.pool,
            ...
            match response {
                Ok(response) => {
                    ...
                    if !next_cursor.is_empty() {
                        ...
                        patchbay_db::queries::channel::replace_channel_receive_cursor(
                            &mut *tx,
                            self.installation_id,
                            crate::TYPE_WEIXIN,
                            &next_cursor,
    ...
    async fn send(
        ...
        self.client
            ...
            .await?;
        Ok(patchbay_channel::SendResult::default())
    }
...
pub fn register(registry: &patchbay_channel::Registry, deps: ChannelDeps) {
    registry.register(
        patchbay_channel::Type(crate::TYPE_WEIXIN.to_string()),
        factory(deps),
    );
