...
pub const DEV_ORIGINS_ENV: &str = "PATCHBAY_PLUGIN_DEV_ORIGINS";
...
/// verification".
pub const DEV_CA_ENV: &str = "PATCHBAY_PLUGIN_DEV_CA";
...
mod tests {
    ...
    fn dev_ca_fails_closed_on_missing_or_garbage_file() {
        assert!(dev_tls_config_from_path("/nonexistent/patchbay-dev-ca.pem").is_none());

        let mut garbage = std::env::temp_dir();
        garbage.push("patchbay-remotemcp-garbage.pem");
    ...
    fn dev_ca_loads_system_pem_bundle_when_available() {
        ...
        path.push("patchbay-remotemcp-system-bundle.pem");
