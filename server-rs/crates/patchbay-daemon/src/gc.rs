...
pub(crate) mod processtree {
    ...
    async fn run_inner(
        ...
        cmd.stdout(Stdio::piped());
        cmd.stderr(Stdio::piped());
        let mut tree = match patchbay_agent::OwnedProcessTree::spawn(&mut cmd).await {
            Ok(tree) => tree,
            Err(error) => {
                return (
                    Vec::new(),
...
fn prune_codex_session_stores(
    ...
    let Some(root) = shared_codex_home().map(|home| home.join("patchbay-sessions").join(namespace))
    else {
        return (0, 0);
    };
...
pub(crate) struct GcConfig {
    ...
    /// regardless of parent issue status (default: 14d on Patchbay Cloud,
...
const REPO_MAINTENANCE_MARKER: &str = ".patchbay-maintenance-pending";
