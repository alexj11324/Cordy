...
use patchbay_protocol::{DaemonHeartbeatAckPayload, RuntimeProfilesChangedPayload};
                    ...
                    move |headers: axum::http::HeaderMap, ws: axum::extract::WebSocketUpgrade| {
                        ...
                        async move {
                            ...
                            ws.on_upgrade(move |mut socket| async move {
                                ...
                                let heartbeat: patchbay_protocol::Message =
                                    serde_json::from_slice(heartbeat.into_data().as_ref()).unwrap();
                                assert_eq!(
                                    heartbeat.r#type,
                                    patchbay_protocol::EVENT_DAEMON_HEARTBEAT
                                );
                                for message in [
                                    patchbay_protocol::Message {
                                        r#type: patchbay_protocol::EVENT_DAEMON_HEARTBEAT_ACK
                                            .to_string(),
                                        payload: json!({
                                    ...
                                    patchbay_protocol::Message {
                                        r#type: patchbay_protocol::EVENT_DAEMON_TASK_AVAILABLE
                                            .to_string(),
                                        payload: json!({
                                            "runtime_id": "runtime-1",
                                            "task_id": "task-1"
                                ...
                                let rpc = loop {
                                    ...
                                    let message: patchbay_protocol::Message =
                                        serde_json::from_slice(message.into_data().as_ref())
                                            .unwrap();
                                    if message.r#type == patchbay_protocol::EVENT_DAEMON_HEARTBEAT {
                                        continue;
                                    }
                                    break message;
                                };
                                assert_eq!(rpc.r#type, patchbay_protocol::EVENT_DAEMON_RPC_REQUEST);
                                assert_eq!(rpc.payload["method"], "tasks.claim");
                                socket
                                    .send(axum::extract::ws::Message::Text(
                                        serde_json::to_string(&patchbay_protocol::Message {
                                            r#type: patchbay_protocol::EVENT_DAEMON_RPC_RESPONSE
                                                .to_string(),
                                            payload: json!({
