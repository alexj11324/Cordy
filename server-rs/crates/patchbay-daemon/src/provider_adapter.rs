...
use patchbay_protocol::DaemonHeartbeatAckPayload;
...
fn preparation_helper_command() -> anyhow::Result<Option<Vec<String>>> {
    ...
    Ok(Some(vec![
        executable
            .into_os_string()
            .into_string()
            .map_err(|_| anyhow::anyhow!("patchbay executable path is not valid UTF-8"))?,
...
/// Real provider adapter for protocol families implemented by `patchbay-agent`.
...
impl ProductionProviderAdapter {
    ...
    async fn resolve_task_model_selection(
        ...
        let catalog = if needs_catalog {
            Some(
                patchbay_agent::registry::discover_models(
                    &target.provider,
                    patchbay_agent::BackendConfig {
                        command: patchbay_agent::RuntimeCommand::new(
            ...
            match catalog.as_ref() {
                Some(Ok(catalog)) => {
                    let (qualified, rewritten) =
                        patchbay_agent::model::qualify_model_id(catalog, &selection.model);
                    if rewritten {
                        tracing::info!(
                            task = %task_id,
        ...
        if !selection.service_tier.is_empty() {
            let valid = if target.provider != "codex" || selection.model.is_empty() {
                ...
                match catalog.as_ref() {
                    Some(Ok(catalog)) => patchbay_agent::model::validate_service_tier(
                        catalog,
                        &target.provider,
                        &selection.model,
        ...
        if !selection.thinking_level.is_empty() {
            let valid = if target.provider == "codex" && selection.model.is_empty() {
                ...
                match catalog.as_ref() {
                    Some(Ok(catalog)) => patchbay_agent::model::validate_thinking_level(
                        catalog,
                        &target.provider,
                        &selection.model,
    ...
    async fn run_task_inner(
        ...
        if let Err(error) = crate::execution_plan::validate_identity(&task, &target) {
            return failed_with_reason(
                error,
                patchbay_task_failure::Reason::INVALID_TASK_IDENTITY.as_str(),
                None,
            );
        }
            ...
            .await
        {
            return failed_with_reason(
                error,
                patchbay_task_failure::Reason::SKILL_BUNDLE_UNAVAILABLE.as_str(),
                None,
            );
        }
...
fn configured_launch_command(config: &Config, target: &RuntimeExecutionTarget) -> Option<String> {
    ...
    config
        ...
        .map(str::to_string)
        .or_else(|| {
            patchbay_agent::provider(&target.provider).map(|provider| {
        ...
        .or_else(|| {
            patchbay_agent::builtin_runtime(&target.provider).map(|runtime| {
    ...
    let payload = match launch_registry.resolve(&workspace_id, &target) {
        ...
        Some(launch) => {
            match resolve_launch_for_execution(&ctx, &config, &verified_launches, launch).await {
                ...
                Ok(launch) => {
                    let config = patchbay_agent::BackendConfig {
                        command: patchbay_agent::RuntimeCommand::new(
                            launch.command_path,
                            launch.fixed_args,
                        ),
                    ...
                    match patchbay_agent::registry::discover_models(
                        &target.provider,
                        config,
                        &cache,
                        ctx.token().clone(),
...
fn redact_value(value: &mut Value, depth: usize) {
    ...
    match value {
        Value::String(text) => *text = patchbay_agent::stderr::sanitize_diagnostic(text),
        Value::Array(values) => {
            for value in values {
...
fn should_retry_with_fresh_session(
    ...
    if result.resume_rejected
        || patchbay_task_failure::unresumable_history(&result.error)
        || patchbay_task_failure::auth_method_unresolved(&result.error)
    {
        return true;
    }
    patchbay_agent::registry::resume_rejection_undetectable(provider)
...
fn fresh_session_may_help(error: &str) -> bool {
    ...
    ![
        patchbay_task_failure::Reason::AGENT_PROVIDER_NETWORK,
        patchbay_task_failure::Reason::AGENT_PROVIDER_CAPACITY_OR_RATE_LIMIT,
        patchbay_task_failure::Reason::AGENT_PROVIDER_QUOTA_LIMIT,
        patchbay_task_failure::Reason::AGENT_PROVIDER_SERVER_ERROR,
        patchbay_task_failure::Reason::AGENT_PROVIDER_AUTH_OR_ACCESS,
        patchbay_task_failure::Reason::AGENT_MISSING_CONFIG,
        patchbay_task_failure::Reason::AGENT_MODEL_NOT_FOUND_OR_UNAVAILABLE,
        patchbay_task_failure::Reason::AGENT_RUNTIME_MISSING_EXECUTABLE,
        patchbay_task_failure::Reason::AGENT_RUNTIME_VERSION_UNSUPPORTED,
        patchbay_task_failure::Reason::AGENT_TIMEOUT,
    ]
    .contains(&reason)
}
    ...
    let (status, comment, failure_reason) = match result.status.as_str() {
        ...
        _ => {
            ...
            let failure_reason = if let Some(reason) = classify_poisoned_error(&comment) {
                ...
                patchbay_task_failure::classify(&comment).to_string()
            };
...
fn task_provider_runtime_env(
    ...
    match target.provider.as_str() {
        ...
        "dsh" => {
            ...
            values.insert("PATCHBAY_DSH_SESSION_ROOT".to_string(), session_root);
