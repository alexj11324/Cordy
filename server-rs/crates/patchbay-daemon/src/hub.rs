...
//! lives in `patchbay-realtime::hub`). The daemon hub indexes connections by
...
//!
//! Port notes vs Go (mirroring `patchbay-realtime/src/hub.rs`):
...
use patchbay_protocol::messages::{
...
impl patchbay_realtime::DaemonRuntimeDeliverer for DaemonHub {
    fn deliver_daemon_runtime(&self, scope_id: &str, frame: &[u8], event_id: &str) {
        self.deliver_daemon_runtime(scope_id, frame, event_id);
    }
}
...
mod tests {
    ...
    use super::*;
    use patchbay_protocol::PENDING_WORK_KIND_MODEL_LIST;
    ...
    #[async_trait]
    impl patchbay_realtime::RelayPublisher for LocalFirstRelayPublisher {
