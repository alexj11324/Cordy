...
pub struct PrepareParams {
    /// Base path for all envs (e.g., ~/patchbay_workspaces).
...
pub struct Environment {
    ...
    pub patchbay_config_root: String,
...
pub(crate) fn ensure_task_temp_dir(
    ...
    let directory = tempfile::Builder::new()
        .prefix("patchbay-task-")
        .tempdir_in(&base)
        .map_err(|error| {
            if override_configured {
                anyhow::anyhow!("PATCHBAY_AGENT_TEMP_BASE: create task temp dir: {error}")
            } else {
...
fn task_temp_base_dir() -> anyhow::Result<(PathBuf, bool)> {
    ...
    #[cfg(not(windows))]
    {
        let Some(configured) = std::env::var_os("PATCHBAY_AGENT_TEMP_BASE") else {
            return Ok((socket_safe_temp_base_dir(), false));
        };
        ...
        anyhow::ensure!(
            path.is_absolute(),
            "PATCHBAY_AGENT_TEMP_BASE must be an absolute path, got {path:?}"
        );
        Ok((path, true))
    }
}
...
pub async fn prepare(params: PrepareParams) -> anyhow::Result<Environment> {
    ...
    let patchbay_config_root = join_path(&[&env_root, "patchbay-config"]);
    std::fs::create_dir_all(&patchbay_config_root)
        .context("execenv: create task-local Patchbay config directory")?;
    restrict_permissions(&patchbay_config_root)
        .context("execenv: restrict task-local Patchbay config directory")?;
...
async fn prepare_body(
    ...
    let mut env = Environment {
        ...
        patchbay_config_root: join_path(&[env_root, "patchbay-config"]),
        ..Default::default()
    };
...
pub fn reuse(params: ReuseParams) -> Option<Environment> {
    ...
    if !env.root_dir.is_empty() {
        env.patchbay_config_root = join_path(&[&env.root_dir, "patchbay-config"]);
        if let Err(err) = std::fs::create_dir_all(&env.patchbay_config_root) {
            tracing::warn!(
                error = %format!("{err:#}"),
                "execenv: restore task-local Patchbay config directory failed; forcing fresh prepare"
            );
            return None;
        }
        if restrict_permissions(&env.patchbay_config_root).is_err() {
            tracing::warn!(
                "execenv: restrict task-local Patchbay config directory failed; forcing fresh prepare"
            );
            return None;
        }
    }
...
pub const MANAGED_ENV_PROVENANCE_MANAGED_BY: &str = "patchbay-daemon-managed-env";
...
pub struct OpenclawConfigPrep {
    ...
    pub profile: String,
    /// Per-invocation CLI deadline; zero uses PATCHBAY_OPENCLAW_CLI_TIMEOUT or
...
///
/// QwenPaw does not read Patchbay's generic `.agent_context/skills` tree: its ACP
...
mod tests {
    ...
    fn test_task_temp_dir_is_private_distinct_and_removed_with_guard() {
        ...
        impl Drop for EnvRestore {
            fn drop(&mut self) {
                match &self.0 {
                    Some(value) => std::env::set_var("PATCHBAY_AGENT_TEMP_BASE", value),
                    None => std::env::remove_var("PATCHBAY_AGENT_TEMP_BASE"),
                }
            }
        }

        let _restore = EnvRestore(std::env::var_os("PATCHBAY_AGENT_TEMP_BASE"));
        let base = tempfile::tempdir().unwrap();
        std::env::set_var("PATCHBAY_AGENT_TEMP_BASE", base.path());
        ...
        std::env::set_var("PATCHBAY_AGENT_TEMP_BASE", "relative/base");
        let error = ensure_task_temp_dir("/tmp/root", "ws", "task").unwrap_err();
        assert!(error.to_string().contains("PATCHBAY_AGENT_TEMP_BASE"));

        std::env::set_var("PATCHBAY_AGENT_TEMP_BASE", "   ");
        ...
        std::env::set_var("PATCHBAY_AGENT_TEMP_BASE", spaced);
        let directory = ensure_task_temp_dir("/tmp/root", "ws", "task").unwrap();
    ...
    fn test_task_temp_dir_reports_unusable_configured_base() {
        ...
        std::env::set_var("PATCHBAY_AGENT_TEMP_BASE", &missing);
        let error = ensure_task_temp_dir("/tmp/root", "ws", "task").unwrap_err();
        assert!(error.to_string().contains("PATCHBAY_AGENT_TEMP_BASE"));
    }
    ...
    fn test_task_temp_dir_accepts_non_unicode_absolute_override() {
        ...
        std::env::set_var("PATCHBAY_AGENT_TEMP_BASE", &base);
    ...
    fn qwenpaw_workspace_deduplicates_skill_slugs() {
        ...
        assert!(workspace.join("skills/a-b-patchbay/SKILL.md").is_file());
        let manifest: Value =
            serde_json::from_slice(&std::fs::read(workspace.join("skill.json")).unwrap()).unwrap();
        assert_eq!(manifest["skills"].as_object().unwrap().len(), 2);
        assert_eq!(
            manifest["skills"]["a-b-patchbay"]["metadata"]["name"],
            "a-b-patchbay"
        );
    }
