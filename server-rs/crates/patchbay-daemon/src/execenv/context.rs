...
/// PATCHBAY_* env vars before invoking `patchbay`.
pub const TASK_CONTEXT_MARKER_REL_PATH: &str = ".patchbay/daemon_task_context.json";

/// The marker discriminator the CLI checks before treating
/// TASK_CONTEXT_MARKER_REL_PATH as daemon-owned.
pub const TASK_CONTEXT_MARKER_MANAGED_BY: &str = "patchbay-daemon-task";
...
/// {workspaces_root}/.patchbay/daemon_task_context.json.
///
...
const SIDECAR_MANIFEST_FILE: &str = ".patchbay_sidecar_manifest.json";
...
/// so write_skill_files can lay down a Patchbay skill without colliding with a
/// user-installed skill of the same slug. First attempt is always the natural
/// baseSlug; on collision we append `-patchbay`, then `-patchbay-2`, `-patchbay-3`, …
...
/// slug first, then `-patchbay`, then numbered variants. Two callers must agree
...
pub(crate) fn skill_slug_candidate(base_slug: &str, attempt: usize) -> String {
    match attempt {
        ...
        1 => format!("{base_slug}-patchbay"),
        n => format!("{base_slug}-patchbay-{n}"),
    }
}
...
fn write_task_context_marker(
    ...
    record_mkdir_all(&dir, manifest.as_deref_mut()).context("create .patchbay dir")?;
...
/// Writes .patchbay/project/resources.json into the working directory when the
...
fn write_project_resources(
    ...
    let dir = join_path(&[work_dir, ".patchbay", "project"]);
    ...
    if let Err(err) = record_write_file(&path, &data, manifest) {
        // .patchbay/project/resources.json is Patchbay-owned and a pre-existing path
...
/// Patchbay skill's natural slug collides with a user-installed skill at the same
/// path, we allocate a collision-free sibling slug (e.g. `issue-review-patchbay`)
...
pub(crate) fn render_issue_context(ctx: &TaskContextForEnv) -> String {
    ...
    b.push_str(&format!(
        "Run `patchbay issue get {} --output json` to fetch the full issue details.\n\n",
        ctx.issue_id
    ));
...
mod tests {
    ...
    fn test_write_skill_files_collision_fallback() {
        ...
        // User directory untouched; skill landed in review-patchbay.
        assert_eq!(
    ...
    fn test_render_issue_context_variants() {
        ...
        assert!(md.contains("patchbay issue get iss_1 --output json"));
