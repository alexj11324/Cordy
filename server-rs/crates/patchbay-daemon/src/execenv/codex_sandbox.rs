...
//! - patchbayManagedBegin/EndMarker → PATCHBAY_MANAGED_BEGIN_MARKER / PATCHBAY_MANAGED_END_MARKER
//! - renderPatchbayManagedBlock   → render_patchbay_managed_block
//! - managedBlockRe            → managed_block_re
//! - upsertPatchbayManagedBlock   → upsert_patchbay_managed_block
...
///     Patchbay CLI can reach the API (openai/codex#10390).
...
const PATCHBAY_MANAGED_BEGIN_MARKER: &str =
    "# BEGIN patchbay-managed (do not edit; regenerated by daemon)";
const PATCHBAY_MANAGED_END_MARKER: &str = "# END patchbay-managed";
...
/// both traps.
fn render_patchbay_managed_block(policy: &CodexSandboxPolicy) -> String {
    let mut b = String::new();
    b.push_str(PATCHBAY_MANAGED_BEGIN_MARKER);
    b.push('\n');
    ...
    b.push_str(PATCHBAY_MANAGED_END_MARKER);
    b.push('\n');
    b
}
...
fn managed_block_re() -> &'static Regex {
    ...
    RE.get_or_init(|| {
        Regex::new(&format!(
            r"(?ms)^{}.*?^{}\n*",
            regex::escape(PATCHBAY_MANAGED_BEGIN_MARKER),
            regex::escape(PATCHBAY_MANAGED_END_MARKER)
        ))
        .expect("static regex")
    })
}

/// Returns the config content with the patchbay-managed block placed at the very
...
/// `[permissions.patchbay]`. Combined with the dotted-key form used by
/// render_patchbay_managed_block, this means the managed block neither leaks into
/// nor inherits from any surrounding table scope.
fn upsert_patchbay_managed_block(content: &str, policy: &CodexSandboxPolicy) -> String {
    // Drop any previously written managed block (wherever it sits).
    let content = managed_block_re().replace_all(content, "").into_owned();
    let block = render_patchbay_managed_block(policy);
...
/// Writes the patchbay-managed sandbox block into the given config.toml according
...
pub(crate) fn ensure_codex_sandbox_config(
    ...
    let updated = upsert_patchbay_managed_block(&existing, policy);
    if updated == data {
        return Ok(());
    }
...
mod tests {
    ...
    // Port of TestRenderAndUpsertPatchbayManagedBlock: idempotent rewrite,
    // hoisted to the top, user content preserved byte-for-byte.
    #[test]
    fn test_upsert_patchbay_managed_block_idempotent() {
        let policy = CodexSandboxPolicy {
            mode: "workspace-write".to_string(),
            network_access: true,
            ..Default::default()
        };
        let once = upsert_patchbay_managed_block("", &policy);
        assert!(once.starts_with(PATCHBAY_MANAGED_BEGIN_MARKER));
        ...
        let twice = upsert_patchbay_managed_block(&once, &policy);
        assert_eq!(once, twice);

        // User content below a trailing table survives and stays after the
        // block.
        let user = "[profiles.foo]\nmodel = \"gpt\"\n";
        let merged = upsert_patchbay_managed_block(user, &policy);
        assert!(merged.starts_with(PATCHBAY_MANAGED_BEGIN_MARKER));
        assert!(merged.ends_with(user));

        // An old managed block sitting mid-file is removed wherever it sits.
        let stale = format!("keep-a\n{once}\nkeep-b\n");
        let cleaned = upsert_patchbay_managed_block(&stale, &policy);
        assert!(cleaned.starts_with(PATCHBAY_MANAGED_BEGIN_MARKER));
        assert!(cleaned.contains("keep-a"));
        assert!(cleaned.contains("keep-b"));
        assert_eq!(cleaned.matches(PATCHBAY_MANAGED_BEGIN_MARKER).count(), 1);
        ...
        let rendered = render_patchbay_managed_block(&full);
    ...
    fn test_strip_legacy_sandbox_directives() {
        ...
        // check), so even the one under [permissions.patchbay] goes. The section
        // header itself survives.
        assert!(got.contains("[permissions.patchbay]"));
    ...
    fn test_ensure_codex_sandbox_config_idempotent() {
        ...
        assert!(migrated.starts_with(PATCHBAY_MANAGED_BEGIN_MARKER));
