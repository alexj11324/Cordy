...
/// Defensive fallback for a surface whose conversation Patchbay never stored.
...
pub(crate) fn build_task_initiator_block(
    ...
    b.push_str("The initiator — not the runtime owner — is who you are answering: apply any per-person privacy or access rules your instructions define. Your Patchbay credentials stay scoped to the runtime owner, and initiator attribution does not change what you may read or write; do not assume the initiator can see everything you can.\n\n");
    b
}

/// DisplayNameForToolkitSlug fallback lives in patchbay-service's runtimeapps
...
`patchbay issue comment list {issue_id} --thread {thread_id} --since {new_comments_since} --compact --output json` \
...
`patchbay issue comment list {issue_id} --since {new_comments_since} --compact --output json`.\n\n"
    ))
}
...
`patchbay issue comment list {issue_id} --thread {thread_id} --tail 30 --compact --output json`.\n\n"
    )
}
...
Write the body file first — never pipe via `--content-stdin` (PowerShell drops non-ASCII; full rules: ## Comment Formatting above):\n\n\
    patchbay issue comment add {issue_id} --parent {trigger_comment_id} --content-file ./reply.md\n\
