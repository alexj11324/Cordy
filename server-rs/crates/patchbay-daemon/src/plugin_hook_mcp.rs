...
//! does NOT go to the plugin from here; it goes back to Patchbay via `invoke`
...
/// Performs one hook call against the Patchbay server (go:37): returns the raw
    ...
    match rpc_method {
        "initialize" => write_result(
            ...
            &json!({
                ...
                "capabilities": {"tools": {}},
                "serverInfo": {"name": "patchbay-plugins", "version": "1"},
            }),
        ),
...
mod tests {
    ...
    async fn started_server_exposes_claim_tool_through_its_overlay_url() {
        ...
        let (config, server) =
            start_task_plugin_hook_mcp(&ctx, "task-1", &fixture_tools(), ok_invoke())
                .await
                .unwrap();
        let url = config.unwrap()["mcpServers"]["patchbay-plugins"]["url"]
            .as_str()
            .unwrap()
            .to_string();
    ...
    async fn initialize_handshake_and_notification() {
        ...
        assert_eq!(decoded["result"]["serverInfo"]["name"], "patchbay-plugins");
