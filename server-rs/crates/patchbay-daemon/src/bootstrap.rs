//! Process bootstrap primitives for the future Patchbay CLI binary.
//!
...
const RESTART_HANDOFF_ENV: &str = "PATCHBAY_DAEMON_RESTART_HANDOFF";
    ...
    Signal: Future<Output = io::Result<()>>,
{
    ...
    let context = BootstrapContext {
        paths: paths.clone(),
        launched_by: std::env::var("PATCHBAY_LAUNCHED_BY").unwrap_or_default(),
    ...
    fn from_env() -> Self {
        Self {
            max_size_bytes: positive_env_u64(
                "PATCHBAY_DAEMON_LOG_MAX_SIZE_MB",
                DEFAULT_LOG_MAX_SIZE_MB,
            ) * 1024
                * 1024,
            max_backups: positive_env_usize(
                "PATCHBAY_DAEMON_LOG_MAX_BACKUPS",
                DEFAULT_LOG_MAX_BACKUPS,
            ),
            max_age: Duration::from_secs(
                positive_env_u64("PATCHBAY_DAEMON_LOG_MAX_AGE_DAYS", DEFAULT_LOG_MAX_AGE_DAYS)
                    * 24
                    * 60
                    * 60,
            ),
...
impl DaemonLogs {
    fn install(
        ...
        let terminal = patchbay_util::logging::stderr_is_terminal();
        ...
        let filter = filter
            .map(str::to_owned)
            .unwrap_or_else(patchbay_util::logging::env_filter);
