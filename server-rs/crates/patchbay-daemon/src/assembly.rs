...
//!
//! CLI profile parsing stays owned by `patchbay-cli`; this module accepts the
...
/// Profile data read by `patchbay-cli` and consumed during daemon assembly.
///
    ...
    fn resolve_with_probe(
        ...
        if profile.token.is_empty() {
            let login_hint = if launch.profile.is_empty() {
                "'patchbay login'".to_string()
            } else {
                format!("'patchbay login --profile {}'", launch.profile)
            };
...
mod tests {
    ...
    fn foreground_args_preserve_explicit_zero_without_credentials() {
        let launch = DaemonLaunchOverrides {
            server_url: "https://patchbay.example".to_string(),
    ...
    fn rejects_missing_auth_before_touching_workspace_root() {
        ...
        assert!(error.to_string().contains("patchbay login"));
        assert!(!root.exists());
    }
    ...
    fn initializes_authenticated_client_and_daemon_cache() {
        ...
        assert!(root.join(".patchbay/daemon_task_context.json").is_file());
    }
}
