...
//!   with patchbay-cli's version template)
...
/// `patchbay --version` output whose first line is rendered by cmd/patchbay:
///
/// ```text
...
pub fn parse_self_version(raw: &str) -> String {
    ...
    if fields.len() >= 2 && fields[0] == "patchbay" {
...
/// `fetchLatestRelease` (update.go:255–274): latest patchbay release from the
/// GitHub API.
...
/// this daemon into a different patchbay binary". Two independent checks on one
...
pub(crate) async fn auto_update_loop(
    ...
    if reload_enabled {
        tracing::info!(
            ...
            current = %settings.cli_version,
            "auto-reload: watching the patchbay binary on disk"
        );
    }
...
/// `trySelfReload` (go:326–411): restarts the daemon when the patchbay binary on
...
pub(crate) async fn try_self_reload(host: &dyn AutoUpdateHost, ctx: &Ctx) {
    ...
    'attempt: {
        ...
        let reason = format!(
            "patchbay binary on disk reports {on_disk}, running {}",
...
mod tests {
    ...
    impl AutoUpdateHost for FakeHost {
        ...
        fn restart_target_binary(&self) -> anyhow::Result<String> {
            Ok("/usr/local/bin/patchbay".to_string())
        }
        fn set_reload_pending(&self, reason: Option<String>) {
    ...
    fn parse_self_version_extracts_ldflags_field() {
        assert_eq!(
            parse_self_version("patchbay 0.3.7 (commit: abc1234, built: 2026-07-29T10:00:00Z)\ngo: go1.26.1, os/arch: darwin/arm64\n"),
            "0.3.7"
        );
