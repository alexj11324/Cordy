...
//! whose production impl wraps patchbay-remotemcp's pinned [`SecureHttpClient`].
...
pub(crate) trait McpUpstream: Send + Sync {
    fn send<'a>(
        ...
        request: http::Request<patchbay_remotemcp::RequestBody>,
    ) -> BoxFuture<'a, anyhow::Result<http::Response<Vec<u8>>>>;
}

pub(crate) struct SecureUpstream(pub patchbay_remotemcp::SecureHttpClient);
...
pub(crate) async fn start_task_remote_mcp_brokers(
    ...
    task_id: &str,
    provider: &str,
    connections: &[patchbay_remotemcp::Connection],
    ...
    for connection in connections {
        ...
        let endpoint = patchbay_remotemcp::validate_public_https_endpoint(
            &connection.endpoint,
            &connection.endpoint_allowed_hosts,
            None,
        )
        ...
        let proxy = Arc::new(RemoteMCPProxyState {
            ...
            upstream: Arc::new(SecureUpstream(patchbay_remotemcp::new_secure_http_client(
                &endpoint,
            ))),
...
pub(crate) fn validate_pinned_remote_mcp_tools(
    approved: &[patchbay_remotemcp::Tool],
    discovered: &[patchbay_remotemcp::Tool],
) -> Result<(), String> {
    let available: std::collections::HashMap<&str, &patchbay_remotemcp::Tool> = discovered
        .iter()
...
pub(crate) fn remote_mcp_server_name(connection: &patchbay_remotemcp::Connection) -> String {
...
pub(crate) struct RemoteMCPProxyState {
    ...
    task_id: String,
    connection: patchbay_remotemcp::Connection,
...
async fn run_proxy_body(
    ...
    let upstream_request = builder
        .body(patchbay_remotemcp::RequestBody::from(raw.to_vec()))
    ...
    let max_response = patchbay_remotemcp::MAX_RESPONSE_BYTES + 1;
    ...
    let mut body = body;
    if body.len() > patchbay_remotemcp::MAX_RESPONSE_BYTES {
        return Err(ProxyFailure {
            result_class: "remote_error",
            id: rpc_id,
            code: -32001,
...
fn tool_approved(connection: &patchbay_remotemcp::Connection, name: &str) -> bool {
    connection
        .approved_tools
        .iter()
...
pub(crate) fn filter_tools_list_response(
    ...
    let mut pinned: Vec<&patchbay_remotemcp::Tool> = approved.iter().collect();
    pinned.sort_by(|a, b| a.name.cmp(&b.name));
    let mut filtered: Vec<Value> = Vec::with_capacity(pinned.len());
    for tool in pinned {
        match by_name.get(&tool.name) {
            Some(current)
                if patchbay_remotemcp::digest_bytes(current.as_bytes()) == tool.schema_digest => {}
...
mod tests {
    ...
    fn pinned_tool_validation_reports_missing_then_drift() {
        let discovered = vec![patchbay_remotemcp::Tool {
    ...
    fn server_name_sanitizes_key_and_truncates_suffix() {
        let connection = patchbay_remotemcp::Connection {
                        ...
                        Some("initialize") => (
                            ...
                            json!({
                                ...
                                "result": {
                                    ...
                                    "capabilities": {"tools": {}},
                                    "serverInfo": {"name": "patchbay-remote-mcp-fixture", "version": "1.0.0"},
                                }
                            })
                            .to_string(),
                        ),
    ...
    fn proxy_state(
        endpoint: url::Url,
        approved: Vec<patchbay_remotemcp::Tool>,
        credential_headers: Vec<(String, String)>,
    ) -> RemoteMCPProxyState {
        RemoteMCPProxyState {
    ...
    fn canned_read_tool() -> patchbay_remotemcp::Tool {
        ...
        patchbay_remotemcp::Tool {
            ...
            schema_digest: patchbay_remotemcp::digest_bytes(schema.as_bytes()),
            risk: "read".into(),
        }
    }
    ...
    async fn proxy_filters_tools_list_and_injects_credential() {
        ...
        approved.push(patchbay_remotemcp::Tool {
            name: "fixture.write".into(),
            description: String::new(),
            input_schema: serde_json::from_str(&write_schema).unwrap(),
            schema_digest: patchbay_remotemcp::digest_bytes(write_schema.as_bytes()),
            risk: "write".into(),
        });
        let state = proxy_state(
            endpoint,
    ...
    async fn proxy_rejects_unapproved_tool_without_calling_upstream() {
        ...
        let approved = vec![patchbay_remotemcp::Tool {
            name: "allowed".into(),
            description: String::new(),
