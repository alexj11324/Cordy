...
/// The sender mapped to a Cordy user.
...
/// Maps the message sender to a Cordy user within the installation,
...
impl MediaIntentLedger for DbMediaIntentLedger {
    async fn record_pending_media_object(
        ...
        cordy_db::queries::channel::record_channel_media_pending_object(
            &self.pool,
            &p.storage_key,
            p.workspace_id,
            p.chat_message_id,
