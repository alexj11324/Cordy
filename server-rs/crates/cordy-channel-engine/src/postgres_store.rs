...
impl InstallationStore for PostgresChannelStore {
    async fn list_active_installations(&self) -> anyhow::Result<Vec<Installation>> {
        let rows =
            cordy_db::queries::channel::list_all_active_channel_installations(&self.pool).await?;
        Ok(rows
            .into_iter()
            .map(|row| Installation {
                id: row.id,
                channel_type: cordy_channel::Type(row.channel_type.clone()),
...
impl LeaseStore for PostgresChannelStore {
    ...
    async fn release(&self, arg: ReleaseLeaseParams) -> Result<(), LeaseError> {
        cordy_db::queries::channel::release_channel_ws_lease(&self.pool, arg.id, Some(&arg.token))
            .await
            .map(|_| ())
...
async fn acquire_or_renew(pool: &PgPool, arg: AcquireLeaseParams) -> Result<(), LeaseError> {
    let acquired = cordy_db::queries::channel::acquire_channel_ws_lease(
        pool,
        Some(&arg.token),
        Some(arg.expires_at),
        arg.id,
