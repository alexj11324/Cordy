...
/// `InboundHandler`.
pub struct Router {
    sets: Mutex<HashMap<cordy_channel::Type, Arc<ResolverSet>>>,
...
impl Router {
    ...
    pub fn register(&self, t: cordy_channel::Type, set: ResolverSet) {
        if t.0.is_empty()
    ...
    async fn process_claimed(
        ...
        //    card.
        if msg.source.chat_type.0 == cordy_channel::ChatType::group().0 && !msg.addressed_to_bot {
            let res = self
        ...
        let session_creator = if msg.source.chat_type.0 == cordy_channel::ChatType::group().0 {
...
/// ErrChatTaskAgentArchived sentinels; until cordy-service exports them
