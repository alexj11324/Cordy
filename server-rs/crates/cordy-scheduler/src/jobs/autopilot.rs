...
use cordy_db::queries::autopilot::{
    advance_trigger_next_run, get_autopilot, get_autopilot_trigger,
    list_schedulable_autopilot_triggers, touch_autopilot_trigger_fired_at,
};
use cordy_service::autopilot::AutopilotService;
use cordy_service::cron::{next_occurrence_after_utc, next_occurrences_utc};
...
pub const AUTOPILOT_SCHEDULE_DISPATCH_JOB: &str = "autopilot_schedule_dispatch";
pub const AUTOPILOT_TRIGGER_SCOPE: &str = "autopilot_trigger";
pub const DEFAULT_AUTOPILOT_SCHEDULE_TIMEZONE: &str = "UTC";

const MAX_SCHEDULE_LATENESS: Duration = Duration::from_secs(5 * 60);
const REPLAY_WINDOW: Duration = Duration::from_secs(24 * 60 * 60);

#[async_trait]
pub trait AutopilotScheduleDispatcher: Send + Sync {
...
#[async_trait]
impl AutopilotScheduleDispatcher for AutopilotService {
    async fn dispatch_autopilot_for_plan(
        &self,
        autopilot: &Autopilot,
        trigger_id: Uuid,
        ...
        AutopilotService::dispatch_autopilot_for_plan(
            self, autopilot, trigger_id, source, payload, planned_at,
        )
        .await
    }
}
...
pub fn autopilot_schedule_dispatch_job(
    pool: PgPool,
    dispatcher: Arc<dyn AutopilotScheduleDispatcher>,
) -> JobSpec {
    let cache = Arc::new(TriggerCache::default());
    JobSpec {
        name: AUTOPILOT_SCHEDULE_DISPATCH_JOB.into(),
        ...
        scopes: autopilot_scopes(pool.clone(), cache.clone()),
        plans_for_scope: Some(autopilot_plans_for_scope(cache)),
        handler: autopilot_handler(pool, dispatcher),
    }
}

fn autopilot_scopes(pool: PgPool, cache: Arc<TriggerCache>) -> ScopeProvider {
    Arc::new(move |_, _| {
        let pool = pool.clone();
        let cache = cache.clone();
        Box::pin(async move {
            let rows = list_schedulable_autopilot_triggers(&pool)
                .await
                .context("autopilot scope: list schedulable triggers")?;
                ...
                next.insert(
                    ...
                    TriggerConfig {
                        ...
                        timezone: row
                            ...
                            .unwrap_or_else(|| DEFAULT_AUTOPILOT_SCHEDULE_TIMEZONE.into()),
                        created_at: row.created_at,
                        last_fired_at: row.last_fired_at,
                    },
                );
                scopes.push(Scope::new(AUTOPILOT_TRIGGER_SCOPE, id));
            }
            cache.replace(next);
            Ok(scopes)
        })
...
fn autopilot_plans_for_scope(cache: Arc<TriggerCache>) -> PlanProvider {
...
fn autopilot_handler(pool: PgPool, dispatcher: Arc<dyn AutopilotScheduleDispatcher>) -> JobHandler {
    Arc::new(move |_, input| {
        let pool = pool.clone();
        let dispatcher = dispatcher.clone();
        Box::pin(async move {
            let trigger_id = Uuid::parse_str(&input.scope.id)
                .context("autopilot handler: scope id is not a valid uuid")?;
            let Some(trigger) = get_autopilot_trigger(&pool, trigger_id)
                .await
            ...
            let Some(autopilot) = get_autopilot(&pool, trigger.autopilot_id)
                .await
                .context("load autopilot")?
            else {
                return Ok(skipped("autopilot_not_found"));
            };
            if autopilot.status != "active" {
                let mut result = skipped("autopilot_inactive");
                result
                    .result
                    .insert("status".into(), json!(autopilot.status));
                return Ok(result);
            }

            let run = dispatcher
                .dispatch_autopilot_for_plan(
                    &autopilot,
                    trigger.id,
                    "schedule",
                    &Value::Null,
            ...
            let timezone = trigger
                ...
                .as_deref()
                .filter(|value| !value.is_empty())
                .unwrap_or(DEFAULT_AUTOPILOT_SCHEDULE_TIMEZONE);
            ...
            if let Some(next) = advanced {
                ...
                let _ = touch_autopilot_trigger_fired_at(&pool, trigger.id).await;
            }
