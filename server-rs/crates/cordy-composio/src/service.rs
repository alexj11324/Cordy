//! The Stage 2 business-integration glue between Cordy and the standalone
//! Composio SDK. It owns Cordy semantics: the signed-state connect
...
/// cordy-db query wrappers in the wiring slice, faked in tests.
...
pub struct UpsertConnectionParams {
    ...
    /// Invariant: composio_user_id == Cordy user id.
...
impl Service {
    ...
    ///
    /// The composio_user_id sent to Composio is the Cordy user id verbatim
...
mod tests {
    ...
    fn service() -> (Service, Arc<FakeSdk>, Arc<FakeStore>) {
        ...
        let svc = Service::new(
            ...
            ServiceConfig {
                ...
                callback_base_url: "https://cordy.test/".into(),
                frontend_base_url: "https://cordy.test".into(),
    ...
    async fn begin_connect_rejects_unknown_toolkit_and_signs_known_one() {
        ...
        assert!(link
            .callback_url
            .starts_with("https://cordy.test/api/integrations/composio/callback?state="));
    }
    ...
    fn callback_redirect_shapes_match_go() {
        ...
        assert_eq!(
            ...
            "https://cordy.test/settings?tab=integrations&connected=github"
        );
        assert_eq!(
            svc.callback_redirect("x", false),
            "https://cordy.test/settings?tab=integrations&error=composio_connect_failed"
        );
    }
