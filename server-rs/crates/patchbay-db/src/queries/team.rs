//! Typed SQL queries for team records.
...
pub async fn add_team_member(
    ...
    let row = sqlx::query(
        r#"INSERT INTO team_member (team_id, member_type, member_id, role)
VALUES ($1, $2, $3, $4)
...
pub async fn archive_team(
    ...
    let row = sqlx::query(
        r#"UPDATE team SET archived_at = now(), archived_by = $2, updated_at = now()
WHERE id = $1
...
pub async fn count_team_members(
    ...
    let row = sqlx::query(r#"SELECT count(*) FROM team_member WHERE team_id = $1"#)
        .bind(team_id)
        .fetch_optional(executor)
        .await?;
    let Some(row) = row else { return Ok(None) };
...
pub async fn create_team(
    ...
    let row = sqlx::query(
        r#"INSERT INTO team (workspace_id, name, description, leader_id, creator_id, avatar_url)
VALUES ($1, $2, $3, $4, $5, $6)
RETURNING id, workspace_id, name, description, leader_id, creator_id, created_at, updated_at, archived_at, archived_by, avatar_url, instructions"#
    )
        .bind(workspace_id)
...
pub async fn get_team(
    ...
    let row = sqlx::query(
        r#"SELECT id, workspace_id, name, description, leader_id, creator_id, created_at, updated_at, archived_at, archived_by, avatar_url, instructions FROM team WHERE id = $1"#
    )
        .bind(id)
        .fetch_optional(executor)
        .await?;
...
pub async fn get_team_by_assignee(
    ...
    let row = sqlx::query(
        r#"SELECT s.id, s.workspace_id, s.name, s.description, s.leader_id, s.creator_id, s.created_at, s.updated_at, s.archived_at, s.archived_by, s.avatar_url, s.instructions FROM team s WHERE s.id = $1 AND s.workspace_id = $2"#
    )
        .bind(id)
        .bind(workspace_id)
        .fetch_optional(executor)
...
pub async fn get_team_in_workspace(
    ...
    let row = sqlx::query(
        r#"SELECT id, workspace_id, name, description, leader_id, creator_id, created_at, updated_at, archived_at, archived_by, avatar_url, instructions FROM team WHERE id = $1 AND workspace_id = $2"#
    )
        .bind(id)
        .bind(workspace_id)
        .fetch_optional(executor)
...
pub async fn list_all_teams(
    ...
    let rows = sqlx::query(
        r#"SELECT id, workspace_id, name, description, leader_id, creator_id, created_at, updated_at, archived_at, archived_by, avatar_url, instructions FROM team WHERE workspace_id = $1 ORDER BY created_at ASC"#
    )
        .bind(workspace_id)
        .fetch_all(executor)
        .await?;
...
pub struct ListTeamMemberPreviewRowsRow {
...
pub async fn list_team_member_preview_rows(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    workspace_id: Uuid,
) -> anyhow::Result<Vec<ListTeamMemberPreviewRowsRow>> {
    let rows = sqlx::query(
        r#"SELECT
    sm.team_id,
    sm.member_type,
...
ORDER BY
    ...
    let mut out = Vec::with_capacity(rows.len());
    for row in &rows {
        out.push(ListTeamMemberPreviewRowsRow {
...
#[derive(Debug, Clone, serde::Serialize)]
pub struct ListTeamMemberPreviewRowsByTeamRow {
    pub team_id: Option<Uuid>,
    pub member_type: String,
    pub member_id: Option<Uuid>,
    pub role: String,
...
pub async fn list_team_member_preview_rows_by_team(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    team_id: Uuid,
) -> anyhow::Result<Vec<ListTeamMemberPreviewRowsByTeamRow>> {
    let rows = sqlx::query(
        r#"SELECT
    sm.team_id,
    sm.member_type,
...
ORDER BY
    ...
    let mut out = Vec::with_capacity(rows.len());
    for row in &rows {
        out.push(ListTeamMemberPreviewRowsByTeamRow {
            team_id: row.try_get(0)?,
            member_type: row.try_get(1)?,
            member_id: row.try_get(2)?,
            role: row.try_get(3)?,
...
#[derive(Debug, Clone, serde::Serialize)]
pub struct ListTeamMemberStatusRowsRow {
...
pub async fn list_team_member_status_rows(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    team_id: Uuid,
) -> anyhow::Result<Vec<ListTeamMemberStatusRowsRow>> {
    let rows = sqlx::query(
        r#"SELECT
...
ORDER BY sm.created_at ASC, atq.dispatched_at DESC NULLS LAST"#,
    ...
    let mut out = Vec::with_capacity(rows.len());
    for row in &rows {
        out.push(ListTeamMemberStatusRowsRow {
            team_member_id: row.try_get(0)?,
            member_type: row.try_get(1)?,
            member_id: row.try_get(2)?,
...
pub async fn list_team_members(
    ...
    let rows = sqlx::query(
        r#"SELECT id, team_id, member_type, member_id, role, created_at FROM team_member WHERE team_id = $1 ORDER BY created_at ASC"#
    )
        .bind(team_id)
        .fetch_all(executor)
        .await?;
...
pub async fn list_teams(
    ...
    let rows = sqlx::query(
        r#"SELECT id, workspace_id, name, description, leader_id, creator_id, created_at, updated_at, archived_at, archived_by, avatar_url, instructions FROM team WHERE workspace_id = $1 AND archived_at IS NULL ORDER BY created_at ASC"#
    )
        .bind(workspace_id)
        .fetch_all(executor)
        .await?;
...
pub async fn list_teams_by_member(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    workspace_id: Uuid,
    member_type: &str,
    member_id: Uuid,
    ...
    let rows = sqlx::query(
        r#"SELECT s.id, s.workspace_id, s.name, s.description, s.leader_id, s.creator_id, s.created_at, s.updated_at, s.archived_at, s.archived_by, s.avatar_url, s.instructions FROM team s
...
pub async fn lock_team_for_autopilot_assignment(
    ...
    let row = sqlx::query(
        r#"SELECT id, workspace_id, name, description, leader_id, creator_id, created_at, updated_at, archived_at, archived_by, avatar_url, instructions FROM team
...
pub async fn lock_team_for_update(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    id: Uuid,
    workspace_id: Uuid,
) -> anyhow::Result<Option<Team>> {
...
pub async fn update_team_member_role(
    ...
    let row = sqlx::query(
        r#"UPDATE team_member SET role = $4
WHERE team_id = $1 AND member_type = $2 AND member_id = $3
RETURNING id, team_id, member_type, member_id, role, created_at"#,
    )
    .bind(team_id)
