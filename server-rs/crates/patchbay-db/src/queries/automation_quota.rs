//! Typed SQL queries for automation_quota records.
...
pub async fn consume_automation_quota_reservation(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    reservation_id: Uuid,
) -> anyhow::Result<Option<AutomationQuotaPeriod>> {
    let row = sqlx::query(
        r#"WITH locked AS (
...
pub async fn create_automation_quota_reservation(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    workspace_id: Uuid,
    ...
    source: &str,
    idempotency_key: &str,
) -> anyhow::Result<Option<AutomationQuotaReservation>> {
    let row = sqlx::query(
        r#"INSERT INTO automation_quota_reservation (
...
RETURNING id, workspace_id, period_start, period_end, policy_revision, subscription_version, source, idempotency_key, state, created_at, finalized_at"#
        ...
        .await?;
    let Some(row) = row else { return Ok(None) };
    Ok(Some(AutomationQuotaReservation {
        id: row.try_get(0)?,
...
pub async fn ensure_automation_quota_period(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    workspace_id: Uuid,
    period_start: Option<DateTime<Utc>>,
    period_end: Option<DateTime<Utc>>,
    ...
    let row = sqlx::query(
        r#"INSERT INTO automation_quota_period (workspace_id, period_start, period_end)
VALUES ($1, $2, $3)
...
pub async fn get_automation_quota_period(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    workspace_id: Uuid,
    period_start: Option<DateTime<Utc>>,
    period_end: Option<DateTime<Utc>>,
    ...
    let row = sqlx::query(
        r#"SELECT workspace_id, period_start, period_end, used_count, reserved_count, blocked_counts, created_at, updated_at FROM automation_quota_period
...
pub async fn get_automation_quota_reservation_by_key(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    workspace_id: Uuid,
    period_start: Option<DateTime<Utc>>,
    period_end: Option<DateTime<Utc>>,
    ...
    let row = sqlx::query(
        r#"SELECT id, workspace_id, period_start, period_end, policy_revision, subscription_version, source, idempotency_key, state, created_at, finalized_at FROM automation_quota_reservation
...
pub async fn increment_automation_quota_blocked(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    source: &str,
    workspace_id: Uuid,
    period_start: Option<DateTime<Utc>>,
    ...
    let row = sqlx::query(
        r#"UPDATE automation_quota_period
...
pub async fn increment_automation_quota_reserved(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    workspace_id: Uuid,
    period_start: Option<DateTime<Utc>>,
    period_end: Option<DateTime<Utc>>,
...
pub async fn list_recoverable_automation_quota_reservations(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    terminal_created_before: Option<DateTime<Utc>>,
    partial_created_before: Option<DateTime<Utc>>,
    row_limit: i32,
) -> anyhow::Result<Vec<AutomationQuotaReservation>> {
    let rows = sqlx::query(
...
LIMIT $3"#
    ...
    for row in &rows {
        out.push(AutomationQuotaReservation {
            id: row.try_get(0)?,
            workspace_id: row.try_get(1)?,
            period_start: row.try_get(2)?,
            period_end: row.try_get(3)?,
...
pub async fn release_automation_quota_reservation(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    reservation_id: Uuid,
) -> anyhow::Result<Option<AutomationQuotaPeriod>> {
    let row = sqlx::query(
