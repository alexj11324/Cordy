...
pub async fn connect(cfg: &patchbay_config::DatabaseConfig) -> anyhow::Result<PgPool> {
    let url = cfg
        .url
        .as_deref()
...
pub async fn check_ready(pool: &PgPool) -> anyhow::Result<()> {
    ...
    let required_versions = patchbay_migrate::required_versions()?;
...
mod tests {
    ...
    async fn connect_fails_fast_without_url() {
        let cfg = patchbay_config::DatabaseConfig::default();
