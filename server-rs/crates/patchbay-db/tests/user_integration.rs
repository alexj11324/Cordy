...
async fn generated_user_queries_roundtrip() {
    ...
    let created = patchbay_db::queries::user::create_user(&pool, "Gen Test", &email, None)
        .await
    ...
    let fetched = patchbay_db::queries::user::get_user_by_email(&pool, &email)
        .await
        .expect("generated get_user_by_email")
        .expect("user exists");
    assert_eq!(fetched.id, created.id);

    let claimed = patchbay_db::queries::user::claim_first_onboarding(&pool, created.id)
        .await
        .expect("generated claim_first_onboarding")
        .expect("first onboarding claim returning row");
    assert!(claimed.onboarded_at.is_some());

    let onboarded = patchbay_db::queries::user::mark_user_onboarded(&pool, created.id)
        .await
    ...
    let patched =
        patchbay_db::queries::user::patch_user_onboarding(&pool, Some(&questionnaire), created.id)
            .await
            .expect("patch_user_onboarding")
            .expect("update returning row");
    assert_eq!(patched.onboarding_questionnaire, questionnaire);

    let preserved = patchbay_db::queries::user::patch_user_onboarding(&pool, None, created.id)
        .await
        .expect("preserve onboarding questionnaire")
        .expect("update returning row");
    assert_eq!(preserved.onboarding_questionnaire, questionnaire);

    let waitlisted = patchbay_db::queries::user::join_cloud_waitlist(
        &pool,
        created.id,
    ...
    let waitlisted_without_reason = patchbay_db::queries::user::join_cloud_waitlist(
        &pool,
        created.id,
    ...
    let onboarded_again = patchbay_db::queries::user::mark_user_onboarded(&pool, created.id)
        .await
        .expect("mark_user_onboarded idempotently")
        .expect("update returning row");
    assert_eq!(onboarded_again.onboarded_at, onboarded.onboarded_at);

    let claimed_again = patchbay_db::queries::user::claim_first_onboarding(&pool, created.id)
        .await
        .expect("claim_first_onboarding after onboarded");
    assert!(claimed_again.is_none());

    let batch = patchbay_db::queries::user::get_users_by_i_ds(&pool, vec![created.id])
        .await
