//! Port of server/pkg/db/queries/channel.sql (generated channel.sql.go).
...
RETURNING id, workspace_id, cordy_user_id, installation_id, channel_type, channel_user_id, config, bound_at"#
    )
        .bind(workspace_id)
        .bind(cordy_user_id)
        .bind(installation_id)
...
pub async fn find_channel_binding_for_member(
    ...
    let row = sqlx::query(
        r#"SELECT b.id, b.workspace_id, b.cordy_user_id, b.installation_id, b.channel_type, b.channel_user_id, b.config, b.bound_at FROM channel_user_binding b
JOIN channel_installation ci ON ci.id = b.installation_id
WHERE b.workspace_id = $1
  AND b.cordy_user_id = $2
...
pub async fn get_channel_user_binding_by_user_id(
    ...
    let row = sqlx::query(
        r#"SELECT id, workspace_id, cordy_user_id, installation_id, channel_type, channel_user_id, config, bound_at FROM channel_user_binding
