...
pub async fn add_autopilot_collaborator(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    autopilot_id: Uuid,
    user_type: &str,
    user_id: Uuid,
    granted_by: Uuid,
) -> anyhow::Result<Option<AutopilotCollaborator>> {
    let row = sqlx::query(
        r#"INSERT INTO autopilot_collaborator (autopilot_id, user_type, user_id, granted_by)
VALUES ($1, $2, $3, $4)
...
RETURNING autopilot_id, user_type, user_id, granted_by, created_at"#,
    ...
    Ok(Some(AutopilotCollaborator {
        autopilot_id: row.try_get(0)?,
...
pub async fn add_autopilot_subscriber(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    autopilot_id: Uuid,
    user_type: &str,
    user_id: Uuid,
    ...
    let r = sqlx::query(
        r#"INSERT INTO autopilot_subscriber (autopilot_id, user_type, user_id)
VALUES ($1, $2, $3)
...
pub async fn archive_autopilot(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    id: Uuid,
) -> anyhow::Result<u64> {
    let r = sqlx::query(
...
pub async fn create_autopilot_rule_version(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    autopilot_id: Uuid,
    workspace_id: Uuid,
...
RETURNING id, autopilot_id, workspace_id, published_by_type, published_by_id, config_summary, created_at"#
        ...
        .await?;
    let Some(row) = row else { return Ok(None) };
    Ok(Some(AutopilotRuleVersion {
        id: row.try_get(0)?,
        autopilot_id: row.try_get(1)?,
...
pub async fn create_autopilot_run(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    autopilot_id: Uuid,
    source: &str,
    status: &str,
...
pub async fn create_autopilot_task(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    agent_id: Uuid,
    runtime_id: Uuid,
    priority: i32,
...
pub async fn create_autopilot_trigger(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    autopilot_id: Uuid,
    kind: &str,
    enabled: bool,
    ...
    published_by_id: Uuid,
) -> anyhow::Result<Option<AutopilotTrigger>> {
    let row = sqlx::query(
        r#"INSERT INTO autopilot_trigger (
...
pub async fn delete_autopilot_collaborator(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    autopilot_id: Uuid,
    user_type: &str,
    user_id: Uuid,
    ...
    let r = sqlx::query(
        r#"DELETE FROM autopilot_collaborator
...
pub async fn delete_autopilot_collaborators_for_autopilot(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    autopilot_id: Uuid,
) -> anyhow::Result<u64> {
    let r = sqlx::query(
...
pub async fn delete_autopilot_subscribers_for_autopilot(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    autopilot_id: Uuid,
) -> anyhow::Result<u64> {
    let r = sqlx::query(
        r#"DELETE FROM autopilot_subscriber
WHERE autopilot_id = $1"#,
    )
    .bind(autopilot_id)
    .execute(executor)
...
pub async fn delete_autopilot_trigger(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    id: Uuid,
) -> anyhow::Result<u64> {
    let r = sqlx::query(r#"DELETE FROM autopilot_trigger WHERE id = $1"#)
        .bind(id)
        .execute(executor)
        .await?;
    Ok(r.rows_affected())
...
pub struct FailAutopilotRunsByIssueRow {
    pub id: Option<Uuid>,
...
pub async fn fail_autopilot_runs_by_issue(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    issue_id: Uuid,
) -> anyhow::Result<Vec<FailAutopilotRunsByIssueRow>> {
    let rows = sqlx::query(
...
SELECT id, autopilot_id, trigger_id, source, status, issue_id, task_id, triggered_at, completed_at, failure_reason, trigger_payload, result, created_at, squad_id, planned_at, webhook_delivery_id, quota_reservation_id, reason_code FROM updated_runs"#
    ...
    for row in &rows {
        out.push(FailAutopilotRunsByIssueRow {
            id: row.try_get(0)?,
            autopilot_id: row.try_get(1)?,
            trigger_id: row.try_get(2)?,
            source: row.try_get(3)?,
...
pub async fn get_active_autopilot_rule_version(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    workspace_id: Uuid,
    autopilot_id: Uuid,
) -> anyhow::Result<Option<AutopilotRuleVersion>> {
    let row = sqlx::query(
        r#"SELECT id, autopilot_id, workspace_id, published_by_type, published_by_id, config_summary, created_at FROM autopilot_rule_version
...
pub async fn get_autopilot(
    ...
    let row = sqlx::query(
        r#"SELECT id, workspace_id, title, description, assignee_id, status, execution_mode, issue_title_template, created_by_type, created_by_id, last_run_at, created_at, updated_at, assignee_type, project_id, pause_reason FROM autopilot
WHERE id = $1"#
    )
        .bind(id)
        .fetch_optional(executor)
...
pub async fn get_autopilot_in_workspace(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    id: Uuid,
    workspace_id: Uuid,
) -> anyhow::Result<Option<Autopilot>> {
...
pub async fn get_autopilot_run(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    id: Uuid,
) -> anyhow::Result<Option<AutopilotRun>> {
    let row = sqlx::query(
...
pub async fn get_autopilot_run_by_issue(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    issue_id: Uuid,
) -> anyhow::Result<Option<AutopilotRun>> {
    let row = sqlx::query(
...
pub async fn get_autopilot_run_by_quota_reservation(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    quota_reservation_id: Uuid,
) -> anyhow::Result<Option<AutopilotRun>> {
    let row = sqlx::query(
...
pub async fn get_autopilot_run_by_trigger_and_planned(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    trigger_id: Uuid,
    planned_at: Option<DateTime<Utc>>,
) -> anyhow::Result<Option<AutopilotRun>> {
...
pub async fn get_autopilot_run_by_webhook_delivery(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    webhook_delivery_id: Uuid,
) -> anyhow::Result<Option<AutopilotRun>> {
    let row = sqlx::query(
...
pub async fn get_autopilot_task_by_run(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    autopilot_run_id: Uuid,
) -> anyhow::Result<Option<AgentTaskQueue>> {
    let row = sqlx::query(
...
pub async fn get_autopilot_trigger(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    id: Uuid,
) -> anyhow::Result<Option<AutopilotTrigger>> {
    let row = sqlx::query(
        r#"SELECT id, autopilot_id, kind, enabled, cron_expression, timezone, next_run_at, webhook_token, label, last_fired_at, created_at, updated_at, provider, signing_secret, event_filters, published_by_type, published_by_id FROM autopilot_trigger
WHERE id = $1"#
    )
        .bind(id)
        .fetch_optional(executor)
...
pub struct GetWebhookTriggerByTokenRow {
    ...
    pub autopilot_workspace_id: Option<Uuid>,
}
...
pub async fn get_webhook_trigger_by_token(
    ...
    let row = sqlx::query(
        r#"SELECT t.id, t.autopilot_id, t.kind, t.enabled, t.cron_expression, t.timezone, t.next_run_at, t.webhook_token, t.label, t.last_fired_at, t.created_at, t.updated_at, t.provider, t.signing_secret, t.event_filters, t.published_by_type, t.published_by_id, a.workspace_id AS autopilot_workspace_id
...
WHERE t.kind = 'webhook'
  AND t.webhook_token = $1"#
    ...
    Ok(Some(GetWebhookTriggerByTokenRow {
        ...
        published_by_type: row.try_get(15)?,
        published_by_id: row.try_get(16)?,
        autopilot_workspace_id: row.try_get(17)?,
    }))
}

pub async fn is_autopilot_collaborator(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    autopilot_id: Uuid,
    user_id: Uuid,
...
pub async fn list_autopilot_collaborators(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    autopilot_id: Uuid,
) -> anyhow::Result<Vec<AutopilotCollaborator>> {
    let rows = sqlx::query(
        r#"SELECT autopilot_id, user_type, user_id, granted_by, created_at FROM autopilot_collaborator
WHERE autopilot_id = $1
ORDER BY created_at ASC, user_id ASC"#
    ...
    let mut out = Vec::with_capacity(rows.len());
    for row in &rows {
        out.push(AutopilotCollaborator {
            autopilot_id: row.try_get(0)?,
            user_type: row.try_get(1)?,
            user_id: row.try_get(2)?,
            granted_by: row.try_get(3)?,
...
pub async fn list_autopilot_i_ds_for_collaborator(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    user_id: Uuid,
) -> anyhow::Result<Vec<Option<Uuid>>> {
    let rows = sqlx::query(
        r#"SELECT autopilot_id FROM autopilot_collaborator
...
pub async fn list_autopilot_subscribers(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    autopilot_id: Uuid,
) -> anyhow::Result<Vec<AutopilotSubscriber>> {
    let rows = sqlx::query(
        r#"SELECT autopilot_id, user_type, user_id, created_at FROM autopilot_subscriber
WHERE autopilot_id = $1
ORDER BY created_at ASC, user_id ASC"#,
    )
    .bind(autopilot_id)
...
pub async fn list_autopilot_triggers(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    autopilot_id: Uuid,
) -> anyhow::Result<Vec<AutopilotTrigger>> {
    let rows = sqlx::query(
        r#"SELECT id, autopilot_id, kind, enabled, cron_expression, timezone, next_run_at, webhook_token, label, last_fired_at, created_at, updated_at, provider, signing_secret, event_filters, published_by_type, published_by_id FROM autopilot_trigger
WHERE autopilot_id = $1
...
pub async fn list_autopilots(
    ...
    workspace_id: Uuid,
    status: Option<&str>,
) -> anyhow::Result<Vec<ListAutopilotsRow>> {
    let rows = sqlx::query(
        r#"SELECT
...
#[derive(Debug, Clone, serde::Serialize)]
pub struct ListSchedulableAutopilotTriggersRow {
    pub id: Option<Uuid>,
    pub autopilot_id: Option<Uuid>,
    pub cron_expression: Option<String>,
    pub timezone: Option<String>,
...
pub async fn list_schedulable_autopilot_triggers(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
) -> anyhow::Result<Vec<ListSchedulableAutopilotTriggersRow>> {
    let rows = sqlx::query(
        r#"SELECT t.id, t.autopilot_id, t.cron_expression, t.timezone, t.created_at, t.last_fired_at
FROM autopilot_trigger t
JOIN autopilot a ON a.id = t.autopilot_id
...
ORDER BY t.id"#,
    ...
    let mut out = Vec::with_capacity(rows.len());
    for row in &rows {
        out.push(ListSchedulableAutopilotTriggersRow {
            id: row.try_get(0)?,
            autopilot_id: row.try_get(1)?,
...
pub async fn pause_autopilots_by_unbound_agents(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    agent_ids: Vec<Uuid>,
) -> anyhow::Result<Vec<Autopilot>> {
    let rows = sqlx::query(
        r#"UPDATE autopilot a
SET status = 'paused',
...
pub async fn recover_partial_autopilot_run(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    id: Uuid,
...
pub async fn rotate_autopilot_trigger_webhook_token(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    id: Uuid,
    webhook_token: Option<&str>,
) -> anyhow::Result<Option<AutopilotTrigger>> {
...
#[derive(Debug, Clone, serde::Serialize)]
pub struct SelectAutopilotsExceedingFailureThresholdRow {
    pub id: Option<Uuid>,
...
pub async fn select_autopilots_exceeding_failure_threshold(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    min_runs: i64,
    fail_ratio_threshold: f64,
    since: Option<DateTime<Utc>>,
) -> anyhow::Result<Vec<SelectAutopilotsExceedingFailureThresholdRow>> {
    let rows = sqlx::query(
        r#"WITH stats AS (
    SELECT autopilot_id,
...
ORDER BY s.failed DESC, a.id ASC"#,
    ...
    let mut out = Vec::with_capacity(rows.len());
    for row in &rows {
        out.push(SelectAutopilotsExceedingFailureThresholdRow {
            id: row.try_get(0)?,
            workspace_id: row.try_get(1)?,
            title: row.try_get(2)?,
...
pub async fn set_autopilot_trigger_publisher(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    id: Uuid,
    published_by_type: Option<&str>,
    published_by_id: Uuid,
...
pub async fn set_autopilot_trigger_publishers_by_autopilot(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    autopilot_id: Uuid,
    published_by_type: Option<&str>,
    published_by_id: Uuid,
...
pub async fn set_autopilot_trigger_signing_secret(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    id: Uuid,
...
pub async fn set_autopilot_trigger_webhook_token(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    id: Uuid,
    webhook_token: Option<&str>,
) -> anyhow::Result<Option<AutopilotTrigger>> {
...
pub async fn system_pause_autopilot(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    id: Uuid,
) -> anyhow::Result<Option<Autopilot>> {
    let row = sqlx::query(
...
pub async fn touch_autopilot_trigger_fired_at(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    id: Uuid,
) -> anyhow::Result<u64> {
    let r = sqlx::query(
...
pub async fn update_autopilot_last_run_at(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    id: Uuid,
) -> anyhow::Result<u64> {
    let r = sqlx::query(
        r#"UPDATE autopilot SET last_run_at = now(), updated_at = now()
WHERE id = $1"#,
    )
    .bind(id)
    .execute(executor)
...
pub async fn update_autopilot_run_completed(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    id: Uuid,
...
pub async fn update_autopilot_run_failed(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    id: Uuid,
...
pub async fn update_autopilot_run_issue_created(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    id: Uuid,
    issue_id: Uuid,
) -> anyhow::Result<Option<AutopilotRun>> {
...
pub async fn update_autopilot_run_running(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    id: Uuid,
    task_id: Uuid,
) -> anyhow::Result<Option<AutopilotRun>> {
...
pub async fn update_autopilot_run_skipped(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    id: Uuid,
    failure_reason: Option<&str>,
    reason_code: Option<&str>,
...
pub async fn update_autopilot_run_skipped_with_result(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    id: Uuid,
    failure_reason: Option<&str>,
    result: &serde_json::Value,
...
#[derive(Debug, Clone, serde::Serialize)]
pub struct UpdateAutopilotRunTerminalWithQuotaRow {
    pub id: Option<Uuid>,
    pub autopilot_id: Option<Uuid>,
    pub trigger_id: Option<Uuid>,
    pub source: String,
...
pub async fn update_autopilot_run_terminal_with_quota(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    terminal_status: &str,
    result: &serde_json::Value,
    failure_reason: Option<&str>,
    ...
    run_id: Uuid,
    consume: bool,
) -> anyhow::Result<Option<UpdateAutopilotRunTerminalWithQuotaRow>> {
    let row = sqlx::query(
        r#"WITH updated_run AS (
    UPDATE autopilot_run AS ar
    SET status = $1::text,
...
SELECT id, autopilot_id, trigger_id, source, status, issue_id, task_id, triggered_at, completed_at, failure_reason, trigger_payload, result, created_at, squad_id, planned_at, webhook_delivery_id, quota_reservation_id, reason_code FROM updated_run"#
        ...
        .await?;
    let Some(row) = row else { return Ok(None) };
    Ok(Some(UpdateAutopilotRunTerminalWithQuotaRow {
        id: row.try_get(0)?,
        autopilot_id: row.try_get(1)?,
        trigger_id: row.try_get(2)?,
        source: row.try_get(3)?,
...
pub async fn update_autopilot_trigger(
    executor: impl sqlx::Executor<'_, Database = sqlx::Postgres>,
    id: Uuid,
    enabled: Option<bool>,
    cron_expression: Option<&str>,
