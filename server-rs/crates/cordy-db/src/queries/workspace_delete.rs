//! Port of server/pkg/db/queries/workspace_delete.sql (generated workspace_delete.sql.go).
...
pub async fn set_workspace_teardown_mode(
    ...
    let r = sqlx::query(r#"SELECT set_config('cordy.workspace_teardown', 'on', true)"#)
        .execute(executor)
        .await?;
    Ok(r.rows_affected())
}
