...
RUN cd server && CGO_ENABLED=0 go build -ldflags "-s -w -X main.version=${VERSION} -X main.commit=${COMMIT} -X main.date=${DATE}" -o bin/orvilo ./cmd/orvilo
RUN cd server && CGO_ENABLED=0 go build -ldflags "-s -w" -o bin/migrate ./cmd/migrate
RUN cd server && CGO_ENABLED=0 go build -ldflags "-s -w" -o bin/orvilo-messaging ./cmd/messaging
...
COPY --from=builder /src/server/bin/orvilo .
COPY --from=builder /src/server/bin/migrate .
COPY --from=builder /src/server/bin/orvilo-messaging .
