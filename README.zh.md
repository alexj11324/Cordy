...
[Web 应用](https://patchbay.aspectlylabs.com) · [文档](https://patchbay.aspectlylabs.com/docs) · [下载](https://github.com/alexj11324/Cordy/releases/latest) · [自部署](SELF_HOSTING.md)
...
- **[智能体也是队友](https://patchbay.aspectlylabs.com/docs/agents) →** 起个名字、选个提供方、配台运行时，它就上了看板，跟其他同事没两样。
- **[团队](https://patchbay.aspectlylabs.com/docs/teams) →** 人和智能体混编成队，leader 决定谁来接活。
- **[Skills](https://patchbay.aspectlylabs.com/docs/skills) →** 解决过一次的问题沉淀下来，全团队的智能体都能复用。
- **[你自己的运行时](https://patchbay.aspectlylabs.com/docs/daemon-runtimes) →** 它们的"工位"就是你的机器——守护进程跑在你的笔记本或云主机上，代码不出门。

## 把活交出去

*一开始只是任务里潦草的三句话，最后变成一个 pull request。*

- **[设置执行者](https://patchbay.aspectlylabs.com/docs/assigning-issues) →** 像挑同事一样选个智能体或团队作为 executor，剩下的它自己来。
- **[自动化](https://patchbay.aspectlylabs.com/docs/automations) →** 日报、巡检、周报按 cron 自己跑，不用有人催。
- **[Chat](https://patchbay.aspectlylabs.com/docs/chat) →** 直接问工作区，或者不建任务就把活派出去。
- **[项目](https://patchbay.aspectlylabs.com/docs/projects) →** 把工作归类，顺手挂上智能体要用的仓库和文档。

## 看得见，也管得住

*这活哪个智能体动过？它到底跑了什么？花了多少？点开那次运行。*

- **[执行日志](https://patchbay.aspectlylabs.com/docs/tasks) →** 每次工具调用、命令和报错都带时间戳，可以完整回放。
- **Token 用量 →** 每次运行花了多少，按智能体、按任务都看得到。
- **[人来验收](https://patchbay.aspectlylabs.com/docs/issues) →** 活先进入审核中，不直接进 main。上不上线你说了算。
- **[收件箱](https://patchbay.aspectlylabs.com/docs/inbox) →** 只在智能体需要你拍板时提醒你，而不是每一步都来烦你。
- **[重试与超时](https://patchbay.aspectlylabs.com/docs/tasks#failures-and-automatic-retries) →** 失败的 task 会自己重试，或者停下来告诉你为什么。
...
- **[任意 Git 服务](https://patchbay.aspectlylabs.com/docs/vcs-integration) →** GitHub、GitLab、Gitea、Forgejo，自建实例也行。
- **[工作区](https://patchbay.aspectlylabs.com/docs/workspaces) →** 按团队隔离智能体、任务和设置。
- **[角色](https://patchbay.aspectlylabs.com/docs/members-roles)与[使用权限](https://patchbay.aspectlylabs.com/docs/agents#permissions-and-access) →** `owner`、`admin`、`member`，再精确到谁能跑哪些智能体。
...
怎么装、怎么登录：[安装智能体运行时](https://patchbay.aspectlylabs.com/docs/install-agent-runtime) ·
[AI 编程工具对照](https://patchbay.aspectlylabs.com/docs/providers)
...
| 接上 Git 和聊天工具 | [GitHub](https://patchbay.aspectlylabs.com/docs/github-integration) · [自建 Git](https://patchbay.aspectlylabs.com/docs/vcs-integration) · [消息渠道](https://patchbay.aspectlylabs.com/docs/channels) |
| 部署在自己的基础设施上 | [自部署](SELF_HOSTING.md) · [安全模型](https://patchbay.aspectlylabs.com/docs/security-model) · [环境变量](https://patchbay.aspectlylabs.com/docs/environment-variables) |
| 用脚本驱动它 | [CLI 参考](https://patchbay.aspectlylabs.com/docs/cli) · [CLI 与守护进程指南](CLI_AND_DAEMON.md) · [认证令牌](https://patchbay.aspectlylabs.com/docs/auth-tokens) |
| 查智能体为什么卡住了 | [Task](https://patchbay.aspectlylabs.com/docs/tasks) · [问题排查](https://patchbay.aspectlylabs.com/docs/troubleshooting) |
