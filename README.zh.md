...
- **[智能体也是队友](https://orvilo.aspectlylabs.com/docs/agents) →** 起个名字、选个提供方、配台运行时，它就上了看板，跟其他同事没两样。
- **[团队](https://orvilo.aspectlylabs.com/docs/teams) →** 人和智能体混编成队，leader 决定谁来接活。
- **[Skills](https://orvilo.aspectlylabs.com/docs/skills) →** 解决过一次的问题沉淀下来，全团队的智能体都能复用。
- **[你自己的运行时](https://orvilo.aspectlylabs.com/docs/daemon-runtimes) →** 它们的"工位"就是你的机器——守护进程跑在你的笔记本或云主机上，代码不出门。

## 把活交出去

*一开始只是任务里潦草的三句话，最后变成一个 pull request。*

- **[设置执行者](https://orvilo.aspectlylabs.com/docs/assigning-issues) →** 像挑同事一样选个智能体或团队作为 executor，剩下的它自己来。
- **[自动化](https://orvilo.aspectlylabs.com/docs/automations) →** 日报、巡检、周报按 cron 自己跑，不用有人催。
- **[Chat](https://orvilo.aspectlylabs.com/docs/chat) →** 直接问工作区，或者不建任务就把活派出去。
- **[项目](https://orvilo.aspectlylabs.com/docs/projects) →** 把工作归类，顺手挂上智能体要用的仓库和文档。

## 看得见，也管得住

*这活哪个智能体动过？它到底跑了什么？花了多少？点开那次运行。*

- **[执行日志](https://orvilo.aspectlylabs.com/docs/tasks) →** 每次工具调用、命令和报错都带时间戳，可以完整回放。
- **Token 用量 →** 每次运行花了多少，按智能体、按任务都看得到。
- **[人来验收](https://orvilo.aspectlylabs.com/docs/issues) →** 活先进入审核中，不直接进 main。上不上线你说了算。
- **[收件箱](https://orvilo.aspectlylabs.com/docs/inbox) →** 只在智能体需要你拍板时提醒你，而不是每一步都来烦你。
- **[重试与超时](https://orvilo.aspectlylabs.com/docs/tasks#failures-and-automatic-retries) →** 失败的 task 会自己重试，或者停下来告诉你为什么。
...
- **[任意 Git 服务](https://orvilo.aspectlylabs.com/docs/vcs-integration) →** GitHub、GitLab、Gitea、Forgejo，自建实例也行。
- **[工作区](https://orvilo.aspectlylabs.com/docs/workspaces) →** 按团队隔离智能体、任务和设置。
- **[角色](https://orvilo.aspectlylabs.com/docs/members-roles)与[使用权限](https://orvilo.aspectlylabs.com/docs/agents#permissions-and-access) →** `owner`、`admin`、`member`，再精确到谁能跑哪些智能体。
- **[安全模型](https://orvilo.aspectlylabs.com/docs/security-model) →** 智能体碰得到什么，碰不到什么。
- **[Slack、飞书、钉钉](https://orvilo.aspectlylabs.com/docs/channels) →** 在团队本来就在聊天的地方，触发和跟进智能体的工作。钉钉由社区维护。
- **[Web、桌面端、移动端](https://orvilo.aspectlylabs.com/docs/desktop-app) →** macOS、Windows、Linux、iPhone，打开都是同一个工作区——iOS 现在要自己从源码编译安装，还没上 App Store。
- **[CLI 与 API](https://orvilo.aspectlylabs.com/docs/cli) →** 界面上能点的，CLI 和 API 里都能调。智能体操作 Orvilo，用的就是你那套 CLI。

---

## 开始使用

不用打开终端：直接在 **[Orvilo Web 应用](https://orvilo.aspectlylabs.com)** 注册，或者下载
...
**1. 登录。** 在浏览器里打开 [Orvilo Web 应用](https://orvilo.aspectlylabs.com)，或者打开
...
怎么装、怎么登录：[安装智能体运行时](https://orvilo.aspectlylabs.com/docs/install-agent-runtime) ·
[AI 编程工具对照](https://orvilo.aspectlylabs.com/docs/providers)
...
| 把我的机器接进来 | [守护进程与运行时](https://orvilo.aspectlylabs.com/docs/daemon-runtimes) · [安装智能体运行时](https://orvilo.aspectlylabs.com/docs/install-agent-runtime) |
| 接上 Git 和聊天工具 | [GitHub](https://orvilo.aspectlylabs.com/docs/github-integration) · [自建 Git](https://orvilo.aspectlylabs.com/docs/vcs-integration) · [消息渠道](https://orvilo.aspectlylabs.com/docs/channels) |
| 部署在自己的基础设施上 | [自部署](SELF_HOSTING.md) · [安全模型](https://orvilo.aspectlylabs.com/docs/security-model) · [环境变量](https://orvilo.aspectlylabs.com/docs/environment-variables) |
| 用脚本驱动它 | [CLI 参考](https://orvilo.aspectlylabs.com/docs/cli) · [CLI 与守护进程指南](CLI_AND_DAEMON.md) · [认证令牌](https://orvilo.aspectlylabs.com/docs/auth-tokens) |
| 查智能体为什么卡住了 | [Task](https://orvilo.aspectlylabs.com/docs/tasks) · [问题排查](https://orvilo.aspectlylabs.com/docs/troubleshooting) |
...
| 把 Orvilo 当产品用 | [文档站](https://orvilo.aspectlylabs.com/docs) |
