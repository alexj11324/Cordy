...
Flags: `--title` (required), `--description`, `--status`, `--priority`, `--executor` / `--executor-id`, `--parent`, `--project`, `--due-date`. Pass `--executor-id <uuid>` (mutually exclusive with `--executor`) when scripting against the IDs returned by `patchbay workspace member list --output json` / `patchbay agent list --output json`.
