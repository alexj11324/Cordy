...
ORVILO_SIGNING_SECRET=whsec_... node server/handler.mjs      # :8788
...
```bash
export ORVILO_PLUGIN_DIR=examples/plugins
export ORVILO_PLUGIN_DEV_ORIGINS=https://127.0.0.1:8788,https://127.0.0.1:8789
export ORVILO_PLUGIN_DEV_CA=/path/to/your-dev-ca.pem
```

Publish it from `ORVILO_PLUGIN_DIR` as `deploy-sentinel`, install the version
...
Note what `ORVILO_PLUGIN_DIR` does and does not cover. It is a publishing
...
and `ORVILO_PLUGIN_DEV_CA` is how you get a self-signed certificate trusted.
